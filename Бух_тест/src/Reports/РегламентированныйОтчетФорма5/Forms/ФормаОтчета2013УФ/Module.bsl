...
Процедура  РасчетПервичнойСтоимостиОсНаКонсервацииБУ();
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	СУММА(ЕСТЬNULL(СтоимостьОстатки.СуммаОстатокДт, 0) - ЕСТЬNULL(АмортизацияОстатки.СуммаОстатокКт, 0)) КАК КонОстаток
	               |ИЗ
	...
	Если СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Истина Тогда
		...
		Исключение
			Если глМенеджерЗвит1С.ФлагОтладки Тогда
				Сообщить("1С:Звіт. Исключение при получении данных из 1С:Звіт ("+ИмяСхемы+", 30, 1, " +Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)+")", СтатусСообщения.Важное);
