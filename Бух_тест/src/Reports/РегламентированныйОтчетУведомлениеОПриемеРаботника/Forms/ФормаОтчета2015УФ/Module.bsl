...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") = Истина Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	НомераДокументов.Дата КАК ДатаПриказа,
		               |	НомераДокументов.Номер КАК НомерПриказа,
		               |	ВЫБОР
		               |		КОГДА РаботникиОрганизаций.ВидЗанятости = &ВидЗанятостиОсновное
		               |			ТОГДА 1
		               |		ИНАЧЕ 2
		               |	КОНЕЦ КАК Категория,
		               |	РаботникиОрганизаций.Период КАК ДатаНачала
		               |ИЗ
					   |	РегистрСведений.КадроваяИсторияСотрудников.СрезПервых(, ГоловнаяОрганизация = &Организация)  КАК РаботникиОрганизаций
					   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаОкончания, ) КАК ФИОФизЛиц
		               |		ПО (ФИОФизЛиц.ФизическоеЛицо = РаботникиОрганизаций.ФизическоеЛицо)
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК НомераДокументов
		               |		ПО РаботникиОрганизаций.Регистратор = НомераДокументов.Документ
		               |ГДЕ
		               |	  РаботникиОрганизаций.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		               |	И РаботникиОрганизаций.ГоловнаяОрганизация = &Организация
		               |	И РаботникиОрганизаций.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Прием)
	...
	Иначе	
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ВЫБОР
		               |		КОГДА РаботникиОрганизаций.ТекущийВидЗанятости = &ВидЗанятостиОсновное
		               |			ТОГДА 1
		               |		ИНАЧЕ 2
		               |	КОНЕЦ КАК Категория,
		               |	РаботникиОрганизаций.ДатаПриема КАК ДатаНачала
		               |ИЗ
		               |	РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК РаботникиОрганизаций
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаОкончания, ) КАК ФИОФизЛиц
		               |		ПО (ФИОФизЛиц.ФизическоеЛицо = РаботникиОрганизаций.ФизическоеЛицо)
		               |ГДЕ
		               |	РаботникиОрганизаций.ДатаПриема МЕЖДУ &ДатаНачала И &ДатаОкончания
		               |	И РаботникиОрганизаций.ГоловнаяОрганизация = &Организация";
	...
	Запрос.УстановитьПараметр("ВидЗанятостиОсновное", Перечисления.ВидыЗанятости.ОсновноеМестоРаботы);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Строка["T1RXXXXG7S"] = Результат.НомерПриказа;
