...
Функция ПолучитьНастройкиОтчета() Экспорт
	...
	Настройки = ФункцииОтчетовКлиентСервер.ПолучитьНастройкиОтчетаПоУмолчанию();  // получим настройки по умолчанию
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ИначеЕсли КлючВарианта = "РасчетнаяВедомость" Тогда
		...
		Попытка
			...
			Макет = ПолучитьМакет("ПФ_MXL_РасчетнаяВедомость");
			...
			Для каждого СтрокаМесяца Из ДаныеОтчета.Строки Цикл
				Для Каждого СтрокаОрганизации Из СтрокаМесяца.Строки Цикл
					ВывестиОрганизациюРасчетнаяВедомость(СтрокаОрганизации, ДокументРезультат, Макеты, ЕстьГруппировкаПоПодразделению, СоответствиеПользовательскихПолей);
				КонецЦикла;
			КонецЦикла;
...
Процедура ВывестиОрганизациюРасчетнаяВедомость(СтрокаОрганизации, ДокументРезультат, Макеты, ЕстьГруппировкаПоПодразделению, СоответствиеПользовательскихПолей)
	...
	Иначе
		...
		Если Документ <> Неопределено Тогда
			...
			Макеты.ШапкаДокумента.Параметры.ФИОРуководителя = ФизическиеЛица.ФамилияИнициалыФизЛица(
					Неопределено,
			...
			Макеты.ШапкаДокумента.Параметры.ФИОГлавногоБухгалтера = ФизическиеЛица.ФамилияИнициалыФизЛица(
					Неопределено,
		...
		Для Каждого СтрокаПодразделения Из СтрокаОрганизации.Строки Цикл
			...
			КонецЦикла;
		КонецЦикла;
		ДополнитьСтраницуРасчетнаяВедомость(НомерСтроки, Макеты, ДокументРезультат, ПромежуточныеИтоги);
		...
		Если Документ <> Неопределено Тогда
			Макеты.Подвал.Параметры.ФИОКассира = ФизическиеЛица.ФамилияИнициалыФизЛица(
					Неопределено,
			...
			Макеты.Подвал.Параметры.ФИОБухгалтера = ФизическиеЛица.ФамилияИнициалыФизЛица(
					Неопределено,
...
Процедура ДополнитьСтраницуРасчетнаяВедомость(НомерСтроки, Макеты, ДокументРезультат, ПромежуточныеИтоги)
...
Функция ДанныеПоНДФЛ(СтрокиСотрудников, МесяцНачисления)
	...
	Запрос.Текст =
		...
		|	МесяцНачисления}
		|ИЗ
		|	РегистрНакопления.НДФЛПоСотрудникам.Обороты(&НачалоПериода, &КонецПериода, Месяц,ДоходНДФЛ <> ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.ВоенныйСбор)) КАК НДФЛПоСотрудникам
		|{ГДЕ
