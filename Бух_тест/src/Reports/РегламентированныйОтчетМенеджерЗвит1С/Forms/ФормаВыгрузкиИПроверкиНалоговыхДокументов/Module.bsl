...
Процедура ЗаполнитьНачальныеНастройки()
	...
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-2, НСтр("ru='В ""1С:Звіт"" обнаружено более одного документа соответствующего документу ""1С""';uk='В ""1С:Звіт"" виявлено більше одного документа відповідного документу ""1С""'"));
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-1, НСтр("ru='Не выгружен в ""1С:Звіт""';uk='Не вивантажений в ""1С:Звіт""'"));
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить( 0, НСтр("ru='Выгружен в ""1С:Звіт""';uk='Вивантажений в ""1С:Звіт""'"));
	...
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-2, НСтр("ru='В ""1С:Звіт"" обнаружено более одного вытяга соответствующего документу ""1С""';uk='В ""1С:Звіт"" виявлено більше одного витяга відповідного документу ""1С""'")); // Несуществующее состояние для входящих 
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-1, НСтр("ru='Не сформирован запрос в ""1С:Звіт""';uk='Не сформований запит в ""1С:Звіт""'")); // -1 Еще не реализовано со стороны 1С:Звіт, пока используется +2
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить( 1, НСтр("ru='Запрос сформирован в ""1С:Звіт""';uk='Запит сформовано в ""1С:Звіт""'"));       //  0 Еще не реализовано со стороны 1С:Звіт, пока используется +2
	...
	СтатусРегистрацииДляВходящихНалоговыхДокументов.Вставить(-1, Перечисления.СтатусыРегистрацииВЕРНННалоговыхДокументов.ПустаяСсылка()); // -1 Еще не реализовано со стороны 1С:Звіт, пока используется +2
	СтатусРегистрацииДляВходящихНалоговыхДокументов.Вставить( 1, Перечисления.СтатусыРегистрацииВЕРНННалоговыхДокументов.ПустаяСсылка());       //  0 Еще не реализовано со стороны 1С:Звіт, пока используется +2
	...
	мПараметрыРаботыЗвит1С.Вставить("ОшибкаВзаимодействияСЗвит1ССтр", НСтр("ru='<Ошибка взаимодействия с ""1С:Звіт"">';uk='<Помилка взаємодії з ""1С:Звіт"">'"));
	
	АвтоЗаголовок = Ложь;
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		ЭтаФорма.Заголовок = НСтр("ru='Выгрузка исходящих налоговых документов в ""1С:Звіт""';uk='Вивантаження вихідних податкових документів в ""1С:Звіт""'");		
	Иначе
		ЭтаФорма.Заголовок = НСтр("ru='Проверка регистрации входящих налоговых документов в ""1С:Звіт""';uk='Перевірка реєстрації вхідних податкових документів в ""1С:Звіт""'");		
	КонецЕсли;
	
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		
		КолонкаПометкаВыполнить_Текст       = НСтр("ru='Выгружать';uk='Вивантажувати'");
		КолонкаПометкаВыполнить_ПолныйТекст = НСтр("ru='Отметка, указывает на то что документ будет выгружен в ""1С:Звіт""';uk='Позначка, що вказує на те, що документ буде вивантажений в ""1С:Звіт""'");
		
		ВыполнитьТекст = НСтр("ru='Выгрузить в ""1С:Звіт""';uk='Вивантажити в ""1С:Звіт""'");
		
	Иначе
		                               
		КолонкаПометкаВыполнить_Текст       = НСтр("ru='Запрос';uk='Запит'");
		КолонкаПометкаВыполнить_ПолныйТекст = НСтр("ru='Отметка, указывает на то что для документа будет сформирован и отправлен запрос состояния регистрации в ЕРНН через ""1С:Звіт""';uk='Позначка, що вказує на те, що для документа буде сформований та відправлений запит стану реєстрації в ЄРПН через ""1С:Звіт""'");
...
Процедура ТаблицаДокументовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		...
		МенюВыбора.Добавить("1СЗвит", "Открыть документ в 1С:Звіт");
...
Процедура Заполнить(Команда)
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаИспользованияЗвит1С) Тогда
		ТекстСообщения = НСтр("ru='Выгрузка в систему ""1С:Звіт"" невозможна, ""1С:Звіт"" не используется для организации';uk='Вивантаження в систему ""1С:Звіт"" неможливе, ""1С:Звіт"" не використовується для організації'")
		                     +" """+Строка(Организация)+"""."+Символы.ПС+НСтр("ru='Не указана дата начала использования ""1С:Звіт"" в карточке организации';uk='Не зазначена дата початку використання ""1С:Звіт"" у картці організації'");
		ПоказатьПредупреждение(,ТекстСообщения);	
		Возврат;
	КонецЕсли;
...
Функция ПроверитьСостояниеРегистрацииВ1СЗвит(МассивДокументов)
	...
	// будем в данной таблице хранить информацию - для какой ссылки какой номер в запросе к 1С:Звіт.
	...
	// Для организации без ЕДРПОУ бесполезно подключаться к 1С:Звіту
			   ...
			   И СтрокаТаблицы.Звит1С_ДокументКорректен Тогда
				...
				Иначе 
					Если (СтрокаТаблицы.Звит1С_Состояние = -1)      // Запит не сформирован
					 ИЛИ (СтрокаТаблицы.Звит1С_Состояние = 2) Тогда // Вытяг не получен. Статус существует временно до реализации со стороны 1С:Звіт (вместо -1 и 0 возвращается статус 2)
...
Функция СформироватьИОтправитьЗапросСостоянияРегистрацииДокументовВЕРНН()
	...
	Состояние("Формирование и отправка запроса состояния регистрации в ЕРНН в ""1С:Звіт""");
	...
	Для каждого СтрокаТаблицыДокументов Из МассивСтрокДокументов Цикл
		...
		Иначе	
			...
			Если СтрокаТаблицыДокументов.Звит1С_ДокументЗарегистрирован И Не ЗначениеЗаполнено(СтрокаТаблицыДокументов.Звит1С_ДатаРегистрации) Тогда
				...
				Сообщить(Строка(ДокументСсылка)+" - "+ТекстСообщения, СтатусСообщения.Информация);
				БылиОшибки = Истина; // Ошибка в работе компоненты 1С:Звіт
				Продолжить;
			КонецЕсли;
...
"Изменены ключевые параметры, выгрузка отменена.';uk='Виявлено звіт ""1С:Звіт"" відповідний документу, що вивантажується з ""1С"" ."
