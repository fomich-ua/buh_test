...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ИначеЕсли РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "УТ" Тогда 
		
		ЗаполнитьАвансовыеПлатежиУТ();
		
		ЗаполнитьРазделПоГруппеУТ(Перечисления.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа);
		ЗаполнитьРазделПоГруппеУТ(Перечисления.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа);
		ЗаполнитьРазделПоГруппеУТ(Перечисления.ГруппыПлательщиковЕдиногоНалога.ТретьяГруппа);
	...
	Если ТипЗнч(ПараметрыЗаполненияЗвит1С) = Тип("Структура") Тогда
		...
		Исключение
			...
			Если глМенеджерЗвит1С.ФлагОтладки Тогда
				Сообщить("1С:Звіт. Исключение при получении данных из 1С:Звіт ("+ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ИмяСхемы+", "+ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ТипПериода+", "+ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.НомерПериода+", " +ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ГодПериода+")", СтатусСообщения.Важное);
