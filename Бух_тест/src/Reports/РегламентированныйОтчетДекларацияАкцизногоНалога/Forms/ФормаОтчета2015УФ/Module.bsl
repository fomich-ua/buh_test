...
Функция СформироватьДанныеДляЗвит1С(ОтчетЗвит1С = Неопределено) Экспорт 
	...
	Если Сред(глМенеджерЗвит1С.ИмяСхемыЗвит1С,2,5) = "02095" Тогда
		...
		|Потім оновіть поля декларації (F6, при включеному авторозрахунку) після заповнення всіх додатків 6!", "Завантаження даних з 1С:Звіт");
	...
	Иначе	
		...
		Если    Результат = Неопределено Тогда
			глМенеджерЗвит1С.ПредупреждениеЗвит1С("В 1С немає даних про продажу в торгових точках розташованих в КОАТУУ " + ТекКОАТУУ, "Завантаження даних з 1С:Звіт");
		КонецЕсли;
