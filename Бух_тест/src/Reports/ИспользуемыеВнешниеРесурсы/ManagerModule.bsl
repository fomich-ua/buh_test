			...
			Для Каждого Разрешение Из Разрешения Цикл
				...
				Если ТипРазрешения = "FileSystemAccess" Тогда
					...
					Если Разрешение.Адрес = "/temp" Тогда
						Разрешение.Адрес = НСтр("ru='Каталог временных файлов';uk='Каталог тимчасових файлів'");
					КонецЕсли;
					
					Если Разрешение.Адрес = "/bin" Тогда
						Разрешение.Адрес = НСтр("ru='Каталог, в который установлен сервер 1С:Предприятия';uk='Каталог, у який установлений сервер 1С:Підприємства'");
					КонецЕсли;
