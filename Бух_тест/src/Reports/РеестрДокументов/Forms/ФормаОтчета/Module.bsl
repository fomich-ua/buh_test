...
Функция ПолучитьТекстЗапроса(ИмяТаблицы)
	...
	Для каждого КлючИЗначение Из ДополнительныеРеквизиты Цикл
		...
		Иначе
			ПолеРеквизита =
				?(Найти(КлючИЗначение.Значение, "Таб.") > 0,
					КлючИЗначение.Значение + " КАК " + КлючИЗначение.Ключ,
					"	" + ?(КлючИЗначение.Значение = "Неопределено", "", "Таб.")
					+ КлючИЗначение.Значение + " КАК " + КлючИЗначение.Ключ);
		КонецЕсли;
	...
	Если Найти(РезультатТекст, "Таб.Организация") > 0 Тогда
		РезультатТекст = СтрЗаменить(РезультатТекст, "Таб.НомерВходящегоДокумента",
			...
			|	КОНЕЦ" + ?(Найти(РезультатТекст, "КАК НомерВходящегоДокумента") = 0, " КАК НомерВходящегоДокумента", ""));
		РезультатТекст = СтрЗаменить(РезультатТекст, "Таб.ДатаВходящегоДокумента",
			...
			|	КОНЕЦ" + ?(Найти(РезультатТекст, "КАК ДатаВходящегоДокумента") = 0, " КАК ДатаВходящегоДокумента", ""));
		РезультатТекст = РезультатТекст + "
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Ключ") Тогда
		...
		СообщениеПользователю.Текст = НСтр("ru='Отчет ""Реестр документов"" может быть вызван только из формы списка документов';uk='Звіт ""Реєстр документів"" може бути викликаний тільки з форми списку документів'");
	...
	Иначе
		Заголовок = НСтр("ru='Список документов';uk='Список документів'");
		ИмяСписка = НСтр("ru='Список документов';uk='Список документів'");
	КонецЕсли;
	...
	Если Параметры.Свойство("ОтборПоТоварамУслугам") Тогда
		...
		Если Не ЭтоРеализация Тогда 
			Элементы.ТоварыУслуги.СписокВыбора[1].Представление = НСтр("Только поступления товаров");
	...
	Если ВыводитьЗаголовок Тогда
		...
		Если ЕстьОтборПоПервичнымДокументам Тогда
			...
			Если ТоварыУслуги = 0 Тогда
				ОбластьОписаниеТоварыУслуги.Параметры.ОписаниеТоварыУслуги     = НСтр("ru='Все документы';uk='Всі документи'");
			ИначеЕсли ТоварыУслуги = 1 Тогда
				Если ЭтоРеализация Тогда
					ОбластьОписаниеТоварыУслуги.Параметры.ОписаниеТоварыУслуги = НСтр("ru='Только реализации товаров';uk='Тільки реалізації товарів'");
				Иначе
					ОбластьОписаниеТоварыУслуги.Параметры.ОписаниеТоварыУслуги = НСтр("ru='Только поступления товаров';uk='Тільки надходження товарів'");
				КонецЕсли;
			ИначеЕсли ТоварыУслуги = 2 Тогда
				Если ЭтоРеализация Тогда
					ОбластьОписаниеТоварыУслуги.Параметры.ОписаниеТоварыУслуги = НСтр("ru='Только оказания услуг';uk='Тільки надання послуг'");
				Иначе
					ОбластьОписаниеТоварыУслуги.Параметры.ОписаниеТоварыУслуги = НСтр("ru='Только полученные услуги';uk='Тільки отримані послуги'");
				КонецЕсли;
			КонецЕсли;
		...
		Если НЕ ПустаяСтрока(ТекстОтбор) Тогда
			ОбластьОтбор.Параметры.ИмяНастроекОтчета = НСтр("ru='Отбор:';uk='Відбір:'");
