...
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".';uk='Щоб відкрити звіт відкрийте картку користувача,"
...
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".';uk='Щоб відкрити звіт відкрийте картку користувача або групи користувачів,"
"перейдіть по посиланню ""Права доступу"", натисніть на кнопку ""Звіт з прав доступу"".'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Расшифровка) = Тип("Строка")
	   И Лев(Расшифровка, СтрДлина("ОткрытьФормуСписка: ")) = "ОткрытьФормуСписка: " Тогда
...
Процедура ВывестиОтчет(Ссылка)
	...
	Запрос.УстановитьПараметр("ТипыВладельцевНастроекПрав", УправлениеДоступомСлужебныйПовтИсп.Параметры(
		).ВозможныеПраваДляНастройкиПравОбъектов.ТипыВладельцев);
	...
	ВладельцыНастроекПрав = УправлениеДоступомСлужебныйПовтИсп.Параметры(
...
"изменение, не ограничено)';uk='(додавання, не обмежено"
"зміна, не обмежена)'");
...
"изменение, не ограничено)';uk='(додавання, обмежено"
"зміна, не обмежена)'");
						ИначеЕсли НЕ ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление*, ограничено"
"изменение, не ограничено)';uk='(додавання*, обмежено"
...
"зміна*, не обмежена)'");
					...
					ИначеЕсли ОписаниеОбъекта.ДобавлениеБезОграничения И НЕ ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление, не ограничено"
"изменение, ограничено)';uk='(додавання, не обмежено"
"зміна, обмежена)'");
						ИначеЕсли НЕ ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление*, не ограничено"
"изменение, ограничено)';uk='(додавання*, не обмежено"
...
"зміна*, обмежена)'");
					...
					Иначе // НЕ ОписаниеОбъекта.ДобавлениеБезОграничения И НЕ ОписаниеОбъекта.ИзменениеБезОграничения
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление, ограничено"
"изменение, ограничено)';uk='(додавання, обмежено"
"зміна, обмежена)'");
						ИначеЕсли НЕ ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление*, ограничено"
"изменение, ограничено)';uk='(додавання*, обмежено"
...
"изменение, не ограничено)';uk='(додавання не доступно"
"зміна, не обмежена)'");
...
"зміна*, не обмежена)'");
					...
					Иначе // НЕ ОписаниеОбъекта.ИзменениеБезОграничения
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление не доступно"
"изменение, ограничено)';uk='(додавання не доступно"
"зміна, обмежена)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ru='(добавление не доступно"
"изменение*, ограничено)';uk='(додавання не доступно"
"зміна*, обмежена)'");
				...
				Иначе // НЕ ОписаниеОбъекта.Добавление И НЕ ОписаниеОбъекта.Изменение
					УточнениеПредставленияОбъекта = НСтр("ru='(добавление не доступно"
"изменение не доступно)';uk='(додавання не доступно"
"зміна не доступна)'");
				КонецЕсли;
			Иначе
...
Функция ВидыОграниченийПравОбъектовМетаданных()
	...
	ИспользуемыеВидыДоступа = УправлениеДоступомСлужебныйПовтИсп.ТипыЗначенийВидовДоступаИВладельцевНастроекПрав(
		).Скопировать(, "ВидДоступа");
		...
		Если СвойстваВидаДоступа = Неопределено Тогда
			...
			Иначе
				Строка.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Настройки прав на %1';uk='Настройки прав на %1'"), МетаданныеВладельцаНастроекПрав.Представление());
			КонецЕсли;
