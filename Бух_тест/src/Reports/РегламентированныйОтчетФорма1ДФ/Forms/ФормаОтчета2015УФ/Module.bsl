	...
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.Подразделение) Тогда
		...
		Иначе
			...
			УсловиеНаПодразделение	= "И 
									   ...
									   |						И (    РаботникиОрганизацийСрезПоследних.ОсновноеРабочееМестоВОрганизации В ИЕРАРХИИ (&Подразделение)
			                      	   |						   ИЛИ РаботникиОрганизацийСрезПоследних.ОсновноеРабочееМестоВОрганизации = &Подразделение))
									   |	ИНАЧЕ НЕ &НеВключатьЧПников
									   |КОНЕЦ)";
...
Процедура ДозаполнитьНаКлиенте(ПараметрыЗаполненияЗвит1С)
	...
	Если ПараметрыЗаполненияЗвит1С.Свойство("ОшибкаПриЗаполнении") Тогда
		глМенеджерЗвит1С.ПредупреждениеЗвит1С(ПараметрыЗаполненияЗвит1С.ОшибкаПриЗаполнении, "Завантаження даних з 1С:Звіт");
	КонецЕсли;
