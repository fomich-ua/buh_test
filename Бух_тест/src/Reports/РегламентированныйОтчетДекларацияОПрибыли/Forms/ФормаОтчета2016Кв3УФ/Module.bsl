...
Функция ЗначениеПоПредыдущейДекларации(ДатаОкончанияПериода,ИмяПоля,ИмяПоказателяРасшНаРазныхЯзыках,ВыводитьПредупреждения )
	...
	// Предупреждения 1С:Звіт выводим только на украинсом языке. Для прочих, по умолчанию
...
Функция ПолучитьЗначениеПоказателяЗвит1С(ОтчетЗвит1С, ИмяПоляЗвит1С)
	...
	// У каждого поля в 1С:Звіт есть состояние "Неопределено". Что 
		...
		Если ПараметрыЗаполненияЗвит1С.Свойство("ПараметрыЗаполненияИзФинОтчетности", ПараметрыЗаполненияИзФинОтчетности) Тогда
			...
			Попытка
				...
				Если Данные = Неопределено Тогда
					...
					глМенеджерЗвит1С.ПредупреждениеЗвит1С(ТекстСообщения, "Завантаження даних з 1С:Звіт");
				Иначе
				R001G3 = 0;	
		...
		Если ПараметрыЗаполненияЗвит1С.Свойство("ПараметрыЗаполненияУбытокПрошлогоГода", ПараметрыЗаполненияУбытокПрошлогоГода) Тогда
			...
			Исключение
				ТекстОшибки = "Помилка алгоритму заповнення звіту!
							  |Виняткова ситуація при отриманні даних з 1С:Звіт ("+ИмяСхемы+", "+ПараметрыЗаполненияУбытокПрошлогоГода.ПараметрыЗапроса.ТипПериода+", "+ПараметрыЗаполненияУбытокПрошлогоГода.ПараметрыЗапроса.НомерПериода+", " +ПараметрыЗаполненияУбытокПрошлогоГода.ПараметрыЗапроса.ГодПериода+")
							  |
							  |"+ОписаниеОшибки();
				глМенеджерЗвит1С.ПредупреждениеЗвит1С(ТекстОшибки, "Завантаження даних з 1С:Звіт");
			КонецПопытки;
