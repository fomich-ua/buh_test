...
"в программе (насколько интенсивно и с какими объектами работают пользователи).';uk=""Дозволяє виконувати моніторинг активності користувачів"
...
"с какими объектами работал пользователь в программе.';uk=""Докладна інформація про те,"
...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации ИЗ ДанныеЖурналаРегистрации Цикл
		...
		// Расчёт количества созданных документов и справочников     
		...
		// Расчёт количества изменённых документов и справочников 		
		...
		// Расчёт количества созданных БизнесПроцессов и Задач 
		Если СтрокаДанныхЖурналаРегистрации.Событие = "_$Data$_.New" Тогда
		...
		// Расчёт количества измененных БизнесПроцессов и задач
		Если СтрокаДанныхЖурналаРегистрации.Событие = "_$Data$_.Update" Тогда
...
Функция СтрокаНеделяГода(ДатаГода)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неделя %1';uk='Тиждень %1'"), НеделяГода(ДатаГода));
КонецФункции
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 (%2)';uk='%1 (%2)'"), Элемент, СчетчикКоличестваСеансовПользователя);
			КонецЕсли;
		КонецЦикла;
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='%1 (сеанс %2)';uk='%1 (сеанс %2)'"), Элемент, МассивРегламентныхЗаданий.Получить(ИндексРегламентногоЗадания+1));
	...
	Для Каждого СтрокаРегламентныеЗадания Из ТаблицаСеансыРегламентныхЗаданий Цикл
		...
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='%1 (%2 из %3)';uk='%1 (%2 з %3)'"), Точка.Значение, 
			...
			Интервал.Текст	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='%1 - %2';uk='%1 - %2'"), Формат(Интервал.Начало, "ДЛФ=T"), Формат(Интервал.Конец, "ДЛФ=T"));
			ПризнакСменыТочки = Ложь;
			// Не заполняем расшифровку для фоновых заданий
			Если СтрокаРегламентныеЗадания.СобытиеМетаданные <> "" Тогда
		...
		Если ТекущДата <> ДатаОдновременноРегламентныхЗаданий Тогда              
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='%1 - %2';uk='%1 - %2'"), Формат(ТекущДата, "ДЛФ=T"), 
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли ПродолжительностьРЗ <= 60 Тогда
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 сек';uk=' %1 сек'"), ПродолжительностьРЗ);
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 мин %2 сек';uk='%1 хв %2 сек'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 ч %2 мин';uk='%1 год %2 хв'"), ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
