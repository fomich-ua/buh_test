...
//
Функция ПолучитьИмяФайлаСПутемКДвоичнымДанным(Знач ПрисоединенныйФайл, ПустойПутьДляПустыхДанных = Ложь) Экспорт
			...
			Иначе // Перечисления.ТипыХраненияФайлов.ВИнформационнойБаза
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						НСтр("ru='Файлы.Не удалось отправить файл при обмене данными';uk='Файли.Не вдалося відправити файл при обміні даними'",
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Файлы.Запись файла на диск';uk='Файли.Запис файлу на диск'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
	...
	Исключение
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Обновление данных присоединенного файла в хранилище файлов';uk='Файли.Оновлення даних приєднаного файлу в сховище файлів'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Функция ИзвлечьТекст(Знач ДвоичныеДанные, Знач Расширение, ИзвлеченныйТекст) Экспорт
	...
	Если ФайловыеФункцииСлужебный.ЭтоПлатформаWindows()
	   И ФайловыеФункцииСлужебный.ИзвлекатьТекстыФайловНаСервере() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Извлечение текста';uk='Файли.Витягування тексту'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
"указан несуществующий справочник ""%2"".';uk='Помилка при визначенні імен довідників для зберігання файлів."
...
"без окончания ""ПрисоединенныеФайлы"".';uk='Помилка при визначенні імен довідників для зберігання файлів."
"У власника файлів типу ""%1"""
		...
		ИначеЕсли КлючИЗначение.Значение = Истина Тогда
			Если ОсновнойСправочникУказан Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"основной справочник указан более одного раза.';uk='Помилка при визначенні імен довідників для зберігання файлів."
"У власника файлів типу ""%1"""
"основний довідник вказано більше одного разу.'"),
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"не имеется справочников для хранения файлов.';uk='Помилка при визначенні імен довідників для зберігання файлів."
"У власника файлів типу ""%1"""
...
"нет справочников для хранения файлов.';uk='У власника файлів ""%1"" типу ""%2"""
"немає довідників для зберігання файлів.'"),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокИсключения + Символы.ПС +
			НСтр("ru='У владельца файлов ""%1"" типа ""%2"""
"нет справочника ""%3"" для хранения файлов.';uk='У власника файлів ""%1"" типу ""%2"""
"немає довідника ""%3"" для зберігання файлів.'"),
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ЗаголовокИсключения + Символы.ПС +
		НСтр("ru='У владельца файлов ""%1"" типа ""%2"""
"не указан основной справочник для хранения файлов.';uk='У власника файлів ""%1"" типу ""%2"""
"не вказано основний довідник для зберігання файлів.'") +
		Символы.ПС + ?(ИмяПараметра = Неопределено, ОкончаниеОшибки,
		НСтр("ru='В этом случае параметр ""%3"" должен быть указан.';uk='В цьому випадку параметр ""%3"" повинен бути вказаний.'")),
		Строка(ВладелецФайлов),
		Строка(ТипЗнч(ВладелецФайлов)),
		Строка(ИмяПараметра));
