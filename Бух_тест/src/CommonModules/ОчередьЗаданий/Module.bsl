...
Функция ПолучитьЗадания(Знач Отбор) Экспорт
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		Отказ = Ложь;
		ИмяСправочника = СправочникЗаданий.СоздатьЭлемент().Метаданные().ПолноеИмя();
...
"Изменение параметра %2 заданий с установленным шаблоном запрещено.';uk='Завдання черги з ідентифікатором %1 створено на основі шаблону."
"Зміна параметра %2 завдань з встановленим шаблоном заборонена.'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
						Идентификатор, ОписаниеПараметра.Имя);
	...
	Попытка
		...
		Если Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
			ШаблонСообщения = НСтр("ru='Задание с идентификатором %1 к изменению не найдено. Область данных: %2';uk='Завдання з ідентифікатором %1 до зміни не знайдене. Область даних: %2'");
...
Процедура УдалитьЗадание(Идентификатор) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
		...
		Если ЗначениеЗаполнено(Задание.Шаблон) Тогда
			ШаблонСообщения = НСтр("ru='Задание очереди с идентификатором %1 создано на основе шаблона."
"Удаление заданий с установленным шаблоном запрещено.';uk='Завдання черги з ідентифікатором %1 створено на основі шаблону."
		...
		Если Отбор И ТипЗнч(КлючИЗначение.Значение) = Тип("Массив") Тогда
			...
			Для каждого ОписаниеОтбора Из КлючИЗначение.Значение Цикл
				Если ТипЗнч(ОписаниеОтбора) <> Тип("Структура") Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
						ТипЗнч(ОписаниеОтбора), КлючИЗначение.Ключ);
					ВызватьИсключение(ТекстСообщения);
				КонецЕсли;
...
"Отсутствует свойство %2.';uk='Передано неприпустимий опис відбору в колекції опису відбору %1."
"Відсутня властивість %2.'");
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
							КлючИЗначение.Ключ, ИмяКлюча);
						ВызватьИсключение(ТекстСообщения);
					КонецЕсли;
				КонецЦикла;
...
"Для вида сравнения %3 ожидается тип Массив.';uk='Переданий неприпустимий тип %1 в описі відбору в колекції опису відбору %2."
"Для виду порівняння %3 очікується тип Масив.'");
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
							ТипЗнч(ОписаниеОтбора.Значение), КлючИЗначение.Ключ, ОписаниеОтбора.ВидСравнения);
						ВызватьИсключение(ТекстСообщения);
					КонецЕсли;
...
Процедура ПроверитьЗначениеНаСоответствиеОписаниюПараметра(Знач Значение, Знач ОписаниеПараметра)
	...
	Если НЕ ОписаниеПараметра.Тип.СодержитТип(ТипЗнч(Значение)) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, 
			ТипЗнч(Значение), ОписаниеПараметра.Имя);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
...
Функция ПолучитьТекстИсключенияДублированиеЗаданийСОдинаковымКлючом() Экспорт
	...
	Возврат НСтр("ru=""Дублирование заданий с одинаковым значения поля 'Ключ' не допустимо."";uk=""Дублювання завдань з однаковим значенням поля 'Ключ' не припустимо.""");
