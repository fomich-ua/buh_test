...
//                                  Для удаления из МассивРеквизитов всех реквизитов табличной части используйте .*, например "Товары.*"
//
...
Функция ИдентификаторCOMСоединителя(Знач ИмяCOMСоединителя) Экспорт
	...
	ТекстИсключения = Нстр("ru='На задан CLSID для класса %1';uk='На заданий CLSID для класу %1'");
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	Если ЭтоLinuxСервер() Тогда
		...
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';uk='Пряме підключення до інформаційної бази недоступно на сервері під управлінням ОС Linux.'");
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	ОбщиеПараметры.Вставить("РаботаВПрограммеЗапрещена", Истина); // Блокировать запуск, если версия ниже минимальной.
...
"Рекомендуется запрещать выполнение регламентных заданий на время обновления.';uk=""Вхід в програму тимчасово неможливий у зв'язку з оновленням на нову версію."
...
"Рекомендуется запрещать выполнение регламентных заданий на время восстановления.';uk=""Вхід в програму тимчасово неможливий до відновлення зв'язку з головним вузлом."
...
Функция ПолучитьДеревоМетаданныхКонфигурации(Отбор = Неопределено) Экспорт
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовРасчета",       НСтр("ru='Планы видов характеристик';uk='Плани видів характеристик'"), БиблиотекаКартинок.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристикОбъект, КоллекцииОбъектовМетаданных);
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Неизвестный тип объекта метаданных ""%1""';uk='Невідомий тип об''єкта метаданих ""%1""'"), ПолноеИмя);
...
"""%1"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
...
""
"Идентификатор ""%1"""
"не найден в справочнике ""Идентификаторы объектов метаданных"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Ідентифікатор ""%1"""
"не знайдений у довіднику ""Ідентифікатори об''єктів метаданих"".'")
...
"""%2"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Ідентифікатору ""%1"""
"найденому в довіднику ""Ідентифікатори об''єктів метаданих"","
...
"Идентификатору ""%1"""
"найденому в справочнике ""Идентификаторы объектов метаданных"","
"соответствует удаленный объект метаданных.';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Ідентифікатору ""%1"""
"знайденому в довіднику ""Ідентифікатори об''єктів метаданих"","
"відповідає вилучений об''єкт метаданих.'")
					+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика(),
					Строка(Идентификатор));
			КонецЕсли;
		Иначе
...
"полное имя которого отличается от заданного в идентификаторе.';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Ідентифікатору ""%1"""
"найденому в довіднику ""Ідентифікатори об''єктів метаданих"","
"відповідає об''єкт метаданих ""%2"","
...
"но значение реквизита ""Пометка удаления"" установлено Истина.';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Ідентифікатор ""%1"""
"знайдений у довіднику ""Ідентифікатори об''єктів метаданих"","
"але значення реквізиту ""Позначка вилучення"" установлена Істина.'")
			+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика(),
			Строка(Идентификатор));
	КонецЕсли;
...
"Прежний тип: ""%1"","
"новый тип: ""%2"".';uk='Не збігаються імена типів перейменованого об''єкта метаданих."
"Колишній тип: ""%1"","
"новий тип: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
...
"%2.';uk='Для типу об''єкта метаданих ""%1"" не потрібно описувати перейменування,"
...
"Описание ошибки: %1';uk='Кореспондент не підтримує версіонування інтерфейсів підсистем."
"Опис помилки: %1'");
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Общий модуль ""%1"" не найден.';uk='Загальний модуль ""%1"" не знайдений.'"), Имя);
	КонецЕсли;
...
Процедура СкопироватьСтрокиВБуферОбмена(ТабличнаяЧасть, ВыделенныеСтроки, Источник = Неопределено) Экспорт
	...
	// В большинстве документов, в которых реализован функционал копирования/вставки строк есть реквизит СчетУчетаБУ и только в одном документе
	// из этого списка - ТребованиеНакладная, реквизит называется по-другому - "Счет". 
	// Чтобы была возможность правильно переносить счет учета из/в этот документ - приведем наименование к общему
...
"%1';uk='Помилка одержання файлу опису web-сервісу:"
...
"либо для него не поддерживается получение модуля менеджера.';uk='Об''єкт метаданих ""%1"" не знайдений,"
	...
	Для Каждого МестоИспользования Из РезультатРазметки.МестаИспользования Цикл
		...
		Иначе
			...
			Иначе
				...
				Текст = НСтр("ru='Неизвестный тип данных %1 для проведения замены %2';uk='Невідомий тип даних %1 для проведення заміни %2'");
...
"%1';uk=""Не вдалося заблокувати один або кілька об'єктів зі списку"
"%1""");
	...
	Для Каждого Ссылка Из СписокСсылок Цикл
		...
		Исключение
			ДобавитьРезультатЗамены(РезультатУдаления, Ссылка, 
				ОписаниеОшибкиЗамены("ОшибкаБлокировки", Ссылка, ПредставлениеСсылки, НСтр("ru='Ошибка блокирования ссылки для удаления';uk='Помилка блокування посилання для вилучення'"))
			);
		КонецПопытки
	КонецЦикла;
	...
	Для Каждого Строка Из ТаблицаПовторногоПоиска Цикл
		...
		Если ИтоговаяТаблица.НайтиСтроки(Фильтр).Количество() = 0 Тогда
			ДобавитьРезультатЗамены(ИтоговаяТаблица, Ссылка, 
				...
				НСтр("ru='Данные были добавлены или изменены другим пользователем';uk='Дані були додані або змінені іншим користувачем'"))
			);
		КонецЕсли;
	КонецЦикла;
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	Возврат НСтр("ru='Поиск и удаление ссылок';uk='Пошук і вилучення посилань'", 
...
Процедура ЗаменитьСсылкуДлиннойТранзакцией(РезультатЗамены, Знач Ссылка, Знач ПараметрыЗамены, Знач ТаблицаПоиска)
	...
	Исключение
		...
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось заблокировать все места использования %1';uk='Не вдалося заблокувати всі місця використання %1'"),
			Ссылка
		);
...
Функция ВычислитьЗначениеДанныхПоПути(Знач Данные, Знач ПутьДанных)
	...
	Для Индекс = 1 По КоличествоЧастейПути Цикл
		...
		Если Тест[ИмяРеквизита] = Неопределено Тогда
			...
			Если Тест[ИмяРеквизита] = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка вычисления %1 для объекта %2. Реквизит %3 не найден';uk=""Помилка обчислення %1 для об'єкта %2. Реквізит %3 не знайдений"""),
