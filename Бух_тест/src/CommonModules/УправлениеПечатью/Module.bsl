...
Функция ПолучитьМакетыИДанныеОбъектов(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач СоставДокументов) Экспорт
	...
	Возврат МакетыИДанныеОбъектовДляПечати(ИмяМенеджераПечати, ИменаМакетов, СоставДокументов);
	
КонецФункции

// Устарела. Следует использовать МакетПечатнойФормы.
//
Функция ПолучитьМакет(ПолныйПутьКМакету) Экспорт
	
	Возврат МакетПечатнойФормы(ПолныйПутьКМакету);
	
КонецФункции

// Устарела. Следует использовать ТабличныйДокументПоДвоичнымДанным.
//
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Редактирование", Метаданные.РегистрыСведений.ПользовательскиеМакетыПечати) Тогда
		Возврат;
	КонецЕсли;
...
// Возвращает список разрешений для загрузки классификатора банков с сайта finance.ua.
//
// Возвращаемое значение:
//  Массив.
//
Функция Разрешения()
	...
	Разрешения.Добавить( 
		РаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты("ОбщийМакет.КомпонентаПечатиQRКода", НСтр("ru='Печать QR кодов.';uk='Друк QR кодів.'"))
	);
		...
		Для Каждого ОписаниеПечатнойФормы Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не задано количество экземпляров для печатной формы ""%1"".';uk='Не задана кількість екземплярів для друкованої форми ""%1"".'"),
...
Функция СтрокаОбязательныхРеквизитов(ДанныеДокумента, ТекстСообщения)
	...
	Для Каждого Элемент Из ОбязательныеРеквизиты Цикл
		Если Не ЗначениеЗаполнено(ДанныеДокумента[Элемент.Ключ]) Тогда
			ТекстСообщения = Нстр("ru='Не заполнен обязательный реквизит: %1';uk=""Не заповнений обов'язковий реквізит: %1""");
...
"""%2""';uk='Неможливо створити QR-код для документа %1"
"Рядок обов''язкових реквізитів повинна бути менше 300 символів:"
"""%2""'");
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Печать %1 не выполнена: выбранная печатная форма недоступна.';uk='Друк %1 не виконано: обрана друкована форма недоступна.'"),
		Объект);
...
Функция ПакетДокументов(ТабличныеДокументы, ОбъектыПечати, ПечататьКомплектами, КоличествоЭкземпляров = 1) Экспорт
	...
	ПакетОтображаемыхДокументов.КоличествоЭкземпляров = КоличествоЭкземпляров;
	
	Возврат ПакетОтображаемыхДокументов;
	
КонецФункции

// Заворачивает табичный документ в пакет отображаемых документов.
...
Процедура ЗаполнитьКомандыПечатиДляСпискаОбъектов(СписокОбъектов, КомандыПечати)
	Для Каждого ОбъектМетаданных Из СписокОбъектов Цикл
		...
		Для Каждого ДобавляемаяКомандаПечати Из КомандыПечатиФормы(ИмяФормыСписка) Цикл
			...
			Отбор = Новый Структура("МенеджерПечати,Идентификатор,Обработчик,СразуНаПринтер,ФорматСохранения");
			ЗаполнитьЗначенияСвойств(Отбор, ДобавляемаяКомандаПечати);
			Если КомандыПечати.НайтиСтроки(Отбор).Количество() > 0 Тогда
...
Процедура ПриПрисоединенииПечатнойФормыКОбъекту(СсылкаНаОбъект, ИмяФайла, АдресВоВременномХранилище) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		МодульПрисоединенныеФайлы.ДобавитьФайл(СсылкаНаОбъект, ИмяФайла, , , , АдресВоВременномХранилище, , НСтр("ru='Печатная форма';uk='Друкована форма'"));
	КонецЕсли;
