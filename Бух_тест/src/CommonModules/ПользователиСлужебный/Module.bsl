...
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';uk='Виконується запуск від імені користувача з роллю ""Повні права"","
"який не зареєстрований у списку користувачів."
"Виконана автоматична реєстрація в списку користувачів."
""
"Для ведення списку й настройки прав користувачів призначений список Користувачі,"
"режим конфігурування 1С:Підприємства для цього використовувати не слід.'");
...
""
"Для ведения списка и настройки прав пользователей предназначен список Пользователи,"
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';uk='Запуск від імені користувача із правом Адміністрування неможливе,"
"тому що він не зареєстрований у списку користувачів."
""
"Для ведення списку й настройки прав користувачів призначений список Користувачі,"
"режим конфігурування 1С:Підприємства для цього використовувати не слід.'");
		КонецЕсли;
...
"Неверное значение параметра Действие: ""%1"".';uk='Помилка в процедурі ПользователиСлужебный.ОбработатьИнтерфейсРолей()"
...
"В параметре ОписаниеПользователяИБ не указано свойство Действие.';uk='Помилка при записі користувача ""%1""."
...
"неверное значение ""%2"" свойства Действие.';uk='Помилка при записі користувача ""%1""."
...
"с которым уже сопоставлен другой пользователем ИБ.';uk='Помилка при записі користувача ""%1""."
...
"""%2"".';uk='Помилка при записі користувача ""%1""."
...
"Продолжить?';uk='У список користувачів програми додається перший користувач,"
...
"Сначала создайте администратора в справочнике Пользователи.';uk='Перший користувач інформаційної бази повинен мати повні права."
...
Функция СвойстваНеуказанногоПользователя() Экспорт
	...
	Свойства.Вставить("ПолноеИмяДляПоиска", НСтр("ru='<Не указан>';uk='<Не вказано>'"));
...
"""\\ИмяДомена\ИмяПользователя"".';uk='Користувач ОС повинен бути у форматі"
...
"не найдена в метаданных.';uk='При оновленні ролей зовнішнього користувача"
"""%1"""
"роль ""%2"""
...
"не знайдена в метаданних.'"),
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Роль не найдена в метаданных';uk='Користувачі.Роль не знайдена в метаданних'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
"для другого внешнего пользователя.';uk=""Об'єкт інформаційної бази вже використається"
"для іншого зовнішнього користувача.""");
...
Функция ФормированиеОкончанияСлова(ПреобразуемоеЧисло, ПараметрыПредметаИсчисления = Неопределено) Экспорт
	...
	Если ПараметрыПредметаИсчисления = Неопределено Тогда
		ПараметрыПредметаИсчисления = НСтр("ru='пользователь,пользователя,пользователей,,,,,,0';uk='користувач,користувача,користувачів,,,,,,0'");
	КонецЕсли;
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			СообщениеПользователю = НСтр("ru='Пользователь ""%1"" не может быть включен в группу ""%2"",';uk='Користувач ""%1"" не може бути включень у групу ""%2"",'");
			СообщениеПользователю = СообщениеПользователю + Символы.ПС + 
									?(Не ТипПользователяСовпадаетСГруппой, 
									НСтр("ru='т.к. в состав ее участников входят только %3.';uk='т. к. до складу її учасників входять тільки %3.'"),
									НСтр("ru='т.к. у группы стоит признак ""Все пользователи заданного типа"".';uk='т. к. у групи коштує ознака ""Всі користувачі заданого типу"".'"));
		Иначе
			Предмет = "";
...
"или у группы стоит признак ""Все пользователи заданного типа"".';uk='Не всі користувачі можуть бути включені в групу ""%2"","
"т.к. до складу її учасників входять тільки %3"
"або в групи коштує ознака ""Всі користувачі заданого типу"".'");
		КонецЕсли;
...
Процедура ОбновитьПредопределенныеВидыКонтактнойИнформацииПользователей() Экспорт
	...
	МодульУправлениеКонтактнойИнформацией.ОбновитьВидКонтактнойИнформации(
	    ...
	    НСтр("ru='Адрес электронной почты пользователя';uk='Адреса електронної пошти користувача'"), Истина, Ложь, Ложь, 1, Истина);
	
	МодульУправлениеКонтактнойИнформацией.ОбновитьВидКонтактнойИнформации(
	    Справочники["ВидыКонтактнойИнформации"].ТелефонПользователя,
	    Перечисления["ТипыКонтактнойИнформации"].Телефон,
	    НСтр("ru='Контактный телефон пользователя';uk='Контактний телефон користувача'"), Истина, Ложь, Ложь, 2, Истина);
...
"Обратитесь к администратору.';uk='Авторизація не виконана. Робота системи буде завершена."
""
"Користувач ""%1"" не знайдений у довідниках"
...
"""%3"".';uk='Спроба змінити состав ролей у користувача %1 з роллю"
"""%2"","
"що надає права на зміну загальних даних:"
"""%3"".'"),
...
"""%3"".';uk='Спроба призначити користувачеві %1 неприпустиму роль"
"""%2"","
"що надає права на зміну загальних даних:"
"""%3"".'"),
					ПользовательИБ.ПолноеИмя,
			...
			ИмяСобытия = НСтр("ru='Пользователи.Ошибка при установке ролей пользователю ИБ';uk='Користувачі.Помилка при установці ролей користувачеві ІБ'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			Для каждого Право Из СвойстваНедоступнойРоли.Права Цикл
				...
				Если ОписаниеРоли.ЭтоУдаляемаяРоль Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Попытка изменить состав ролей у пользователя ""%1"" с ролью"
"""%2"","
"предоставляющей право ""%3"".';uk='Спроба змінити склад ролей у користувача ""%1"" з роллю"
"""%2"","
"що надає право ""%3"".'"),
				...
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Попытка назначить пользователю ""%1"" недопустимую роль"
"""%2"","
"предоставляющую право ""%3"".';uk='Спроба призначити користувачеві ""%1"" неприпустиму роль"
"""%2"","
"що надає право ""%3"".'"),
						Строка(ПользовательИБ),
						Роль.Представление(),
	...
	Если УдаляемыеРоли.Количество() = 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно удалить недоступную роль у пользователя ""%1"".';uk='Неможливо видалити недоступну роль у користувача ""%1"".'"),
			ПользовательИБ.ПолноеИмя);
	ИначеЕсли УдаляемыеРоли.Количество() > 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно удалить недоступные роли у пользователя ""%1"".';uk='Неможливо видалити недоступні ролі у користувача ""%1"".'"),
			ПользовательИБ.ПолноеИмя);
	КонецЕсли;
...
"""%2"".';uk='Користувачу ""%1"" не може бути додана роль"
"""%2"".'"),
			ПользовательИБ.ПолноеИмя,
	...
	ИначеЕсли ДобавляемыеРоли.Количество() > 1 Тогда
		...
		КонецЦикла;
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" не могут быть добавлены роли:%2.';uk='Користувачу ""%1"" не можуть бути додані ролі:%2.'"), 
			ПользовательИБ.ПолноеИмя,
			Роли);
	КонецЕсли;
...
Процедура ПроверитьИзмененияРеквизитовПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи")
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1""."
"Реквизит Служебный не допускается изменять в подписках на события.';uk='Помилка при записі користувача ""%1""."
"Реквізит Службовий не допускається змінювати в підписках на події.'"),
	...
	Если БлокируемыеРеквизиты.Подготовлен <> ПользовательОбъект.Подготовлен Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1""."
"Реквизит Подготовлен не допускается изменять в подписках на события.';uk='Помилка при записі користувача ""%1""."
...
"Обновление реквизита выполняется автоматически.';uk='Помилка при записі користувача ""%1""."
...
"Оновлення реквізиту виконується автоматично.'"),
	...
	Если СтарыйПользователь.ПометкаУдаления = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1""."
"Нельзя помечать на удаление пользователя, которому разрешен вход в программу.';uk='Помилка при записі користувача ""%1""."
"Не можна відмічати для вилучення користувача, якому дозволений вхід у програму.'"),
	...
	Если СтарыйПользователь.Недействителен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1""."
"Нельзя пометить недействительным пользователя, которому разрешен вход в программу.';uk='Помилка при записі користувача ""%1""."
"Не можна позначити недійсним користувача, якому дозволений вхід у програму.'"),
	...
	Если СтарыйПользователь.Подготовлен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1""."
"Нельзя пометить подготовленным пользователя, которому разрешен вход в программу.';uk='Помилка при записі користувача ""%1""."
