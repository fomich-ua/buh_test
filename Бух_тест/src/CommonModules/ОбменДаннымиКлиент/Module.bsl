...
Процедура ОткрытьПомощникНастройкиОбменаДанными(Знач ИмяПланаОбмена, НастройкаОбменаССервисом = Ложь) Экспорт
	...
	Если Найти(ИмяПланаОбмена, "ИдентификаторНастройки") > 0 Тогда
		
		ЧислоСимволовИмяПланаОбмена = Найти(ИмяПланаОбмена, "ИдентификаторНастройки") - 1;
...
//     ИмяСвойства           - Строка             - Имя свойства с именем файла, устанавливаемого в объекте. Источник начального значения.
//     СтандартнаяОбработка  - Булево             - Флаг стандартной обработки, устанавливается в Ложь
//     ПараметрыДиалога      - Структура          - Необязательные дополнительные параметры диалога выбора каталога
//     ОповещениеЗавершения  - ОписаниеОповещения - Необязательное оповещение, которое будет вызвано со следующими параметрами:
//                                 Результат               - Строка - Выбранное значение (массив строк, если был множественный выбор);
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	ТекстПредупреждения = НСтр("ru='Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для цієї операції необхідно встановити розширення для веб-клієнта 1С:Підприємство.'");
...
//                                Результат               - Массив - Содержит структуры с полями Имя, Хранение, ОписаниеОшибки для каждого файла
//                                ДополнительныеПараметры - Неопределено
//
//     ИменаФайлов          - Массив                  - Массив имен файлов для передачи
//     ИдентификаторФормы   - УникальныйИдентификатор - Параметр для хранилища
//     ТекстПредупреждения  - Строка                  - Текст предупреждения о необходимости установки расширения для работы с файлами
//
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru='Для передачи файла на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для передачі файлу на сервер необхідно встановити розширення для веб-клієнта 1С:Підприємство.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для передачі файлів на сервер необхідно встановити розширення для веб-клієнта 1С:Підприємство.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередатьФайлНаСерверЗавершениеПодключенияРасширения(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ДанныеФайлов Цикл
		...
		Если Не Файл.Существует() Или Файл.ЭтоКаталог() Тогда
			БылиОшибки = Истина;
			Элемент.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл ""%1"" не существует или к нему нет доступа';uk='Файл ""%1"" не існує або до нього немає доступу'"), ИмяФайла);
		Иначе
			СписокДляПередачи.Добавить( Новый ОписаниеПередаваемогоФайла(ИмяФайла) );
		КонецЕсли;
	...
	Если Не БылиОшибки Тогда
		...
		ПоместитьФайлы(СписокДляПередачи, ПомещенныеФайлы, , Ложь, ДополнительныеПараметры.ИдентификаторФормы);
		
		Для Индекс = 0 По ПомещенныеФайлы.ВГраница() Цикл
			ДанныеФайлов[Индекс].Хранение = ПомещенныеФайлы[Индекс].Хранение;
		КонецЦикла;
	КонецЕсли;
	
	// Оповещаем исходного вызывающего
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.Оповещение, ДанныеФайлов);
КонецПроцедуры

// Передает файл на сервер интерактивно, без расширения для работы с файлами
//
// Параметры:
//     ОповещениеЗавершения - ОписаниеОповещения - Экспортная процедура, которая будет вызвано со следующими параметрами:
...
Процедура ВыбратьИПередатьФайлНаСервер(ОповещениеЗавершения, Знач ПараметрыДиалога = Неопределено, Знач ИдентификаторФормы = Неопределено) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ВыбратьИПередатьФайлыНаСерверЗавершение", ЭтотОбъект, Новый Структура);
	Оповещение.ДополнительныеПараметры.Вставить("ОповещениеЗавершения", ОповещениеЗавершения);
	...
	Если Не ПодключитьРасширениеРаботыСФайлами() Тогда
		...
		НачатьПомещениеФайла(Оповещение, , , Истина, ИдентификаторФормы);
		Возврат;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(ДиалогВыбора, ПараметрыДиалога);
	
	Если ДиалогВыбора.Выбрать() Тогда
		НачатьПомещениеФайла(Оповещение, , ДиалогВыбора.ПолноеИмяФайла, Ложь, ИдентификаторФормы);
	КонецЕсли;
...
Процедура ВыбратьИСохранитьФайлНаКлиенте(Знач ПолучаемыйФайл, Знач ПараметрыДиалога = Неопределено) Экспорт
	...
	ЗаполнитьЗначенияСвойств(ДиалогСохранения, ПараметрыДиалога);
	
	Получаемые = Новый Массив;
	Получаемые.Добавить( Новый ОписаниеПередаваемогоФайла(ПолучаемыйФайл.Имя, ПолучаемыйФайл.Хранение) );
	
	Полученные = Новый Массив;
	ПолучитьФайлы(Получаемые, Полученные, ДиалогСохранения, Истина);
КонецПроцедуры
...
//
Процедура УстановитьЗначенияСтруктурыПоУмолчанию(Результат, Знач ЗначенияПоУмолчанию) Экспорт
...
Процедура ВыполнитьОбменДаннымиОбработкаКоманды(УзелИнформационнойБазы, Владелец,
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ОбменДаннымиXDTO") Тогда
			
			МодульОбменДаннымиXDTOКлиентПовтИсп = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбменДаннымиXDTOКлиентПовтИсп");
			Если МодульОбменДаннымиXDTOКлиентПовтИсп.ЭтоПланОбменаXDTO(УзелИнформационнойБазы) Тогда
				ИмяФормы = "Обработка.ПомощникИнтерактивногоОбменаДаннымиЧерезXDTO.Форма";
			КонецЕсли;
...
Процедура ОбработчикОткрытияФормыПослеЗакрытияТекущей(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму(ДополнительныеПараметры.ИмяФормы, ДополнительныеПараметры.Параметры,
		ПараметрыОткрытия.Владелец, ПараметрыОткрытия.Уникальность, ПараметрыОткрытия.Окно, ПараметрыОткрытия.НавигационнаяСсылка, ПараметрыОткрытия.ОписаниеОповещенияОЗакрытии, ПараметрыОткрытия.РежимОткрытияОкна
	);
...
// Открывает форму сообщения о неудачном обновлении при ошибке в ПРО при выполнении обновления информационной базы
//
Процедура ИмяФормыСообщенияОНеудачномОбновлении(ИмяОткрываемойФормы) Экспорт
	
	ИмяОткрываемойФормы = "РегистрСведений.ПравилаДляОбменаДанными.Форма.СообщениеОНеудачномОбновлении";
...
Процедура ВыполнитьОбновлениеИнформационнойБазы(ЗавершениеРаботыСистемы = Ложь) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
		...
		МодульОбновлениеКонфигурацииКлиент.ПриВыполненииОбновленияИнформационнойБазы(ЗавершениеРаботыСистемы);
	Иначе
...
Процедура ЗаполнитьДанныеСтруктуры(Форма)
	...
	СоответсвующиеРеквизиты = Форма.ИменаРеквизитов;
	...
	СоответсвующиеРеквизиты = Форма.ИменаРеквизитовБазыКорреспондента;
