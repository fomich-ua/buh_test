...
Процедура ВыполнитьНазначаемуюКомандуНаСервере(Форма, ИмяЭлемента, РезультатВыполнения) Экспорт
	...
	Если Найденные.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Команда не найдена';uk='Команда не знайдена'");
	КонецЕсли;
...
"Для включения подсистемы обратитесь к администратору программы.';uk='Елемент ""%1"" не підключений, тому що підсистема ""%2"" відключена у настройках програми."
...
Функция НазначаемыеОбъектыМетаданныхПоВидуВнешнегоОбъекта(Вид) Экспорт
	...
	Для Каждого ТипИлиМетаданные Из МассивТиповИлиМетаданных Цикл
		...
		Если ОбъектМетаданных = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола() Тогда
			...
			НовоеНазначение.Представление = НСтр("ru='Рабочий стол';uk='Робочий стіл'");
		Иначе
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Команда %1 не обнаружена!';uk='Команда %1 не виявлена!'"), ИдентификаторКоманды);
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.БезопасныйРежим
		...
		// ничего не делаем
	Иначе
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.';uk='Для підключення обробки, що запускається в небезпечному режимі, потрібні адміністративні права.'");
		Возврат Результат;
	КонецЕсли;
	
	Если НЕ Объект.ЭтоНовый() И РегистрационныеДанные.Вид <> Объект.Вид Тогда
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='Невозможно сменить вид дополнительного отчета или обработки.';uk='Неможливо змінити вид додаткового звіту чи обробки.'");
		Возврат Результат;
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> (РегистрационныеДанные.Вид = ВидДополнительныйОтчет ИЛИ РегистрационныеДанные.Вид = ВидОтчет) Тогда
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';uk='Вид обробки, зазначений у відомостях про зовнішню обробку, не відповідає її розширенню.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрЗаменить(НСтр("ru='Для команды ""%1"" не определен способ запуска.';uk='Для команди ""%1"" не визначений спосіб запуску.'"), "%1", КомандаОписание.Представление));
		КонецЕсли;
...
"Возможно, он не подходит для этой версии программы.';uk='Неможливо підключити додатковий звіт з файлу."
...
"Возможно, она не подходит для этой версии программы.';uk='Неможливо підключити додаткову обробку з файлу."
