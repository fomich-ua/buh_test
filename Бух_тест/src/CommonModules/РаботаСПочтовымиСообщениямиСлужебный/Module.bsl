...
Процедура ПроверитьВозможностьОтправкиТестовогоСообщения(Знач УчетнаяЗапись, Знач Пароль = Неопределено)
	...
	ПараметрыПисьма.Вставить("Тело", НСтр("ru='Это сообщение отправлено подсистемой электронной почты 1С:Предприятие';uk='Це повідомлення відправлене підсистемою електронної пошти 1С:Підприємство'"));
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ru='Проверка учетной записи электронной почты';uk='Перевірка облікового запису електронної пошти'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
