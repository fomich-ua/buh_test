...
"- Версия конфигурации не предусматривает обновление без установки монопольного режима.';uk='Неможливо виконати оновлення інформаційної бази:"
...
"  после обновления параметров работы программы.';uk='Неприпустиме звертання до неоновлених параметрів роботи програми"
...
"Требуется запретить авторегистрацию для следующих объектов метаданных: %2';uk=""У складі плану обміну %1 не повинне бути об'єктів із установленою ознакою авторегістрації."
...
"  выполнились процедуры обновления данных информационной базы.';uk='Для розробника: можливо потрібно оновити допоміжні дані,"
"які впливають на роботу програми. Для виконання оновлення можна:"
...
"Подробности см. в журнале регистрации.';uk=""Для обов'язкових подій не визначені обробники."
...
"быть выполнено в разделенном режиме модели сервиса.';uk='Оновлення параметрів роботи програми не може"
"бути виконано в розділеному режимі моделі сервісу.'");
	КонецЕсли;
...
"- Версія конфігурації не передбачає оновлення без установки монопольного режиму.'");
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';uk=""Ідентифікатори об'єктів метаданих.Потрібно завантажити критичні зміни""",
...
"регистрации в событии ""Идентификаторы объектов метаданных.Требуется загрузить критичные изменения"").';uk='З головного вузла не завантажені зміни довідника ""Ідентифікатори об''єктів метаданих"":"
...
"""%1"".';uk='Неприпустимо відмічати для вилучення напередвизначений елемент:"
"""%1"".'"),
...
"""%1"".';uk='Неприпустимо пов''язувати з ім''ям напередвизначеного елемент, який відмічений для вилучення:"
"""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
"""%1"".';uk='Неприпустимо видаляти напередвизначений елемент"
"""%1"".'"),
		Строка(Источник.Ссылка));
...
"Необходимо использовать вариант языка ""%2"".';uk='Варіант убудованої мови конфігурації ""%1"" не підтримується."
"Необхідно використовувати варіант мови ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.4.365") < 0 Тогда
		ВызватьИсключение НСтр("ru='Для запуска необходима версия платформы 1С:Предприятие 8.3.4.365 или выше.';uk='Для запуску необхідна версія платформи 1С:Підприємство 8.3. 4.365 або вище.'");
	КонецЕсли;
...
"версии не ниже 8.3.4 или ""Не использовать"".';uk='Режим сумісності конфігурації з 1С:Підприємством версії %1 не підтримується."
"Для запуску встановіть в конфігурації режим сумісності з 1С:Підприємством"
"версії не нижче 8.3.4 або ""Не використовувати"".'"),
			НедоступныйРежим);
	КонецЕсли;
...
"Правильный формат, например: ""2.1.3.70"".';uk='Не правильно заповнена властивість конфігурації Версія: ""%1""."
"Правильний формат, наприклад: ""2.1.3.70"".'"),
		...
		Если НулеваяВерсия Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Версия не может быть нулевой.';uk='Не правильно заповнена властивість конфігурації Версія: ""%1""."
...
"АдминистраторСистемы и ПолныеПрава или указаны лишние роли.';uk='У конфігурації у властивості ОсновныеРоли не зазначені стандартні ролі"
...
""
"Событие ""%1"""
"уже добавлено.';uk='Помилка при підготовці списку подій."
""
"Подія ""%1"""
"уже додано.'"),
				Событие);
...
"""%1""."
""
"Ошибка в имени модуля ""%2"".';uk='Помилка при підготовці обробників події"
"""%1""."
""
...
"""%1""."
""
"Модуль ""%2"" уже добавлен.';uk='Помилка при підготовці обробників події"
"""%1""."
""
...
"""%3"".';uk='Помилка при підготовці обробників події"
"""%1""."
""
