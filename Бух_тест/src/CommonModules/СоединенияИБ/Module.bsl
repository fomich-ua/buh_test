...
Процедура ПриДобавленииПараметровРаботыКлиентскойЛогикиСтандартныхПодсистемПриЗапуске(Параметры) Экспорт
	...
	Иначе
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='с %1';uk='з %1'"), ТекущийРежим.Начало);
	КонецЕсли;
...
""
"Приложение временно недоступно.';uk='Адміністратором прикладної програми встановлено блокування роботи користувачів %1 %2."
""
"Прикладна програма тимчасово недоступна.'"),
	...
	Если Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Администратором приложения установлена блокировка работы пользователей %1 %2."
""
"Войти в заблокированное приложение?';uk='Адміністратором прикладної програми встановлено блокування роботи користувачів %1 %2."
""
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ПараметрыБлокировки.Установлена Тогда
		Если ТекущаяДатаСеанса < ПараметрыБлокировки.Начало Тогда
			...
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Запланирована с %1';uk='Запланована з %1'"), 
		...
		ИначеЕсли ПараметрыБлокировки.Конец <> Дата(1, 1, 1) И ТекущаяДатаСеанса > ПараметрыБлокировки.Конец И ПараметрыБлокировки.Начало <> Дата(1, 1, 1) Тогда
			Важность = Ложь;
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не действует (истек срок %1)';uk='Не діє (закінчився строк %1)'"), 
...
"Для того щоб дозволити роботу користувачів, скористайтеся консоллю кластера серверів або запустіть ""1С:Підприємство"" з параметрами:"
