...
//
Функция СформироватьПредставлениеВерсии(ОписаниеВерсии) Экспорт
...
Функция ВыбратьЦепочкуТрансляции(Знач ЦепочкиТрансляции) Экспорт
...
Функция ВыполнитьТрансляцию(Знач ИсходныйОбъект, Знач ОписаниеИсходнойВерсии, Знач ОписаниеРезультирующейВерсии) Экспорт
	...
	Иначе
		...
		Для Каждого ЭтапТрансляцииИнтерфейса Из ЦепочкаТрансляцииИнтерфейса Цикл
			...
			Разряды = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Версия, ".");
			Итератор = 0;
			ВерсияЧислом = 0;
...
//
Функция ПолучитьЦепочкуТрансляции(Знач ОписаниеИсходнойВерсии, Знач ОписаниеРезультирующейВерсии) Экспорт
...
//
Функция ПолучитьЗависимостиПакета(Знач ПакетОбъектаСообщения) Экспорт
...
Функция ТранслироватьОбъект(Знач Объект, Знач ПравилаТрансляцииИнтерфейсов)
	...
	Если ЦепочкаТрансляцииИнтерфейса = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось определить обработчик трансляции для пакета {%1}, невозможно выполнение стандартной трансляции для обработки данного свойства';uk='Не вдалося визначити обробник трансляції для пакета {%1}, неможливе виконання стандартної трансляції для обробки цієї властивості'"), 
...
Функция СтандартнаяОбработка(Знач Объект, Знач ПакетРезультирующегоОбъекта, Знач ПравилаТрансляцииИнтерфейсов)
	...
	Для Каждого Свойство Из ТипРезультирующегоОбъекта.Свойства Цикл
		...
		Если СвойствоОригинала = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %1!';uk='Не вдалося виконати стандартну обробку конвертації типу %1 у тип %2: властивість %3 не визначено для типу %1!'"),
				"{" + ТипИсходногоОбъекта.URIПространстваИмен + "}" + ТипИсходногоОбъекта.Имя,
	...
	Для Каждого Свойство Из ТипИсходногоОбъекта.Свойства Цикл
		...
		Если ТранслируемоеСвойство = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %2!';uk='Не вдалося виконати стандартну обробку конвертації типу %1 у тип %2: властивість %3 не визначено для типу %2!'"),
				"{" + ТипИсходногоОбъекта.URIПространстваИмен + "}" + ТипИсходногоОбъекта.Имя,
				"{" + ТипРезультирующегоОбъекта.URIПространстваИмен + "}" + ТипРезультирующегоОбъекта.Имя,
				Свойство.ЛокальноеИмя);
		КонецЕсли;
