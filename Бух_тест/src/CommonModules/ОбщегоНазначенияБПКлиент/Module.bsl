...
// 	Пока версия ознакомительная, не предупреждаем
	Возврат;
...
""
"';uk='Версія конфігурації, яка зараз використовується, була випущена більше двох місяців тому."
""
"За цей час випущена нова версія, в якій відображені зміни законодавства і форм звітності. Нові версії всіх конфігурацій розміщуються на диску ІТС та на користувацькому сайті."
""
"'");
	...
	Если ПараметрыРаботыКлиента.ЭтоАдминистраторСистемы И ПараметрыРаботыКлиента.ЭтоГлавныйУзел Тогда
		...
		#Если ВебКлиент Тогда
			Параметры.Вставить("ТекстСообщения", ТекстСообщения + НСтр("ru='Обновление конфигурации недоступно в веб-клиенте, рекомендуется открыть программу в тонком клиенте.';uk='Оновлення конфігурації недоступне у веб-клієнті, рекомендується відкрити програму в тонкому клієнті.'"));
			ОткрытьФорму("ОбщаяФорма.НерекомендуемаяВерсияКонфигурации", Параметры);
			Возврат;
		#КонецЕсли
		
		Параметры.Вставить("ТекстСообщения", ТекстСообщения + НСтр("ru='Вы можете обновить конфигурацию прямо сейчас.';uk='Ви можете оновити конфігурацію прямо зараз.'"));
		Параметры.Вставить("ОткрытьОбновлениеПрограммы", Истина);
	Иначе
		Параметры.Вставить("ТекстСообщения", ТекстСообщения + НСтр("ru='Для обновления конфигурации обратитесь к ответственному за обновление.';uk='Для оновлення конфігурації зверніться до відповідального за оновлення.'"));
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// Предложение загружить данные из 1С:Предприятия 7.7
