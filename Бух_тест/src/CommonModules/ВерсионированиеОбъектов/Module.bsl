...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Обновление сведений о записанных версиях объектов.';uk=""Оновлення відомостей про записані версії об'єктів.""");
...
"%3';uk='Не вдалося оновити відомості про версії №%1 об''єкту ""%2"" з причини:"
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1';uk='Процедурі ОбновитьСведенияОВерсияхОбъектов не вдалося обробити деякі записи регістра відомостей ВерсииОбъектов (пропущені): %1'"), 
					Выборка.Количество());
...
Процедура ЗаписатьВерсиюПоверхПредыдущей(Ссылка, Объект, НомерВерсии, СведенияОВерсииОбъекта)
	...
	Если НаборЗаписей.Количество() = 0 Тогда
		...
		ДанныеВерсии.Вставить("Комментарий", НСтр("ru='Версия создана при синхронизации данных.';uk='Версія створена при синхронізації даних.'"));
...
"Техническая информация об ошибке: %1';uk=""Не вдалося перейти на обрану версію."
...
Процедура ДобавлениеСведенийОВерсииОбъектаПриОбменеДанными(Объект, АвторВерсии)
	...
	Если ТипЗнч(Объект) <> Тип("УдалениеОбъекта") И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(Объект.Метаданные()) Тогда
		...
		СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru='Версия получена при синхронизации данных.';uk='Версія отримана при синхронізації даних.'"));
...
"%1';uk=""У реєстрі зберігаються настройки для неіснуючого об'єкта метаданих"
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 %2';uk='%1 %2'"), РазмерДанных, ЕдиницаИзмерения);
...
Функция ПолучитьПредставлениеРеквизитаНаЯзыке(Знач ИмяРеквизита) Экспорт
	...
	ИначеЕсли ИмяРеквизита = "Posted" Тогда
		Возврат НСтр("en='Posted';ru='Проведен';uk='Проведено'");
	...
	ИначеЕсли ИмяРеквизита = "Parent" Тогда
		Возврат НСтр("en='Parent';ru='Родитель';uk='Батьківський елемент'");
