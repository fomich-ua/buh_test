...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа, ДобавитьКоличествоЗначений = Истина) Экспорт
	...
	Иначе
		...
		ПрописьЧислаИПредмета = ЧислоПрописью(
			КоличествоЗначений,
			"Л = ru_RU",
			НСтр("ru='-го значения,-х значений,-и значений,,,,,,0';uk='-го значення,-х значень,-і значень,,,,,,0'"));
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Форма.НадписьВидДоступа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонНадписи, Строка(ТекущиеДанные.ВидДоступаПредставление));
...
Процедура ЗаполнитьСвойстваВидовДоступаВФорме(Форма) Экспорт
	...
	Для каждого Строка Из Параметры.ВидыДоступа Цикл
...
Процедура ОбработкаПроверкиЗаполненияНаСервереФормыРедактированияРазрешенныхЗначений(
	...
	ОтборВидовДоступа = ОтборВТаблицахФормыРедактированияРазрешенныхЗначений(
		Форма);
	...
	Пока НЕ Отказ И ИндексВидаДоступа >= 0 Цикл
		...
		Пока НЕ Отказ И ИндексЗначенияДоступа >= 0 Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаЗначенияДоступа.ЗначениеДоступа) Тогда
