...
"он не подключен к подсистеме ""%2""';uk='Помилка одержання опису звіту ""%1"","
"він не підключений до підсистеми ""%2""'"),
...
"Поиск отчетов временно недоступен.';uk='Повне оновлення індексу пошуку звітів, передбачених у програмі."
...
"Поиск отчетов временно недоступен.';uk='Повне оновлення індексу пошуку звітів, збережених користувачами."
"Пошук звітів тимчасово недоступний.'");
...
Функция ПодключитьОтчетОбъект(СсылкаВариантаИлиОтчета) Экспорт
	...
	Если Результат.Ссылка = Неопределено Тогда
		Результат.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вариант отчета ""%1"" не найден в программе';uk='Варіант звіту ""%1"" не знайдений у програмі'"),
			Строка(СсылкаВариантаИлиОтчета));
		Возврат Результат;
	ИначеЕсли ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для варианта ""%1"" отчет ""%2"" записан как внешний и не может быть подключен из программы';uk='Для варіанта ""%1"" звіт ""%2"" записаний як зовнішній і не може бути підключений з програми'"),
			Строка(СсылкаВариантаИлиОтчета),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если Результат.Метаданные = Неопределено Тогда
		Результат.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для варианта ""%1"" отчет ""%2"" не найден в программе';uk='Для варіанта ""%1"" звіт ""%2"" не знайдений у програмі'"),
			Строка(СсылкаВариантаИлиОтчета),
			Результат.Имя);
		Возврат Результат;
	КонецЕсли;
...
Процедура СформироватьОтчет(ПараметрыФормированияОтчета, АдресХранилища) Экспорт
	...
	Если ОтчетОбъект = Неопределено Тогда
		ОшибкаПоВарианту(Неопределено, НСтр("ru='Не удалось получить объект отчета ""%1""';uk='Не вдалося одержати об''єкт звіту ""%1""'"), ОтчетСсылка);
		Возврат;
	КонецЕсли;
...
Функция ДеревоНастроекВариантовОтчетовКонфигурации() Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.СистемнаяИнформация.ПостроенНаСКД Тогда
			...
			//	Исключение
			//		ТекстОшибки = НСтр("ru='Не удалось прочитать настройки по умолчанию отчета ""%1"":';uk='Не вдалося прочитати настройки по умовчанню звіту ""%1"":'");
			//		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", ОтчетМетаданные.Имя);
			//		ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			//		ПредупреждениеПоВарианту(Неопределено, ТекстОшибки);
			//	КонецПопытки;
...
"так же числится как старое имя ""%4"" (актуальное имя ""%5"").';uk='Помилка реєстрації змін імені варіанта звіту ""%1"":"
...
"варианта отчета ""%4"" (актуальное имя ""%5"").';uk='Помилка реєстрації змін імені варіанта звіту ""%1"":"
...
Процедура ЗавершениеПроцедуры(ПредставлениеПроцедуры)
	...
	ИнформацияПоВарианту(Неопределено, НСтр("ru='Завершение процедуры ""%1"".';uk='Завершення процедури ""%1"".'"), ПредставлениеПроцедуры);
КонецПроцедуры
...
Процедура ОтложенноеОбновлениеОбщихДанных(Полное)
	...
	КонецЕсли;
	Если Полное Тогда
		ПредставлениеПроцедуры = НСтр("ru='Отложенное обновление общих данных (полное)';uk='Відкладене оновлення загальних даних (повне)'");
	Иначе
		ПредставлениеПроцедуры = НСтр("ru='Отложенное обновление общих данных (по изменениям)';uk='Відкладене оновлення загальних даних (по змінам)'");
	КонецЕсли;
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОтложенноеПолноеОбновлениеРазделенныхДанных(Параметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Отложенное обновление разделенных данных (полное)';uk='Відкладене оновлення розділених даних (повне)'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = НСтр("ru='Обновить ключи вариантов отчетов';uk='Оновити ключі варіантів звітів'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = НСтр("ru='Обновить предопределенные';uk='Оновити напередвизначені'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьСоставОбласти()
	...
	ПредставлениеПроцедуры = НСтр("ru='Обновить разделенные вариантов отчетов';uk='Оновити розділені варіантів звітів'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш, ОбщиеДанные)
	...
	Если ОбщиеДанные Тогда
		Кэш.ОбновитьОбласти = Истина;
		Суффикс = НСтр("ru='общие данные';uk='загальні дані'");
	Иначе
		Суффикс = НСтр("ru='область данных';uk='область даних'");
	КонецЕсли;
	
	ПредставлениеПроцедуры = НСтр("ru='Удаление вариантов удаленных отчетов';uk='Вилучення варіантів вилучених звітів'") + " (" + Суффикс + ")";
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
"Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".';uk='Для звіту ""%2"" не заповнені настройки пошуку: найменування полів або параметрів і відборів."
"Детальніше - див. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".'");
	...
	Если СхемаКД = Неопределено Тогда
		Если Не ЗначениеЗаполнено(ВариантОбъект.НаименованияПолей)
			...
			ТекстОшибки = НСтр("ru='Для варианта ""%1"" отчета ""%2"" не заполнены настройки поиска: наименования полей или параметров и отборов.';uk='Для варіанта ""%1"" звіту ""%2"" не заповнені настройки пошуку: найменування полів або параметрів і відборів.'");
