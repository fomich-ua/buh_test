...
//
// Параметры:
//  ВидыОпераций - Структура, описывающая значения перечисления ОперацииСНаборамиРазрешений:
//                   * Ключ - Строка - имя значения перечисления,
//                   * Значение - ПеречислениеСсылка.ОперацииСНаборамиРазрешений,
...
//  ФормаВладелец - УправляемаяФорма или Неопределено - форма, по отношению которой мастер открывается в псевдо-модальном
//    режиме,
...
//  РежимОтключения - Булево - флаг того, что мастер вызывается при выполнении отключения использования для информационной
//    базы профилей безопасности,
...
//  РежимПроверки - Булево - флаг того, что мастер вызывается для проверки завершения операция, в рамках которой до этого
//    были применены запросы на использование внешних ресурсов.
//
// Результатом операции является открытие формы
// "Обработка.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.Форма.ИнициализацияЗапросаРазрешений", для которой в качестве
// описания оповещения о закрытии установлена процедура ПослеИнициализацииЗапросаРазрешенийНаИспользованиеВнешнихРесурсов().
//
Процедура НачатьИнициализациюЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(
		...
		ОповещениеОЗакрытии,
		РежимВключения = Ложь,
		РежимОтключения = Ложь,
		РежимВосстановления = Ложь,
		РежимПроверки = Ложь) Экспорт
...
//
// Параметры:
//  Результат - КодВозвратаДиалога - результат выполнения предыдущей операции мастера применения разрешений на использование
//    внешних ресурсов (используемые значения - ОК и Отмена),
//  Состояние - Структура, описывающая состояние мастера настройки разрешений (см. СостояниеЗапросаРазрешенийНаИспользованиеВнешнихРесурсов))).
//
...
// в качестве описания оповещения о закрытии установлена процедура ПослеНастройкиРазрешенийНаИспользованиеВнешнихРесурсов() или аварийное
...
// Параметры:
//  Результат - КодВозвратаДиалога - результат выполнения предыдущей операции мастера применения разрешений на использование
//    внешних ресурсов (используемые значения - ОК, Пропустить и Отмена). Значение Пропустить используется в том случае, если
//    внесение изменений в настройки профилей безопасности не вносилось, но запросы на использование внешних ресурсов должны
//    считаться успешно примененными (например, в том случае, если использование всех запрашиваемых внешних ресурсов ранее уже
//    предоставлялось),
//  Состояние - Структура - описывающая состояние мастера настройки разрешений (см. СостояниеЗапросаРазрешенийНаИспользованиеВнешнихРесурсов))).
//
// Результатом операции является открытии формы
// "Обработка.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.Форма.ЗавершениеЗапросаРазрешений", для которой
// в качестве описания оповещения о закрытии установлена процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов() или аварийное
// прерывание работы мастера.
//
...
Процедура ЗапланироватьПроверкуПримененияРазрешенийПослеЗакрытияФормыВладельца(ВладелецФормы, ИдентификаторыЗапросов) Экспорт
	...
	Если ТипЗнч(ВладелецФормы) = Тип("УправляемаяФорма") Тогда
		...
		Состояние.ИдентификаторыЗапросов = ИдентификаторыЗапросов;
...
// Выполняет оригинальное описание оповещения о закрытии формы-владельца, вместо которого выполнялся
// запуск мастера настройки разрешений на использование внешних ресурсов в режиме проверки завершения операции,
// в рамках которой ранее применялись запросы разрешений на использование внешних ресурсов.
//
// Параметры:
//  Результат - Произвольный, результат закрытия форм мастера настройки разрешений на использование внешних
//    ресурсов. В теле процедуры не используется, параметр требуется для назначения процедуры в качестве описания
//    оповещения о закрытия формы.
//  Состояние - Структура - описывающая состояние проверки завершения операции (см.
//    СостояниеПроверкиПримененияРазрешенийПослеЗакрытияФормыВладельца()).
//
// Результатом выполнения данной процедуры является завершение процесса проверки завершения операции, в рамках
//  которой ранее применялись разрешения на использование внешних ресурсов.
//
Процедура ПослеПроверкиПримененияРазрешенийПослеЗакрытияФормыВладельца(Результат, Состояние) Экспорт
...
Функция СостояниеПроверкиПримененияРазрешенийПослеЗакрытияФормыВладельца()
	...
	// Идентификаторы запросов на использование внешних ресурсов, при применении которых
	// были предоставлены разрешения в профилях безопасности в кластере серверов (и для которых
	// выполняется проверка завершения операции, в рамках которой было применены запросы на
	// использование внешних ресурсов) - Массив(УникальныйИдентификатор).
	Результат.Вставить("ИдентификаторыЗапросов", Новый Массив());
...
Функция ДлительностьОжиданияПримененияИзменений()
	...
	Возврат 20; // Интервал обновления rphost'ами текущих настроек профилей безопасности из rmngr.
