...
Функция НачалоЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые данные.Календарные графики';uk='Дані, які поставляються.Календарні графіки'", 
			Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный формат файла данных. Ожидается начало элемента %1';uk='Невірний формат файлу даних. Очікується початок елементу %1'"), Имя));
		Возврат Ложь;
	КонецЕсли;
...
Функция КонецЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.КонецЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые данные.Календарные графики';uk='Дані, які поставляються.Календарні графіки'", 
			Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный формат файла данных. Ожидается конец элемента %1';uk='Невірний формат файлу даних. Очікується кінець елемента %1'"), Имя));
		Возврат Ложь;
	КонецЕсли;
