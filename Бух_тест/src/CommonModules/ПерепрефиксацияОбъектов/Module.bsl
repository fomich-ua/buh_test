...
//  НачалоПериодаПерепрефиксации - Дата - Дата, начиная с которой будет установлен новый префикс ИБ у документов, бизнес-процессов и задач.
...
//  ОбъектовОбработано - Число - В этот параметр возвращается количество объектов, для которых был изменен номер или код.
//
...
Процедура СлужебнаяУстановитьПрефиксИБИПерепрефиксоватьОбъектыСозданныеВЭтойИБ(
	...
	Если СтандартныйПрефикс Тогда
		...
		УстановитьНовыйПрефиксИВыполнитьОбработкуДанныхСоСтандартнымФорматомКода(НовыйПрефикс, СДаты, ПредыдущийПрефиксИнформационнойБазы,, СчетчикОбработанныхОбъектов);
		
	Иначе
		
		УстановитьНовыйПрефиксИВыполнитьОбработкуДанныхСНестандартнымФорматомКода(НовыйПрефикс, СДаты, ПредыдущийПрефиксИнформационнойБазы,, СчетчикОбработанныхОбъектов);
...
Процедура СлужебнаяУстановитьПрефиксИБИПерепрефиксоватьВсеОбъекты(
	...
	Если СтандартныйПрефикс Тогда
		...
		УстановитьНовыйПрефиксИВыполнитьОбработкуДанныхСоСтандартнымФорматомКода(НовыйПрефикс, СДаты,, Истина, СчетчикОбработанныхОбъектов);
		
	Иначе
		
		УстановитьНовыйПрефиксИВыполнитьОбработкуДанныхСНестандартнымФорматомКода(НовыйПрефикс, СДаты,, Истина, СчетчикОбработанныхОбъектов);
...
Процедура СлужебнаяУстановитьПрефиксИБИСоздатьОбъекты(
	...
	УстановитьПрефиксИБИСоздатьИзменитьОбъекты(НовыйПрефикс, СДаты, ПредыдущийПрефиксИнформационнойБазы, Ложь, СчетчикОбработанныхОбъектов);
...
Процедура СлужебнаяУстановитьПрефиксИБИПерепрефиксоватьПоследниеОбъекты(
	...
	УстановитьПрефиксИБИСоздатьИзменитьОбъекты(НовыйПрефикс, СДаты, ПредыдущийПрефиксИнформационнойБазы, Истина, СчетчикОбработанныхОбъектов);
...
"Недостаточная длина %1. Минимальная длина %1 объекта должна составлять %3 символов.';uk=""Перетворення %1 об'єкта %2 не може бути виконано."
"Недостатня довжина %1. Мінімальна довжина %1 об'єкта винна становити %3 символів.""");
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
					?(ЭтоДокумент, НСтр("ru='номера';uk='номера'"), НСтр("ru='кода';uk='коду'")),
					Строка(Объект),
...
Процедура УстановитьНовыйПрефикс(
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОбъектМодифицирован Тогда
			...
			Объект[?(ЭтоДокумент, "Номер", "Код")] = КодНовый;
...
//
// Параметры:
//  ФункциональнаяОпцияИспользуется - Булево - признак наличия в конфигурации функциональной опции ПрефиксИнформационнойБазы.
//
...
//  ПрефиксОрганизации - Строка - прификс организации.
//
