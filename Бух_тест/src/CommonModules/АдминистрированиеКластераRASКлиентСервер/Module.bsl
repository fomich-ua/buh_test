...
Функция БлокировкаСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ) Экспорт
	...
	Возврат СвойстваИнформационнойБазы(ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ, СловарьСвойствБлокировкиСеансовИРегламентныхЗаданий());
...
//  ПроверятьПараметрыАдминистрированияИнформационнойБазы - Булево, флаг необходимости проверки параметров администрирования кластера.
//
Процедура ПроверитьПараметрыАдминистрирования(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ = Неопределено,
	...
	Если ПроверятьПараметрыАдминистрированияИнформационнойБазы Тогда
		...
		а = СвойстваИнформационнойБазы(ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ, Словарь);
...
//    2. Структура (упрощенный вариант), ключ - Имя свойства сеанса (см. выше), значение - значение, с которым выполняется
...
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Сеанс Из Сеансы Цикл
		...
		Исключение
			...
			ОтборСеансов = Новый Структура();
			ОтборСеансов.Вставить("Номер", Сеанс.Получить("session-id"));
			
			Если ПолучитьСеансы(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ, ОтборСеансов, Истина).Количество() > 0 Тогда
				ВызватьИсключение;
			Иначе
				Продолжить;
			КонецЕсли;
...
//            ВидСравнения.ИнтервалВключаяОкончание (только для числовых значений),
//        Значение - Число, Строка, Дата, Булево, СписокЗначений, Массив, Структура - значение, с которым
//          сравнивается значение соответствующего свойства соединения. При ВидСравнения.ВСписке и ВидСравнения.НеВСписке
//          в качестве значения должны передаваться СписокЗначений или Массив, содержащие набор значений, с которыми
//          будет выполняться сравнение. При ВидСравнения.Интервал, ВидСравнения.ИнтервалВключаяГраницы,
//          ВидСравнения.ИнтервалВключаяНачало и ВидСравнения.ИнтервалВключаяОкончание в качестве значения должны
//          передаваться структуры с полями С и По, значения которых образуют интервал, с которым будет выполняться
//          соответствующее сравнение,
//    2. Структура (упрощенный вариант), ключ - Имя свойства соединения (см. выше), значение - значение, с которым выполняется
//      сравнение. При использовании данного варианта описания фильтра сравнение всегда выполняется на равенство.
//
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Соединение Из Соединения Цикл
		...
		Исключение
			...
			ОтборСоединений = Новый Структура();
			ОтборСоединений.Вставить("Номер", Соединение.Получить("conn-id"));
			
			ОписанияСоединений = ПолучитьСоединения(
			...
			Если ОписанияСоединений.Количество() > 0 Тогда
				ВызватьИсключение;
			Иначе
				Продолжить;
			КонецЕсли;
...
//  безопасности безопасного режима. Если для информационной базы не назначен профиль безопасности - возвращается пустая строка.
//
...
//
Функция ПолучитьКластеры(Знач ПараметрыАдминистрированияКластера, Знач Фильтр = Неопределено)
...
//
Функция ПолучитьКластер(Знач ПараметрыАдминистрированияКластера)
...
// Возвращаемое значение: Массив(Структура).
//
Функция ПолучитьРабочиеСерверы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Фильтр = Неопределено)
...
// Возвращаемое значение: Массив(Структура).
//
Функция ПолучитьИнформационныеБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Фильтр = Неопределено)
...
// Возвращаемое значение: Строка, внутренний идентификатор кластера серверов.
//
Функция ПолучитьИнформационнуюБазу(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы)
...
// Возвращаемое значение: Массив(Структура).
//
Функция ПолучитьРабочиеПроцессы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Фильтр = Неопределено)
	...
	Для Каждого РабочийСервер Из РабочиеСерверы Цикл
		...
		Для Каждого РабочийПроцесс Из РабочиеПроцессыСервера Цикл
			Если РабочийПроцесс.Получить("running") Тогда
...
//    использования.
//
// Возвращаемое значение: Массив(Структура), Массив(Соответствие) - массив структур, описывающих свойства сеансов (описание структур -
//  см. АдминистрированиеКластераКлиентСервер.СвойстваСеанса()) или массив соответствий, описывающих свойства сеансов в нотации
//  утилиты rac (при ИспользоватьСловарь = Ложь).
//
...
//    использования.
//
// Возвращаемое значение: Массив(Структура), Массив(Соответствие) - массив структур, описывающих свойства соединений (описание структур -
//  см. АдминистрированиеКластераКлиентСервер.СвойстваСоединения()) или массив соответствий, описывающих свойства соединений в нотации
//  утилиты rac (при ИспользоватьСловарь = Ложь).
//
...
//  ПараметрыАдминистрированияИнформационнойБазы - Структура, описывающая параметры подключения к информационной базе,
//    описание - см. АдминистрированиеКластераКлиентСервер.ПараметрыАдминистрированияИнформационнойБазыКластера(),
//  Словарь - Структура, соответсвие имен свойств для API и потока вывода утилиты rac.
//
// Возвращаемое значение: Структура - описание информационной базы, составленное по переданному словарю.
//
...
Функция ПривестиЗначение(Знач Значение, Знач ИмяПараметра = "")
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		Если Найти(Значение, """") > 0 Или Найти(Значение, " ") > 0 Или Найти(Значение, "-") > 0 Или Найти(Значение, "!") > 0 Тогда
...
Функция ПривестиЭлементВывода(ЭлементВывода)
	...
	Если Лев(ЭлементВывода, 1) = """" Тогда
		ЭлементВывода = Прав(ЭлементВывода, СтрДлина(ЭлементВывода) - 1);
	КонецЕсли;
	
	Если Прав(ЭлементВывода, 1) = """" Тогда
		ЭлементВывода = Лев(ЭлементВывода, СтрДлина(ЭлементВывода) - 1);
	КонецЕсли;
...
"- Для компьютеров с ОС Linux требуется установить пакет 1c-enterprise83-server*.';uk='Неможливо виконати операцію адміністрування кластера серверів через: файл %1 не знайдений!"
""
"Для адміністрування кластера через сервер адміністрування (ras) потрібно встановити на даному"
"комп''ютері клієнт сервера адміністрування (rac)."
"Для його установки:"
"- Для комп''ютерів з ОС Windows потрібно перестановити платформу, установивши компонент ""Адміністрування сервера 1С:Підприємства"";"
...
Функция КаталогИсполняемыхФайловПлатформы() Экспорт
	...
	Если Не Прав(Результат, 1) = СимволСлеша Тогда
		Результат = Результат + СимволСлеша;
	КонецЕсли;
...
Функция ПарсерВывода(Знач ПотокВывода, Знач Словарь, Знач Фильтр = Неопределено)
	...
	Для Шаг = 1 По РазмерВывода Цикл
		...
		ПоложениеРазделителя = Найти(ЭлементПотока, ":");
...
//  к соответствию. Ключи соответсвия - имена свойств, значения соответствия - значения свойств.
//
// Параметры:
//  ЭлементРезультата - Строка, элемент потока вывода,
//  Результата - Массив, в который должен быть добавлен распарсенный объект,
//  Словарь - Структура, выступающая в качестве словаря сопоставления имен свойств объектов
//    В нотации утилиты rac и в нотации API,
//  Фильтр - Структура, условия фильтрации объектов (только для потоков вывода команд, возвращающих
//    коллекции объектов).
...
//  Ключ - Строка, имя свойства в API (см. АдминистрированиеКластераКлиентСервер.СвойстваБлокировкиСеансовИРегламентныхЗаданий()),
