...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки,
	...
	Попытка
		...
		Если УзелПланаОбмена <> Неопределено Тогда
			...
			Запрос.УстановитьПараметр("Пользователь", УзелПланаОбмена.Ссылка);
...
Функция НайденЗапретИзмененияИлиЗагрузкиДанных(Запрос,
	...
	Если ЗапретыДанных.Строки.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(Текст) Тогда
			Если ПоискЗапретовЗагрузки Тогда
				Текст = Текст
					+ НСтр("ru=' запрещено загружать в запрещенный период.';uk=' заборонено завантажувати в заборонений період.'")
					+ Символы.ПС
					+ Символы.ПС;
			Иначе
				Текст = Текст
					+ НСтр("ru=' запрещено изменять или перемещать в запрещенный период.';uk=' заборонено змінювати або переміщати в заборонений період.'")
					+ Символы.ПС
					+ Символы.ПС;
			КонецЕсли;
		КонецЕсли;
		...
		Если ТранзакцияАктивна() Тогда
			...
			ЗаписьЖурналаРегистрации(
				?(ПоискЗапретовЗагрузки,
				  ...
				  НСтр("ru='Даты запрета изменения.Найдены запреты изменения';uk='Дати заборони зміни.Знайдені заборони зміни'",
				       ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
	...
	ИначеЕсли Константы.ЗапретИзмененияДанныхДо01042011.Получить() Тогда
		Проверки = РезультатыЗапроса[1].Выгрузить();
		Текст = ПолучитьПредставлениеДанных(ИдентификаторДанных, ТаблицаДанных);
		
		Проверка = Проверки.Найти(0, "КлючСтроки");
		Если Проверка <> Неопределено И Проверка.Дата <=  '20110331235959' Тогда
			
			Если ЗначениеЗаполнено(Текст) Тогда
				Если ПоискЗапретовЗагрузки Тогда
					Текст = Текст
						+ НСтр("ru=' запрещено загружать в запрещенный период (до 01.04.2011).';uk=' заборонено завантажувати в заборонений період (до 01.04.2011).'")
						+ Символы.ПС
						+ Символы.ПС;
				Иначе
					Текст = Текст
						+ НСтр("ru=' запрещено изменять или перемещать в запрещенный период (до 01.04.2011).';uk=' заборонено змінювати або переміщати в заборонений період (до 01.04.2011).'")
						+ Символы.ПС
						+ Символы.ПС;
				КонецЕсли;
			КонецЕсли;			
...
Функция ПолучитьПредставлениеДанных(ИдентификаторДанных, ТаблицаДанных)
	...
	Если Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) Тогда
		ПредставлениеДанных = НСтр("ru='Записи регистра сведений ""%1""';uk='Запису регістру відомостей ""%1""'");
		
	ИначеЕсли Метаданные.РегистрыНакопления.Содержит(ОбъектМетаданных) Тогда
		ПредставлениеДанных = НСтр("ru='Записи регистра накопления ""%1""';uk='Записи регістру накопичення ""%1""'");
		
	ИначеЕсли Метаданные.РегистрыБухгалтерии.Содержит(ОбъектМетаданных) Тогда
		ПредставлениеДанных = НСтр("ru='Записи регистра бухгалтерии ""%1""';uk='Записи регістру бухгалтерії ""%1""'");
		
	ИначеЕсли Метаданные.РегистрыРасчета.Содержит(ОбъектМетаданных) Тогда
		ПредставлениеДанных = НСтр("ru='Записи регистра расчета ""%1""';uk='Запису регістру розрахунку ""%1""'");
	...
	Если ЗначениеЗаполнено(ПредставлениеДанных) Тогда
		...
		Если КоличествоПолей = 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ru='с полем';uk='з полем'")  + " " + Строка(ИдентификаторДанных);
			
		ИначеЕсли КоличествоПолей > 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ru='с полями';uk='з полями'") + " " + Строка(ИдентификаторДанных);
		КонецЕсли;
	Иначе
		...
		ИначеЕсли Метаданные.ПланыВидовРасчета.Содержит(ОбъектМетаданных) Тогда
			ПредставлениеДанных = НСтр("ru='План видов расчета ""%1""';uk='План видів розрахунку ""%1""'");
			
		ИначеЕсли Метаданные.БизнесПроцессы.Содержит(ОбъектМетаданных) Тогда
			ПредставлениеДанных = НСтр("ru='Бизнес-процесс';uk='Бізнес-процес'");
			
		ИначеЕсли Метаданные.Задачи.Содержит(ОбъектМетаданных) Тогда
			ПредставлениеДанных = НСтр("ru='Задача';uk='Задача'");
