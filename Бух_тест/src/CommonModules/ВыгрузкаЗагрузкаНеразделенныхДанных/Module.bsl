...
//          контейнера, используемый в процессе выгрузи данных. Подробнее см. комментарий
//          к программному интерфейсу обработки ВыгрузкаЗагрузкаДанныхМенеджерКонтейнера,
//        МенеджерВыгрузкиОбъекта - ОбработкаОбъект.ВыгрузкаЗагрузкаДанныхМенеджерВыгрузкиДанныхОбъектаИнформационнойБазы -
//          менеджер выгрузки текущего объекта. Подробнее см. комментарий к програмнмому интерфейсу обработки
//          ВыгрузкаЗагрузкаДанныхМенеджерВыгрузкиДанныхОбъектаИнформационнойБазы. Параметр передается только при вызове
...
Процедура ПередВыгрузкойТипа(Контейнер, Сериализатор, ОбъектМетаданных, Отказ) Экспорт
	...
	Если Не ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
...
Процедура КонтрольИспользованияСсылокНаНеразделенныеДанныеВРазделенных() Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Обнаружены ошибки в структуре метаданных конфигурации: ';uk='Виявлені помилки в структурі метаданих конфігурації: '") + ТекстОшибки;
...
Процедура ПроверитьНаличиеДублейЕстественныхКлючей(Знач ОбъектМетаданных, Знач ПоляЕстественногоКлюча)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%1", ТекстВыборкиПоля);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%2", Сред(ТекстВыборкиПоля, 1, СтрДлина(ТекстВыборкиПоля) - 3));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%3", ТекстДополнительногоЗапроса);
	...
	ТекстСообщения = ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
		НСтр("ru='У некоторых объектов %1: "
"%2"
""
"дублируются поля:"
"%3.';uk=""У деяких об'єктів %1: "
"%2"
""
"дублюються поля:"
"%3."""),
...
Процедура ПроверитьПоляЕстественногоКлюча(Знач ОбъектМетаданных, Знач ПоляЕстественногоКлюча)
	...
	Если ПоляЕстественногоКлюча = Неопределено Или ПоляЕстественногоКлюча.Количество() = 0 Тогда
		...
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для типа данных %1 не указаны естественные ключи для замены ссылок."
"Проверьте обработчик ПриОпределенииТиповТребующихЗагрузкиВЛокальнуюВерсию.';uk='Для типу даних %1 не вказані природні ключі для заміни посилань."
...
Процедура КонтрольСсылокНаНеразделенныеДанныеПриВыгрузке(Контейнер, Объект, ПоляДляКонтроляСсылокНаНеразделенныеДанные)
	...
	ОбъектМетаданных = Объект.Метаданные();
	ПолноеИмяОбъектаМетаданных = ОбъектМетаданных.ПолноеИмя();
	СтруктураИмениОбъекта = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПолноеИмяОбъектаМетаданных, ".");
	
	Для Каждого ПолеДляКонтроляСсылокНаНеразделенныеДанные Из ПоляДляКонтроляСсылокНаНеразделенныеДанные Цикл
		
		СтруктураИмениПоля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПолеДляКонтроляСсылокНаНеразделенныеДанные, ".");
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект метаданных %1 не поддерживается!';uk=""Об'єкт метаданих %1 не підтримується!""", Метаданные.ОсновнойЯзык.КодЯзыка),
				ПолноеИмяОбъектаМетаданных
			);
		КонецЕсли;
...
Процедура КонтрольСсылкиНаНеразделенныеДанныеПриВыгрузке(Контейнер, Знач ПроверяемаяСсылка, Знач ИсходныйОбъект, Знач ИсходныйОбъектМетаданных, Знач ИмяПоля)
	...
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоПеречисление(ОбъектМетаданных) Тогда
	...
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоСсылочныеДанныеПоддерживающиеПредопределенныеЭлементы(ОбъектМетаданных) Тогда
