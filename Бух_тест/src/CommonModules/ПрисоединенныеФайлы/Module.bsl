...
Функция ПолучитьДвоичныеДанныеФайла(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ФайлОбъект = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найден присоединенный файл ""%1"".';uk='Не знайдений приєднаний файл ""%1"".'"),
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Получение файла из тома';uk='Файли.Отримання файла з тома'",
	...
	Если ПолучатьСсылкуНаДвоичныеДанные Тогда
		...
		Иначе
			...
			Исключение
				СообщениеОбОшибке = ТекстОшибкиПриПолученииФайла(ИнформацияОбОшибке(), ПрисоединенныйФайл);
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Файлы.Открытие файла';uk='Файли.Відкриття файлу'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.Справочники[ПрисоединенныйФайл.Метаданные().Имя],
					ПрисоединенныйФайл,
...
"Ссылка на новый файл не заполнена.';uk='Помилка при додаванні приєднаного файлу."
...
"в присоединенные файлы подсистемы Присоединенные файлы.';uk='Помилка при конвертації приєднаних файлів підсистеми Робота з файлами"
...
"""%1"".';uk='Не заповнений власник у файлі"
"""%1"".'"),
		...
		Если ОбновлениеИнформационнойБазы.ВыполняетсяОбновлениеИнформационнойБазы() Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Ошибка записи файла при обновлении ИБ';uk='Файли.Помилка запису файлу при оновленні ІБ'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				Источник.Ссылка,
...
Процедура ВызватьИсключениеОткрытияФормы(Форма) Экспорт
	...
	ВызватьИсключение НСтр("ru='Самостоятельное использование формы не предусмотрено.';uk='Самостійне використання форми не передбачено.'");
...
"занятый для редактирования.';uk='""%1"" не може бути вилучений,"
