...
Процедура ПроверитьКорректностьИмениМетодаВнешнегоОбъекта(Знач ИмяМетода)
	...
	Для Каждого ЗапрещенныйСимвол Из ЗапрещенныеСимволы Цикл
		...
		Если Найти(ИмяМетода, ЗапрещенныйСимвол) > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не является корректным именем метода для COM-объекта или объекта внешней компоненты!';uk=""%1 не є коректним іменем методу для Com-Об'єкта або об'єкта зовнішнього компонента!"""),
				ИмяМетода);
...
"из сценария!';uk='Некоректний виклик функції загального модуля ДополнительныеОтчетыИОбработкиВБезопасномРежиме!"
