...
Процедура ПередПроверкойИдентификаторовОбъектовМетаданныхВПодчиненномУзлеРИБ(Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Обмен данными.Загрузка справочника ""Идентификаторы объектов метаданных""';uk='Обмін даними.Завантаження довідника ""Ідентифікатори об''єктів метаданих""'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ВызватьИсключение
			НСтр("ru='Из главного узла не загружены изменения справочника ""Идентификаторы объектов метаданных"":"
"ошибка загрузки данных. См. подробности в журнале регистрации.';uk='З головного вузла не завантажені зміни довідника ""Ідентифікатори об''єктів метаданих "":"
"помилка завантаження даних. Див. подробиці в журналі реєстрації.'");
	КонецПопытки;
	УстановитьПривилегированныйРежим(Истина);
	УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
	УстановитьПривилегированныйРежим(Ложь);
...
""
"После этого запустите программу.';uk='Завантажено зміни програми, отримані з головного вузла."
"Завершіть роботу програми. Відкрийте програму в конфігураторі й"
"виконайте команду ""Оновити конфігурацію бази даних (F7)""."
""
...
"параметр запуска ЗарегистрироватьПолноеИзменениеИОМДляПодчиненныхУзловРИБ.';uk='З головного вузла не завантажені зміни довідника ""Ідентифікатори об''єктів метаданих"":"
"при перевірці виявлено, що потрібно завантажити критичні зміни (див. подробиці в журналі"
"реєстрації в події ""Ідентифікатори об''єктів метаданих.Потрібно завантажити критичні зміни"")."
""
"Якщо недоступно повідомлення обміну даними, тоді запустіть знову програму,"
"настройте параметри підключення й повторіть синхронізацію."
"Якщо з головного вузла не отримані всі необхідні зміни, тоді"
"у головному вузлі виконайте оновлення інформаційної бази,"
"повторіть синхронізацію даних у головному й підлеглому вузлах."
""
"Щоб виконати оновлення інформаційної бази в головному вузлі, один раз виконайте"
"запуск програми з параметром запуску ЗапуститьОбновлениеИнформационнойБазы."
"Щоб при цьому зареєструвати зміни всього довідника, укажіть додатковий"
"параметр запуску ЗарегистрироватьПолноеИзменениеИОМДляПодчиненныхУзловРИБ.'");
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование = НСтр("ru='Синхронизация данных с другими программами';uk='Синхронізація даних з іншими програмами'");
	ОписаниеПрофиля.Описание = НСтр("ru='Дополнительно назначается тем пользователям, которым должны быть доступны средства"
"для мониторинга и синхронизации данных с другими программами.';uk='Додатково призначається тим користувачам, яким повинні бути доступні засоби"
"для моніторингу й синхронізації даних з іншими програмами.'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Если ДлительнаяОперация Тогда
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ru='Ожидание получения данных от базы-корреспондента...';uk='Очікування одержання даних від бази-кореспондента...'",
		...
		Если СтруктураНастроекОбмена.ИспользоватьПередачуБольшогоОбъемаДанных Тогда
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) Тогда
				...
				Попытка
					...
					Если ПараметрыОбмена.ДлительнаяОперация Тогда
						ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ru='Ожидание загрузки данных в базе-корреспонденте...';uk='Очікування завантаження даних у базі-кореспонденті...'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
						Возврат;
					КонецЕсли;
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
			...
			Если СтруктураПараметров.Ошибка Тогда
				ЕстьОшибки = Истина;
				НСтрока = НСтр("ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';uk='Помилка при переході на синхронізацію даних через універсальний формат: %1. Обмін скасовано.'",
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		Если НЕ ЭтоПланОбменаXDTO Тогда
			...
			Если ПравилаКонвертацииОбъектов = Неопределено Тогда
				...
				НСтрока = НСтр("ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';uk='Не задані правила конвертації в другій інформаційній базі для плану обміну %1. Обмін скасовано.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Для синхронизации данных через прямое подключение требуется использовать ОС Windows.';uk='Синхронізація даних через пряме підключення на сервері під управлінням ОС Linux недоступно."
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Иначе
		СтрокаСообщения = НСтр("ru='%1, %2; Объектов обработано: %3';uk=""%1, %2; Об'єктів оброблено: %4""", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("ru='Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]';uk='Обмін даними.[ИмяПланаОбмена].[ДействиеПриОбмене]'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Реквизит ""%1"".';uk='Складений тип даних для значень за замовчуванням не підтримується."
"Реквізит ""%1"".'");
...
"Реквизит ""%1"".';uk='Вибір значень за замовчуванням підтримується тільки для довідників."
"Реквізит ""%1"".'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Реквизит.ПолноеИмя());
				ВызватьИсключение СтрокаСообщения;
			КонецЕсли;
...
"Для предупреждения возможного возникновения ошибок при работе с программой рекомендуется актуализировать правила обмена из файла.';uk='Для плану(ов) обміну %1 використовуються правила обміну, завантажені з файлу."
"Ці правила можуть бути несумісні з новою версією програми."
"Для попередження можливого виникнення помилок при роботі із програмою рекомендується актуалізувати правила обміну з файлу.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';uk='Користувачу, вказаному для підключення до іншої програмі, повинні бути призначені ролі ""Адміністратор системи"" і ""Повні права""'");
	...
	ИначеЕсли ЭтоОбменРИБ Тогда
		...
		Пока ФайлОбмена.Прочитать() Цикл
			Если ФайлОбмена.ЛокальноеИмя = "Changes"
				...
				Продолжить;
			ИначеЕсли Найти(ФайлОбмена.ЛокальноеИмя, "Config") = 0 
				И Найти(ФайлОбмена.ЛокальноеИмя, "Signature") = 0
				И Найти(ФайлОбмена.ЛокальноеИмя, "Nodes") = 0
...
"См. подробности в журнале регистрации.';uk='Помилка завантаження пріоритетних даних з повідомлення обміну."
...
""
"После этого запустите программу.';uk='Завантажені зміни програми, отримані з головного вузла."
...
"Синхронизация отменена.';uk='Виявлено повернення до конфігурації бази даних."
...
Функция ОписаниеОграниченийПередачиДанных(Знач ИмяПланаОбмена, Знач Настройка, Знач ВерсияКорреспондента, Знач ИдентификаторНастройки = "") Экспорт
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не задан узел информационной базы с которым нужно производить обмен информацией. Обмен отменен.';uk='Не заданий вузол інформаційної бази з яким потрібно проводити обмін інформацією. Обмін скасовано.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ВидТранспортаОбмена) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не задан вид транспорта обмена. Обмен отменен.';uk='Не заданий вид транспорту обміну. Обмін скасовано.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Узел информационной базы помечен на удаление. Обмен отменен.';uk='Вузол інформаційної бази позначений на вилучення. Обмін скасовано.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.';uk='Не можна організувати обмін даними з поточним вузлом інформаційної бази. Обмін скасовано.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли ПустаяСтрока(СтруктураНастроекОбмена.УзелИнформационнойБазыКод)
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Один из узлов обмена имеет пустой код. Обмен отменен.';uk='Один з вузлів обміну має порожній код. Обмін скасовано.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';uk='Файл зовнішньої обробки для налагодження вивантаження не існує. Обмін відмінний.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковЗагрузки Тогда
		...
		Если Не ФайлОбработкиЗагрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';uk='Файл зовнішньої обробки для налагодження завантаження не існує. Обмін відмінний.'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// правила обмена должны быть указаны
		НСтрока = НСтр("ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';uk='Не задані правила конвертації для плану обміну %1. Завантаження даних скасоване.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
"Возможно не заполнены все реквизиты, обязательные к заполнению.';uk=""Не вдалося провести документ %1, отриманий з іншої інформаційної бази. %2"
"Можливо не заповнені всі реквізити, обов'язкові до заповнення.""",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Возможно не заполнены все реквизиты, обязательные к заполнению.';uk=""Не вдалося записати об'єкт %1, отриманий з іншої інформаційної бази. %2"
"Можливо не заповнені всі реквізити, обов'язкові до заповнення.""",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Строка(Объект), ПричинаОшибки);
...
Функция ПредставлениеДатыСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Последняя синхронизация: %1';uk='Остання синхронізація: %1'"),
...
""
"%2';uk='Завдання ""%1"" завершене аварійно."
""
"%2'"),
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта(Метаданные.ОбщиеФормы.ОбменыДанными.ПолноеИмя());
...
Процедура ФормаУзлаПриСозданииНаСервере(Форма, Отказ) Экспорт
	...
	Заголовок = Форма.Объект.Наименование + " (%ПереопределяемоеИмя%)";
	Форма.Заголовок = СтрЗаменить(Заголовок, "%ПереопределяемоеИмя%", ПереопределяемоеИмяУзлаПланаОбмена(Форма.Объект.Ссылка, "ЗаголовокУзлаПланаОбмена"));
