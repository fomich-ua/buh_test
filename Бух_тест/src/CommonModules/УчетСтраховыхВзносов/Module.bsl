...
Процедура РассчитатьВзносы(ВременнаяСсылка, НаборыЗаписей, ДанныеЗаполнения, ДокументСсылка, Организация, МесяцНачисления, ПоВсемДокументам = Истина) Экспорт
	...
	Запрос.Текст =
	   ...
	   |    ЛЕВОЕ СОЕДИНЕНИЕ
	   |    РегистрСведений.ШкалаСтавокНалогов.СрезПоследних(КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ),Организация = &Организация) КАК ШкалаСтавокНалоговПоОрганизации
	   |    ПО ШкалаСтавокНалоговПоОрганизации.Налог = ПараметрыКатегорийЕСВ.ЕСВ
	   |    ЛЕВОЕ СОЕДИНЕНИЕ
	   |    РегистрСведений.ШкалаСтавокНалогов.СрезПоследних(КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ),Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК ШкалаСтавокНалогов
