...
Функция БлокировкаСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ = Неопределено) Экспорт
	...
	Если Результат.ДатаС = ПустаяДата() Тогда
		Результат.ДатаС = Неопределено;
	КонецЕсли;
	
	Если Результат.ДатаПо = ПустаяДата() Тогда
...
Процедура УстановитьБлокировкуСеансовИЗаданийИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач СвойстваБлокировкиСеансовИЗаданий) Экспорт
	...
	Для Каждого Свойство Из СвойстваБлокировкиСеансовИЗаданий Цикл
		УстанавливаемаяБлокировка.Вставить(Свойство.Ключ, Свойство.Значение);
	КонецЦикла;
	
	Если УстанавливаемаяБлокировка.ДатаС = Неопределено Тогда
		УстанавливаемаяБлокировка.ДатаС = ПустаяДата();
	КонецЕсли;
	Если УстанавливаемаяБлокировка.ДатаПо = Неопределено Тогда
		УстанавливаемаяБлокировка.ДатаПо = ПустаяДата();
	КонецЕсли;
...
Функция МенеджерАдминистрирования(Знач ПараметрыАдминистрирования)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неизвестный тип подключения: %1!';uk='Невідомий тип підключення: %1!'"), ПараметрыАдминистрирования.ТипПодключения);
