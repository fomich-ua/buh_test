...
"установить расширение для веб-клиента 1С:Предприятие.';uk='Для виконання команди ""%1"" необхідно"
"встановити розширення для веб-клієнта 1С:Підприємство.'");
...
Процедура ПоказатьПредупреждениеОНеобходимостиРасширенияРаботыСКриптографией(ОбработчикРезультата, ПредставлениеКоманды = "") Экспорт
	ТекстПредупреждения = НСтр("ru='Для выполнения команды ""%1"" необходимо"
"установить расширение работы с криптографией.';uk='Для виконання команди ""%1"" необхідно"
"установити розширення роботи із криптографією.'");
	Если ЗначениеЗаполнено(ПредставлениеКоманды) Тогда
		ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%1", ПредставлениеКоманды);
	Иначе
		ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, " ""%1""", "");
...
"открыть существующий без обновления?';uk='Розмір файлу в робочому каталозі й у сховище файлів відрізняється."
""
...
""
"Взять файл из хранилища файлов и заменить им существующий или"
"открыть существующий без обновления?';uk='Файл у сховищі файлів, відмічений як зайнятий для редагування,"
...
"из хранилища файлов c потерей изменений и открыть?';uk='Файл у робочому каталозі має більш пізню дату зміни (новіший),"
