...
Процедура ПередВыгрузкойТипа(Контейнер, Сериализатор, ОбъектМетаданных, Отказ) Экспорт
	...
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоНаборЗаписейПоследовательности(ОбъектМетаданных) Тогда
	...
	Иначе
		...
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхГраницПоследовательностей.ПередВыгрузкойОбъекта()!';uk=""Об'єкт метаданих %1 не може бути оброблений обробником ВыгрузкаЗагрузкаДанныхГраницПоследовательностей.ПередВыгрузкойОбъекта()!"""),
	...
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоНаборЗаписейПоследовательности(ОбъектМетаданных) Тогда
		...
		Для Каждого ФайлГраниц Из ФайлыГраниц Цикл
			...
			Для Каждого СтрокаТаблицы Из ТаблицаГраниц Цикл 
				...
				МоментВремени = Новый МоментВремени(СтрокаТаблицы.Период, СтрокаТаблицы.Регистратор.Ссылка);
