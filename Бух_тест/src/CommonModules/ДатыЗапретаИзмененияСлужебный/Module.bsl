...
"При пересчете возникли ошибки:';uk='Перераховані деякі поточні значення відносних дат заборони."
"При перерахунку виникли помилки:'");
		Иначе
			ОписаниеРезультата =
				НСтр("ru='Не пересчитаны текущие значения дат запрета."
"При пересчете возникли ошибки:';uk='Не перераховані поточні значення дат заборони."
"При перерахунку виникли помилки:'");
		КонецЕсли;
	...
	Иначе
		Если ЕстьОбновленныеДаты Тогда
			ОписаниеРезультата =
				НСтр("ru='Успешно пересчитаны текущие значения относительных дат запрета.';uk='Успішно перераховані поточні значення відносних дат заборони.'");
		Иначе
			ОписаниеРезультата =
				НСтр("ru='Сегодня уже пересчитаны текущие значения относительных дат запрета.';uk='Сьогодні вже перераховані поточні значення відносних дат заборони.'");
		КонецЕсли;
	КонецЕсли;
	
	Если ЗаписатьОписаниеРезультатаВЖурналРегистрации Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Даты запрета изменения.Пересчет относительных дат';uk='Дати заборони зміни.Перерахунок відносних дат'",
...
"Подробности в журнале регистрации.';uk='Не вдалося перерахувати всі відносні дати заборони."
...
"не задано поле даты.';uk='Для перевірки заборони зміни "
...
"не задано поле дати.'"),
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Поля[0]) Тогда
			ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле даты: %2.';uk='Для перевірки заборони зміни "
"в джерелі даних для таблиці ""%1"""
"невірно задано поле дати: %2.'"),
				Таблица,
				ИсточникДанных.ПолеДаты));
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ИсточникДанных.ПолеОбъекта) Тогда
			...
			Если НЕ ЗначениеЗаполнено(Поля[0]) Тогда
				ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано поле объекта: %2.';uk='Для перевірки заборони зміни "
"в джерелі даних для таблиці ""%1"""
"невірно задано поле об''єкта: %2.'"),
					Таблица,
					ИсточникДанных.ПолеОбъекта));
			КонецЕсли;
			Если Найти(ПоляРегистра, "," + Поля[0] + ",") = 0 Тогда
...
Функция ПолучитьСтруктуруПолейОбъекта(ОбъектМетаданных, ИсточникиДанных, Таблица) Экспорт
	...
	Для каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		ДобавитьПоле(ОбъектМетаданных,
		             ...
		             ИсточникДанных.ПолеДаты,
		             Таблица,
		             НСтр("ru='поле даты';uk='поле дати'"));
		...
		Если ЗначениеЗаполнено(ИсточникДанных.ПолеОбъекта) Тогда
			...
			ДобавитьПоле(ОбъектМетаданных,
			             ...
			             ИсточникДанных.ПолеОбъекта,
			             Таблица,
			             НСтр("ru='поле объекта';uk=""поле об'єкта"""));
		КонецЕсли;
	КонецЦикла;
...
Функция ЗапретИзмененияДанныхНеИспользуется()
	...
	Возврат НеИспользуется И НЕ ЗапретИзмененияДанныхДо01042011;
...
"источники данных для таблицы ""%1"".';uk='Для перевірки заборони зміни не знайдені"
...
Процедура ДобавитьПоле(ОбъектМетаданных,
	...
	Если Поля.Количество() = 0 Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"не задано %2.';uk='Для перевірки заборони зміни "
"в джерелі даних для таблиці ""%1"""
"не задано %2.'"),
			Таблица,
			ВидПоляДляСообщений));
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Поля[0]) Тогда
		ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для проверки запрета изменения "
"в источнике данных для таблицы ""%1"""
"неверно задано %2: ""%3""';uk='Для перевірки заборони зміни "
"в джерелі даних для таблиці ""%1"""
"невірно задано %2: ""%3""'"),
			Таблица,
			ВидПоляДляСообщений,
...
"в источнике данных для таблицы ""%1"""
"неверно задано %2:"
"не задано поле заданной табличной части ""%3"".';uk='Для перевірки заборони зміни "
"в джерелі даних для таблиці ""%1"""
"невірно задано %2:"
"не задано поле заданої табличної частини ""%3"".'"),
				Таблица,
				ВидПоляДляСообщений,
				Поле));
...
"в источнике данных для таблицы ""%1"""
"неверно задано %2:"
"неверно задано поле заданной табличной части ""%3"".';uk='Для перевірки заборони зміни "
"в джерелі даних для таблиці ""%1"""
"невірно задано %2:"
"невірно задано поле заданої табличної частини ""%3"".'"),
				Таблица,
				ВидПоляДляСообщений,
				Поле));
		КонецЕсли;
