...
"%1';uk='Не вдалося встановити блокування області даних через:"
"%1'");
				ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, 
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Блокировка области данных';uk='Блокування області даних'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ПроверитьОтсутствиеДругихСеансов Тогда
		...
		Если КонфликтующиеСеансы.Количество() > 0 Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';uk='Операція не може бути виконана, т. я. в додатку працюють інші користувачі: %1'",
...
Процедура СкопироватьДанныеОбласти(Знач ОбластьИсточник, Знач ОбластьПриемник) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Копирование области данных';uk='Копіювання області даних'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
		ВызватьИсключение(НСтр("ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей';uk='Змінити поділ сеансу можливо лише з сеансу запущеного без вказання роздільників'"));
	КонецЕсли;
...
"""1С:Библиотека технологии сервиса"" версии не младше %1!';uk='У конфігурацію не впроваджена бібліотека ""1С:Бібліотека технології сервісу""."
...
"""1С:Бібліотека технології сервісу"" версії не молодше %1!'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ТребуемаяВерсияБТС());
...
""
"Используемая версия: %1, требуется версия не младше %2!';uk='Для використання конфігурації в моделі сервісу з поточною версією БСП потрібно"
"оновити використовувану версію бібліотеки ""1С:Бібліотека технології сервісу""!"
""
...
"Проверьте наличие и корректность внедрения подсистемы ""%1"".';uk='Неможливо виконати операцію з причини - в конфігурації не впроваджена підсистема ""%1""."
...
Процедура ПриПроверкеВключенияБезопасногоРежимаРазделенияДанных() Экспорт
	...
	Если БезопасныйРежим() = Ложь
		...
		Если РазделениеПереключено Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка публикации';uk='Помилка публікації'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				НСтр("ru='При публикации не включен режим безопасного разделения данных';uk='При публікації не включений режим безпечного розділення даних'"));
			ВызватьИсключение(НСтр("ru='Информационная база опубликована некорректно. Работа сеанса будет прекращена.';uk='Інформаційна база опублікована некоректно. Робота сеансу буде припинена.'"));
		КонецЕсли;
...
""
"Попробуйте запустить приложение через несколько минут.';uk='Запуск програми тимчасово недоступний."
...
Функция ПолучитьМенеджерЗаписиОбластиДанных(Знач ОбластьДанных, Знач Статус)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Подготовка области данных';uk='Підготовка області даних'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Функция ПодготовитьОбластьДанныхКИспользованиюИзЭталонной(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, 
	...
	ОбновлениеИнформационнойБазыСлужебный.ВыполнитьОбновлениеИнформационнойБазы();
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "*";
	Обработчик.Процедура = "РаботаВМоделиСервиса.СоздатьНеразделенныеПредопределенныеЭлементы";
	Обработчик.Приоритет = 99;
	Обработчик.ОбщиеДанные = Истина;
	Обработчик.МонопольныйРежим = Ложь;
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
	...
	Если ОбменЧерезФС Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Получение файла из хранилища';uk='Отримання файлу зі сховища'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбменЧерезФС = Ложь;
		КонецПопытки;
...
Функция ПоместитьФайлВХранилище(Знач АдресДанныеФайл, Знач ПараметрыПодключения, Знач ИмяФайла = "")
	...
	Если ОбменЧерезФС Тогда
		...
		Попытка
			...
			ЗаписатьВЖурналСобытиеФайловогоХранилища(
				...
				ОбменЧерезФС);
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Добавление файла.Обмен через ФС';uk='Додавання файлу.Обмін через ФС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбменЧерезФС = Ложь;
		КонецПопытки;
	...
	Если Не ОбменЧерезФС Тогда
		...
		Если Описание.Данные = Неопределено тогда
			...
			Иначе
				ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Добавление файла в хранилище. Не найден файл %1.';uk='Додавання файлу в сховище. Не знайдений файл %1.'"), СвойстваФайла.ПолноеИмя));
			КонецЕсли;
		Иначе
...
Процедура УдалитьВременныеФайлы(Знач ИмяФайла)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции удаления временного файла';uk='Виконання операції вилучення тимчасового файлу'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
"Дескриптор файла: %1';uk='Зазначений файл початкових даних не підходить для даної конфігурації."
"Дескриптор файлу: %1'"),
...
"Следующие объекты метаданных не удовлетворяют этому критерию: %3.';uk=""Всі об'єкти метаданих, що не входять до складу роздільників БСП (%1),"
"повинні бути включені до складу підписок на події (%2), що контролюють"
"неможливість запису нерозділених даних в розділених сеансах."
...
"Следующие объекты не удовлетворяют этому критерию: %2';uk=""Всі об'єкти метаданих конфігурації повинні бути розділені не більш ніж одним роздільником БСП (%1)."
"Наступні об'єкти не задовольняють цьому критерію: %2"""),
...
"%2 по порядку!';uk='Виявлено порушення структури метаданих конфігурації: загальний реквізит %1 повинен"
...
Процедура СоздатьНеразделенныеПредопределенныеЭлементы() Экспорт
	...
	Иначе
		...
		// При выключенном разделении данных действий не требуется.
		Возврат;
		
	КонецЕсли;
	
	ОсновнойРазделитель        = РазделительОсновныхДанных();
	ВспомогательныйРазделитель = РазделительВспомогательныхДанных();
