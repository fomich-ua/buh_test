		...
		Пока Выборка.Следующий() Цикл 
			Попытка
				...
				Если ЗначениеЗаполнено(Выборка.Шаблон)
					...
					ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Выполнение';uk='Черга завдань.Виконання'", 
...
"По причине:"
"%3';uk='Помилка при виконанні обробника помилок"
"Псевдонім методу: %1"
...
"%3'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Очередь регламентных заданий.Ошибка обработчика ошибок';uk='Черга регламентних завдань.Помилка обробника помилок'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
// Служебная процедура, которая снимает задания обработчика ошибок и свои собственные. Необходима для случая, когда задание
// исполнение "ОбработатьОшибку" не смогло завершиться
//
// Параметры:
...
// ИнформацияОбОшибке - ОписаниеОшибки - Не используется, необходимо только потому, что этот параметр должен присутствовать 
...
"Псевдоним метода: %1"
"Уровень рекурсии: %2';uk='Був виконаний обробник зняття завдань."
"Псевдонім методу: %1"
"Рівень рекурсії: %2'");
			ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонКомментария,
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Очередь регламентных заданий.Снятие заданий обработки ошибок';uk='Черга регламентних завдань.Зняття завдань обробки помилок'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Если ТранзакцияАктивна() Тогда
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Перед стартом выполнения обработчика %1 есть активные транзакции!';uk='Перед стартом виконання обробника %1 є активні транзакції!'"),
				ИмяМетода);
			
		ЗаписьЖурналаРегистрации(НСтр("ru='Очередь регламентных заданий.Выполнение';uk='Черга регламентних завдань.Виконання'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
	...
	Попытка
		...
		Если Не(РазделительУстановлен) И ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса!';uk='По завершенні виконання обробника %1 не було вимкнено поділ сеансу!'"),
				ИмяМетода);
		...
		ИначеЕсли РазделительУстановлен И ЗначениеРазделителя <> РаботаВМоделиСервиса.ЗначениеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По завершении выполнения обработчика %1 было изменено значение разделителя сеанса!';uk='По завершенні виконання обробника %1 було змінено значення роздільника сеансу!'"),
				ИмяМетода);
	...
	Попытка
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные().ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
			...
			Если ЗначениеЗаполнено(Задание.Шаблон)
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'", 
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда // Задание не было выполнено, запланируем обработчик ошибки
			
			ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'", 
...
Процедура ВыполнитьЗаданиеОчереди(Знач Ссылка, Знач ИсполняющееФоновоеЗадание, 
	...
	Попытка
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Старт';uk='Черга регламентних завдань.Старт'", 
		...
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено успешно';uk='Черга регламентних завдань.Завершено успішно'", 
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка);
