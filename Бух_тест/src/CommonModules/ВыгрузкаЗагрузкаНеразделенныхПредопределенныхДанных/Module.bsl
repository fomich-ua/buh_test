	...
	Для Каждого ПравилоКонтроля Из ПравилаКонтроля Цикл
		...
		Если ПараметрыВыгрузки.ВыгружаемыеТипы.Найти(ОбъектМетаданных) <> Неопределено Тогда
			...
			Для Каждого ИмяПоля Из ПравилоКонтроля.Значение Цикл
				...
				СтруктураИмениПоля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяПоля, ".");
				...
				Иначе
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Объект метаданных %1 не поддерживается!';uk=""Об'єкт метаданих %1 не підтримується!"""), ОбъектМетаданных.ПолноеИмя());
				...
				Для Каждого ВозможныйТипПоля Из ВозможныеТипыПоля Цикл
					...
					Если Не ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоСсылочныеДанныеПоддерживающиеПредопределенныеЭлементы(ОбъектВозможногоТипа) Тогда
...
Процедура ВызватьИсключениеНеУдалосьОпределитьПоле(Знач ИмяПоля, Знач ИмяОбъекта)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось построить запрос получения значения поля %1 объекта мтеданных %2!';uk=""Не вдалося побудувати запит отримання значення поля %1 об'єкта метаданих %2!"""),
		ИмяПоля, ИмяОбъекта
	);
