...
Функция ВходВПрограммуРазрешен(ОписаниеПользователяИБ) Экспорт
	...
	Если ТипЗнч(ОписаниеПользователяИБ) = Тип("СправочникСсылка.Пользователи")
	 Или ТипЗнч(ОписаниеПользователяИБ) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		УникальныйИдентификатор = ОбщегоНазначения.ПолучитьЗначениеРеквизита(
...
//
// Параметры:
//  ПользовательИБ - ПользовательИнформационнойБазы - используется, когда нужно сопоставить
//                   существующего администратора с новым или существующим пользователем
//                   в справочнике Пользователи.
//
// Возвращаемое значение:
//  Неопределено                  - пользователь в справочнике, с которым сопоставлен пользователь ИБ
//                                  с административными правами, уже существует.
//  СправочникСсылка.Пользователи - пользователь в справочнике, с которым был сопоставлен первый администратор или
//                                  администратор, указанный в параметре ПользовательИБ.
//
...
Функция ПолноеИмяНеуказанногоПользователя() Экспорт
	...
	Возврат НСтр("ru='<Не указан>';uk='<Не вказано>'");
...
Функция ПрочитатьПользователяИБ(Знач Идентификатор,
	...
	Если ПользовательИБ = Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователь информационной базы ""%1"" не найден.';uk='Користувач інформаційної бази ""%1"" не знайдений.'"), Идентификатор);
		Возврат Ложь;
	КонецЕсли;
...
"%1,"
"так как он уже существует.';uk='Неможливо створити користувача інформаційної бази"
" %1,"
...
"%1,"
"старый пароль указан не верно.';uk='При записі користувача інформаційної бази"
"%1,"
...
"%1 произошла ошибка:"
""
"""%2"".';uk='При записі користувача інформаційної бази"
"%1 відбулася помилка:"
""
"""2"".'"),
			ПользовательИБ.Имя,
...
"%1 произошла ошибка:"
""
"""%2"".';uk='При вилученні користувача інформаційної бази"
"%1 відбулася помилка:"
""
"""%2"".'"),
				ПользовательИБ.Имя,
...
"Имя (для входа): ""%1"""
"превышает длину 64 символа.';uk='Помилка запису користувача інформаційної бази"
"Ім''я (для входу): ""%1"""
"перевищує довжину 64 символу.'"),
						...
						ИначеЕсли Найти(ЗначениеСвойства, ":") > 0 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Ошибка записи пользователя информационной базы"
"Имя (для входа): ""%1"""
"содержит запрещенный символ "":"".';uk='Помилка запису користувача інформаційної бази"
"Ім''я (для входу): ""%1"""
...
"соответствует более одного элемента в справочнике Пользователи:';uk='Користувачеві сервісу з ідентифікатором ""%2"""
"відповідає більше одного елемента в довіднику Користувачі:'"),
						НСтр("ru='Пользователю ИБ ""%1"" с идентификатором ""%2"""
"соответствует более одного элемента в справочнике Пользователи:';uk='Користувачеві ІБ ""%1"" з ідентифікатором ""%2"""
"відповідає більше одного елемента в довіднику Користувачі:'") ),
						ИмяДляВхода,
						ТекущийНеоднозначныйИдентификатор);
			КонецЕсли;
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка записи пользователя ИБ';uk='Користувачі.Помилка запису користувача ІБ'",
