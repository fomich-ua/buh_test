...
//
Функция СписокРеквизитовНаселенныйПункт(Код=Неопределено, КлючСтраны) Экспорт
	
	Если КлючСтраны = "АдресРФ" Тогда
		Возврат СписокРеквизитовНаселенныйПунктКЛАДР();
	ИначеЕсли КлючСтраны = "АдресУкраины" Тогда
		Возврат СписокРеквизитовНаселенныйПунктКОАТУУ();
...
Функция ДесериализацияКонтактнойИнформации(Знач Текст, Знач ОжидаемыйВид = Неопределено, РезультатыЧтения = Неопределено) Экспорт
	...
	Если КонтактнаяИнформацияКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Исключение
			...
			Если ТипЗнч(ОжидаемыйВид) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
				ТекстОшибки = СтрЗаменить(НСтр("ru='Некорректный формат XML контактной информации для ""%1"", значения полей были очищены.';uk='Некоректний формат XML контактної інформації для ""%1"", значення полів були очищені.'"),
		...
		Если ТекстОшибки = Неопределено Тогда
			...
			ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается веб-страница';uk='Помилка десеріалізації контактної інформації, очікується веб-страница'");
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, не указан ожидаемый тип';uk='Помилка десеріалізації контактної інформації, не зазначений очікуваний тип'");
	КонецЕсли;
...
Функция АдресВведенВСвободнойФорме(XDTOИнформация, НовоеЗначение = Неопределено) Экспорт
	...
	ОбъектXDTO = ?(НадоСериализовать, ДесериализацияКонтактнойИнформации(XDTOИнформация), XDTOИнформация);
	Если Не ЭтоПострочныйАдрес(ОбъектXDTO) Тогда
		// Не поддерживаем
		Возврат Ложь;
	КонецЕсли;
	
	АдресПострочно = ОбъектXDTO.Состав.Состав;
	Если ТипЗнч(НовоеЗначение) <> Тип("Булево") Тогда
		// Читаем
		Возврат Не ПустаяСтрока(АдресПострочно.Адрес_по_документу);
	КонецЕсли;
		
	// Устанавливаем
	Если НовоеЗначение Тогда
		АдресПострочно.Адрес_по_документу = ОбъектXDTO.Представление;
	Иначе
		АдресПострочно.Сбросить("Адрес_по_документу");
	КонецЕсли;
...
//  Возвращает флаг того, что переданный адрес - построчный
//
//  Параметры:
...
//      Булево - результат проверки
//
Функция ЭтоПострочныйАдрес(XDTOАдрес) Экспорт
	Возврат ПострочныйАдрес(XDTOАдрес)<>Неопределено;
КонецФункции

//  Возвращает извлеченный XDTO построчного адреса или Неопределено для адреса иностранного
//
//  Параметры:
//      ОбъектИнформации - ОбъектXDTO - Контактная информация или XDTO адреса
//
//  Возвращаемое значение:
//      ОбъектXDTO - укр адрес
//      Неопределено - нет укр адреса
//
Функция ПострочныйАдрес(ОбъектИнформации) Экспорт
	...
	Если ТипЗнч(ОбъектИнформации) = ТипXDTO Тогда
		...
		Если ТипЗнч(Адрес) = ТипXDTO И Адрес.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины") Тогда
			Результат = Адрес;
		КонецЕсли;
...
Функция ПочтовыйИндексАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	АдресПострочно = ПострочныйАдрес(XDTOАдрес);
	...
	Если НовоеЗначение = Неопределено Тогда
		// Чтение
		Результат = АдресПострочно.Получить( КонтактнаяИнформацияКлиентСерверПовтИсп.XPathПочтовогоИндекса() );
	...
	ЗаписьИндекса = АдресПострочно.Получить( КонтактнаяИнформацияКлиентСерверПовтИсп.XPathПочтовогоИндекса() );
	Если ЗаписьИндекса = Неопределено Тогда
		ЗаписьИндекса = АдресПострочно.ДопАдрЭл.Добавить( ФабрикаXDTO.Создать(XDTOАдрес.ДопАдрЭл.ВладеющееСвойство.Тип) );
...
Функция ОпределитьПочтовыйИндексАдреса(XDTOАдрес) Экспорт
	...
	Если XDTOАдрес.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		XDTOАдресПострочно = XDTOАдрес.Состав;
	Иначе 
		XDTOАдресПострочно = XDTOАдрес;
	КонецЕсли;
	
	Если XDTOАдресПострочно = Неопределено Или XDTOАдресПострочно.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
	...
	Если Вариант = "КЛАДР" Тогда
		Возврат ОпределитьПочтовыйИндексАдресаКЛАДР(XDTOАдресПострочно);
	КонецЕсли;
...
Функция РайонАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
			АдресПострочно = XDTOАдрес;
		ИначеЕсли XDTOТип=ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины") Тогда
			АдресПострочно = XDTOАдрес;
		Иначе
			АдресПострочно = XDTOАдрес.Состав;
		КонецЕсли;
		
		Если ТипЗнч(АдресПострочно) = Тип("ОбъектXDTO") Тогда
			Возврат ЗначениеСвойстваПоXPath(АдресПострочно, КонтактнаяИнформацияКлиентСерверПовтИсп.XPathРайона() );
		КонецЕсли;
...
Функция ЗданияИПомещенияАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	// Чтение
	ПредставленияДопЭлементов = КонтактнаяИнформацияКлиентСерверПовтИсп.ПредставленияОбъектовАдресации(Локализация.КодЯзыкаИнформационнойБазы());
	Для Каждого ДопЭлемент Из АдресПострочно.ДопАдрЭл Цикл
		Если ДопЭлемент.Номер <> Неопределено Тогда
			...
			Если ТипОбъекта <> Неопределено Тогда
				...
				Если НоваяСтрока <> Неопределено Тогда
					...
					НоваяСтрока.Сокращение = НРег(ПредставленияДопЭлементов.Получить(ТипОбъекта.Наименование));
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	АдресПострочно = ПострочныйАдрес(ОбъектАдреса);
	Если АдресПострочно = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	НужныРФПараметрыВНаименовании = (ТипЗнч(АдресПострочно)=Тип("ОбъектXDTO") И АдресПострочно.Тип()=ФабрикаXDTO.Тип(КонтактнаяИнформацияКлиентСерверПовтИсп.ПространствоИмен(), "АдресРФ"));
	
	Если АдресПострочно.СвРайМО = Неопределено Тогда
		Район = "";
	ИначеЕсли АдресПострочно.СвРайМО.Район <> Неопределено Тогда
		Район = АдресПострочно.СвРайМО.Район;
	ИначеЕсли НужныРФПараметрыВНаименовании И АдресПострочно.СвРайМО.СвМО <> Неопределено Тогда
		Район = КонтактнаяИнформацияКлиентСервер.ПолноеНаименование(
			АдресПострочно.СвРайМО.СвМО.МунОбр2, "",
			АдресПострочно.СвРайМО.СвМО.МунОбр1, "");
	Иначе
		Район = "";;
	КонецЕсли;
	
	Возврат КонтактнаяИнформацияКлиентСервер.ПолноеНаименование(
		АдресПострочно.НаселПункт, "",
		АдресПострочно.Город,  "",
		Район, "",
		?(НужныРФПараметрыВНаименовании, АдресПострочно.Округ, АдресПострочно.Область), "",
		?(НужныРФПараметрыВНаименовании, АдресПострочно.СубъектРФ, ""), ""
	);
...
Функция ПредставлениеАдреса(XDTOАдрес, ВидИнформации) Экспорт
	...
	Если ТипЗнч(АдресПострочно) <> Тип("ОбъектXDTO") Тогда
		Возврат ?(ПустаяСтрока(Страна), "", Страна + ", ") + СокрЛП(АдресПострочно);
	КонецЕсли;
	...
	Если АдресПострочно.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		Представление = КонтактнаяИнформацияКлиентСервер.ПолноеНаименование(
			ПочтовыйИндексАдреса(АдресПострочно), "",
			АдресПострочно.СубъектРФ, "",
			АдресПострочно.Округ, "",
			РайонАдреса(АдресПострочно), "",
			АдресПострочно.Город, "",
			АдресПострочно.ВнутригРайон, "",
			АдресПострочно.НаселПункт, "",
			АдресПострочно.Улица, "");
			
		// Строения и помещения
		НомерНеВыведен = Истина;
		Данные = ЗданияИПомещенияАдреса(АдресПострочно);
	...
	ИначеЕсли АдресПострочно.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины") Тогда

		// Значимые части
		Представление = КонтактнаяИнформацияКлиентСервер.ПолноеНаименование(
			ПочтовыйИндексАдреса(АдресПострочно), "",
			АдресПострочно.Область, "",
			РайонАдреса(АдресПострочно), "",
			АдресПострочно.Город, "",
			АдресПострочно.ВнутригРайон, "",
			АдресПострочно.НаселПункт, "",
	...
	Иначе
		...
		Представление = СокрЛП(АдресПострочно);
	КонецЕсли;
...
Функция ОшибкиЗаполненияАдреса(Знач XDTOАдрес, Знач ВидИнформации = Неопределено, Знач РезультатГруппами = Ложь) Экспорт
	...
	Если ТипЗнч(АдресРФ) <> Тип("ОбъектXDTO") Или АдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		Если ФлагиПроверки.АдресТолькоУкраинский Тогда
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации) Экспорт
	...
	Иначе
		...
		Адрес = XTDOКонтактная.Состав;
		Если Не ЭтоПострочныйАдрес(Адрес) Тогда
			Возврат Результат;
		КонецЕсли;
		...
		Если XTDOКонтактная.Представление <> Представление Тогда
			...
			ДобавитьОшибкуЗаполненияАдреса(Результат[0], "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.';uk='Представлення адреси для виду контактної інформації ""%1"" відрізняється від даних в адресі.'"),
	...
	НеСоответствуетДлинаПолей = ГруппаОшибокАдреса("НеСоответствуетДлинаПолей",
		НСтр("ru='Не соответствует установленной длина полей:';uk='Не відповідає встановленої довжина полів:'"));
	...
	Если ИсключатьПроверкуДомаВАдресе(АдресРФ) Тогда
		...
		Если ЗданиеНеУказано Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				...
				НСтр("ru='Не указан дом или корпус';uk='Не вказаний будинок або корпус'"), 
	...
	Иначе
		...
		Если ДанныеДома = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				КонтактнаяИнформацияКлиентСерверПовтИсп.XPathНомераДополнительногоОбъектаАдресации("Дом"),
				НСтр("ru='Не указан дом или владение (домовладение).';uk='Не вказаний будинок або володіння (домоволодіння).'"),
				НСтр("ru='Дом';uk='Будинок'")
			);
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		Если ПустаяСтрока(КонтактнаяИнформацияКлиентСервер.Сокращение(Регион)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "СубъектРФ",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Не указано сокращение в названии региона ""%1"".';uk='Не зазначене скорочення в назві регіону ""%1"".'"), Регион
				), НСтр("ru='Регион';uk='Регіон'"));
		КонецЕсли;
		Если СтрДлина(Регион) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Название региона ""%1"" должно быть короче 50 символов.';uk='Назва регіону ""%1"" повинне бути коротше 50 символів.'"), Регион
				), НСтр("ru='Регион';uk='Регіон'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='В названии региона ""%1"" есть не кириллические символы.';uk='У назві регіону ""%1"" є не кириличні символи.'"), Регион
				), НСтр("ru='Регион';uk='Регіон'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(КонтактнаяИнформацияКлиентСервер.Сокращение(Район)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				...
				), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Название района ""%1"" должно быть короче 50 символов.';uk='Назва району ""%1"" повинне бути коротше 50 символів.'"), Район
				), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='В названии района ""%1"" есть не кириллические символы.';uk='У назві району ""%1"" є не кириличні символи.'"), Район
				), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если ПустаяСтрока(КонтактнаяИнформацияКлиентСервер.Сокращение(Город)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Не указано сокращение в названии города ""%1"".';uk='Не зазначене скорочення в назві міста ""%1"".'"), Город
				), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
		Если СтрДлина(Город) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Название города ""%1"" должно быть короче 50 символов.';uk='Назва міста ""%1"" повинне бути коротше 50 символів.'"), Город
				), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='В названии города ""%1"" есть не кириллические символы.';uk='У назві міста ""%1"" є не кириличні символи.'"), Город
				), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если ПустаяСтрока(КонтактнаяИнформацияКлиентСервер.Сокращение(НаселенныйПункт)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Не указано сокращение в названии населенного пункта ""%1"".';uk='Не зазначене скорочення в назві населеного пункту ""%1"".'"), НаселенныйПункт
				), НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
		Если СтрДлина(НаселенныйПункт) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Название населенного пункта ""%1"" должно быть короче 50 символов.';uk='Назва населеного пункту ""%1"" повинне бути коротше 50 символів.'"), НаселенныйПункт
				), НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='В названии населенного пункта ""%1"" есть не кириллические символы.';uk='У назві населеного пункту ""%1"" є не кириличні символи.'"), НаселенныйПункт
				), НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если ПустаяСтрока(КонтактнаяИнформацияКлиентСервер.Сокращение(Улица)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Не указано сокращение в названии улицы ""%1"".';uk='Не зазначене скорочення в назві вулиці ""%1"".'"), Улица
				), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Название улицы ""%1"" должно быть короче 50 символов.';uk='Назва вулиці ""%1"" повинне бути коротше 50 символів.'"), Улица
				), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='В названии улицы ""%1"" есть не кириллические символы.';uk='У назві вулиці ""%1"" є не кириличні символи.'"), Улица
				), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Индекс) Тогда
		...
		Если СтрДлина(Индекс) <> 6 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Индекс) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				НСтр("ru='Почтовый индекс должен состоять из 6 цифр.';uk='Поштовий індекс винний складатися з 6 цифр.'"),
	...
	Для Каждого ДанныеЗдания Из ЗданияПомещения.Здания Цикл
		Если СтрДлина(ДанныеЗдания.Значение) > 10 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеЗдания.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Значение поля ""%1"" должно быть короче 10 символов.';uk='Значення поля ""%1"" повинне бути коротше 10 символів.'"), ДанныеЗдания.Тип
				), ДанныеЗдания.Тип);
		КонецЕсли;
	КонецЦикла;
	Для Каждого ДанныеПомещения Из ЗданияПомещения.Помещения Цикл
		Если СтрДлина(ДанныеПомещения.Значение) > 10 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеПомещения.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru='Значение поля ""%1"" должно быть короче 10 символов.';uk='Значення поля ""%1"" повинне бути коротше 10 символів.'"), ДанныеПомещения.Тип
				), ДанныеПомещения.Тип);
		КонецЕсли;
	КонецЦикла;
	...
	Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) Тогда
		Если НазванияГородовФедеральногоЗначения().Найти(ВРег(Регион)) = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Город",
				НСтр("ru='Город может быть не указан только в регионе - городе федерального значения.';uk='Місто може бути не зазначений тільки в регіоні - місті федерального значення.'"),
				НСтр("ru='Город';uk='Місто'")
			);
	...
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресРФ) Тогда
		...
		Если ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Улица) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Улица",
				НСтр("ru='Если неопределен населенный пункт, то должна быть указана улица.';uk='Якщо невизначений населений пункт, то повинна бути зазначена вулиця.'"), 
				НСтр("ru='Улица';uk='Вулиця'")
			);
		КонецЕсли;
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина) Экспорт
	...
	Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		...
		Если Не ЭтоПострочныйАдрес(XDTOАдрес) Тогда
			Возврат Список;
		КонецЕсли;
...
Функция РезультатыАвтоПодбораНаселенногоПунктаКЛАДР(Текст, СкрыватьНеактуальныеАдреса, ПредупреждатьОНеактуальных, ВыбиратьСтрок)
	...
	ПредупреждениеНеактуальности = НСтр("ru='Адрес ""%1"" неактуален.';uk='Адреса ""%1"" неактуальний.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если УникальныеПредставления[ПредставлениеВСписке] <> Неопределено Тогда
			...
			Если ПустаяСтрока(ПочтовыйИндекс) Тогда
				Описание = "";
			Иначе
				Описание = СтрЗаменить(НСтр("ru='индекс: %1';uk='індекс: %1'"), "%1", ПочтовыйИндекс);
			КонецЕсли;
...
Функция НаселенныеПунктыПоПредставлениюКЛАДР(Текст, СкрыватьНеактуальныеАдреса, ВыбиратьСтрок, УточняющаяУлица)
	...
	ПредупреждениеНеактуальности = НСтр("ru='Населенный пункт неактуален.';uk='Населений пункт неактуальний.'");
	КартинкаНеактуальности = БиблиотекаКартинок.КонтактнаяИнформацияНеактуально;
	КартинкаАктуальности   = Неопределено;
...
Функция УлицыПоПредставлениюКЛАДР(КодНаселенногоПункта, Текст, СкрыватьНеактуальныеАдреса, ВыбиратьСтрок)
	...
	ПредупреждениеНеактуальности = НСтр("ru='Улица неактуальна.';uk='Вулиця неактуальна.'");
	КартинкаНеактуальности = БиблиотекаКартинок.КонтактнаяИнформацияНеактуально;
	КартинкаАктуальности   = Неопределено;
...
Функция СписокРеквизитовНаселенныйПунктКЛАДР(Код = Неопределено)
	...
	Результат.Вставить("Регион",          ЭлементАдреснойСтруктуры(НСтр("ru='Регион';uk='Регіон'"),    "СубъектРФ") );
	Результат.Вставить("Район",           ЭлементАдреснойСтруктуры(НСтр("ru='Район';uk='Район'"),     "СвРайМО/Район") );
	Результат.Вставить("Город",           ЭлементАдреснойСтруктуры(НСтр("ru='Город';uk='Місто'"),     "Город") );
	Результат.Вставить("НаселенныйПункт", ЭлементАдреснойСтруктуры(НСтр("ru='Нас.пункт';uk='Нас.пункт'"), "НаселПункт", ,Истина));
	
	// Подсказки для интерфейса
	Результат.Регион.Вставить("Подсказка",          НСтр("ru='Регион адреса';uk='Регіон адреси'"));
	Результат.Район.Вставить("Подсказка",           НСтр("ru='Район адреса';uk='Район адреси'"));
	Результат.Город.Вставить("Подсказка",           НСтр("ru='Город адреса';uk='Місто адреси'"));
	Результат.НаселенныйПункт.Вставить("Подсказка", НСтр("ru='Населенный пункт адреса';uk='Населений пункт адреси'"));
...
Процедура ЗаполнитьОшибкиАдресаКЛАДР(XDTOАдресРФ, Результат)
	...
	СущностьПоляРегион = НСтр("ru='Субъект РФ';uk=""Суб'єкт РФ""");
	...
	Если МодульКЛАДР.ЧислоЗаполненныхАдресныхОбъектов() = 0 Тогда
		...
		Результат.Добавить(Новый Структура("СущностьПоля, ПутьXPath", СущностьПоляРегион, ПутьXPathРегион),
			НСтр("ru='Адресный классификатор не загружен.';uk='Адресний класифікатор не завантажений.'"));
		Возврат;
	КонецЕсли;
	...
	Если ПустаяСтрока(КодРегиона) Тогда
		...
		Результат.Добавить(Новый Структура("СущностьПоля, ПутьXPath", СущностьПоляРегион, ПутьXPathРегион),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Регион ""%1"" не является субъектом РФ по классификатору';uk='Регіон ""%1"" не є суб''єктом РФ по класифікатору'"), 
				Данные.Регион));
		Возврат;
	КонецЕсли;
	...
	Если Не МодульКЛАДР.АдресныйЭлементЗагружен(Данные.Регион) Тогда
		Результат.Добавить(Новый Структура("СущностьПоля, ПутьXPath", СущностьПоляРегион, ПутьXPathРегион), 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Адресный классификатор по региону ""%1"" не загружен';uk='Адресний класифікатор по регіону ""%1"" не завантажений'"),
				Данные.Регион));
		Возврат;
	КонецЕсли;
...
Функция СписокРеквизитовНаселенныйПунктКОАТУУ(Код=Неопределено)
	
	// Первый уровень - внутренние идентификаторы
	Результат = Новый Структура("Область, Район, Город, НаселенныйПункт",
		// Второй уровень - одинаково для всех классификаторов
		ЭлементАдреснойСтруктуры(НСтр("ru='Область';uk='Область'"),   "Область"),
		ЭлементАдреснойСтруктуры(НСтр("ru='Район';uk='Район'"),     "СвРайМО/Район"),
		ЭлементАдреснойСтруктуры(НСтр("ru='Город';uk='Місто'"),     "Город"),
		ЭлементАдреснойСтруктуры(НСтр("ru='Нас.пункт';uk='Нас.пункт'"), "НаселПункт", ,Истина)
	);
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	НазваниеУкраины = ВРег(Справочники.СтраныМира.Украина.Наименование);
	
	ЭлементКвартира = Неопределено;
	ЭлементКорпус   = Неопределено;
	ЭлементДом      = Неопределено;
	
	АдресУкраины = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины"));
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		Если ИмяПоля="ИНДЕКС" Тогда
			ЭлементИндекс = СоздатьДопАдрЭл(АдресУкраины);
		...
		ИначеЕсли ИмяПоля = "СТРАНА" Тогда
			...
			Если ВРег(ЭлементСписка.Значение) <> НазваниеУкраины Тогда
		...
		ИначеЕсли ИмяПоля = "РЕГИОН" ИЛИ ИмяПоля = "ОБЛАСТЬ" Тогда
			АдресУкраины.Область = ЭлементСписка.Значение;

		ИначеЕсли ИмяПоля = "РАЙОН" Тогда
			Если АдресУкраины.СвРайМО = Неопределено Тогда
				АдресУкраины.СвРайМО = ФабрикаXDTO.Создать( АдресУкраины.Тип().Свойства.Получить("СвРайМО").Тип )
			КонецЕсли;
			АдресУкраины.СвРайМО.Район = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "ГОРОД" Тогда
			АдресУкраины.Город = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "НАСЕЛЕННЫЙПУНКТ" Тогда
			АдресУкраины.НаселПункт = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "УЛИЦА" Тогда
			АдресУкраины.Улица = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "ТИПДОМА" Тогда
			Если ЭлементДом = Неопределено Тогда
				ЭлементДом = СоздатьНомерДопАдрЭл(АдресУкраины);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "ТИПКОРПУСА" Тогда
			Если ЭлементКорпус = Неопределено Тогда
				ЭлементКорпус = СоздатьНомерДопАдрЭл(АдресУкраины);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "ТИПКВАРТИРЫ" Тогда
			Если ЭлементКвартира = Неопределено Тогда
				ЭлементКвартира = СоздатьНомерДопАдрЭл(АдресУкраины);
			КонецЕсли;
	...
	Адрес.Состав = ?(АдресУкраинский, АдресУкраины, Результат.Представление);
...
Функция СоздатьНомерДопАдрЭл(АдресПострочно)
	ДопАдрЭл = СоздатьДопАдрЭл(АдресПострочно);
	ДопАдрЭл.Номер = ФабрикаXDTO.Создать(ДопАдрЭл.Тип().Свойства.Получить("Номер").Тип);
	Возврат ДопАдрЭл.Номер;
КонецФункции

Функция СоздатьДопАдрЭл(АдресПострочно)
	СвойствоДопАдрЭл = АдресПострочно.ДопАдрЭл.ВладеющееСвойство;
	ДопАдрЭл = ФабрикаXDTO.Создать(СвойствоДопАдрЭл.Тип);
	АдресПострочно.ДопАдрЭл.Добавить(ДопАдрЭл);
	Возврат ДопАдрЭл;
КонецФункции

Функция СвРайМО(АдресПострочно)
	Если АдресПострочно.СвРайМО <> Неопределено Тогда
		Возврат АдресПострочно.СвРайМО;
	КонецЕсли;
	
	АдресПострочно.СвРайМО = ФабрикаXDTO.Создать( АдресПострочно.Свойства().Получить("СвРайМО").Тип );
	Возврат АдресПострочно.СвРайМО;
КонецФункции
...
Функция ДесериализацияАдресаПоПредставлениюКЛАДР(Знач Представление)
	...
	НаименованиеРоссии = СокрЛП(Справочники.СтраныМира.НайтиПоКоду("643").Наименование);
