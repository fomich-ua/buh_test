...
//	ПараметрыКоманды.СопровождающийТекст = НСтр("ru='Выполняется команда...';uk='Виконується команда...'");
...
"Команда ""%2"" не может быть выполнена.';uk='Для звіту або обробки ""%1"" не призначена основна форма,"
...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	ТекстСообщения = НСтр("ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента 1С:Предприятие.';uk='Для вивантаження зовнішньої обробки (звіту) в файл рекомендується встановити розширення для веб-клієнта 1С:Підприємство.'");
