		...
		Иначе
			...
			Исключение
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Ошибка анализа контактной информации, возможно неверный формат значений полей.';uk='Помилка аналізу контактної інформації, можливо невірний формат значень полів.'"), ,
...
//                              для получения флагов, влияющих на проверку: АдресТолькоУкраинский, ОбязательноеЗаполнение, ВключатьСтрануВПредставление.
...
Функция АдресПоДокументуКонтактнойИнформации(XMLСтрока, Знач НовоеЗначение = Неопределено) Экспорт
	...
	АдресПострочно = КонтактнаяИнформацияСлужебный.ПострочныйАдрес(XDTOАдрес.Состав);
	Если АдресПострочно=Неопределено Тогда
		ВызватьИсключение НСтр("ru='Невозможно определить адрес по документу, ожидается построчный адрес';uk='Неможливо визначити адресу за документом, очікується порядкова адреса'");
	КонецЕсли;
	
	Если НовоеЗначение <> Неопределено Тогда
		АдресПострочно.Адрес_по_документу = НовоеЗначение;
		XMLСтрока = КонтактнаяИнформацияСлужебный.СериализацияКонтактнойИнформации(XDTOАдрес);
	КонецЕсли;
	
	Возврат Строка(АдресПострочно.Адрес_по_документу);
КонецФункции
...
Функция РегионАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	АдресПострочный = КонтактнаяИнформацияСлужебный.ПострочныйАдрес(Адрес);
	Возврат ?(АдресПострочный = Неопределено, "", СокрЛП(АдресПострочный.СубъектРФ));
КонецФункции
...
Функция ГородАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	АдресПострочный = КонтактнаяИнформацияСлужебный.ПострочныйАдрес(Адрес);
	Возврат ?(АдресПострочный = Неопределено, "", СокрЛП(АдресПострочный.Город));
КонецФункции
...
Функция НомерТелефонаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	ВызватьИсключение НСтр("ru='Невозможно определить номер, ожидается телефона или факс.';uk='Неможливо визначити номер, очікується телефону або факс.'");
КонецФункции
...
Функция Действие(Форма, Тип, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь)
	...
	Если (Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница
		...
		Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			Команда.Подсказка = НСтр("ru='Заполнить адрес';uk='Заповнити адреса'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница Тогда
			...
			Команда.Подсказка = НСтр("ru='Перейти по ссылке';uk='Перейти по посиланню'");
...
//            * АдресТолькоУкраинский        - Булево - Истина, если используются только украинские адреса, Ложь - иначе.
//            * ПроверятьКорректность        	 - не используется.
...
Процедура ОбновитьВидКонтактнойИнформации(Вид, Тип, Подсказка, МожноИзменятьСпособРедактирования, РедактированиеТолькоВДиалоге, ОбязательноеЗаполнение, Порядок = Неопределено, РазрешитьВводНесколькихЗначений = Ложь, НастройкиПроверки = Неопределено) Экспорт
	...
	ОшибкаПараметров   = НСтр("ru='Некорректно заполнены настройки проверки адреса';uk='Некоректно заповнені настройки перевірки адреси'");
...
// Параметры:
//     Вид - СправочникСсылка.ВидыКонтактнойИнформации, Строка   - Ссылка на вид контактной информации или идентификатор предопределенного элемента
//     Наименование - Строка 									 - Наименование вида контактной информации. (на языке ИБ)
//     Тип - ПеречислениеСсылка.ТипыКонтактнойИнформации, Строка - Тип контактной информации или его идентификатор.
//     Подсказка - Строка                                        - Подсказка к виду контактной информации. (на языке ИБ)
//     МожноИзменятьСпособРедактирования - Булево                - Истина, если есть возможность изменять настройки вида, Ложь - иначе.
//     РедактированиеТолькоВДиалоге - Булево                     - Истина, если редактировать только в диалоге, Ложь - иначе.
//     ОбязательноеЗаполнение                                    - Булево - Истина, если требуется обязательное заполнение поля, Ложь - иначе.
//     Порядок - Число, Неопределено                             - Порядок вида контактной информации, положение в списке относительно других элементов: 
...
//            * АдресТолькоУкраинский        - Булево - Истина, если используются только украинские адреса, Ложь - иначе.
//            * ПроверятьКорректность        		  - не используется.
//            * ЗапрещатьВводНекорректного   		  - не используется.
//            * СкрыватьНеактуальныеАдреса   		  - не используется.
//            * ВключатьСтрануВПредставление - Булево - Истина, если требуется включать наименование страны в представление адреса, Ложь - иначе.
//        Для типа АдресЭлектроннойПочты - структура, содержащая поля:
//            * ПроверятьКорректность      - Булево - Истина, если требуется проверка адреса электронной почты на корректность, Ложь - иначе.
//            * ЗапрещатьВводНекорректного - Булево - Истина, если требуется запрещать пользователю записывать некорректный адрес (Только если ПроверятьКорректность = Истина), Ложь - иначе.
...
// Если добавляется предопределённый вид КИ к уже существующим в ИБ, не рекомендуется назначать порядок в явном виде.
//
Процедура ОбновитьВидКонтактнойИнформацииУкр(Вид, Наименование, Тип, Подсказка, МожноИзменятьСпособРедактирования, РедактированиеТолькоВДиалоге, ОбязательноеЗаполнение, Порядок = Неопределено, РазрешитьВводНесколькихЗначений = Ложь, НастройкиПроверки = Неопределено) Экспорт
...
Процедура ОбновитьГруппуВидКонтактнойИнформацииУкр(Вид, Наименование) Экспорт
...
Функция ДоступныеДляКопированияАдреса(Знач ЗначенияПолейДляАнализа, Знач ВидАдреса) Экспорт
	...
	ТекущийТолькоУкраинский = ВидАдреса.АдресТолькоУкраинский;
	...
	Для Каждого Адрес Из ЗначенияПолейДляАнализа Цикл
		...
		Иначе
			Если ТекущийТолькоУкраинский Тогда
				// В адрес, ограниченный Украиной, нельзя копировать иностранный адрес
				...
				Если Не КонтактнаяИнформацияСлужебный.ЭтоПострочныйАдрес(XDTOАдрес) Тогда
					ДопустимыйИсточник = Ложь;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
