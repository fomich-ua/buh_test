...
#КонецЕсли
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору системы.';uk='Немає прав для виводу друкованої форми на принтер, зверніться до адміністратора системи.'"));
		Возврат;
	КонецЕсли;
...
Функция ИнициализироватьМакетОфисногоДокумента(Знач ДвоичныеДанныеМакета, Знач ТипМакета, Знач ИмяМакета = "") Экспорт
	...
	Если Не ПодключитьРасширениеРаботыСФайлами() Тогда
		ПоказатьПредупреждение(, НСтр("ru='Расширение для работы с файлами не подключено. Печать невозможна.';uk='Розширення для роботи з файлами не підключено. Друк неможливий.'"));
		Возврат Неопределено;
	КонецЕсли;
