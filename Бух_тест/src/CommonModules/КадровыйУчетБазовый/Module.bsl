...
Процедура СоздатьВТДанныеДокументовПриемНаРаботу(МенеджерВременныхТаблиц, МассивОбъектов) Экспорт
	...
	Если ТипЗнч(МассивОбъектов[0]) = Тип("СправочникСсылка.Сотрудники") Тогда
        ...
        ТекстУсловия = "ПриемНаРаботу.Ссылка = &Документ"
	Иначе	
...
Функция СформироватьПоЭлементамСправочникаЗапросДляП1П4(МассивОбъектов, ИмяРеквизитаДатаСобытия)
	...
	Запрос.Текст = 
		...
		|	КадровыеДанныеСотрудников.Организация.КодПоЕДРПОУ,
...
Функция ТаблицаНачисленийСотрудниковПоВидам(МенеджерВременныхТаблиц)
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	ПриемНаРаботу.Ссылка КАК ДокументСсылка
	|
	|ИЗ
	|	ВТДанныеДокументов КАК ПриемНаРаботу";
	Результат = Запрос.Выполнить().Выгрузить();
	
	Запрос.УстановитьПараметр("ДокументСсылка", Результат.Получить(0).ДокументСсылка);
	Запрос.Текст = "ВЫБРАТЬ
	|	ПриемНаРаботуНачисления.Размер КАК ТарифнаяСтавка
	|ПОМЕСТИТЬ ВТОклады
	|ИЗ
	|	Документ.ПриемНаРаботу.Начисления КАК ПриемНаРаботуНачисления
	|ГДЕ
	|	ПриемНаРаботуНачисления.Ссылка = &ДокументСсылка
	|	И ПриемНаРаботуНачисления.Начисление.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ПовременнаяОплатаТруда)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ПриемНаРаботуНачисления.Размер) КАК Доплата
	|ПОМЕСТИТЬ ВТДоплаты
	|ИЗ
	|	Документ.ПриемНаРаботу.Начисления КАК ПриемНаРаботуНачисления
	|ГДЕ
	|	ПриемНаРаботуНачисления.Ссылка = &ДокументСсылка
	|	И ПриемНаРаботуНачисления.Начисление.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаНачислений.ФиксированнойСуммой)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПриемНаРаботуНачисления.Размер КАК ПроцентНадбавки,
	|	ПриемНаРаботуНачисления.Начисление КАК Надбавка
	|ПОМЕСТИТЬ ВТНадбавки
	|ИЗ
	|	Документ.ПриемНаРаботу.Начисления КАК ПриемНаРаботуНачисления
	|ГДЕ
	|	ПриемНаРаботуНачисления.Ссылка = &ДокументСсылка
	|	И ПриемНаРаботуНачисления.Начисление.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаНачислений.Процентом)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура СформироватьКадровыеПриказы(ПараметрыЗадания = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	НачисленияИндексации = РасчетЗарплаты.НачисленияПоКатегории(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.Индексация, Новый Структура("СпособРасчета", Перечисления.СпособыРасчетаНачислений.ИндексацияЗарплаты));
