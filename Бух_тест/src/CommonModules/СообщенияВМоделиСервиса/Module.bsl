...
Процедура ОшибкаНеизвестноеИмяКанала(Знач КаналСообщений) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Неизвестное имя канала сообщений %1';uk=""Невідоме ім'я каналу повідомлень %1""");
...
Процедура ЗаписатьСобытиеНачалоОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Начало обработки';uk='Повідомлення в моделі сервісу.Початок обробки'",
...
Процедура ЗаписатьСобытиеОкончаниеОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Окончание обработки';uk='Повідомлення в моделі сервісу.Закінчення обробки'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
...
Процедура ДоставитьБыстрыеСообщения() Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Доставка быстрых сообщений';uk='Доставка швидких повідомлень'",
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
			Если ОбщегоНазначения.ЗначениеРазделителяСеанса() <> Сообщение.Body.Zone Тогда
				ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Отправка сообщения';uk='Повідомлення в моделі сервісу.Відправлення повідомлення'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриОтправкеСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Отправка';uk='Повідомлення в моделі сервісу.Відправка'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Процедура ПриПолученииСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Получение';uk='Повідомлення в моделі сервісу.Отримання'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Процедура ТранслироватьСообщениеВВерсиюКорреспондентаПриНеобходимости(Сообщение, Знач ИнформацияПодключения, Знач ПредставлениеПолучателя) Экспорт
	...
	Если Не ИнформацияПодключения.Свойство("URL") 
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не задан URL сервиса обмена сообщениями с информационной базой %1';uk='Не заданий URL сервісу обміну повідомленнями з інформаційною базою %1'"), ПредставлениеПолучателя);
	КонецЕсли;
