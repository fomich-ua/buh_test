...
Процедура ПередЗагрузкойТипа(Контейнер, ОбъектМетаданных, Отказ) Экспорт
	...
	Если ОбъектПоддерживаетТекущиеИтоги(ОбъектМетаданных) Тогда
...
Процедура ЗаполнитьТаблицуНаборовПоКоллекцииМетаданных(Наборы, Коллекция)
	...
	Для Каждого ОбъектМетаданных Из Коллекция Цикл
		...
		Если ОбъектПоддерживаетИтоги(ОбъектМетаданных) Тогда
...
Функция ОбъектПоддерживаетИтоги(Знач ОбъектМетаданных)
	
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоРегистрСведений(ОбъектМетаданных) Тогда
		
		Возврат (ОбъектМетаданных.РазрешитьИтогиСрезПервых ИЛИ ОбъектМетаданных.РазрешитьИтогиСрезПоследних);
		
	ИначеЕсли ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоРегистрНакопления(ОбъектМетаданных) Тогда
		
		Возврат Истина;
		
	ИначеЕсли ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоРегистрБухгалетрии(ОбъектМетаданных) Тогда
...
Функция ОбъектПоддерживаетТекущиеИтоги(Знач ОбъектМетаданных)
	
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоРегистрНакопления(ОбъектМетаданных) Тогда
		
		Возврат (ОбъектМетаданных.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Остатки);
