...
Процедура ЗарплатаКадрыОбновлениеС12()Экспорт
	...
	КонвертироватьДанныеОБанковскихКарточкахРаботников();
	
	КонвертироватьДанныеСведенийОВыплатахРаботникам(СоответствиеКонвертированныхОбъектов);
	...
	УстановитьРучнуюКорректировкуДокументов();
...
Процедура ВидыВыплатКонвертация()
	...
	Пока Выборка.Следующий() Цикл 
		Если НЕ Выборка.Предопределенный И ЗначениеЗаполнено(Выборка.УДАЛИТЬВидРасчета)
			 И Выборка.Начисления.Количество() = 0 Тогда
			ВидыВыплатОбъект = Выборка.ПолучитьОбъект();
			НС = ВидыВыплатОбъект.Начисления.Добавить();
			НС.Начисление = ВидыВыплатОбъект.УДАЛИТЬВидРасчета;
...
Процедура СпособыОтраженияЗарплатыВРеглУчетеКонвертация()
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ Выборка.Предопределенный Тогда
			...
			Если Выборка.УДАЛИТЬВзносыОсновногоНачисления = NULL ИЛИ Выборка.СпособОтраженияЕСВФОТ = NULL Тогда
				Продолжить;
			КонецЕсли;	
			Если НЕ Выборка.УДАЛИТЬВзносыОсновногоНачисления Тогда
				СпособыОтраженияОбъект.СтратегияОтраженияЕСВФОТ = Перечисления.СтратегииОтраженияВУчетеЕСВ.КакНачисление;
			ИначеЕсли Выборка.УДАЛИТЬВзносыОсновногоНачисления И Выборка.СпособОтраженияЕСВФОТ = Справочники.СпособыОтраженияЗарплатыВРеглУчете.ПустаяСсылка() Тогда
...
Процедура КонвертироватьДанныеРегистраРасчетаНачисления(СоответствиеКонвертированныхОбъектов);
	...
	Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
		...
		Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
			...
			СтрокаТаблицы.НалоговыйПериодЕСВ = УчетСтраховыхВзносов.ОпределитьНалоговыйПериод(ЗаписьНабора.ПериодРегистрации, ЗаписьНабора.ПериодДействия, ЗаписьНабора.ВидРасчета.ВидЕСВ, ЗаписьНабора.ВидРасчета.КатегорияНачисленияИлиНеоплаченногоВремени);
			СтрокаТаблицы.НалоговыйПериодНДФЛ = УчетНДФЛ.ОпределитьНалоговыйПериод(ЗаписьНабора.ПериодРегистрации, ЗаписьНабора.ПериодДействия, ЗаписьНабора.ВидРасчета.ДоходНДФЛ, ЗаписьНабора.ВидРасчета.КатегорияНачисленияИлиНеоплаченногоВремени);
...
Процедура КонвертироватьПределыСтраховыхВзносов()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ПределыСтраховыхВзносов.УдалитьНалог КАК Налог
	|ИЗ
	|	РегистрСведений.ПределыСтраховыхВзносов КАК ПределыСтраховыхВзносов
	|ГДЕ
	|	ПределыСтраховыхВзносов.УдалитьНалог.Предопределенный = ИСТИНА";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НаборЗаписей = РегистрыСведений.ПределыСтраховыхВзносов.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.УдалитьНалог.Установить(Выборка.Налог);
...
Функция ДанныеКадровойИстории(КлючиПорцииДанных, ТолькоДвиженияНепереносимыхРегистраторов = Ложь)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьРаботникиОрганизаций.Сотрудник,
	...
	|	УдалитьРаботникиОрганизаций.Должность,
	|	УдалитьРаботникиОрганизаций.ПричинаИзмененияСостояния,
	|	УдалитьРаботникиОрганизаций.ПринятНаНовоеРабочееМесто,
	|	УдалитьРаботникиОрганизаций.ДатаСозданияРабочегоМеста,
	//|	УдалитьРаботникиОрганизаций.ЗанимаемыхСтавок,
	|	УдалитьРаботникиОрганизаций.Период,
...
Процедура КонвертироватьДанныеУчетаВзаиморасчетовССотрудниками(СоответствиеКонвертируемыхОбъектов)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
		...
		Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
			...
			КонецЕсли;	
			Если ЗаписьНабора.УДАЛИТЬСпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк
				Тогда ЗаписьНабора.СпособВыплаты = Перечисления.ВидыМестВыплатыЗарплаты.ЗарплатныйПроект
			ИначеЕсли ЗаписьНабора.УДАЛИТЬСпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу
				Тогда ЗаписьНабора.СпособВыплаты = Перечисления.ВидыМестВыплатыЗарплаты.Касса
			КонецЕсли;
			ЗаписьНабора.Сотрудник = Справочники.Сотрудники.ПустаяСсылка();
		КонецЦикла;
...
Процедура КонвертироватьДанныеВзаиморасчетовСДепонентами(СоответствиеКонвертируемыхОбъектов)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
		...
		НаборЗаписей.Прочитать();
		Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
			Если ЗначениеЗаполнено(ЗаписьНабора.УдалитьСотрудник.ФизическоеЛицо) Тогда
				ЗаписьНабора.ФизическоеЛицо = ЗаписьНабора.УдалитьСотрудник.ФизическоеЛицо; 
				ЗаписьНабора.УдалитьСотрудник = Справочники.Сотрудники.ПустаяСсылка();
			КонецЕсли;
		КонецЦикла;
		...
		Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.СписаниеДепонентовВДоходыОрганизаций") Тогда	
			...
			Для Каждого СтрокаДокумента Из ДокументОбъект.Работники Цикл
				СтрокаДокумента.ФизическоеЛицо = СтрокаДокумента.УдалитьСотрудник.ФизическоеЛицо;	
				СтрокаДокумента.УдалитьСотрудник = Справочники.Сотрудники.ПустаяСсылка();
			КонецЦикла;
...
Процедура СформироватьПередставлениеАдреса(Адрес, Представление)
	СтруктураАдреса = КонтактнаяИнформацияСлужебный.ПредыдущаяСтруктураКонтактнойИнформацииXML(Адрес);
...
Процедура КонвертироватьДанныеОБанковскихКарточкахРаботников()
	...
	|ИЗ
	|	РегистрСведений.УдалитьПараметрыВыплатыЗПРаботников.СрезПоследних(
	|			,
	|			НЕ Организация В
	|						(ВЫБРАТЬ
...
Процедура КонвертироватьДанныеСведенийОВыплатахРаботникам(СоответствиеКонвертируемыхОбъектов)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
		Пока Выборка.Следующий() Цикл
		НаборЗаписей = РегистрыСведений.УдалитьСведенияОВыплатахРаботникамОрганизацийПоПлатежнымВедомостям.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(Выборка.Регистратор);
		НаборЗаписей.Прочитать();
...
Функция ДанныеСведенийОВыплатахРаботникам()
	...
	|	Регистр.Регистратор КАК Регистратор
	|ИЗ
	|	РегистрСведений.УдалитьСведенияОВыплатахРаботникамОрганизацийПоПлатежнымВедомостям КАК Регистр
	|
	|УПОРЯДОЧИТЬ ПО
	|	Регистратор";
