...
"Не рекомендуется разрешать редактирование из-за риска рассогласования данных.';uk='Елемент ""%1"" уже використовується в інших місцях у програмі."
...
"Не рекомендуется разрешать редактирование из-за риска рассогласования данных.';uk='Обрані елементи (%1) уже використовуються в інших місцях у програмі."
"Не рекомендується дозволяти редагування через ризик неузгодженості даних.'");
	...
	Иначе
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Разрешено редактирование реквизитов объектов (%1)';uk=""Дозволено редагування реквізитів об'єктів (%1)"""), Параметры.МассивСсылок.Количество());
			ПоказатьОповещениеПользователя(НСтр("ru='Редактирование реквизитов разрешено';uk='Редагування реквізитів дозволене'"),, ТекстСообщения);
		КонецЕсли;
		ВыполнитьОбработкуОповещения(Параметры.ОбработкаПродолжения, Истина);
	КонецЕсли;
