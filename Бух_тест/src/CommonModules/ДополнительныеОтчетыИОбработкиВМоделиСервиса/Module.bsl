...
"доступно только сеансов с установленным разделением данных!';uk='Використання функції ДополнительныеОтчетыИОбработкиВМоделисервиса.ПоставляемаяОбработка()"
...
"доступно только сеансов с установленным разделением данных!';uk='Використання функції ДополнительныеОтчетыИОбработкиВМоделисервиса.ИспользуемаяОбработка()"
"доступне тільки для сеансів з установленим поділом даних!'");
...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, Знач СправочникиИДокументы, Знач НастройкиРасположенияКоманд, Знач ВариантыДополнительногоОтчета, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, які поставляються.Ініційована установка обробки, яка поставляється, в область даних'",
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемаяОбработка) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных';uk='Додаткові звіти та обробки, які поставляються.Установка в область даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';uk='Додаткові звіти та обробки, які поставляються.Установка в область даних відкладена'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных';uk='Додаткові звіти та обробки, які поставляються.Вилучення з області даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';uk='Додаткові звіти та обробки, які поставляються.Помилка вилучення з області даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиДополнительнойОбработкиВОбластьДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';uk='Додаткові звіти та обробки, які поставляються.Помилка установки в область даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		ПоставляемаяОбработка,
		...
		Для Каждого КонтролируемыйРеквизит Из КонтролируемыеРеквизиты Цикл
			...
			Если ИсходныйРеквизит <> РезультирующийРеквизит Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, полученной из каталога дополнительных обработок менеджера сервиса!';uk='Неприпустима спроба зміни значення реквізиту %1 для додаткової обробки %2, отриманої з каталогу додаткових обробок менеджера сервісу!'"), 
...
Процедура ПриПолученииРазрешенийСессииБезопасногоРежима(Знач КлючСессии, ОписанияРазрешений, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не обнаружена поставляемая обработка для ключа запуска %1!';uk='Не виявлена обробка, що поставляється, для ключа запуску %1!'"),
				Строка(КлючСессии));
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Дополнительная обработка с идентификатором %1 не является поставляемой!';uk='Додаткова обробка з ідентифікатором %1 не є такою, що поставляється!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';uk='Додаткові звіти та обробки, які поставляються.Завантаження обробки, яка поставляється, скасоване'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				,
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Завантаження обробки, що поставляється'", 
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
	...
	Если ПоставляемаяОбработка.ВерсияGUID <> ОписаниеПоставляемойОбработки.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';uk='Додаткові звіти та обробки, які поставляються.Завантаження обробки, яка поставляється, завершене'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
		...
		Для Каждого ИнсталляцияОбработки Из ИспользуемыеОбработки Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Запланована актуалізація настройок обробки, яка поставляється'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
		...
		Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, які поставляються.Запланована відкладена установка обробки, яка поставляється, в область даних'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
...
Процедура ПриСериализацииВладельцаРазрешенийНаИспользованиеВнешнихРесурсов(Знач Владелец, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ТипЗнч(Владелец) = Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		...
		Результат = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(Обработки.НастройкаРазрешенийНаИспользованиеВнешнихРесурсовВМоделиСервиса.ПакетАдминистрированиеРазрешений(), "PermissionsOwnerExternalModule"));
