...
Функция РежимРаботыПрограммы() Экспорт
	...
	// Права пользователя
	РежимРаботы.Вставить("ЭтоАдминистраторПрограммы", Пользователи.ЭтоПолноправныйПользователь(,,Ложь)); // АА, АС, ЛКС, ЛФ
...
Функция РазделенныеОбъектыМетаданных(Знач Разделитель) Экспорт
	...
	Если МетаданныеОбщегоРеквизита = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Общий реквизит %1 не обнаружен в конфигурации!';uk='Загальний реквізит %1 не виявлений у конфігурації!'"), Разделитель);
	КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для общего реквизита %1 не используется разделение данных!';uk='Для загального реквізиту %1 не використовується розподіл даних!'"), Разделитель);
	...
	Для Каждого МетаданныеПоследовательности Из Метаданные.Последовательности Цикл
		...
		Если МетаданныеПоследовательности.Документы.Количество() = 0 Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Получение разделенных объектов метаданных';uk=""Одержання розділених об'єктів метаданих""", 
	...
	Если ТребуетсяВыполнитьОбновление Тогда
		...
		Иначе
			...
			НаименованиеЗадание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Обновление кэша версий. Идентификатор записи %1. Тип данных %2.';uk='Оновлення кеша версій. Ідентифікатор запису %1. Тип даних %2.'"),
				Идентификатор,
				ТипДанных);
				...
				Исключение
					Для каждого ИсходноеЗадание Из Задания Цикл
						...
						Если Задание.ИнформацияОбОшибке <> Неопределено Тогда
							ЗаписьЖурналаРегистрации(НСтр("ru='Обновление кэша версий';uk='Оновлення кеша версій'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						...
						Иначе
							...
							ВызватьИсключение(НСтр("ru='Неизвестная ошибка при выполнения задания обновления кэша версий';uk='Невідома помилка при виконання завдання оновлення кеша версій'"));
						КонецЕсли;
					КонецЦикла;
					
					ВызватьИсключение(НСтр("ru='Неизвестная ошибка при обновлении кэша версий';uk='Невідома помилка при оновленні кеша версій'"));
				КонецПопытки;
			КонецЕсли;
...
"Тип данных: %2';uk='Помилка при оновленні кеша версій. Дані не отримані."
...
Функция WSПрокси(Знач АдресWSDL, Знач URIПространстваИмен, Знач ИмяСервиса,
	Знач ИмяТочкиПодключения = "", Знач ИмяПользователя, Знач Пароль, Знач Таймаут = 0) Экспорт
...
Функция ОпцииИнтерфейса() Экспорт
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При получении опций интерфейса произошла ошибка:"
"%1';uk='При одержанні опцій інтерфейсу відбулася помилка:"
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
