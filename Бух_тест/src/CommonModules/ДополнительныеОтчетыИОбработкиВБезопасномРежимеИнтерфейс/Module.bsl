...
Функция ИдентификаторCOMКлассаВРежимеОбратнойСовместимости(Знач ProgId)
	...
	Если CLSID = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Разрешение на использование COM-класса %1 не может быть предоставлено дополнительной обработке, работающей в режиме обратной совместимости с механизмом разрешений, реализованным в версии БСП 2.1.3."
"Для использования COM-класса требуется переработать дополнительную обработку для работы без режима обратной совместимости';uk='Дозвіл на використання COM-класу %1 не може бути надано додатковій обробці, що працює в режимі зворотної сумісності з механізмом дозволів, реалізованим у версії БСП 2.1.3."
"Для використання COM-класу потрібно переробити додаткову обробку для роботи без режиму зворотної сумісності'"),
				  ProgId
		);
...
Функция ИдентификаторыCOMКлассовВРежимеОбратнойСовместимости()
	...
	// V83.ComConnector
	Результат.Вставить(ОбщегоНазначения.ИмяCOMСоединителя(), ОбщегоНазначения.ИдентификаторCOMСоединителя(ОбщегоНазначения.ИмяCOMСоединителя()));
	// Word.Application
