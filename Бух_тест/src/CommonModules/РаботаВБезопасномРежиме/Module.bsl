...
Процедура ВыполнитьМетодОбъекта(Обработка, Знач ИмяПроцедуры, Знач Параметры = Неопределено) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректное значение параметра ИмяПроцедуры (%1)';uk='Некоректне значення параметра ИмяПроцедуры (%1)'"),
			ИмяПроцедуры);
	КонецПопытки;
...
"Не найден общий модуль ""%2"".';uk='Неправильний формат параметра ИмяЭкспортнойПроцедуры (%1):"
...
"или для этой коллекции не поддерживается безопасное выполнение.';uk='Неправильний формат параметра ИмяЭкспортнойПроцедуры (%1):"
"Не знайдена колекція об''єктів метаданих ""%2"","
"або для цієї колекції не підтримується безпечне виконання.'"),
				ИмяЭкспортнойПроцедуры,
				ИмяОбъекта);
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяЭкспортнойПроцедуры (%1):"
"Имя метода ""%2"" не соответствует требованиям образования имен переменных.';uk='Неправильний формат параметра ИмяЭкспортнойПроцедуры (%1):"
"Ім''я методу ""%2"" не відповідає вимогам утворення імен змінних.'"),
			ИмяЭкспортнойПроцедуры,
			ИмяМетода);
	КонецПопытки;
...
"заново включить его с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';uk='Неможливе виконання обробників установки параметрів сеансу через: профіль безпеки %1 не знайдений."
""
"Можливо, він був вилучений через консоль кластера."
"У цьому випадку потрібно відключити використання профілю безпеки через консоль кластера й"
"заново включити його за допомогою інтерфейсу конфігурації (відповідні команди перебувають у розділі настройок програми).'"),
					Профиль
				);
			КонецЕсли;
			ДоступенПривилегированныйРежим = Вычислить("ДоступенПереходВПривилегированныйРежим()");
...
"В этом случае требуется отключить использование профиля безопасности через консоль кластера и"
"заново включить его с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';uk='Неможливе виконання обробників установки параметрів сеансу через: профіль безпеки %1 не містить дозволу на використання привілейованого режиму."
""
"Можливо, він був відредагований через консоль кластера."
"У цьому випадку потрібно відключити використання профілю безпеки через консоль кластера й"
"заново включити його за допомогою інтерфейсу конфігурації (відповідні команди перебувають у розділі настройок програми).'",
					Профиль
				);
...
"совместно с включенными профилями безопасности.';uk='Неможливе виконання обробників установки параметрів сеансу через: %1."
""
"Можливо, для інформаційної бази через консоль кластера був установлений профіль безпеки,"
"не допускає виконання зовнішніх модулів без установки безпечного режиму."
"У цьому випадку потрібно відключити використання профілю безпеки через консоль кластера"
" і включити його за допомогою інтерфейсу конфігурації (відповідні команди перебувають у розділі"
"настройок програми). При цьому програма буде автоматично коректно настроєна на використання"
"разом із включеними профілями безпеки.'"),
