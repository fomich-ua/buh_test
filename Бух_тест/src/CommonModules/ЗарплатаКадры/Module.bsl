...
Процедура СоздатьВТФИООтветственныхЛиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, ИменаПолейОтветственныхЛиц, ИмяВременнойТаблицыДанныхДокумента) Экспорт
	...
	КадровыйУчет.СоздатьНаДатуВТКадровыеДанныеФизическихЛиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, СписокФизическихЛицСПериодом.ВыгрузитьКолонку("ФизическоеЛицо"), МассивНеобходимыхДанных, Периоды[0].Период);
...
Функция КраткийСоставСотрудников(Сотрудники, ДатаАктуальности, Физлица = Неопределено, ФизлицаСотрудников = Неопределено) Экспорт
	...
	Для Каждого Физлицо Из ПервыеФизлица Цикл
		...
		КраткийСостав = КраткийСостав + ФизическиеЛица.ФамилияИнициалыФизЛица(Неопределено, ФИО.Фамилия, ФИО.Имя, ФИО.Отчество);
	КонецЦикла;
...
// см. УправлениеДоступом.ЗаполнитьСвойстваВидаДоступа
//
Процедура УправлениеДоступомЗаполнитьСвойстваВидаДоступа(ВидыДоступа) Экспорт
	...
	//Если Свойства.ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.ГруппыФизическихЛиц Тогда
	//	Свойства.Таблицы.Добавить("Справочник.ФизическиеЛица");
	//	Свойства.ВидДоступаБезГруппЗначенияДоступа = Ложь;
	//ИначеЕсли Свойства.ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.ПодразделенияОрганизаций Тогда
	//	Свойства.Таблицы.Добавить("Справочник.ПодразделенияОрганизаций");
	//КонецЕсли;
	
КонецПроцедуры

// Обслуживание подсистемы УправлениеДоступом
// см. УправлениеДоступом.ЗаполнитьИспользованиеВидаДоступа
//
Процедура УправлениеДоступомЗаполнитьИспользованиеВидаДоступа(ВидДоступа, Использование) Экспорт
	
	//Если ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.ГруппыФизическихЛиц Тогда
	//	Использование = ПолучитьФункциональнуюОпцию("ОграничиватьДоступНаУровнеЗаписейФизическиеЛица");
	//ИначеЕсли ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.ПодразделенияОрганизаций Тогда
	//	Использование = ПолучитьФункциональнуюОпцию("ОграничиватьДоступНаУровнеЗаписейПодразделенияОрганизаций");
	//КонецЕсли;
...
Функция ОтладитьЗапрос(ИсходныйЗапрос, ИмяВременнойТаблицы = "") Экспорт
