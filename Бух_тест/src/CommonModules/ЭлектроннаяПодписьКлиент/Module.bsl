...
Процедура УстановитьРасширение(ОбработчикРезультата, ТекстВопроса = "", ЗаголовокВопроса = "") Экспорт
	...
	Если ПустаяСтрока(ЗаголовокВопроса) Тогда
		ЗаголовокВопроса = НСтр("ru='Расширение работы с криптографией';uk='Розширення роботи із криптографією'");
	КонецЕсли;
	
	Если ПустаяСтрока(ТекстВопроса) Тогда
		ТекстВопроса = НСтр("ru='Для продолжения необходимо установить"
"расширение работы с криптографией.';uk='Для продовження необхідно встановити"
...
"тип провайдера, алгоритмы подписи и хеширования.';uk='Не зазначений провайдер ЕП."
"У настройках криптографії вкажіть провайдера ЕП,"
"тип провайдера, алгоритми підпису й хешування.'");
	КонецЕсли;
