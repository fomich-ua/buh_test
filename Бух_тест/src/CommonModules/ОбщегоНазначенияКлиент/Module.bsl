...
//    ТекстСообщения = НСтр("ru='Для печати документа необходимо установить расширение работы с файлами.';uk='Для друку документа необхідно встановити розширення роботи з файлами.'");
...
"Перезапустить сейчас?';uk='Для завершення перереєстрації компоненти comcntr необхідно перезапустити програму."
...
//
// Пример: 
//  ТекстПредупреждения = НСтр("ru='Закрыть помощник?';uk='Закрити помічник?'");
...
Процедура РасширениеРаботыСФайламиПодключеноЗавершение(Действие, Параметры) Экспорт
	...
	Если ПустаяСтрока(Параметры.ТекстПредупреждения) Тогда
		ТекстСообщения = НСтр("ru='Действие недоступно, так как не установлено расширение для веб-клиента 1С:Предприятие.';uk='Дія недоступна, так як не встановлено розширення для веб-клієнта 1С:Підприємство.'")
	Иначе
