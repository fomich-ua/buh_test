...
//  URLВебСервисаПолучателя    - Строка - Веб-адрес подключаемой конечной точки.
//  ИмяПользователяПолучателя  - Строка - Пользователь для аутентификации в подключаемой конечной точке 
//                                        при работе через web-сервис подсистемы обмена сообщениями.
//  ПарольПолучателя           - Строка - Пароль пользователя в подключаемой конечной точке.
//  URLВебСервисаОтправителя   - Строка - Веб-адрес этой информационной базы со стороны подключаемой конечной точки.
//  ИмяПользователяОтправителя - Строка - Пользователь для аутентификации в этой информационной базе 
//                                        при работе через web-сервис подсистемы обмена сообщениями.
//  ПарольОтправителя          - Строка - Пароль пользователя в этой информационной базе.
//  КонечнаяТочка - ПланОбменаСсылка.ОбменСообщениями, Неопределено - Если подключение конечной точки завершилось успешно,
//                                        то в этот параметр возвращается ссылка на узел плана обмена,
...
//  КонечнаяТочка - ПланОбменаСсылка.ОбменСообщениями - Ссылка на узел плана обмена, который соответствует конечной точке.
//  URLВебСервисаПолучателя    - Строка - Веб-адрес конечной точки.
//  ИмяПользователяПолучателя  - Строка - Пользователь для аутентификации в конечной точке 
//                                        при работе через web-сервис подсистемы обмена сообщениями.
//  ПарольПолучателя           - Строка - Пароль пользователя в конечной точке.
//  URLВебСервисаОтправителя   - Строка - Веб-адрес этой информационной базы со стороны конечной точки.
//  ИмяПользователяОтправителя - Строка - Пользователь для аутентификации в этой информационной базе 
//                                        при работе через web-сервис подсистемы обмена сообщениями.
//  ПарольОтправителя          - Строка - Пароль пользователя в этой информационной базе.
//
...
Процедура ОтправитьСообщениеПолучателю(КаналСообщений, ТелоСообщения, Получатель, ЭтоБыстроеСообщение)
	...
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Получатель, "Заблокирована") Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка отправки сообщения заблокированной конечной точке ""%1"".';uk='Спроба відправки повідомлення заблокованій кінцевій точці ""%1"".'"),
			Строка(Получатель));
	КонецЕсли;
