...
"указано имя подсистемы ""%2"", которое уже зарегистрировано ранее.';uk='Помилка при підготовці описів підсистем:"
...
"не совпадает с версией конфигурации в метаданных ""%4"".';uk='Помилка при підготовці описів підсистем:"
...
"не збігається з версією конфігурації в метаданих ""%4"".'"),
	...
	Для каждого КлючИЗначение Из ВсеТребуемыеПодсистемы Цикл
		Если ОписанияПодсистем.ПоИменам.Получить(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при подготовке описаний подсистем:"
"не найдена подсистема ""%1"" требуемая для подсистем: %2.';uk='Помилка при підготовці описів підсистем:"
...
""
"Об''єкт метаданних не знайдений по повному імені:"
"""%1"".'"),
...
"в справочнике ""Идентификаторы объектов метаданных"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных ()."
""
...
""
"Для об''єкта метаданих ""%1"""
"знайдено кілька ідентифікаторів"
"у довіднику ""Ідентифікатори об''єктів метаданих "".'")
			+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика(),
			ПолноеИмяОбъектаМетаданных);
	КонецЕсли;
...
"которому соответствует удаленный объект метаданных.';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Для об''єкта метаданих ""%1"""
"знайдено ідентифікатор у довіднику ""Ідентифікатори об''єктів метаданих "","
"якому відповідає вилучений об''єкт метаданих.'")
			+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика(),
				ПолноеИмяОбъектаМетаданных);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"Для объекта метаданных ""%1"""
"найден идентификатор в справочнике ""Идентификаторы объектов метаданных"","
"который соответствует другому объекту метаданных ""%2"".';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных ()."
""
"Для об''єкта метаданих ""%1"""
"знайдено ідентифікатор у довіднику ""Ідентифікатори об''єктів метаданих "","
"який відповідає іншому об''єкту метаданих ""%2"".'")
			+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика(),
				ПолноеИмяОбъектаМетаданных,
...
"- РассылкаОтчетов,"
"- УправлениеДоступом.';uk=""Неможливо відключити довідник Ідентифікатори об'єктів метаданих,"
...
Функция ПодготовленныеОбработчикиСерверногоСобытия(Событие, Служебное = Ложь, ПерваяПопытка = Истина)
	...
	Если Обработчики = Неопределено Тогда
		Если Служебное Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найдено серверное служебное событие ""%1"".';uk='Не знайдене серверна службова подія ""%1"".'"), Событие);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найдено серверное событие ""%1"".';uk='Не знайдене серверна подія ""%1"".'"), Событие);
		КонецЕсли;
	КонецЕсли;
