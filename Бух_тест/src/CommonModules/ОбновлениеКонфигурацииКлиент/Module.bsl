...
//	ПроверитьОбновленияДляНовойРедакции - Булево - признак необходимости проверять обновления для новой редакции платформы
//
...
"%1';uk='Не вдалося створити тимчасовий каталог для перевірки наявності оновлень."
"%1'");
...
"%1';uk='Не вдалося перевірити наявність оновлень."
...
Процедура ПриВыполненииОбновленияИнформационнойБазы(ЗавершениеРаботыСистемы = Ложь) Экспорт
	...
	ОткрытьФорму("Обработка.ОбновлениеКонфигурации.Форма.Форма", ПараметрыФормы);
...
Функция ПолучитьПараметрыОбновления(ПроверитьОбновленияДляНовойРедакции = Ложь) Экспорт
	...
	СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации"		, НСтр("ru='Обновление информационной базы';uk='Оновлення інформаційної бази'"));
...
//
Функция АдресРесурсовДляПроверкиНаличияОбновления(ПроверитьОбновленияДляНовойРедакции = Ложь) Экспорт
	...
	КонецЕсли;
	Результат = ОбновлениеКонфигурацииКлиентСервер.ДобавитьКонечныйРазделительПути(НастройкиОбновления.АдресРесурсаДляПроверкиНаличияОбновления) +
...
Функция СледующаяРедакцияПлатформы()
...
Функция АдресСтраницыИнформацииОПолученииДоступаКПользовательскомуСайту() Экспорт
	...
	АдресСтраницы = ОбновлениеКонфигурацииКлиентПереопределяемый.АдресСтраницыИнформацииОПолученииДоступаКПользовательскомуСайту();
...
Процедура ПроверитьОбновлениеПоРасписанию() Экспорт
	...
	Если Параметры.ИсточникОбновления <> -1 И Параметры.ИмяСтраницы = НаименованиеСтраницыДоступногоОбновления Тогда
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),,
			НСтр("ru='Обнаружена новая версия конфигурации:';uk='Виявлено нову версію конфігурації:'") + " " +
...
"(источник: %2; приемник: %3)';uk='Помилка при копіюванні: %1"
"(джерело: %2; приймач: %3)'"), 
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ИмяФайлаИсточник, ИмяФайлаНазначение);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Предупреждение", Сообщение);
		Возврат Ложь;
	КонецПопытки;
	Возврат Истина;
КонецФункции
...
Функция ПолучитьПараметрыРелизовНаДискеИТС(ДискИТС, Знач ВыдаватьСообщения = Истина) Экспорт
	...
	Если НЕ СкопироватьФайл(
		ОбновлениеКонфигурацииКлиентСервер.ДобавитьКонечныйРазделительПути(ДискИТС) + 
	...
	Если ТипЗнч(ПараметрыРелизовНаДискеИТС) <> Тип("Структура") Тогда // ошибки при чтении файла
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru='Обновления на диске 1С:ИТС не найдены.';uk='Оновлення на диску 1С:ІТС не знайдені.'"));
		КонецЕсли;
		Возврат Неопределено;		
   	КонецЕсли;
...
Функция ПрочитатьАдресаРелизовНаДискеИТС(Знач ИмяФайла, Знач ДискИТС)
	...
	Попытка
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			ЧтениеXML.Прочитать();
			ЗначениеПутиКаталога = ОбновлениеКонфигурацииКлиентСервер.ДобавитьКонечныйРазделительПути(ДискИТС) + 
				ОбновлениеКонфигурацииКлиентСервер.ДобавитьКонечныйРазделительПути(СтроковыеФункцииКлиентСервер.СократитьДвойныеКавычки(
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", 
			НСтр("ru=' ПрочитатьАдресаРелизовНаДискеИТС:';uk=' ПрочитатьАдресаРелизовНаДискеИТС:'")
				+ " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Неопределено;
	КонецПопытки;
...
Функция ПроверятьОбновлениеДляСледующейРедакцииПлатформы()
	...
	ПроверятьОбновление = ОбновлениеКонфигурацииКлиентПереопределяемый.ПроверятьОбновлениеДляСледующейРедакцииПлатформы();
	Если ПроверятьОбновление = Неопределено Тогда
...
Процедура ПриЗавершенииРаботыСистемы(Предупреждения) Экспорт
	...
	Если ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса = Истина Тогда
		...
		ДействиеПриУстановленномФлажке.Форма = "Обработка.ОбновлениеКонфигурации.Форма.Форма";
...
Функция ПроверитьЛегальностьСкачиванияОбновления(ПараметрыЗапроса) Экспорт
	...
	ПроверятьЛегальность = ОбновлениеКонфигурацииКлиентПереопределяемый.ИспользоватьПроверкуЛегальностиСкачиванияОбновления();
	Если ПроверятьЛегальность = Неопределено Тогда
...
"Повторите попытку позднее';uk='Сервіс перевірки легальності одержання оновлень тимчасово недоступний."
...
"Проверьте параметры подключения к Интернету';uk='Не вдалося підключитися до сервісу перевірки легальності одержання оновлень."
...
"Обратитесь к администратору';uk='Помилка виконання перевірки легальності одержання оновлення."
"Зверніться до адміністратора'");
	...
	Иначе
		...
		Результат = СтруктураОтветаВебСервера(Нстр("ru='Неожиданный ответ сервиса проверки легальности получения обновлений';uk='Несподівана відповідь сервісу перевірки легальності одержання оновлень'"));
...
"Не удалось получить WSDL-описание с сервера проверки легальности скачивания обновления (%1): %2.';uk='Помилка при створенні опису веб-сервіса."
...
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления: %2.';uk='Помилка при створенні опису веб-сервіса (%1)."
...
"Отсутствует элемент описания типов данных (<wsdl:types ...>).';uk='Помилка при створенні опису веб-сервіса (%1)."
...
"Відсутній елемент опису типів даних (<wsdl:types ...>).'"), АдресWSDL);
	...
	Если УзелОписанияСхемы = Неопределено Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1)."
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:"
"Отсутствует элемент описания типов данных (<xs:schema ...>)';uk='Помилка при створенні опису веб-сервіса (%1)."
"Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:"
...
"%2';uk='Помилка при створенні опису веб-сервіса (%1)."
...
"%2'"), АдресWSDL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1)."
"Ошибка при создании фабрики XDTO из WSDL-описания веб-сервиса проверки легальности скачивания обновления: %2';uk='Помилка при створенні опису веб-сервіса (%1)."
"Помилка при створенні фабрики XDTO з WSDL-опису веб-сервіса перевірки легальності завантаження оновлення: %2'"),
	...
	Если НЕ ЗначениеЗаполнено(URIСервиса) Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1)."
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:"
"Отсутствует URI пространства имен в WSDL-описании.';uk='Помилка при створенні опису веб-сервіса (%1)."
"Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:"
"Відсутній URI простору імен в WSDL-описі.'"), АдресWSDL);
	...
	Если УзлыСервисов.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1)."
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:"
"Отсутствует описание веб-сервисов в WSDL-описании (<wsdl:service ...>).';uk='Помилка при створенні опису веб-сервіса (%1)."
"Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:"
"Відсутнє опис веб-сервісів в WSDL-описі (<wsdl:service ...>).'"), АдресWSDL);
	...
	Если УзлыПортов.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1)."
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:"
"Отсутствует описание портов в WSDL-описании (<wsdl:port ...>).';uk='Помилка при створенні опису веб-сервіса (%1)."
"Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:"
"Відсутнє опис портів в WSDL-описі (<wsdl:port ...>).'"), АдресWSDL);
	...
	Если НЕ ЗначениеЗаполнено(АдресПорта) Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1)."
"Ошибка чтения WSDL-описания веб-сервиса интернет-поддержки пользователей:"
"Не удалось определить URL заданного порта сервиса (%2).';uk='Помилка при створенні опису веб-сервіса (%1)."
...
"Ошибка SOAP: %2';uk='Помилка при виклику операції isReady сервісу (%1)"
...
"%1"
"Тело ответа:"
"%2';uk='Помилка читання відповіді SOAP:"
" %1"
"Тіло відповіді:"
...
"%2"
"Тело ответа:"
"%3';uk=""Помилка читання об'єкта (%1) у конверті SOAP:"
" %2"
"Тіло відповіді:"
