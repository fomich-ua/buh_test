...
"может быть зависимость только от права Чтения.';uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом."
"У наборі значень доступу вказаний вид доступу ПравоИзменения"
"таблиці з идентификтором ""%1""."
"В обмеженні перевірки ролі (як додаткового права)"
...
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом."
"Набір значень доступу містить відомий вид доступу ""%2"","
...
"""ПравоЧтения"", ""ПравоИзменения"", якщо вони використовуються.'"),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
"Набор значений доступа содержит неизвестный вид доступа ""%2"".';uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом."
"Набір значень доступу містить невідомий вид доступу ""%2"".'"),
				ТипЗнч(СсылкаНаОбъект),
				Строка.ВидДоступа);
		КонецЕсли;
...
"общего модуля УправлениеДоступом.';uk='Помилка в процедурі ПриСозданииФормыЗначенияДоступа"
...
"доступа у реквизита ГруппаДоступа.';uk='Для значень доступу типу ""%1"""
...
"общего модуля УправлениеДоступом.';uk='Помилка в процедурі ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа"
"загального модуля УправлениеДоступом.'");
...
//
// Параметры:
//  СсылкаНаОбъект - СправочникСсылка, ДокументСсылка и другие типы ссылок тех объектов
//                   метаданных, для которых заполняются наборы значений доступа.
//
...
//
//  Чтобы добавить значение доступа нужно указать полное имя предопределенного элемента,
// например, "Справочник.ГруппыПользователей.ВсеПользователи".
//
...
//	ОписаниеПрофиля.Наименование  = НСтр("ru='Пользователь';uk='Користувач'");
...
"%1.';uk='Помилка в параметрах процедури ПереименоватьПравоВНастройкахПравобъектов"
"загального модуля УправлениеДоступом."
""
...
""
"Указывать требуется только специальные виды доступа"
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';uk='Об''єкт ""%1"" сформував набір значень доступу,"
"який містить відомий вид доступу ""%2"", який не потрібно вказувати."
""
"Вказувати потрібно тільки спеціальні види доступу"
"""ПравоЧтения"", ""ПравоИзменения"", якщо вони використовуються.'"),
				ТипЗнч(СсылкаНаОбъект),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект ""%1"" сформировал набор значений доступа,"
"содержащий неизвестный вид доступа ""%2"".';uk='Об''єкт ""%1"" сформував набір значень доступу,"
...
Функция СвойстваГруппЗначенияДоступа(ТипЗначенияДоступа, ЗаголовокОшибки)
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС + Символы.ПС +
			НСтр("ru='Для значений доступа типа ""%1"""
"не используются группы значений доступа.';uk='Для значень доступу типу ""%1"""
