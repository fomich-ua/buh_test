...
Функция ОтносительныйКаталогОбменаИнформацией(Знач Каталог)
	...
	Если ПустаяСтрока(Каталог) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат каталога обмена информацией ""%1"".';uk='Неправильний формат каталогу обміну інформацією ""%1"".'"),
			Каталог);
	КонецЕсли;
...
Процедура ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДанными) Экспорт
	...
	Если СтрокаСценария.НомерИнформационнойБазы = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неизвестное действие ""%1"" при обмене данными между областями данных.';uk='Невідома дія ""%1"" при обміні даними між областями даних.'"),
...
"Фактическое значение ""%2"".';uk='Помилка призначення коду вузла кореспондента."
...
Процедура СохранитьДанныеСессии(Знач Сообщение, Знач Представление = "") Экспорт
	...
	СтрокаСообщения = НСтр("ru='Сессия обмена сообщениями системы ""%1"" успешно завершена.%2';uk='Сесія обміну повідомленнями системи ""%1"" успішно завершена.%2'",
...
"Описание ошибки из корреспондента: %3';uk='Помилка виконання сесії обміну повідомленнями системи ""%1"".%2"
...
Функция СобытиеЖурналаРегистрацииНастройкаСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Настройка синхронизации данных';uk='Обмін даними в моделі сервісу.Настройка синхронізації даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииМониторСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Монитор синхронизации данных';uk='Обмін даними в моделі сервісу.Монітор синхронізації даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Синхронизация данных';uk='Обмін даними в моделі сервісу.Синхронізація даних'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСессииОбменаСообщениямиСистемы()
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Сессии обмена сообщениями системы';uk='Обмін даними в моделі сервісу.Сесії обміну повідомленнями системи'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
"Обратитесь к администратору.';uk='Зміна нерозподілених даних (%1), що завантажуються з програми, в Автономному робочому місці заборонена."
