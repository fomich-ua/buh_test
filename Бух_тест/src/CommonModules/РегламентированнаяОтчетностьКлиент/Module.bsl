...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ДЛЯ РАБОТЫ С 1С:Звіт
	...
	Иначе
		...
		Если ПравоДоступаКОтчету = Ложь ИЛИ ПравоДоступаКОтчету = Неопределено Тогда
			...
			Если ВыводитьСообщенияОбОшибках Тогда                                                                                                                            
				Сообщить(НСтр("ru='Недостаточно прав на использование модуля взаимодействия с ""1С:Звіт""!';uk='Недостатньо прав на використання модуля взаємодії зі ""1С:Звіт""!'"));
			КонецЕсли;			
...
// Функция выполняет запуск  1С:Звіт"
//
...
// Функция выполняет запуск запуск пункта меню "Выгрузка налоговых документов в 1С:Звіт"
//
...
// Функция выполняет запуск пункта меню "Проверка входящих налоговых документов в "1С:Звіт"
//
...
Процедура ПроверитьВозможностьОткрытияДочернейФормыРегламентиованногоОтчета(ЭтаФорма, ВыполняемоеОповещение) Экспорт	
	...
	// Не во всех конфигурациях использующих рег отчеты есть поддержка 1С:Звіт
	Попытка
	...
	ТекстВопроса = НСтр("ru = 'Внимание! Начиная с "+ДатаСтрока+" для формирования регламентированных отчетов по организации """+ОрганизацияСтрока+""" используется ""1С:Звіт"". 
	                    |
	                    |Прервать выполнение операции и запустить ""1С:Звіт""?'; uk = 'Увага! Починаючи з "+ДатаСтрока+" для формування регламентованих звітів по організації """+ОрганизацияСтрока+""" використовується ""1С:Звіт"".
	                    |
...
Процедура ПроверитьВозможностьОткрытияДочернейФормыРегламентиованногоОтчетаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
    ...
    ИначеЕсли Ответ = КодВозвратаДиалога.Да Тогда
        ...
        Если МенеджерЗвит1С.ЗапуститьЗвит1С(Истина) Тогда
            // Если запустили 1С:Звіт, форма нам больше не нужна
            ЭтаФорма.Закрыть();
        КонецЕсли;
