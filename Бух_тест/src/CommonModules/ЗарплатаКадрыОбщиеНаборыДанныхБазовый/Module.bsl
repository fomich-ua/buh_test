...
Функция ПолучитьТекстОбщегоЗапроса(ИсточникДанных) Экспорт
	...
	Если ВРег(ИсточникДанных) = ВРег("НачисленияУдержания") Тогда
		...
		|	NULL
		|ИЗ
		|   РегистрНакопления.НДФЛПоСотрудникам.Обороты(&НачалоПериода {(&НачалоПериода)}, &КонецПериода {(&КонецПериода)}, Месяц, ДоходНДФЛ <> ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.ВоенныйСбор) {(Организация), (ФизическоеЛицо)}) КАК НДФЛПоСотрудникамОбороты
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
