		...
		Если ИскатьКлюч И СтруктураПриемник.Свойство(КлючИЗначение.Ключ) Тогда
			...
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Пересечение структур источника и приемника по ключу ""%1"".';uk='Перетинання структур джерела й приймача по ключу ""%1"".'"),
					КлючИЗначение.Ключ);
			КонецЕсли
		КонецЕсли;
...
// Устарела. Следует использовать ОбщегоНазначения.ПодсистемаСуществует или ОбщегоНазначенияКлиент.ПодсистемаСуществует.
...
// Устарела. Следует использовать ОбщегоНазначения.ОбщийМодуль или ОбщегоНазначенияКлиент.ОбщийМодуль.
...
// Переносит отборы из параметров формы в динамический список.
//
// Параметры:
//   Форма - УправляемаяФорма - Форма, из параметров открытия которой следует перенести отборы.
//   ДинамическийСписок - ДинамическийСписок - Необязательный. Список, в котором следует установить отборы.
//       Если не передан, то ожидается реквизит формы с именем "Список" соответствующего типа.
//
// Важно:
//   После завершения процедуры коллекция Форма.Параметры.Отбор очищается.
//   Поэтому если коллекция "Форма.Параметры.Отбор" используется в коде формы,
//   то эту процедуру следует вызывать после исполнения этого кода.
//   Например в подвале обработчика ПриСозданииНаСервере.
//
Процедура ПеренестиОтборыВДинамическийСписок(Форма, ДинамическийСписок = Неопределено) Экспорт
	Перем ВидСравнения;
...
//
// Возвращаемое значение:
//   ОтображениеОбычнойГруппы - ОтображениеОбычнойГруппы.СлабоеВыделение.
//
...
//
// Возвращаемое значение:
//   ОтображениеОбычнойГруппы - ОтображениеОбычнойГруппы.ОбычноеВыделение.
//
Функция ОтображениеОбычнойГруппыОтступ() Экспорт
...
//
// Возвращаемое значение:
//   ОтображениеОбычнойГруппы - ОтображениеОбычнойГруппы.СильноеВыделение.
//
Функция ОтображениеОбычнойГруппыРамкаГруппы() Экспорт
...
//   Поддерживаются только те таблицы, которые могут содержать предопределенные элементы:
	...
	Если Условие <> Истина Тогда
		...
		Если Не ПустаяСтрока(КонтекстПроверки) Тогда
			ТекстИсключения = ТекстИсключения + " " +
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='в %1';uk='в %1'"), КонтекстПроверки);
		КонецЕсли;
...
"Очікувалося: %3; передане значення: %4 (тип %5).'");
	Проверить((ЭтоОписаниеТипов И ОжидаемыеТипы.СодержитТип(ТипЗнч(ЗначениеПараметра)))
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НедопустимыйПараметр, 
			...
			?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, НСтр("ru='Неопределено';uk='Невизначено'")), ТипЗнч(ЗначениеПараметра)));
...
"В структуре ожидалось свойство %3 (тип %4).';uk='Неприпустиме значення параметра %1 (Структура) в %2. "
...
"Ожидалось: %4; передано значение: %5 (тип %6).';uk='Неприпустиме значення властивості %1 у параметрі %2 (Структура) в %3. "
		...
		Для каждого Свойство Из ОжидаемыеТипыСвойств Цикл
			...
			Проверить((ЭтоОписаниеТипов И ОжидаемыйТипСвойства.СодержитТип(ТипЗнч(ЗначениеСвойства)))
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НедопустимоеСвойство, 
					...
					?(ЗначениеСвойства <> Неопределено, ЗначениеСвойства, НСтр("ru='Неопределено';uk='Невизначено'")), ТипЗнч(ЗначениеСвойства)));
