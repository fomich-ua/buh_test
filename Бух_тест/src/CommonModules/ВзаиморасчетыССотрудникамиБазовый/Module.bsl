...
Процедура РассчитатьНалоги(ТаблицаЗарплат, ДанныеДляЗаполнения, ДанныеДляРасчета) Экспорт
	...
	Иначе
		//Вариант 2 - определим категорию начисления у предопределенного способа выплаты
		Если ДанныеДляРасчета.СпособВыплаты = Справочники.ВидыВыплат.Аванс
		 или ДанныеДляРасчета.СпособВыплаты = Справочники.ВидыВыплат.АвансПредварительныйРасчет Тогда
			КатегорияНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПовременнаяОплатаТруда;
	 	Иначе
		КонецЕсли;
	...
	Запрос.Текст =
		...
		|	 ИНАЧЕ Зарплата.СуммаГрязными - ЕСТЬNULL(Взносы.Сумма,0) - ЕСТЬNULL(НДФЛ.Сумма,0) 
		|	КОНЕЦ КАК КВыплате,
		...
		|    	Взносы.ФизическоеЛицо
		|	) КАК Взносы
		|   ПО Взносы.ФизическоеЛицо = Зарплата.ФизическоеЛицо
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|	(ВЫБРАТЬ 
		|		НДФЛ.ФизическоеЛицо,
		...
		|    	НДФЛ.ФизическоеЛицо
		|	) КАК НДФЛ
		|   ПО НДФЛ.ФизическоеЛицо = Зарплата.ФизическоеЛицо
		|;
	    |
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		ТаблицаЗарплат.Зарплата = РезультатыЗапроса[0].Выгрузить();
