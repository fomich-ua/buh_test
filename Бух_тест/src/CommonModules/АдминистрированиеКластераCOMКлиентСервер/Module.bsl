...
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Сеанс Из Сеансы Цикл
		...
		Исключение
			...
			Если ПолучитьСеансы(СоединениеСАгентомСервера, Кластер, ОписаниеИнформационнойБазы, ОтборСеансов, Ложь).Количество() > 0 Тогда
				ВызватьИсключение;
			Иначе
				Продолжить;
			КонецЕсли;
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	ПроцессыИСоединения = ПолучитьСоединения(
		COMСоединитель,
		СоединениеСАгентомСервера,
		Кластер,
		ПараметрыАдминистрированияИБ.ИмяВКластере,
	...
	Для Каждого ПроцессИСоединение Из ПроцессыИСоединения Цикл
		
		Попытка
			
			ПроцессИСоединение.СоединениеСРабочимПроцессом.Disconnect(ПроцессИСоединение.Соединение);
			
		Исключение
			
			// Соединение могло уже завершиться к моменту вызова Disconnect.
			
			ОтборСоединений = Новый Структура("Номер", ПроцессИСоединение.Соединение.Номер);
...
Функция COMСоединитель()
	...
	#Иначе
		...
		Возврат Новый COMОбъект(ОбщегоНазначения.ИмяCOMСоединителя());
...
Функция ПолучитьКластер(СоединениеСАгентомСервера, Знач ПортКластера, Знач ИмяАдминистратораКластера, Знач ПарольАдминистратораКластера)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='На рабочем сервере %1 не найден кластер %2';uk='На робочому сервері %1 не знайдений кластер %2'"),
...
Функция СоединениеСРабочимПроцессом(COMСоединитель, СоединениеСАгентомСервера, Кластер)
	...
	Для Каждого РабочийПроцесс Из СоединениеСАгентомСервера.GetWorkingProcesses(Кластер) Цикл
		Если РабочийПроцесс.Running Тогда
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='В кластере серверов %1:%2 не найдено активных рабочих процессов.';uk='В кластері серверів %1:%2 не знайдено активних робочих процесів.'"),
		Кластер.HostName,
...
Функция ПолучитьОписаниеИБ(СоединениеСАгентомСервера, Кластер, Знач ИмяВКластере)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='В кластере серверов %1:%2 не найдена информационная база ""%3""!';uk='В кластері серверів %1:%2 не знайдена інформаційна база ""%3""!'"),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяВКластере
	);
	...
	Для Каждого ИнформационнаяБаза Из СоединениеСРабочимПроцессом.GetInfoBases() Цикл
		...
		Если ИнформационнаяБаза.Name = ИмяВКластере Тогда
			...
			Если Не ЗначениеЗаполнено(ИнформационнаяБаза.DBMS) Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").';uk='Неправильні ім''я й пароль адміністратора інформаційної бази %1 у кластері серверів %2:%3 (ім''я: ""%4"").'"),
					ИмяВКластере,
					Кластер.HostName, Кластер.MainPort,
					ИмяАдминистратораИБ);
...
//    2. Структура (упрощенный вариант), ключ - Имя свойства соединения (см. выше), значение - значение, с которым выполняется
//      сравнение. При использовании данного варианта описания фильтра сравнение всегда выполняется на равенство,
//  Описания - Булево, если значение равно Ложь, функция вернет массив COMОбъектов, реализующих интерфейс IConnectionShort,
//    иначе - массив структур, описывающих свойства соединений (поля структур - см. возвращаемое значение функции
//    АдминистрированиеКластераКлиентСервер.СвойстваСоединения()).
//
// Возвращаемое значение: Массив(COMОбъект), Массив(Структура).
//
Функция ПолучитьСоединения(COMСоединитель, СоединениеСАгентомСервера, Кластер, Знач ИмяВКластере, Знач ИмяАдминистратораИБ, Знач ПарольАдминистратораИБ, Знач Фильтр = Неопределено, Знач Описания = Ложь)
...
Функция ПолучитьПрофильБезопасности(СоединениеСАгентомСервера, Кластер, ИмяПрофиля)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='В кластере серверов %1:%2 не найден профиль безопасности ""%3""!';uk='В кластері серверів %1:%2 не знайдений профіль безпеки ""%3""!'"),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяПрофиля
	);
...
//  ИмяПрофиля - Строка, имя профиля безопасности.
//
// Возвращаемое значение: Массив(COMОбъект) - массив COM-объектов, реализующих интерфейс ISecurityProfileVirtualDirectory.
//
...
//  ИмяПрофиля - Строка, имя профиля безопасности.
//
// Возвращаемое значение: Массив(COMОбъект) - массив COM-объектов, реализующих интерфейс ISecurityProfileInternetResource.
//
