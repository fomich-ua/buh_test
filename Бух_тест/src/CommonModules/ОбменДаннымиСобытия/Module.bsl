...
"Описание ошибки:"
"%2';uk=""Помилка виконання правил реєстрації об'єктів для плану обміну %1."
"Опис помилки:"
"%2"""),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"План обмена: [ИмяПланаОбмена];"
"Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';uk='Помилка при виконанні обробника: ""[ИмяОбработчика]"";"
"План обміну: [ИмяПланаОбмена];"
...
"Описание ошибки: [Описание]';uk='Помилка при виконанні обробника: ""[ИмяОбработчика]""; План обміну: [ИмяПланаОбмена]; Об''єкт метаданих: [ОбъектМетаданныхИмя]"
"Опис помилки: [Описание]'");
	...
	Если ЕстьКоллизия Тогда
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Комментарий = НСтр("ru='Принятая версия (автоматическое разрешение конфликта).';uk='Прийнята версія (автоматичний дозвіл конфлікту).'");
				
				ЭлементДанных.ДополнительныеСвойства.СведенияОВерсииОбъекта = Новый ФиксированнаяСтруктура(СведенияОВерсииОбъекта);
				
			Иначе
				
				Комментарий = НСтр("ru='Отклоненная версия (автоматическое разрешение конфликта).';uk='Відхилена версія (автоматичний дозвіл конфлікту).'");
...
Процедура ЗарегистрироватьЗапретЗагрузкиДанныхПоДате(ЭлементДанных, Отправитель, СообщениеОбОшибке)
	...
	Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(Отправитель.Метаданные()) Тогда
	 	СсылкаНаОтправителя = Отправитель.Ссылка;		
	КонецЕсли;
	Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(СсылкаНаОтправителя) И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементДанных.Метаданные()) Тогда
		...
		Если ОбщегоНазначения.СсылкаСуществует(СсылкаНаОбъект) Тогда
			...
			ПриСозданииВерсииОбъекта(ОбъектВБазе,,, НСтр("ru='Версия создана при синхронизации данных.';uk='Версія створена при синхронізації даних.'"));
		...
		ПриСозданииВерсииОбъекта(ЭлементДанных, СсылкаНаОтправителя, ТипВерсииОбъекта, СтрокаСообщенияОбОшибке);
...
"Объект этой информационной базы был заменен версией объекта из второй информационной базы.';uk=""Виник конфлікт змін об'єктів."
"Об'єкт цієї інформаційної бази був замінений версією об'єкта із другої інформаційної бази.""");
		
	Иначе
		
		ТекстПредупрежденияЖР = НСтр("ru='Возник конфликт изменений объектов."
"Объект из второй информационной базы не принят. Объект этой информационной базы не изменен.';uk=""Виник конфлікт змін об'єктів."
