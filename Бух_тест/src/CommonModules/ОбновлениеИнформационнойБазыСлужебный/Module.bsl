...
"(роли ""Администратор системы"" и ""Полные права"").';uk='Вхід у програму тимчасово неможливий у зв''язку з оновленням на нову версію."
...
"Обратитесь к администратору сервиса за подробностями.';uk=""Вхід у додаток тимчасово неможливий у зв'язку з оновленням на нову версію."
...
"Зверніться до адміністратора сервісу за подробицями.""");
	Иначе
		...
		Сообщение =
			НСтр("ru='Вход в программу временно невозможен в связи с обновлением на новую версию."
"Обратитесь к администратору за подробностями.';uk=""Вхід у програму тимчасово неможливий у зв'язку з оновленням на нову версію."
...
"Будет выполнен переход с другой программы.';uk='Змінилося ім''я конфігурації на ""%1""."
...
"Будет выполнено обновление информационной базы.';uk='Змінився номер версії конфігурації: з ""%1"" на ""%2""."
"Буде виконане оновлення інформаційної бази.'"),
	...
	Попытка
		...
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И
			...
			ЗапланироватьОтложенноеОбновление(ИтерацииОбновления);
...
"Ожидалось: %3; передано значение: %4 (тип %5).';uk='Неприпустиме значення параметра %1 в %2. "
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Иначе
			...
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = НСтр("ru='Выполняется обновление информационной базы с версии %1 на версию %2.';uk='Виконується оновлення інформаційної бази з версії %1 на версію %2.'");
			Иначе
				Сообщение = НСтр("ru='Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.';uk='Виконується оновлення дані бібліотеки %3 з версії %1 на версію %2.'");
			КонецЕсли;
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение,
		...
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = НСтр("ru='Выполнено обновление информационной базы с версии %1 на версию %2.';uk='Виконано оновлення інформаційної бази з версії %1 на версію %2.'");
			Иначе
				Сообщение = НСтр("ru='Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.';uk='Виконано оновлення дані бібліотеки %3 з версії %1 на версію %2.'");
			КонецЕсли;
...
""
"Подробности ошибки:"
"%1';uk='Неможливо виконати оновлення інформаційної бази:"
...
"""%1"""
"произошла ошибка:"
"""%2"".';uk='При виклику обробника переходу з іншої програми"
"""%1"""
"відбулася помилка:"
"""%2"".'"),
				ИмяОбработчика,
...
Процедура ЗапланироватьОтложенноеОбновление(ИтерацииОбновления)
...
"не должен быть установлен признак ""МонопольныйРежим"".';uk='У відкладеного обробника ""%1"""
...
"так как он выполняется только при переходе на версию %2';uk='Обробник оновлення %1 не може бути відключений, "
...
"""%1"""
"произошла ошибка:"
"""%2"".';uk='При виклику обробника оновлення:"
"""%1"""
"відбулася помилка:"
"""%2"".'"),
			ИмяОбработчика,
...
"При активных транзакциях, открытых выше по стеку, исключение также необходимо пробрасывать выше по стеку.';uk='Помилка виконання обробника оновлення %1:"
...
"При активних транзакціях, відкритих вище по стеку, виключення також необхідно прокидати вище по стеку.'");
		...
		Иначе
			ШаблонКомментария = НСтр("ru='Ошибка выполнения обработчика обновления %1:"
"Обработчиком обновления была закрыта лишняя транзакция, открытая ранее (выше по стеку).';uk='Помилка виконання обробника оновлення %1:"
"Обробником оновлення була закрита зайва транзакція, відкрита раніше (вище по стеку).'");
	...
	Иначе
		Если ТранзакцияАктивна() Тогда
			ШаблонКомментария = НСтр("ru='Ошибка выполнения обработчика обновления %1:"
"Открытая внутри обработчика обновления транзакция осталась активной (не была закрыта или отменена).';uk='Помилка виконання обробника оновлення %1:"
...
"Правильный формат, например: ""2.1.3.70"".';uk='В обробника не правильно заповнена властивість Версія: ""%1""."
"Правильний формат, наприклад: ""2.1.3.70"".'"),
			...
			Если НулеваяВерсия Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У обработчика не правильно заполнено свойство Версия: ""%1""."
"Версия не может быть нулевой.';uk='В обробника не правильно заповнена властивість Версія: ""%1""."
...
"свойство ВыполнятьВГруппеОбязательных.';uk='В обробника не правильно заповнена властивість Приоритет або"
...
"Допустимое значение: ""Монопольно"", ""Отложенно"", ""Оперативно"".';uk='В обробника ""%1"" не правильно заповнена властивість РежимВыполнения."
...
"свойство НачальноеЗаполнение.';uk='В обробника не правильно заповнена властивість Опциональный або"
	...
	Для Каждого ДеревоОбработчиковБиблиотека Из СведенияОбОбновлении.ДеревоОбработчиков.Строки Цикл
		...
		Для Каждого ДеревоОбработчиковВерсия Из ДеревоОбработчиковБиблиотека.Строки Цикл
			...
			Для Каждого ОбработчикОбновления Из ДеревоОбработчиковВерсия.Строки Цикл
				...
				Попытка
					СообщениеОЗапущенномОбработчике = НСтр("ru='Выполняется процедура обновления ""%1"".';uk='Виконується процедура оновлення ""%1"".'");
