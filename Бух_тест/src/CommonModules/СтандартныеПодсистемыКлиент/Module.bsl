...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Иначе
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, 
			НСтр("ru='Не установлены разделители';uk='Не встановлені роздільники'"), ПараметрыКлиента.ПодробнаяИнформация);
	КонецЕсли;
...
"Необходимо предварительно обновить версию платформы 1С:Предприятие.';uk='Вхід у програму неможливий."
"Необхідно попередньо оновити версію платформи 1С:Підприємство.'");
		Иначе
			ТекстСообщения = НСтр("ru='Вход в программу невозможен."
"Необходимо обратиться к администратору для обновления версии платформы 1С:Предприятие.';uk='Вхід у програму неможливий."
"Необхідно звернутися до адміністратора для оновлення версії платформи 1С:Підприємство.'");
		КонецЕсли;
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения = 
				НСтр("ru='Рекомендуется завершить работу программы и обновить версию платформы 1С:Предприятия."
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';uk='Рекомендується завершити роботу програми й оновити версію платформи 1С:Підприємства."
"У противному випадку деякі можливості програми будуть недоступні або будуть працювати некоректно.'");
		Иначе
			ТекстСообщения = 
...
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';uk='Рекомендується завершити роботу програми й звернутися до адміністратора для оновлення версії платформи 1С:Підприємства."
"У противному випадку деякі можливості програми будуть недоступні або будуть працювати некоректно.'");
		КонецЕсли;
	КонецЕсли;
...
"Обратитесь к администратору за подробностями.';uk=""Вхід у програму тимчасово неможливий до оновлення зв'язку з головним вузлом."
...
"Все функции - Стандартные - Активные пользователи.';uk='Для того щоб відкрити список активних користувачів, перейдіть у меню"
...
"дополнительным параметром необходимо указать значение разделителя.';uk='При вказівці параметра запуску ВойтиВОбластьДанных,"
