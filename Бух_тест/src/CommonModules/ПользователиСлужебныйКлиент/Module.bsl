...
"и открыть нужный отчет или форму.';uk='Для перегляду настройок іншого користувача необхідно запустити програму від його імені "
"і відкрити потрібний звіт або форму.'");
...
"в командном интерфейсе программы.';uk='Для перегляду настройок робочого стола перейдіть до розділу ""Робочий стіл"""
"у командному інтерфейсі програми.'"));
					Возврат;
				КонецЕсли;
...
"командного интерфейса программы.';uk='Для перегляду настройок командного інтерфейсу виберіть потрібний розділ"
"командного інтерфейсу програми.'"));
					Возврат;
				КонецЕсли;
				
				Если РодительЭлемента <> Неопределено Тогда
					ТекстПредупреждения = НСтр("ru='Для просмотра данной настройки необходимо открыть ""%1"" "
"и затем перейти к форме ""%2"".';uk='Для перегляду даної настройки необхідно відкрити ""%1"" "
...
Функция ФормированиеПоясненияПриКопировании(ПредставлениеНастройки, КоличествоНастроек, ПояснениеКомуСкопированыНастройки) Экспорт
	...
	Если КоличествоНастроек = 1 Тогда
		...
		ТекстПояснения = НСтр("ru='""%1"" скопирована %2';uk='""%1"" скопійована %2'");
	...
	Иначе
		...
		ТекстПояснения = НСтр("ru='Скопировано %1 %2';uk='Скопійовано %1 %2'");
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПояснениеПользователи(КоличествоПользователей, Пользователь) Экспорт
	...
	Если КоличествоПользователей = 1 Тогда
		ПояснениеКомуСкопированыНастройки = НСтр("ru='пользователю ""%1""';uk='користувачеві ""%1""'");
	...
	Иначе
		ПояснениеКомуСкопированыНастройки = НСтр("ru='%1 пользователям';uk='%1 користувачам'");
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
