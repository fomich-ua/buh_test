...
"Подобрать?';uk='Існує можливість підібрати країну світу із класифікатора."
"Підібрати?'");
								
		ОбработчикОповещения = Новый ОписаниеОповещения("ОповещениеВопросОПодбореИзКлассификатора", ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Нет);
	КонецЕсли;
КонецПроцедуры
