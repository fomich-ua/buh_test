...
Процедура ОбновитьПроизводственныеКалендари(ТаблицаКалендарей) Экспорт
	...
	|	ВЫРАЗИТЬ(КлассификаторТаблица.Description"+Язык+" КАК СТРОКА(100)) КАК Наименование
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаписатьДанныеПроизводственногоКалендаря(СправочникОбъект.Ссылка,НомерТекущегоГода,	Справочники.ПроизводственныеКалендари.РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(СправочникОбъект.Код, НомерТекущегоГода));
	КонецЦикла;
...
Процедура ОбновитьДанныеПроизводственныхКалендарей(ТаблицаДанных) Экспорт
	...
	|			ИЛИ КлассификаторТаблица.ДатаПереноса <> ДанныеПроизводственногоКалендаря.ДатаПереноса)";
...
Функция УсловийОбновленияГрафиковРаботы(ПроизводственныйКалендарь, Год)
	...
	НоваяСтрока.КодПроизводственногоКалендаря = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПроизводственныйКалендарь, "Код");
	НоваяСтрока.Год = Год;
