...
"либо при наличии роли ""Полные права"".';uk='Визначену групу доступу Адміністратори"
...
"або при наявності ролі ""Повні права"".'");
		...
		Для каждого ТекущаяСтрока Из ЭтотОбъект.Пользователи Цикл
			Если ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
				...
				ВызватьИсключение НСтр("ru='Предопределенная группа доступа Администраторы"
"может содержать только пользователей."
""
"Группы пользователей, внешние пользователи и"
"группы внешних пользователей недопустимы.';uk='Наперед визначена група доступу Адміністратори"
...
"у предопределенной группы доступа Администраторы.';uk='Напередвизначений профіль Адміністратор може бути тільки"
"у напередвизначеної групи доступу Адміністратори.'");
	...
	Если НЕ ЭтоГруппа Тогда
		...
		Если НЕ ПометкаУдаления
		   И СтараяПометкаУдаления = Истина
		   И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Профиль, "ПометкаУдаления") = Истина Тогда
