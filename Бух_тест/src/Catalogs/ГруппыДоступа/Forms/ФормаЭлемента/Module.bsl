...
""
"Установить профиль в группе доступа (нет - открыть только для просмотра)?';uk='У групи доступу Адміністратори повинен бути профіль Адміністратор."
""
...
""
"Обновить виды доступа в группе доступа (нет - открыть только для просмотра)?';uk='Змінився склад видів доступу профілю цієї групи доступу."
""
...
"так как ее профиль помечен на удаление.';uk='Група доступу не впливає на права учасників"
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач ""%2"" неприпустимий у визначеній групі доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач ""%2"" в рядку %1 неприпустимий у визначеній групі доступу Адміністратори.'");
			...
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				...
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група користувачів ""%2"" в рядку %1 неприпустима в наперед визначеній групі доступу Адміністратори.'");
			Иначе
				...
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група зовнішніх користувачів ""%2"" в рядку %1 неприпустима в наперед визначеній групі доступу Адміністратори.'");
			КонецЕсли;
...
Процедура ПредставлениеТипаПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоказатьВыборИзСписка(
		Новый ОписаниеОповещения("ПредставлениеТипаПользователейНачалоВыбораЗавершение", ЭтотОбъект),
		ТипыПользователей,
		Элемент,
...
"Их нельзя удалить в этом списке.';uk='Користувачі груп відображаються для відомості,"
...
"Их нельзя удалить в этом списке.';uk='Зовнішні користувачі груп відображаються для відомості,"
...
// Продолжение обработчика события ПредставлениеТипаПользователейНачалоВыбора
&НаКлиенте
Процедура ПредставлениеТипаПользователейНачалоВыбораЗавершение(ВыбранныйЭлемент, Неопределен) Экспорт
...
Процедура ЗаполнитьСписокТиповПользователей()
	...
	ТипыПользователей.Добавить(
		Неопределено,
		НСтр("ru='Произвольные участники';uk='Довільні учасники'"));
	
	ТипыПользователей.Добавить(
		Справочники.Пользователи.ПустаяСсылка(),
		НСтр("ru='Обычные пользователи';uk='Звичайні користувачі'"));
