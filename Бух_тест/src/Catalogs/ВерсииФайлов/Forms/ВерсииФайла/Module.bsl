...
Процедура СделатьАктивнойВыполнить()
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Смена активной версии разрешена только для не занятых файлов.';uk='Зміна активної версії дозволена тільки для не зайнятих файлів.'"));
	КонецЕсли;
...
Процедура Сравнить(Команда)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru='Сравнение версий не поддерживается в веб-клиенте.';uk='Порівняння версій не підтримується у веб-клієнті.'"));
		Возврат;
	#КонецЕсли
...
"   Текстовый документ OpenDocument (.odt)';uk='Порівняння версій підтримується тільки для файлів наступних типів: "
...
Процедура СравнениеВерсийАвтомат(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 2 Тогда
		...
		ПараметрыВыполнения.ДанныеФайла1 = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаДляОткрытия(
			Неопределено, Ссылка1, УникальныйИдентификатор);
		
		ПараметрыВыполнения.ДанныеФайла2 = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаДляОткрытия(
