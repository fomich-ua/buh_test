...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ПоказатьВес = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Объект.Владелец, "ДополнительныеЗначенияСВесом");
	КонецЕсли;
...
Процедура УстановитьЗаголовок()
	...
	Если НЕ ПустаяСтрока(ИмяСвойства) Тогда
		...
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (Создание)';uk='%1 (Створення)'"), ИмяСвойства);
		КонецЕсли;
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (Значение свойства %2)';uk='%1 (Значення властивості %2)'"),
				Объект.Наименование,
				ИмяСвойства);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Значение свойства %1 (Создание)';uk='Значення властивості %1 (Створення)'"), ИмяСвойства);
		КонецЕсли;
	КонецЕсли;
