...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПользователиИнфБазы
...
Процедура ПользователиИнфБазыПриАктивизацииСтроки(Элемент)
	...
	Элементы.ПользователиИнфБазыПерейтиКПользователю.Доступность                = МожноПерейтиКПользователю;
	Элементы.ПользователиИнфБазыКонтекстноеМенюПерейтиКПользователю.Доступность = МожноПерейтиКПользователю;
	
	Элементы.ПользователиИнфБазыСопоставить.Доступность       = МожноСопоставить;
	Элементы.ПользователиИнфБазыСопоставитьСНовым.Доступность = МожноСопоставить;
	
	Элементы.ПользователиИнфБазыОтменитьСопоставление.Доступность = МожноОтменитьСопоставление;
...
Процедура ПользователиИнфБазыПередУдалением(Элемент, Отказ)
	...
	Если Не ЗначениеЗаполнено(Элементы.ПользователиИнфБазы.ТекущиеДанные.Ссылка) Тогда
...
Процедура ОтменитьСопоставление(Команда)
	...
	Если Элементы.ПользователиИнфБазы.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
"при обновлении информационной базы, поэтому не рекомендуется отменять сопоставление по любой другой причине.';uk='Скасування зіставлення користувача інформаційної бази з користувачем у довіднику."
""
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПользователиИнфБазы.ДобавленВКонфигураторе");
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПользователиИнфБазы.ИзмененВКонфигураторе");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;

	ОтборЭлемента = ГруппаОтбора1.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПользователиИнфБазы.УдаленВКонфигураторе");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПользователиИнфБазы.ПользовательОС");
...
Процедура ЗаполнитьПользователейИБ()
	...
	Если Элементы.ПользователиИнфБазы.ТекущаяСтрока <> Неопределено Тогда
		Строка = ПользователиИнфБазы.НайтиПоИдентификатору(Элементы.ПользователиИнфБазы.ТекущаяСтрока);
	Иначе
		Строка = Неопределено;
	КонецЕсли;
	...
	Строки = ПользователиИнфБазы.НайтиСтроки(Отбор);
	Если Строки.Количество() > 0 Тогда
		Элементы.ПользователиИнфБазы.ТекущаяСтрока = Строки[0].ПолучитьИдентификатор();
	КонецЕсли;
...
Процедура УдалитьТекущегоПользователяИБЗавершение(Ответ, УдалитьСтроку) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если Не Отказ И УдалитьСтроку Тогда
			ПользователиИнфБазы.Удалить(Элементы.ПользователиИнфБазы.ТекущиеДанные);
		КонецЕсли;
	КонецЕсли;
...
Процедура СопоставитьПользователяИБСЭлементом(Элемент, ИмяСправочника) Экспорт
	...
	ПараметрыФормы.Вставить("ИдентификаторПользователяИБ",
		Элементы.ПользователиИнфБазы.ТекущиеДанные.ИдентификаторПользователяИБ);
...
Процедура ОтменитьСопоставлениеНаСервере()
	...
	ТекущаяСтрока = ПользователиИнфБазы.НайтиПоИдентификатору(
		Элементы.ПользователиИнфБазы.ТекущаяСтрока);
