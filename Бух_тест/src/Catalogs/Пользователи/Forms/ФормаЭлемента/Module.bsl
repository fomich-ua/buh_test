...
"создание новых пользователей.';uk='У демонстраційному режимі не підтримується"
...
"пометку на удаление с этого пользователя.';uk='Щоб дозволити вхід у програму, потрібно зняти"
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИнфБазыПолноеИмя <> Объект.Наименование Тогда
			...
			УточнениеСвойств.Вставить(0, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Полное имя ""%1""';uk='Повне ім''я ""%1""'"), ПользовательИнфБазыПолноеИмя));
		КонецЕсли;
...
"%1.';uk='Наступні властивості користувача інформаційної бази відрізняються від зазначених у цій формі:"
