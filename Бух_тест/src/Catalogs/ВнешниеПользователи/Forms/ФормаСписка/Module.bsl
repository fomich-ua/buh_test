...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.ПеренестиОтборыВДинамическийСписок(ЭтотОбъект, ВнешниеПользователиСписок);
КонецПроцедуры
...
&НаКлиенте
Процедура ФлагПоказыватьНедействительныхПользователейПриИзменении(Элемент)
...
"флажок ""Показывать пользователей дочерних групп"".';uk='Для перетаскування користувача в групи необхідно відключити"
"прапорець ""Показувати користувачів дочірніх груп"".'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если КоличествоПользователей = 1 Тогда
			...
			Если ДействиеИсключитьПользователя Тогда
				ШаблонВопроса = НСтр("ru='Исключить пользователя ""%2"" из группы ""%4""?';uk='Виключити користувача ""%2"" із групи ""%4""?'");
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ШаблонВопроса = НСтр("ru='%1 пользователя ""%2"" в группу ""%3""?';uk='%1 користувача ""%2"" у групу ""%3""?'");
			Иначе
				ШаблонВопроса = НСтр("ru='%1 пользователя ""%2"" в эту группу?';uk='%1 користувача ""%2"" у цю групу?'");
			КонецЕсли;
		...
		Иначе
			...
			Если ДействиеИсключитьПользователя Тогда
				ШаблонВопроса = НСтр("ru='Исключить пользователей (%2) из группы ""%4""?';uk='Виключити користувачів (%2) із групи ""%4""?'");
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ШаблонВопроса = НСтр("ru='%1 пользователей (%2) в группу ""%3""?';uk='%1 користувачів (%2) у групу ""%3""?'");
			Иначе
				ШаблонВопроса = НСтр("ru='%1 пользователей (%2) в эту группу?';uk='%1 користувачів (%2) у дану групу?'");
			КонецЕсли;
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПоказыватьНедействительныхПользователей(Команда)
	ПоказыватьНедействительныхПользователей = НЕ Элементы.ПоказыватьНедействительныхПользователей.Пометка;
	ПереключитьОтображениеНедействительныхПользователей(ПоказыватьНедействительныхПользователей);
КонецПроцедуры
...
Процедура ПереключитьОтображениеНедействительныхПользователей(ПоказатьНедействительных)
	...
	Элементы.ПоказыватьНедействительныхПользователей.Пометка = ПоказатьНедействительных;
...
Процедура ГруппыВнешнихПользователейПеретаскиваниеЗавершение(СообщениеПользователю)
	...
	Иначе
		...
		Если СообщениеПользователю.Пользователи <> Неопределено Тогда
			Отчет = НСтр("ru='Следующие пользователи не были включены в выбранную группу:';uk='Наступні користувачі не були включені в обрану групу:'");
