	...
	Если Наименование <> "" И ВариантыОтчетов.НаименованиеЗанято(Отчет, Ссылка, Наименование) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='""%1"" занято, необходимо указать другое наименование.';uk='""%1"" зайняте, необхідно вказати інше найменування.'"),
				Наименование
			),
			,
			"Наименование");
...
"необходимо установить пометку удаления конфликтующего варианта отчета.';uk='Перед зняттям позначки вилучення варіанта звіту"
...
Процедура ПроиндексироватьНастройки()
	Если Пользовательский Или ТипОтчета = Перечисления.ТипыОтчетов.Дополнительный Тогда
		Попытка
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				КлючВарианта,
				Строка(Отчет));
			ВариантыОтчетов.ОшибкаПоВарианту(Ссылка, ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе
		// Для предопределенных вариантов отчетов данные хранятся в неразделенном справочнике.
		Если НаименованияПолей <> "" Тогда
			НаименованияПолей = "";
		КонецЕсли;
		Если НаименованияПараметровИОтборов <> "" Тогда
...
Процедура ПроверитьЗаполнениеПредопределенного(Отказ)
	...
	ИначеЕсли Не ТипыОтчетовСовпадают() Тогда
		ТекстОшибки = ПротиворечивыеЗначенияПолей("ТипОтчета", "Отчет");
	ИначеЕсли ТипОтчета = Перечисления.ТипыОтчетов.Внутренний
		И Не ЗначениеЗаполнено(ПредопределенныйВариант) Тогда
...
Функция НеЗаполненоПоле(ИмяПоля)
	Возврат СтрЗаменить(НСтр("ru='Не заполнено поле ""%1""';uk='Не заповнене поле ""%1""'"), "%1", ИмяПоля);
КонецФункции

Функция ТипыОтчетовСовпадают()
	Если ТипЗнч(Отчет) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		ОжидаемыйТип = Перечисления.ТипыОтчетов.Внутренний;
	ИначеЕсли ТипЗнч(Отчет) = Тип("Строка") Тогда
		ОжидаемыйТип = Перечисления.ТипыОтчетов.Внешний;
	Иначе
		ОжидаемыйТип = Перечисления.ТипыОтчетов.Дополнительный;
	КонецЕсли;
	Возврат ТипОтчета = ОжидаемыйТип;
КонецФункции

Функция ПротиворечивыеЗначенияПолей(ИмяПоля1, ИмяПоля2)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Противоречивые значения полей ""%1"" и ""%2""';uk='Суперечливі значення полів ""%1"" та ""%2""'"),
		ИмяПоля1,
		ИмяПоля2
	);
КонецФункции
