...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		...
		РезультатВыполнения.ВыводСообщения.Текст = НСтр("ru='Необходимо добавить указать варианты отчетов';uk='Необхідно додати вказати варіанти звітів'");
	...
	Если КоличествоОшибок > 0 Тогда
		РезультатВыполнения = Новый Структура;
		ТекстОшибок = НСтр("ru='Варианты отчетов, исключенные из списка выбранных:';uk='Варіанти звітів, виключені зі списку обраних:'");
		ТекущаяПричина = 0;
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					ТекстОшибок = ТекстОшибок + НСтр("ru='Помеченные на удаление:';uk='Відмічені для вилучення:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					ТекстОшибок = ТекстОшибок + НСтр("ru='Недостаточно прав для изменения:';uk='Недостатньо прав для зміни:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					ТекстОшибок = ТекстОшибок + НСтр("ru='Отчет отключен или недоступен по правам:';uk='Звіт відключений або недоступний по правах:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					ТекстОшибок = ТекстОшибок + НСтр("ru='Вариант отчета отключен по функциональной опции:';uk='Варіант звіту відключено по функціональній опції:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			ТекстСообщения = НСтр("ru='Нет прав для размещения в разделах выбранных вариантов отчетов.';uk='Немає прав для розміщення в розділах обраних варіантів звітів.'");
			Элементы.ДеревоПодсистем.Доступность = Ложь;
			РезультатВыполнения.Вставить("Отказ", Истина);
		Иначе
			ТекстСообщения = НСтр("ru='Нет прав для размещения в разделах некоторых вариантов отчетов (%1).';uk='Немає прав для розміщення в розділах деяких варіантів звітів (%1).'");
