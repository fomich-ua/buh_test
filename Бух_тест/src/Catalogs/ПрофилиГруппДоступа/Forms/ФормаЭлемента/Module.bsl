		...
		Если Строка.Пометка И Лев(Строка.Синоним, 1) = "?" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" не найдена в метаданных.';uk='Роль ""%1"" не знайдена в метаданних.'"),
					Строка.Синоним),
				"Роли",
				ЭлементыДерева.Индекс(Строка),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%2"" в строке %1 не найдена в метаданных.';uk='Роль ""%2"" у рядку %1 не знайдена в метаданних.'"),
					"%1", Строка.Синоним));
		КонецЕсли;
	КонецЦикла;
...
Процедура ОписаниеПоставляемогоПрофиля(Команда)
	...
	ТекстовыйДокумент.Показать(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Описание поставляемого профиля ""%1""';uk='Опис профілю, що поставляється, ""%1""'"),
		Объект.Наименование));
...
"значениями доступа и добавлены недостающие виды доступа.';uk='Оновити групи доступу, що використовують цей профіль?"
""
