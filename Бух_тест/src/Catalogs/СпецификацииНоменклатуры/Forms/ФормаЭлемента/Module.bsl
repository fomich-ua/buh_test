...
Процедура ИсходныеКомплектующиеПриИзмененииНоменклатурыСервер(ТекущаяСтрока)
	...
	Если ЗначениеЗаполнено(Объект.ИсходныеКомплектующие[ТекущаяСтрока].Номенклатура) Тогда
		Объект.ИсходныеКомплектующие[ТекущаяСтрока].ЕдиницаИзмерения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ИсходныеКомплектующие[ТекущаяСтрока].Номенклатура, "БазоваяЕдиницаИзмерения").БазоваяЕдиницаИзмерения;
		Объект.ИсходныеКомплектующие[ТекущаяСтрока].Коэффициент = 1;
	КонецЕсли;
...
Процедура ИсходныеКомплектующиеПриИзмененииЕдиницыИзмеренияСервер(ТекущаяСтрока)
	...
	Если ЗначениеЗаполнено(Объект.ИсходныеКомплектующие[ТекущаяСтрока].ЕдиницаИзмерения) Тогда
		Объект.ИсходныеКомплектующие[ТекущаяСтрока].Коэффициент = Справочники.Номенклатура.ПолучитьКоэффициентЕдиницыИзмерения(Объект.ИсходныеКомплектующие[ТекущаяСтрока].Номенклатура, Объект.ИсходныеКомплектующие[ТекущаяСтрока].ЕдиницаИзмерения);
	КонецЕсли;
