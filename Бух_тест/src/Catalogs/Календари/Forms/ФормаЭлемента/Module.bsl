	...
	Для Каждого СтрокаРасписания Из Форма.Объект.РасписаниеРаботы Цикл
		...
		ПредставлениеИнтервалов = ПредставлениеИнтервалов 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1-%2, ", 
				Формат(СтрокаРасписания.ВремяНачала, "ДФ=ЧЧ:мм; ДП="), 
				Формат(СтрокаРасписания.ВремяОкончания, "ДФ=ЧЧ:мм; ДП="));
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			Форма.ЗаполненностьИнформационныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru='График работы заполнен до %1';uk='Графік роботи заповнений до %1'"), 
													Формат(Форма.ДатаЗаполненности, "ДЛФ=D"));
		Иначе											
			#Если Клиент Тогда
			...
			Форма.ЗаполненностьИнформационныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru='График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2';uk='Графік роботи заповнений до %1, з урахуванням горизонту планування графік повинен бути заповнений до %2'"), 
													Формат(Форма.ДатаЗаполненности, "ДЛФ=D"),
													Формат(КонецГоризонтаПланирования, "ДЛФ=D"));
