...
"необходимо их закрыть и открыть заново.';uk='Для застосування змін у відкритих вікнах"
"необхідно їх закрити й відкрити заново.'");
...
Процедура ОбновлениеИзФайлаПослеВыбораФайла(ПомещенныеФайлы, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ДополнительныеОтчетыИОбработкиКлиент.ВернутьРезультатПослеПоказаПредупреждения(
			НСтр("ru='Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).';uk='Розширення файлу не відповідає розширенню зовнішнього звіту (ERF) або обробки (EPF).'"),
...
"3. ""[Open]"" - отменить загрузку и открыть список конфликтующих отчетов.';uk='Внутрішнє ім''я звіту ""[Name]"" уже зайнято іншими додатковими звітами ([Count]): "
"[List]."
""
"Виберіть:"
"1. ""[Continue]"" - виконати завантаження, відключивши публікацію цього звіту."
"2. ""[Disable]"" - виконати завантаження, відключивши публікацію всіх інших конфліктуючих звітів."
"3. ""[Open]"" - скасувати завантаження й відкрити список конфліктуючих звітів.'");
		Иначе
...
"3. ""[Open]"" - отменить загрузку и открыть список конфликтующих обработок.';uk='Внутрішнє ім''я обробки ""[Name]"" уже зайнято іншими додатковими обробками ([Count]): "
"[List]."
""
"Виберіть:"
"1. ""[Continue]"" - виконати завантаження, відключивши публікацію цієї обробки."
"2. ""[Disable]"" - виконати завантаження, відключивши публікацію всіх інших конфліктуючих обробок."
"3. ""[Open]"" - скасувати завантаження й відкрити список конфліктуючих обробок.'");
		КонецЕсли;
...
"3. ""[Open]"" - отменить загрузку и открыть карточку другого отчета.';uk='Внутрішнє ім''я звіту ""[Name]"" уже зайнято іншим додатковим звітом: "
"[List]."
""
"Виберіть:"
"1. ""[Continue]"" - виконати завантаження, відключивши публікацію цього звіту."
"2. ""[Disable]"" - виконати завантаження, відключивши публікацію іншого звіту."
"3. ""[Open]"" - скасувати завантаження й відкрити картку іншого звіту.'");
...
"3. ""[Open]"" - отменить загрузку и открыть карточку другой обработку.';uk='Внутрішнє ім''я обробки ""[Name]"" уже зайнято іншою додатковою обробкою: "
"[List]."
""
"Виберіть:"
"1. ""[Continue]"" - виконати завантаження, відключивши публікацію цієї обробки."
"2. ""[Disable]"" - виконати завантаження, відключивши публікацію іншої обробки."
"3. ""[Open]"" - скасувати завантаження й відкрити картку інший обробку.'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = НСтр("ru='Вариант отчета ""%1"" не зарегистрирован.';uk='Варіант звіту ""%1"" не зареєстрований.'");
...
"не может использоваться в регламентных заданиях.';uk='Команда з варіантом запуску ""%1"""
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если КоличествоКоманд = 0 Тогда
		...
		Элементы.СтраницаКоманды.Заголовок = НСтр("ru='Команды';uk='Команди'");
	...
	Иначе
		...
		Элементы.СтраницаКоманды.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Команды (%1)';uk='Команди (%1)'"),
	...
	Если КоличествоРазрешений = 0 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок, не требующих разрешений на использование профилей безопасности!';uk='%1 не є коректним режимом підключення для додаткових звітів і обробок, що не потребують дозволів на використання профілів безпеки!'"),
				БезопасныйРежим);
		КонецЕсли;
...
"профиля безопасности %1, в котором будут разрешены только следующие операции:';uk='Додатковий звіт або обробка буде підключатися до програми з використанням ""персонального"""
"профілю безпеки %1, в якому будуть дозволені тільки наступні операції:'"), БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!';uk='%1 не є коректним режимом підключення для додаткових звітів і обробок, що потребують дозволів на використання профілів безпеки!'"),
				БезопасныйРежим);
		КонецЕсли;
...
Функция ПолучитьТаблицыРазрешений()
	...
	Для Каждого СтрокаИсходнойТаблицы Из ИсходнаяТаблица Цикл
		...
		Если Набор = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Неизвестный вид разрешения!';uk='Невідомий вид дозволу!'");
		КонецЕсли;
