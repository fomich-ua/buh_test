...
Процедура УстановитьЗаголовок()
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (Группа значений свойства %2)';uk='%1 (Група значень властивості %2)'"),
				Объект.Наименование,
				ИмяСвойства);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Группа значений свойства %1 (Создание)';uk='Група значень властивості %1 (Створення)'"), ИмяСвойства);
		КонецЕсли;
	КонецЕсли;
