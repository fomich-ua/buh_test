...
Функция СвойстваВидовДоступа()
	...
	ВидДоступаПользователи = ВидыДоступа.Добавить();
	ВидДоступаВнешниеПользователи = ВидыДоступа.Добавить();
	
	ЗаполнитьНеизмененныеСвойстваВидовДоступаПользователиИВнешниеПользователи(
		ВидДоступаПользователи, ВидДоступаВнешниеПользователи);
	...
	// - тип значений доступа ВнешниеПользователи, ГруппыВнешнихПользователей используется только для вида доступа ВнешниеПользователи.
	// - имена видов доступа Объект, Условие, НастройкиПрав, ПравоЧтения, ПравоИзменения не указаны.
	// - тип групп значений не совпадает с типом значений.
...
"не указан в определяемом типе ""Значение доступа"".';uk='Тип ПланВидовХарактеристикСсылка.УдалитьВидыДоступа,"
"необхідний для переходу на нові версії програми"
"не вказано в типі ""Значення доступу"", що визначається.'");
		
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Параметры.ЗаголовокОшибки + ОписаниеОшибки);
	КонецЕсли;
...
Процедура ЗаполнитьЗначенияДоступаСГруппами(Строка, ЗначенияДоступаСГруппами, Свойства, Параметры)
	...
	Если Параметры.ТипыПодпискиОбновитьГруппыЗначенийДоступа.Получить(ТипОбъекта) = Неопределено
	   И НЕ ОбщегоНазначения.ЭтоПеречисление(МетаданныеТипаЗначений) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Параметры.ЗаголовокОшибки +
			НСтр("ru='Тип значения доступа ""%1"", использующий группы значений,"
"не указан в подписке на событие ""Обновить группы значений доступа"".';uk='Тип значення доступу ""%1"", що використовує групи значень,"
"не вказано у підписці на подію ""Оновити групи значень доступу"".'"),
			Строка(ТипОбъекта));
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьНеизмененныеСвойстваВидовДоступаПользователиИВнешниеПользователи(
		ВидДоступаПользователи, ВидДоступаВнешниеПользователи)
	
	ВидДоступаПользователи.Имя                    = "Пользователи";
	ВидДоступаПользователи.Представление          = НСтр("ru='Пользователи';uk='Користувачі'");
	ВидДоступаПользователи.ТипЗначений            = Тип("СправочникСсылка.Пользователи");
	ВидДоступаПользователи.ТипГруппЗначений       = Тип("СправочникСсылка.ГруппыПользователей");
	ВидДоступаПользователи.НесколькоГруппЗначений = Истина;
	
	ВидДоступаВнешниеПользователи.Имя                    = "ВнешниеПользователи";
	ВидДоступаВнешниеПользователи.Представление          = НСтр("ru='Внешние пользователи';uk='Зовнішні користувачі'");
	ВидДоступаВнешниеПользователи.ТипЗначений            = Тип("СправочникСсылка.ВнешниеПользователи");
	ВидДоступаВнешниеПользователи.ТипГруппЗначений       = Тип("СправочникСсылка.ГруппыВнешнихПользователей");
	ВидДоступаВнешниеПользователи.НесколькоГруппЗначений = Истина;
...
"Однако это не тип ссылки.';uk='Тип ""%1"" вказано, як тип груп значень, для виду доступу ""%2""."
"Однак це не тип посилання.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип значений, для вида доступа ""%2""."
"Однако это не тип ссылки.';uk='Тип ""%1"" вказано, як тип значень, для виду доступу ""%2""."
"Однак це не тип посилання.'");
		КонецЕсли;
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Параметры.ЗаголовокОшибки + ОписаниеОшибки, Тип, ВидДоступа.Имя);
	КонецЕсли;
	...
	Если ПроверкаТиповГрупп Тогда
		Если ПроверкаПересечения Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип значений, для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать, как тип групп значений.';uk='Тип ""%1"" вказано, як тип значень, для виду доступу ""%2""."
...
"Для вида доступа ""%3"" его нельзя указать.';uk='Тип груп значень ""%1"" вже вказаний для виду доступу ""%2""."
"Для виду доступу ""%3"" його не можна вказати.'");
	...
	Иначе
		Если ПроверкаПересечения Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип групп значений, для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать, как тип значений.';uk='Тип ""%1"" вказано, як тип груп значень, для виду доступу ""%2""."
"Для виду доступу ""%3"" його не можна вказати, як тип значень.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип значений ""%1"" уже указан для вида доступа ""%2""."
"Для вида доступа ""%3"" его нельзя указать.';uk='Тип значення ""%1"" вже вказаний для виду доступу ""%2""."
"Для виду доступу ""%3"" його не можна вказати.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ВсеТипы.Получить(Тип) <> Неопределено Тогда
		Если НЕ (ДляТогоЖеВидаДоступаОшибкиНет И ВидДоступа.Имя = ВсеТипы.Получить(Тип)) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Параметры.ЗаголовокОшибки + ОписаниеОшибки, Тип, ВсеТипы.Получить(Тип), ВидДоступа.Имя);
		КонецЕсли;
...
"не указан в определяемом типе ""Значение доступа"".';uk='Тип груп значень доступу ""%1"" види доступу ""%2"""
"не вказано в типі ""Значення доступу"", що визначається.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип значений доступа ""%1"" вида доступа ""%2"""
"не указан в определяемом типе ""Значение доступа"".';uk='Тип значень доступу ""%1"" види доступу ""%2"""
"не вказано в типі ""Значення доступу"", що визначається.'");
		КонецЕсли;
	КонецЕсли;
