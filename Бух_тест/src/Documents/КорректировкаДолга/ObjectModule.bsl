...
Процедура ПроводкиПоДоговорамСУпрощеннымНалоговымУчетом(СтруктураШапкиДокумента, ТаблицаПродажи, ТаблицаПриобретения)
	...
	Для каждого Строка Из СтрокиОплат Цикл
		...
		Если  СуммаПроводки <> 0 
			...
			Проводка.Содержание                             = ?(ЭтоПродажа,НСтр("ru='НДС: налоговые обязательства';uk=""ПДВ: податкові зобов'язання"""), НСтр("ru='НДС: налоговый кредит';uk='ПДВ: податковий кредит'")) + ?(ЭтоВозврат, НСтр("ru=', (сторно) оплата';uk=', (сторно) оплата'"), НСтр("ru=', оплата';uk=', оплата'"));
	...
	Для каждого Строка Из СтрокиОтгрузок Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНДС 
			...
			Проводка.Сумма                      = СуммаПроводки;
			Проводка.Содержание                 = ?(ЭтоПродажа,НСтр("ru='НДС: налоговые обязательства';uk=""ПДВ: податкові зобов'язання"""), НСтр("ru='НДС: налоговый кредит';uk='ПДВ: податковий кредит'")) + ?(ЭтоВозврат, НСтр("ru=', (сторно) отгрузка';uk=', (сторно) відвантаження'"), НСтр("ru=', отгрузка';uk=', відвантаження'"));
			Проводка.НомерЖурнала               = "";
		...
		Если    СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности
			...
			Если  СтруктураШапкиДокумента.ЕстьНДС 
				...
				Если  СуммаПроводки <> 0 
					...
					Проводка.Сумма                                  = СуммаПроводки;
					Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ИзменениеЗадолженности  Тогда
					    Проводка.Содержание                             = ?(ЭтоПродажа,НСтр("ru='НДС: Изменение задолженности: обязательства';uk=""ПДВ: Зміна заборгованості: зобов'язання"""), НСтр("ru='НДС: Изменение задолженности: налоговый кредит';uk='ПДВ: Зміна заборгованості: податковий кредит'")) + ?(ЭтоВозврат, НСтр("ru=', (сторно) отгрузка';uk=', (сторно) відвантаження'"), НСтр("ru=', отгрузка';uk=', відвантаження'"));
					Иначе//списание	
						Проводка.Содержание                             = ?(ЭтоПродажа,НСтр("ru='НДС: Списание задолженности: обязательства';uk=""ПДВ: Списання заборгованості: зобов'язання"""), НСтр("ru='НДС: Списание задолженности: налоговый кредит';uk='ПДВ: Списання заборгованості: податковий кредит'")) + ?(ЭтоВозврат, НСтр("ru=', (сторно) отгрузка';uk=', (сторно) відвантаження'"), НСтр("ru=', отгрузка';uk=', відвантаження'"));
					КонецЕсли;
			...
			Если  СтруктураШапкиДокумента.ЕстьНДС
				...
				Если  СуммаПроводки <> 0 
					...
					Проводка.Организация                            = Организация;
					Проводка.Сумма                                  = СуммаПроводки;
					Проводка.Содержание                             = ?(ЭтоПродажа,НСтр("ru='НДС: Перенос задолженности: обязательства';uk=""ПДВ: Перенос заборгованості: зобов'язання"""), НСтр("ru='НДС: Перенос задолженности: налоговый кредит';uk='ПДВ: Перенос заборгованості: податковий кредит'")) + ?(ЭтоВозврат, НСтр("ru=', (сторно) отгрузка';uk=', (сторно) відвантаження'"), НСтр("ru=', отгрузка';uk=', відвантаження'"));
					Проводка.НомерЖурнала                           = "";
