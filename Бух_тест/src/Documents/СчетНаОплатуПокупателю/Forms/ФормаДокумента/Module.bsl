...
Процедура ТоварыЕдиницаИзмеренияПриИзменении(Элемент)
	...
	ДанныеСтрокаТаблицы = Новый Структура("Номенклатура, ЕдиницаИзмерения, Коэффициент, Количество, Цена, Сумма, СуммаБезСкидки, СуммаСкидки");
...
Процедура ОрганизацияОбработатьИзменение()
	...
	Если НЕ ТекущаяОрганизацияПлательщикНДС = ПлательщикНДС Тогда
		//изменился статус организацию-плательщика
		
		ПересчитатьНДС = Ложь;
		ПерезаполнитьСтавкуНДСДо = Ложь;
		ПерезаполнитьСтавкуНДСПосле = Ложь;
		
		МетаданныеДокумента = Объект.Ссылка.Метаданные();
		ПараметрыОбъекта = Новый Структура("ПлательщикНДС", ПлательщикНДС);
		...
		Если Не ПлательщикНДС Тогда
			...
			ПерезаполнитьСтавкуНДСПосле = Истина;
			
			Объект.СуммаВключаетНДС = Истина;
		Иначе
			//организацию-неплательщика поменяли на плательщика;
			ПересчитатьНДС = Ложь;
			ПерезаполнитьСтавкуНДСДо = Истина;
		КонецЕсли;
		
		Если ПерезаполнитьСтавкуНДСДо Тогда
			Для каждого Строка Из Объект.Товары Цикл
		...
		Если ПерезаполнитьСтавкуНДСПосле Тогда
			Для каждого Строка Из Объект.Товары Цикл
				ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(Строка, ПараметрыОбъекта , "Товары", МетаданныеДокумента);
			КонецЦикла;
			Для каждого Строка Из Объект.Услуги Цикл
...
Процедура ВыгрузкаПервичныхДокументовВЗвит1С(Команда)
	...
	Если НЕ КонтрагентИспользуетЭДО(Объект.Контрагент)  Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'В карточке контрагента не установлен признак ""Использовать 1С:ЭДО""!'; uk = 'В картці контрагента не встановлена ознака ""Використовувати 1С:ЕДО""!'"));
		Возврат;
	КонецЕсли;
