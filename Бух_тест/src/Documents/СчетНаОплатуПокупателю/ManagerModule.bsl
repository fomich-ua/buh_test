...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Счет (для обмена через ""1С:ЭДО"")'; uk = 'Рахунок (для обміну через ""1С:ЕДО"")'");
	...
	Для каждого Ссылка Из МассивОбъектов Цикл	
		...
		Если Тип = "Счет" Тогда
			...
			Если ТипЗнч(Шапка.СтруктурнаяЕдиница) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
				...
				МФО	 		= Шапка.СтруктурнаяЕдиницаМФО;
		...
		Если Тип = "Счет" Тогда
			ОбластьМакета.Параметры.РеквизитыПоставщика =  НСтр("ru='Т/с ';uk='П/р '",КодЯзыкаПечать) + НомерСчета + НСтр("ru=', Банк ';uk=', Банк '",КодЯзыкаПечать) + Банк + НСтр("ru=', МФО ';uk=', МФО '",КодЯзыкаПечать) + МФО + Символы.ПС + 
														ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПоставщике, "ЮридическийАдрес,/,Телефоны,Факс,Email,/,КодПоЕДРПОУ,КодПоДРФО,ИНН,НомерСвидетельства,/,ИнформацияОСтатусеПлательщикаНалогов,",,КодЯзыкаПечать);
		КонецЕсли;
	    ТабДокумент.Вывести(ОбластьМакета);
		...
		// Вывести подписи
		Если СПечатью Тогда 
			ДанныеКартинкиПечать = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИДвоичныеДанные(Шапка.Поставщик.ФайлФаксимильнаяПечать);
			Если ЗначениеЗаполнено(ДанныеКартинкиПечать) Тогда 
				ДанныеКартинкиПечать = ДанныеКартинкиПечать.ДвоичныеДанные;
			КонецЕсли;
		Иначе
			ДанныеКартинкиПечать = Неопределено;
		КонецЕсли;
		Если СПечатью Тогда 
			ДанныеКартинкиПодпись = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИДвоичныеДанные(Шапка.Поставщик.ФайлФаксимильнаяПодпись);
			Если ЗначениеЗаполнено(ДанныеКартинкиПодпись) Тогда 
				ДанныеКартинкиПодпись = ДанныеКартинкиПодпись.ДвоичныеДанные;
			КонецЕсли;
		Иначе
...
Функция ПечатьПредварительныйПросмотрСчетаНаОплату(Организация, ОбъектыПечати, ПараметрыВывода)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СчетЗаказ_СчетЗаказ";
	...
	ОбластьМакета = Макет.ПолучитьОбласть("СуммаПрописью");
	ОбластьМакета.Параметры.ИтоговаяСтрока = "Всего наименований 10, на сумму 1200 гривен";
	ОбластьМакета.Параметры.СуммаПрописью = "Одна тясяча двести гривен 00 копеек";
	ТабличныйДокумент.Вывести(ОбластьМакета);

	ДанныеКартинкиПечать  = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИДвоичныеДанные(Организация.ФайлФаксимильнаяПечать);
	ДанныеКартинкиПодпись = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИДвоичныеДанные(Организация.ФайлФаксимильнаяПодпись);
	Если Не ЗначениеЗаполнено(ДанныеКартинкиПечать.ДвоичныеДанные) И ЗначениеЗаполнено(ДанныеКартинкиПодпись.ДвоичныеДанные) Тогда
		ОбластьМакета = Макет.ПолучитьОбласть("ПодвалСчетаСФаксимильнойПодписью");
		ОбластьМакета.Рисунки.ФаксимильнаяПодписьБезПечати.Картинка = Новый Картинка(ДанныеКартинкиПодпись.ДвоичныеДанные);
	ИначеЕсли ЗначениеЗаполнено(ДанныеКартинкиПечать.ДвоичныеДанные) Или ЗначениеЗаполнено(ДанныеКартинкиПодпись.ДвоичныеДанные) Тогда
		ОбластьМакета = Макет.ПолучитьОбласть("ПодвалСчетаСФаксимильнойПечатьюИПодписью");
		ОбластьМакета.Рисунки.ФаксимильнаяПечать.Картинка  = Новый Картинка(ДанныеКартинкиПечать.ДвоичныеДанные);
		ОбластьМакета.Рисунки.ФаксимильнаяПодпись.Картинка = Новый Картинка(ДанныеКартинкиПодпись.ДвоичныеДанные);
	Иначе
		ОбластьМакета = Макет.ПолучитьОбласть("Подписи");
	КонецЕсли;
	ТабличныйДокумент.Вывести(ОбластьМакета);
