	...
	Для Каждого Строка Из ТаблицаТоваров Цикл
		...
		Для Каждого Запись из НайденныеСтроки Цикл
			...
			Если Списать > 0 Тогда
				...
				Строка.СуммаПоступления = ?(СуммаВключаетНДС, Строка.СуммаПоступления - СуммаНДС,
												Строка.СуммаПоступления);
												
				СуммаНДСВыручка  = УчетНДС.РассчитатьСуммуНДС(Строка.Выручка, Истина, СуммаВключаетНДС, СтавкаНДС);
				Строка.Выручка = ?(НЕ СуммаВключаетНДС, Строка.Выручка + СуммаНДСВыручка,
												Строка.Выручка);
				...
				Иначе 
					Движение.Выручка 			= Строка.Выручка * Списать /Строка.Количество;
					Движение.СуммаПоступления	= Строка.СуммаПоступления * Списать /Строка.Количество;
				КонецЕсли;
