...
Функция ВариантыДополненияКНаименованиюПриПродажеНижеОбычнойЦены(Объект)
	...
	Иначе
		...
		Список.Добавить(СписокВариантов[0].Представление);
		Список.Добавить(СписокВариантов[1].Представление);
		Список.Добавить(СписокВариантов[2].Представление);
	КонецЕсли;
...
Процедура ВидДоговораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыВыбора = Новый Структура("ТекущаяСтрока", ПолучитьСсылкуНаВидДоговораПоГКПоНаименованию(Объект.ВидДоговора));
	
	ОткрытьФорму("Справочник.ВидыДоговоровПоГК.ФормаВыбора", ПараметрыВыбора, Элемент);
...
Процедура ПодготовитьФормуНаСервере() Экспорт
	...
	Если ДоговорУказан Тогда
		ВалютаВзаиморасчетов = РеквизитыДоговора.ВедениеВзаиморасчетовНУ;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.УслугиЗаполниитьСводнуюПоУсловнойПродажеУслуги.Доступность   = Ложь; 
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНалоговаяНакладная.УсловнаяПродажа") Тогда 
		...
		Элементы.УслугиЗаполниитьСводнуюПоУсловнойПродажеУслуги.Доступность   = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьТабличнуюЧасть(ИмяТЧ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ТекстПредупреждения = НСтр("ru='Не выбран документ-основание. Заполнение невозможно.';uk='Не обраний документ-основа. Заповнення неможливо.'");
...
&НаСервере
Процедура ЗаполниитьСводнуюПоУсловнойПродажеНаСервере()
	
	Объект.ТипПричиныНевыдачиПокупателю = 9;
	Объект.АвторасчетНДС = Ложь;
	Объект.Сводная = Истина;
	
	Документ = РеквизитФормыВЗначение("Объект", Тип("ДокументОбъект.НалоговаяНакладная"));
	
	Документ.ЗаполнитьСводуюУсловнуюПродажу();
...
&НаКлиенте
Процедура ЗаполниитьСводнуюПоУсловнойПродажеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	ЗаполниитьСводнуюПоУсловнойПродажеНаСервере();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполниитьСводнуюПоУсловнойПродажеУслуги(Команда)
	
	Если Объект.Услуги.Количество() > 0 Тогда
		
		ТекстВопроса = НСтр("ru='Очистить табличную часть перед заполнением?';uk='Очистити табличну частину перед заповненням?'");
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполниитьСводнуюПоУсловнойПродажеЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да, НСтр("ru='Заполнение';uk='Заповнення'"));
        Возврат; 
	КонецЕсли;
...
Функция ПолучитьСписокТиповДляВыбораДокументаОснования(Объект)
	...
	Иначе
		Для каждого ТипРеквизита Из МассивТиповДокументаОснования Цикл
			...
			Если ТипРеквизита = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда	
				Продолжить;
			Иначе
