...
Процедура АвтозаполнениеДоходыИНалоги()
	...
	ДоходыТекстЗапроса = 
	   ...
	   |	0 КАК НДФЛ,
	   |	0 КАК ДоходНДФЛ,
	   |	ВзаиморасчетыССотрудниками.СуммаВзаиморасчетовРасход КАК Выплачено,
   	   |	0,
	   |	0,
	   |	0

	   |ИЗ
	   |	РегистрНакопления.ВзаиморасчетыССотрудниками.Обороты(&парамДатаНач, &парамДатаКон, , ФизическоеЛицо = &парамФизическоеЛицо И Организация.Ссылка = &парамОрганизация И (ПериодВзаиморасчетов МЕЖДУ &парамДатаНач И &парамДатаКон)) КАК ВзаиморасчетыССотрудниками
	   |
	   |;
	   |
	   |ВЫБРАТЬ
