...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если МассивОбъектов[0].ВидСправки = Перечисления.ВидыСправокОДоходах.Произвольная Тогда
		...
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МакетПроизвольная") Тогда
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МакетПроизвольная", НСтр("ru='Справка о доходах';uk='Довідка про доходи'"), ПечатьПроизвольная(МассивОбъектов, ОбъектыПечати, ПараметрыВывода));
		КонецЕсли;
	ИначеЕсли МассивОбъектов[0].ВидСправки = Перечисления.ВидыСправокОДоходах.ДоходыИНалоги Тогда
		...
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МакетДоходыИНалоги") Тогда
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МакетДоходыИНалоги", НСтр("ru='Справка о доходах';uk='Довідка про доходи'"), ПечатьДОходыИНалоги(МассивОбъектов, ОбъектыПечати, ПараметрыВывода));
		КонецЕсли;
...
Функция ПечатьПроизвольная(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетПроизвольная");
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл
		...
		Иначе
			...
			Формат(ДанныеСотрудника[0].ДатаПриема, "ДЛФ=Д")+ НСтр("ru='г.';uk='р.'",КодЯзыкаПечать) + " по " + Формат(ДанныеСотрудника[0].ДатаУвольнения-86400, "ДЛФ=Д")+ НСтр("ru='г.';uk='р.'",КодЯзыкаПечать)  + ТекстДоговор + НСтр("ru=' на должности ';uk=' на посаді '",КодЯзыкаПечать) + ДанныеФизЛица.Должность;
		КонецЕсли;
...
Функция ПечатьДоходыИНалоги(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетДоходыИНалоги");
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл
		...
		Иначе
			ОбластьМакета.Параметры.ДатаКон			= Формат(?(НЕ ЗначениеЗаполнено(Формат(ДанныеСотрудника[0].ДатаУвольнения, "ДЛФ=Д")),РезультатПериодДоходов.ОкончаниеПериодаДоходов,МИН(ДанныеСотрудника[0].ДатаУвольнения-86400,РезультатПериодДоходов.ОкончаниеПериодаДоходов)), "ДЛФ=Д");
		КонецЕсли;	
...
Функция ПечатьБезработица(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетБезработица");
...
Функция ПечатьПенсия(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетПенсия");
...
Функция ПечатьСоцстрах(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетСоцстрах");
...
Функция ПечатьСубсидия(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетСубсидия");
...
Функция ПечатьСубсидия2015(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетСубсидия2015");
...
Функция ПечатьСоцстрах2015(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетСоцстрах2015");
...
Функция ПечатьПенсия2015(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.СправкаОДоходах.ПФ_MXL_МакетПенсия2015");
