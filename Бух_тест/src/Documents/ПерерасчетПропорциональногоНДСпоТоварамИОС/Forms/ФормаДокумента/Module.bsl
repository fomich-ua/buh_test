...
Процедура ЗаполнитьНалоговыеДокументы(Перезаполнить = Ложь)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ННДок.НомерСтроки КАК НомерСтроки,
				   ...
				   |	И ННДок.Ссылка.ТипПричиныНевыдачиПокупателю = 9
	...
	Для каждого Период_ Из ВходящиеНалоговыеДокументы Цикл
		...
		ИсходнаяСводнаяНН = НалоговыеДокументыПоСтрокам.НайтиСтроки(Новый Структура("СодержаниеСтрокиНН", Период_.СодержаниеСтрокиНН));
		...
		Иначе	
			...
			НомерСтрокиНН   = ИсходнаяСводнаяНН[0].НомерСтроки;
		...
		СтрокаП2.Сумма	 		  = Период_.СуммаПоставки;																				
		СтрокаП2.СуммаНДС 		  = Период_.НДСПоставки;										
		
		СтрокаП2.ДатаКорректировки = П2.Дата;
		СтрокаП2.Причина		   = "Коригування податкових зобов'язань, згідно з п. 199.1 ПКУ";
		//СтрокаП2.ИзменениеСуммы    = Период_.СуммаКорректировкиНДСОбязательств;										
