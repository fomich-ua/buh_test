...
"Для объектов таблицы ""%2"" права не настраиваются.';uk='Помилка в процедурі РегистрыСведений.НастройкиПравОбъектов.Прочитать()"
""
...
"Требуется исправить данные регистра.';uk='Помилка в процедурі РегистрыСведений.НастройкиПравОбъектов.Прочитать()"
""
	...
	Попытка
		...
		Если НаследованиеИзменено Тогда
			...
			РегистрыСведений.НаследованиеНастроекПравОбъектов.ОбновитьРодителейВладельца(
				СсылкаНаОбъект, , Истина, ОбъектыСИзмененниями);
		КонецЕсли;
...
""
"Тип владельцев прав ""%1"" не указан в измерении Объект.';uk='Помилка в процедурі ОбновитьВспомогательныеДанныеРегистра"
...
"Тип власників прав ""%1"" не вказаний у вимірюванні Об''єкт.'"),
	...
	Попытка
		...
		УправлениеДоступомСлужебный.ОбновитьРегистрСведений(
			РегистрыСведений.НастройкиПравОбъектов, Изменения, ЕстьИзменения);
...
"общего модуля УправлениеДоступомПереопределяемый.';uk='Помилка в процедурі ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов"
"загального модуля УправлениеДоступомПереопределяемый.'")
	...
	ТипыПодпискиОбновитьГруппыЗначенийДоступа = УправлениеДоступомСлужебныйПовтИсп.ТипыОбъектовВПодпискахНаСобытия(
	...
	Для каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ЗаголовокОшибки + НСтр("ru='Не найден владельц прав ""%1"".';uk='Не знайдений власник прав ""%1"".'"),
...
"не указан в определяемом типе ""Владелец настроек прав"".';uk='Тип власника прав ""%1"""
...
"для корректного заполнения регистра НаборыЗначенийДоступа.';uk='Тип власника прав ""%1"""
...
"но обнаружен в описании вида доступа ""%2"".';uk='Тип власника прав ""%1"""
...
"але виявлений в описі виду доступу ""%2"".'"),
			...
			Если ТипыПодпискиОбновитьГруппыЗначенийДоступа.Получить(ТипОбъекта) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ЗаголовокОшибки +
					НСтр("ru='Тип владельца прав ""%1"""
"не указан в подписке на событие ""Обновить группы значений доступа"".';uk='Тип власника прав ""%1"""
"не вказано у підписці на подію ""Оновити групи значень доступу"".'"),
			...
			ИерархическиеТаблицы.Вставить(ТипСсылки,  ИерархическийОбъектМетаданных(ОбъектМетаданныхВладельца));
			ИерархическиеТаблицы.Вставить(ТипОбъекта, ИерархическийОбъектМетаданных(ОбъектМетаданныхВладельца));
...
"повторно определено право ""%2"".';uk='Для власника прав ""%1"""
...
"В этом случае отдельных таблиц указывать не нужно.';uk='Для власника прав ""%1"""
...
"У цьому випадку окремих таблиць вказувати не потрібно.'")
			...
			Если ЗначениеЗаполнено(ОбщиеПрава[Свойство]) Тогда
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки = НСтр("ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для чтения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для чтения для права ""%3"".';uk='Для власника прав ""%1"""
"для права ""%2"" в таблицях для читання вказаний символ ""*""."
"Однак символ ""*"" вже зазначено в таблицях для читання для права ""%3"".'")
				Иначе
					ОписаниеОшибки = НСтр("ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для изменения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для изменения для права ""%3"".';uk='Для власника прав ""%1"""
"для права ""%2"" в таблицях для зміни вказаний символ ""*""."
...
"Имеет смысл использовать только символ ""*"".';uk='Для власника прав ""%1"""
...
"Має сенс використовувати тільки символ ""*"".'");
		...
		ИначеЕсли Метаданные.НайтиПоПолномуИмени(Значение) = Неопределено Тогда
			...
			Если Свойство = "ЧтениеВТаблицах" Тогда
				ОписаниеОшибки = НСтр("ru='Для владельца прав ""%1"""
"для права ""%2"" не найдена таблица для чтения ""%3"".';uk='Для власника прав ""%1"""
"для права ""%2"" не знайдена таблиця для читання ""%3"".'")
			Иначе
				ОписаниеОшибки = НСтр("ru='Для владельца прав ""%1"""
"для права ""%2"" не найдена таблица для изменения ""%3"".';uk='Для власника прав ""%1"""
...
Функция ИерархическийОбъектМетаданных(ОписаниеОбъектаМетаданных)
	...
	Если НЕ Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных) Тогда
		Возврат Ложь;
	КонецЕсли;
