...
""
"Указаны неверные параметры.';uk='Помилка в процедурі ОбновитьГруппировкиПользователей"
...
Функция УдалитьЛишниеЗаписи(ЕстьИзменения = Неопределено)
	...
	ТипыГруппИЗначенийДоступа = УправлениеДоступомСлужебныйПовтИсп.Параметры(
		).СвойстваВидовДоступа.ЗначенияДоступаСГруппами.ТипыГруппЗначений;
...
"'");
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки +
			НСтр("ru='Для типа ""%1"""
"не настроено использование групп значений доступа.';uk='Для типу ""%1"""
...
"со специальным реквизитом ГруппаДоступа.';uk='У типу значень доступу ""%1"""
		...
		Иначе
			Если МетаданныеТипа.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ЗаголовокОшибки +
					НСтр("ru='У типа значений доступа ""%1"""
"не создан специальный реквизит ГруппаДоступа.';uk='У типу значень доступу ""%1"""
...
"%3'"),
	...
	Если УправлениеДоступом.ОграничиватьДоступНаУровнеЗаписей() Тогда
		...
		ТипыГруппЗначений = УправлениеДоступомСлужебныйПовтИсп.Параметры(
			).СвойстваВидовДоступа.ЗначенияДоступаСГруппами.ТипыГруппЗначений;
		...
		Если СвойстваВидаДоступа.НесколькоГруппЗначений Тогда
			Для каждого Строка Из НовыеЗначения[ИмяТабличнойЧасти] Цикл
...
Процедура ОбновитьПользователей(Пользователи1 = Неопределено,
	...
	ФиксированныйОтбор = Новый Структура;
	ФиксированныйОтбор.Вставить("ГруппаДанных", 1);
...
Процедура ОбновитьГруппыПользователей(ГруппыПользователей = Неопределено,
	...
	ФиксированныйОтбор = Новый Структура;
	ФиксированныйОтбор.Вставить("ГруппаДанных", 2);
...
""
"Указаны неверные параметры.';uk='Помилка в процедурі ОбновитьГруппыИсполнителей"
...
"Вказані невірні параметри.'");
	...
	ФиксированныйОтбор = Новый Структура;
	ФиксированныйОтбор.Вставить("ГруппаДанных", 3);
...
Процедура ОбновитьОбъектыАвторизации(ОбъектыАвторизации = Неопределено, ЕстьИзменения = Неопределено)
	...
	ФиксированныйОтбор = Новый Структура;
	ФиксированныйОтбор.Вставить("ГруппаДанных", 4);
