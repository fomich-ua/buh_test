...
Процедура ОбновитьДанныеРегистра(ГруппыДоступа = Неопределено, ЕстьИзменения = Неопределено) Экспорт
	...
	Попытка
		...
		Для каждого СвойстваВидаДоступа Из СвойстваВидовДоступа Цикл
			Если УправлениеДоступомСлужебный.ВидДоступаИспользуется(СвойстваВидаДоступа.Ссылка) Тогда
				ИспользуемыеВидыДоступа.Добавить().ВидДоступа = СвойстваВидаДоступа.Ссылка;
			КонецЕсли;
		КонецЦикла;
