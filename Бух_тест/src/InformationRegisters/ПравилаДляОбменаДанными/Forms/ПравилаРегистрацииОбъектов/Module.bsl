...
Процедура ВыгрузитьПравила(Команда)
	...
	ВариантыВыгрузки.Добавить(Ложь,   НСтр("ru='Обычным файлом';uk='Звичайним файлом'") );
	ВариантыВыгрузки.Добавить(Истина, НСтр("ru='В архиве zip';uk='В архіві zip'")   );
...
""
"[ИнформацияОПравилах]';uk='Використання правил, завантажених з файлу,"
...
Процедура ВыгрузитьПравилаЗавершение(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение Тогда
		...
		ФильтрИмени   = НСтр("ru='Архивы ZIP (*.zip)';uk='Архіви ZIP (*.zip)'") + "|*.zip";
	Иначе
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("ru='Правила для обмена данными';uk='Правила для обміну даними'");
	Иначе
