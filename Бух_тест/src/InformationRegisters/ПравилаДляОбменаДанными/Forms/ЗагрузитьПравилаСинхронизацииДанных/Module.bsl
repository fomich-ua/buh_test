...
"или найти в %2';uk='Комплект правил можна скачати з %1"
"або знайти в %2'");
	...
	Если Не ПустаяСтрока(РасположениеКомплектаПравил.ПутьКФайлуКомплектаПравилВКаталогеШаблонов) Тогда
		...
		КаталогШаблонов = КаталогШаблонов();
		
		РасположениеФайла = КаталогШаблонов() + РасположениеКомплектаПравил.ПутьКФайлуКомплектаПравилВКаталогеШаблонов;
		
		Если ФайлСуществуетНаКлиенте(РасположениеФайла) Тогда
			ШаблонКаталогОбновлений = Новый ФорматированнаяСтрока(ШаблонКаталогОбновлений,,,, РасположениеФайла);
		КонецЕсли;
		
	КонецЕсли;
	
	ШаблонПользовательскийСайт = Нстр("ru='сайта поддержки пользователей системы ""1C:Предприятие 8""';uk='сайта підтримки користувачів системи ""1С:Підприємство 8""'");
	...
	ТекстПодсказки = ПодставитьПараметрыВФорматированнуюСтроку(ШаблонПодсказки, ШаблонПользовательскийСайт, ШаблонКаталогОбновлений);
...
&НаКлиенте
Процедура ДекорацияИнформацияОПолученииПравилОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	
	Если Найти(НавигационнаяСсылка, "http") = 0 Тогда
		СтандартнаяОбработка = Ложь;
...
"Использовать типовые правила конвертации?';uk='Правила з файлу не завантажені. Закриття призведе до використання типових правил конвертації."
...
"RegistrationRules.xml - правила регистрации для текущей программы.';uk='Некоректний формат файлу комплекту правил. Очікується zip архів, що містить три файли:"
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		Если ОписаниеОшибки.ВидОшибки = "ТребуетсяПовторЗапросаРазрешений" Тогда
			
			ПередЗагрузкойПравил(АдресПомещенногоФайла, ИмяФайла);
...
"Перейти в журнал регистрации?';uk='У процесі завантаження правил були виявлені помилки."
...
Процедура ЗагрузитьПравилаНаСервере(Отказ, АдресВременногоХранилища, ИмяФайлаПравил, ОписаниеОшибки)
	...
	Если Не Отказ Тогда
		...
		Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() И РежимОтладки
			И РежимСовместимости <> ЗаписьПравилКонвертации.РежимСовместимости Тогда
			
			РежимСовместимости = ЗаписьПравилКонвертации.РежимСовместимости;
			ОписаниеОшибки = Новый Структура("ВидОшибки", "ТребуетсяПовторЗапросаРазрешений");
			Возврат;
...
Функция СоздатьЗапросНаИспользованиеВнешнихРесурсов()
	...
	СтруктураЗаписи.Вставить("РежимСовместимости", РежимСовместимости);
