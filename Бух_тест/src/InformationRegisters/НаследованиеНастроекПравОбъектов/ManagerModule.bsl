...
Процедура ОбновитьРодителейВладельца(ВладелецНастроекПрав, ЕстьИзменения, ОбновитьИерархию = Ложь, ОбъектыСИзмененниями = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка при обновлении иерархии владельцев прав по значениям доступа.';uk='Помилка при оновленні ієрархії власників прав за значеннями доступу.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Если ВозможныеПрава.ПоТипам.Получить(ТипВладельца) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки +
			НСтр("ru='Для типа ""%1"""
"не настроено использование настроек прав объектов.';uk='Для типу ""%1"""
