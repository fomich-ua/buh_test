...
"установленным ""По информационным базам"".';uk='Звіти по датах заборони завантаження даних,"
"встановлених ""З інформаційних баз"".'");
		
		Команды.ДатыЗапретаПоПользователям.Заголовок
			= НСтр("ru='Даты запрета по информационным базам';uk='Дати заборони за інформаційними базами'");
		
		Команды.ДатыЗапретаПоПользователям.Подсказка
			= НСтр("ru='Даты запрета загрузки данных"
"по информационным базам и программам.';uk='Дати заборони завантаження даних"
"по інформаційним базам і програмам.'");
		
		Команды.ДатыЗапретаПоРазделамОбъектамДляПользователей.Заголовок =
			НСтр("ru='Даты запрета по разделам и объектам для информационных баз';uk=""Дати заборони за розділами і об'єктами для інформаційних баз""");
		
		Команды.ДатыЗапретаПоРазделамОбъектамДляПользователей.Подсказка =
			НСтр("ru='Даты запрета загрузки данных"
"по разделам и объектам для информационных баз и программ.';uk=""Дати заборони завантаження даних"
"за розділами і об'єктами для інформаційних баз і програм.""");
	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Иначе
				...
				КонецЕсли;
				ТекстНадписи = Символы.ПС +
					НСтр("ru='По %2 возможно изменение данных с %3 по %4';uk='З %2 можлива зміна даних з %3 по %4'") + Символы.ПС +
					НСтр("ru='После %2 будет запрещено изменение данных по %4';uk='Після %2 буде заборонено зміну даних до %4'") + Символы.ПС;
			КонецЕсли;
		Иначе
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Запрещено изменение данных по %1';uk='Заборонена зміна даних до %1'") + ТекстНадписи,
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Для каждого ЗначениеСписка Из Контекст.СписокТиповПользователей Цикл
			Если ТипЗнч(ЗначениеСписка.Значение) = ТипЗнч(Пользователь) Тогда
				...
				Иначе
					Возврат ЗначениеСписка.Представление + ": " +
						НСтр("ru='<Все информационные базы>';uk='<Усі інформаційні бази>'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстВопросаПересчитатьДатыЗапрета()
	...
	Возврат НСтр("ru='Очистить незаполненные строки и пересчитать относительные даты запрета?';uk='Очистити незаповнені рядки і перерахувати відносні дати заборони?'");
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удалить все даты запрета, кроме установленных %1?';uk='Видалити усі дати заборони, крім встановлених %1?'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Значение %1 не изменяется.';uk='Значення %1 не змінюється.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Комментарий %1 не изменяется.';uk='Коментар %1 не змінюється.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
"поэтому будет сохранена установка даты запрета ""%2"".';uk='Установка дати заборони ""%1"" не настроєна,"
...
"способ указания даты запрета ""%3"".';uk='Спосіб зазначення дати заборони ""%1"""
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удалить даты запрета %1?';uk='Видалити дати заборони %1?'"), НРег(ЗначениеДляВсехПользователей));
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 даты запрета не установлены.';uk='%1 дати заборони не встановлені.'"), ЗначениеДляВсехПользователей);
...
"для разделов.';uk='Розділи вже заповнені,"
...
Функция ТекстСообщенияРазделыУжеЗаполненыМожноУстановитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ru='Разделы уже заполнены,"
"можно установить даты запрета"
"для разделов и объектов.';uk=""Розділи вже заповнені,"
"можна встановити дати заборони"
...
"можно установить дату запрета.';uk=""Об'єкт вже вибраний,"
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 разделы всегда показываются.';uk='%1 розділи завжди показуються.'"), ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 <Общая дата> всегда показывается.';uk='%1 <Загальна дата> завжди показується.'"), ЗначениеДляВсехПользователей);
