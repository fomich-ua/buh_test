	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Иначе
				...
				КонецЕсли;
				ТекстНадписи = Символы.ПС
				             + НСтр("ru='По %2 возможно изменение данных с %3 по %4';uk='З %2 можлива зміна даних з %3 по %4'")
				             + Символы.ПС
				             + НСтр("ru='После %2 будет запрещено изменение данных по %4';uk='Після %2 буде заборонено зміну даних до %4'")
				             + Символы.ПС;
			КонецЕсли;
		Иначе
			Контекст.Элементы.СвойствоКоличествоДнейРазрешенияИзменения.ТекущаяСтраница =
