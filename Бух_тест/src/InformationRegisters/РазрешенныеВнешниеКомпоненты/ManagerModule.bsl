...
"некорректное имя макета %1!';uk=""Не вдалося сформувати дозвіл на використання зовнішнього компонента:"
"некоректне ім'я макета %1!"""), XDTOПредставление.TemplateName);
				КонецЕсли;
				
				Если Макет = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: макет %1 не обнаружден в составе конфигурации!';uk='Не вдалося сформувати дозвіл на використання зовнішнього компонента,"
"поставляє в макеті %1: макет %1 не виявлений у складі конфігурації!'"), XDTOПредставление.TemplateName);
				КонецЕсли;
				...
				Если Метаданные.НайтиПоПолномуИмени(XDTOПредставление.TemplateName).ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ДвоичныеДанные Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты:"
"макет %1 не содержит двоичных данных!';uk='Не вдалося сформувати дозвіл на використання зовнішнього компонента:"
"макет %1 не містить двоїчних даних!'"), XDTOПредставление.TemplateName);
				КонецЕсли;
				...
				Если ПустаяСтрока(ФайлМанифеста) Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: в архиве не обнаружен файл MANIFEST.XML!';uk='Не вдалося сформувати дозвіл на використання зовнішнього компонента,"
"поставляє в макеті %1: в архіві не виявлений файл MANIFEST.XML!'"), XDTOПредставление.TemplateName);
				КонецЕсли;
