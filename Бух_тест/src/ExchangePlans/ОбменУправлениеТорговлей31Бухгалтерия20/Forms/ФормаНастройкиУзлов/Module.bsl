...
Процедура ПолучитьОписаниеКонтекста()
	...
	Если ЗначениеЗаполнено(ДатаНачалаВыгрузкиДокументов) Тогда
		ДатаНачалаВыгрузкиДокументовОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Данные будут синхронизироваться, начиная с %1';uk='Дані будуть синхронізуватися, починаючи з %1'"),
			Формат(ДатаНачалаВыгрузкиДокументов, "ДЛФ=DD")
		);
	Иначе
		ДатаНачалаВыгрузкиДокументовОписание = НСтр("ru='Данные будут синхронизироваться за весь период ведения учета в программах';uk='Дані будуть синхронізуватися за весь період ведення обліку в програмах'");
	КонецЕсли;
	
	// отбор по Организациям
	Если ИспользоватьОтборПоОрганизациям Тогда
		ОрганизацииОписание = НСтр("ru='Только по организациям:';uk='Тільки за організаціями:'") + Символы.ПС + ИспользуемыеЭлементы("Организации");
	Иначе
	...
	Если ВыгружатьЦеныНоменклатуры Тогда
		ВидыЦенНоменклатурыОписание = НСтр("ru='Только по видам цен номенклатуры:';uk='Тільки за видами цін номенклатури:'") + Символы.ПС + ИспользуемыеЭлементы("ВидыЦенНоменклатуры");
	Иначе
		ВидыЦенНоменклатурыОписание = НСтр("ru='По всем видам цен номенклатуры.';uk='За усіма видами цін номенклатури.'");
	КонецЕсли;
