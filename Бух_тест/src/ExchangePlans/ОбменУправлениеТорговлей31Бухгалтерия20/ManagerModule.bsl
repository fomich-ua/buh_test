...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат "Настройки обмена для информационной базы Управление торговлей для Украины ред. 3.1";
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Управление торговлей для Украины, редакция 3.1';uk='Управління торгівлею для України, редакція 3.1'");
...
Функция КраткаяИнформацияПоОбмену(IDНастройки = "") Экспорт
	...
	ПоясняющийТекст = НСтр("ru='Позволяет синхронизировать данные между конфигурациями ""Бухгалтерия для Украины"", ред. 2.0, и ""Управление торговлей для Украины"", ред. 3.1."
"Синхронизация данных выполняется в двустороннем режиме на уровне документов и справочной информации.';uk='Дозволяє синхронізувати дані між конфігураціями ""Бухгалтерія для України"", ред. 2.0, і ""Управління торгівлею для України"", ред. 3.1."
...
Процедура ВыполнитьКорректировкуРегистраСоответствия() Экспорт
	...
	|				Обмен.Ссылка
	|			ИЗ
	|				ПланОбмена.ОбменУправлениеТорговлей31Бухгалтерия20 КАК Обмен
	|			ГДЕ
	...
	Запрос.УстановитьПараметр("ЭтотУзел", ПланыОбмена.ОбменУправлениеТорговлей31Бухгалтерия20.ЭтотУзел());
...
Процедура ИнициализироватьПравилаОтправкиСправочниковИДокументов() Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		|	ОбменУправлениеТорговлей31Бухгалтерия20.Ссылка
		|ИЗ
		|	ПланОбмена.ОбменУправлениеТорговлей31Бухгалтерия20 КАК ОбменУправлениеТорговлей31Бухгалтерия20
		|ГДЕ
		|	(ОбменУправлениеТорговлей31Бухгалтерия20.ПравилаОтправкиДокументов = &ПустаяСтрока
		|			ИЛИ ОбменУправлениеТорговлей31Бухгалтерия20.ПравилаОтправкиСправочников = &ПустаяСтрока
		|			ИЛИ ОбменУправлениеТорговлей31Бухгалтерия20.РежимВыгрузкиСправочников = Значение(Перечисление.РежимыВыгрузкиОбъектовОбмена.ПустаяСсылка))
		|	И ОбменУправлениеТорговлей31Бухгалтерия20.ПометкаУдаления = ЛОЖЬ");
...
Процедура ИнициализироватьРежимВыгрузкиСправочниковИДокументов() Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|	ПланОбмена.ОбменУправлениеТорговлей31Бухгалтерия20 КАК ОбменУправлениеТорговлей31Бухгалтерия20
		|ГДЕ
		|	ОбменУправлениеТорговлей31Бухгалтерия20.РежимВыгрузкиДокументов = Значение(Перечисление.РежимыВыгрузкиОбъектовОбмена.ПустаяСсылка)
		|	И ОбменУправлениеТорговлей31Бухгалтерия20.ПометкаУдаления = ЛОЖЬ");
