...
Функция ПрочитатьФрагментПотока()
	...
	Попытка
		...
		ПотокЧтения.Прочитать();
	Исключение
		ТекстЖР = ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка копирования фрагмента исходного файла. Частично скопированный фрагмент:"
"%1';uk='Помилка копіювання фрагмента вихідного файлу. Частково скопійований фрагмент:"
"%1'"),
				ЗаписьФрагмента.Закрыть());
		ЗаписьЖурналаРегистрации(НСтр("ru='Выгрузка/загрузка данных.Ошибка чтения XML';uk='Вивантаження/завантаження даних.Помилка читання XML'", 
			ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстЖР);
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ВызватьИсключениеЧтенияXML(Знач Фрагмент, Знач ТекстОшибки)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка при чтении данных из файла %1: при чтении фрагмента"
""
"%2"
""
"произошла ошибка:"
""
"%3.';uk='Помилка при читанні даних з файлу %1: при читанні фрагменту"
""
"%2"
""
"выдбулася помилка:"
