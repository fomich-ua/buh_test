...
//
// Параметры:
//    ОбъектДанных - ЛюбаяСсылка, Структура - Ссылка или структура со значениями измерений набор записей. Для этих данных 
...
//                                                   Иначе: 0 = нет, 1 = запрещена, 2 = разрешена, Неопределено для плана обмена
...
//        * Пометка                       - Булево       - Если указан объект, то 0 = нет регистрации, 1 - есть, иначе всегда 0
...
Функция ПредставлениеСсылки(ОбъектПредставления) Экспорт
	...
	Если МодульОбщегоНазначения <> Неопределено Тогда
		...
		Исключение
			// Нет или поломан метод получения предмета
		КонецПопытки;
	КонецЕсли;
...
Функция БСП_ОбновитьИЗарегистрироватьИОМГлавногоУзла(Знач Узел) Экспорт
	...
	Если (Не ДоступнаРаботаРИБ)                                      // Недоступна работа с ИОМ из-за версии БСП
		...
		Или (Не МетаПланОбменаУзла.РаспределеннаяИнформационнаяБаза) // Узел-параметр не РИБ
	Тогда 
		Возврат Результат;
	КонецЕсли;
	...
	// Актуализируем
	МодульСтандартныеПодсистемыСервер = ОбщегоНазначенияОбщийМодуль("СтандартныеПодсистемыСервер");
	МодульСтандартныеПодсистемыСервер.ОбновитьПараметрыРаботыПрограммы();
