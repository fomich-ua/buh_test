...
// ИменаСкрываемыхМетаданных   - СписокЗначений   - Содержит имена метаданных, которые будут исключены из дерева регистрации.
//
// При работе с подсистемой дополнительных отчетов и обработок используются дополнительные параметры:
//
// ДополнительнаяОбработкаСсылка - Произвольный - Ссылка на элемент справочника дополнительных отчетов и обработок, вызывающий форму.
//                                                При использовании должен быть заполнен параметр "ОбъектыНазначения"
// ОбъектыНазначения             - Массив       - Объекты для обработки. Будет использован первый элемент для открытия формы регистрации
//                                                объекта на узлах. При использовании должен быть заполнен параметр "ИдентификаторКоманды"
//
...
"на узле ""%1""?';uk='Зареєструвати результат запиту"
"на вузлі ""%1""?'"); 
	Иначе
		Текст = НСтр("ru='Отменить регистрацию результата запроса"
"на узле ""%1""?';uk='Скасувати реєстрацію результату запиту"
"на вузлі ""%1""?'");
	КонецЕсли;
...
"на узле ""%1""?';uk='Зареєструвати дані для відновлення підпорядкованого вузла РІБ"
"на вузлі ""%1""?'"),
		"%1", УзелОбменаСсылка
	);
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ru='Отпр. %1, прин. %2';uk='Відпр. %1, прийн. %2'");
...
Процедура ОбработкаВыбораУзлаОбмена()
	...
	Если Объект.ДоступнаРаботаРИБ                             // Доступна работа с ИОМ по версии БСП
		...
		И МетаПланОбменаУзла.РаспределеннаяИнформационнаяБаза // Текущий узел - РИБ
	Тогда
...
"на узле ""%0""';uk='Зареєстровано %1 змін з %2"
"на вузлі ""%0""'");
		Иначе
...
"на узле ""%0"".';uk='Скасовано реєстрацію %1 змін "
"на вузлі ""%0"".'");
		КонецЕсли;
	Иначе
...
"у %1 объекта(ов)';uk=""Номер повідомлення змінений на %3"
"в %1 об'єкта(ів)""");
	КонецЕсли;
...
"на узле ""%1""?';uk='Скасувати реєстрацію ""%2"""
"на вузлі ""%1""?'"); 
	Иначе
		Текст = НСтр("ru='Отменить регистрацию выбранных констант"
"на узле ""%1""?';uk='Скасувати реєстрацію обраних констант"
"на вузлі ""%1""?'"); 
	КонецЕсли;
...
"на узле ""%1""?';uk='Скасувати реєстрацію обраних об''єктів"
"на вузлі ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
""
"Изменение регистрации большого количества объектов может занять продолжительное время!';uk='Зареєструвати %1 для вивантаження на вузлі ""%2""?"
""
...
""
"Изменение регистрации большого количества объектов может занять продолжительное время!';uk='Скасувати реєстрацію %1 для вивантаження на вузлі ""%2""?"
""
"Зміна реєстрації великої кількості об''єктів може забрати тривалий час!'");
...
"Настроить сейчас?';uk='У настройках не зазначена обробка для виконання запитів."
"Настроїти зараз?'");
...
"командами ""Зарегистрировать"" или ""Отменить регистрацию""';uk='%1."
""
"%2"
...
"1С:Предприятие 8.3 с отключенным режимом совместимости или выше';uk='Обробка призначена для запуску на версії платформи"
"1С:Підприємство 8.3 з відключеним режимом сумісності або вище'");
