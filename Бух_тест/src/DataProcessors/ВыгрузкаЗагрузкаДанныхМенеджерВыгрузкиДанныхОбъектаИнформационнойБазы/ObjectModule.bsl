...
Перем ТекущаяЗаписьСопоставляемыхСсылок;
...
Процедура Инициализировать(Контейнер, ОбъектМетаданных, Обработчики, Сериализатор, ПотокЗаписиПересоздаваемыхСсылок) Экспорт
	...
	ТекущаяЗаписьСопоставляемыхСсылок = ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоСсылочныеДанные(ТекущийОбъектМетаданных);
	Если ТекущаяЗаписьСопоставляемыхСсылок Тогда
		ТекущийПотокЗаписиСопоставляемыхСсылок = Обработки.ВыгрузкаЗагрузкаДанныхПотокЗаписиСопоставляемыхСсылок.Создать();
		ТекущийПотокЗаписиСопоставляемыхСсылок.Инициализировать(ТекущийКонтейнер, ТекущийОбъектМетаданных, ТекущийСериализатор);
	КонецЕсли;
...
Процедура ВыгрузитьДанныеОбъектаМетаданных()
	...
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоКонстанта(ТекущийОбъектМетаданных) Тогда
		
		ВыгрузитьКонстанту(ПотокЗаписи)
		
	ИначеЕсли ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоСсылочныеДанные(ТекущийОбъектМетаданных) Тогда
		
		ВыгрузитьСсылочныйОбъект(ПотокЗаписи);
		
	ИначеЕсли ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоНаборЗаписей(ТекущийОбъектМетаданных) Тогда
		
		Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоНезависимыйНаборЗаписей(ТекущийОбъектМетаданных) Тогда
	...
	Иначе
		...
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неожиданный объект метаданных: %1';uk=""Несподіваний об'єкт метаданих: %1"""),
...
Процедура ВыгрузитьСсылочныйОбъект(ПотокЗаписи)
	...
	ЭтоПланОбмена = ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоПланОбмена(ТекущийОбъектМетаданных);
...
Процедура ВыгрузитьНаборЗаписейПодчиненныйРегистратору(ПотокЗаписи)
	...
	Если ВыгрузкаЗагрузкаДанныхСлужебный.ЭтоНаборЗаписейПерерасчета(ТекущийОбъектМетаданных) Тогда
		
		ИмяПоляРегистратора = "ОбъектПерерасчета";
		
		Подстроки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекущийОбъектМетаданных.ПолноеИмя(), ".");
