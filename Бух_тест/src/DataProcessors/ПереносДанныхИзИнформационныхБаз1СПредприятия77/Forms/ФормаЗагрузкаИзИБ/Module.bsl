...
Функция ПодключитьсяКИБ()
	...
	Если Элементы.ТаблицаПути.ТекущиеДанные = Неопределено Тогда
		...
		ЗафиксироватьОшибку(ЭтаФорма, ТекстСообщения, НСтр("ru='Возможно, на Вашем компьютере не установлена платформа 1С:Предприятие 7.7, либо используется БАЗОВАЯ версия программы. Воспользуйтесь вариантом загрузки данных из файла, предварительно сформировав его в информационной базе, из которой производится перенос данных.';uk=""Можливо, на Вашому комп'ютері не встановлена платформа 1С:Підприємство 7.7, або використовується БАЗОВА версія програми. Скористайтеся варіантом завантаження даних з файлу, попередньо сформувавши його в інформаційній базі, з якої виконується перенесення даних."""));
		Ошибка = Истина;
		Возврат 0;
	КонецЕсли;
...
Функция ВыгрузитьИзИБ(v7)
	...
	Если ПроверкаДанных Тогда
		Если ИдентификаторКонфигурации <> "UBUTK" Тогда
			ТекстСообщения = Нстр("ru='Проверка данных перед выгрузкой выполняется только для конфигурации ""1С:Бухгалтерия 7.7""';uk='Перевірка даних перед вивантаженням виконується тільки для конфігурації ""1С:Бухгалтерія 7.7""'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли ВозрастРелизаКонфигурации < 317 Тогда
			ТекстСообщения = Нстр("ru='Для проверки данных перед выгрузкой из ""1С:Бухгалтерии 7.7"" требуется релиз не ниже 7.70.317';uk='Для перевірки даних перед вивантаженням з ""1С:Бухгалтерії 7.7"" потрібний реліз не нижче 7.70.317'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Если НЕ ФайлДанных.Существует() Тогда
			...
			Для НомерСтроки = 1 По СтрЧислоСтрок(СписокОшибок) Цикл
				ШаблонСообщения = НСтр("ru='Ошибка выгрузки данных из информационной базы 1С:Предприятия 7.7: %1';uk='Помилка вивантаження даних з інформаційної бази 1С:Підприємства 7.7: %1'");
		...
		Иначе
			...
			СписокОшибок = СтрЗаменить(Результат, ",", Символы.ПС);
			Для НомерСтроки = 1 По СтрЧислоСтрок(СписокОшибок) Цикл
				ШаблонСообщения = НСтр("ru='Ошибка получения данных из информационной базы 1С:Предприятия 7.7: %1';uk='Помилка отримання даних з інформаційної бази 1С:Підприємства 7.7: %1'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтрПолучитьСтроку(СписокОшибок, НомерСтроки));
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
