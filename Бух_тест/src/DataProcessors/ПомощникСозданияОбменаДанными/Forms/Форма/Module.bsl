...
"Работа помощника не может быть завершена.';uk='Виконується створення синхронізації даних."
...
"Отказаться от настройки и использовать значения по умолчанию?';uk='Виконується настройка підпорядкованого вузла розподіленої інформаційної бази."
...
Процедура НадписьРезервноеКопированиеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "РезервноеКопирование" Тогда
		МодульРезервноеКопированиеИБКлиент = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("РезервноеКопированиеИБКлиент");
...
Функция НаименованиеВторойБазы()
	...
	ТекстЗапроса = "ВЫБРАТЬ
		...
		|	И СозданныеОбмены.Ссылка <> &ЭтотУзел";
	...
	Запрос.УстановитьПараметр("ЭтотУзел",     ПланыОбмена[Объект.ИмяПланаОбмена].ЭтотУзел());
...
"Перейти в журнал регистрации?';uk='При створенні синхронізації даних виникли помилки."
...
"%1';uk='Параметри підключення до додатка в сервісі:"
"%1'");
...
"Перейти в журнал регистрации?';uk='Помилка установки підключення."
"Перейти в журнал реєстрації?'");
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	ТекстПредупреждения = НСтр("ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.';uk='Для передачі файлу настройок синхронізації даних необхідно встановити розширення для веб-клієнта 1С:Підприємство.'");
...
"Следует настроить хотя бы один вариант подключения.';uk='Не зазначені параметри підключення для синхронізації даних."
...
"Префикс второй информационной базы: %5';uk='%1%2%3Префікс цієї інформаційної бази: %4"
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі створення настройок синхронізації даних."
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі аналізу даних."
"Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
"Продолжить?';uk='Не всі дані зіставлені. Наявність незіставлених даних"
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі синхронізації довідників."
"Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі збереження настройок."
"Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
"Для решения проблем воспользуйтесь журналом регистрации.';uk='Виникли помилки на етапі синхронізації даних."
"Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
"Измените значение префикса или используйте существующую синхронизацию.';uk='Значення префікса другої інформаційної бази не унікально."
