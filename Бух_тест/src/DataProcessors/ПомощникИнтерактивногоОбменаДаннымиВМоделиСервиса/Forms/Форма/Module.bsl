...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
		ТекстВопроса = Локализация.СтрШаблонУкр(НСтр("ru='Восстановить настройки ""%1""?';uk='Відновити настройки ""%1""?'"), ПредставлениеНастройки
		);
...
Процедура УстановитьКорреспондентаВЗаголовок(ВладелецЗаголовка)
	...
	ВладелецЗаголовка.Заголовок = Локализация.СтрШаблонУкр(ВладелецЗаголовка.Заголовок, Строка(Объект.УзелИнформационнойБазы));
