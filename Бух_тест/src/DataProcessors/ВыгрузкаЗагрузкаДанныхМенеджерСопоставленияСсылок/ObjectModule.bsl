...
Процедура ВыполнитьСопоставлениеСсылок() Экспорт
	...
	ИмяКолонки = ВыгрузкаЗагрузкаДанныхСлужебный.ИмяКолонкиСИсходнойСсылкой(ТекущийКонтейнер);
		...
		Для Каждого ОписаниеФайла Из СловариСопоставленияСсылок Цикл
			...
			Если СтандартнаяОбработка Тогда
				...
				Обработки.ВыгрузкаЗагрузкаДанныхМенеджерСопоставленияСсылок.ВыполнитьСтандартноеСопоставлениеСсылок(
					ТекущийПотокЗаменыСсылок, ОбъектМетаданных, ТаблицаИсходныхСсылок, ИмяКолонки);
			...
			Иначе
				...
				СоответствиеСсылок = ОбработчикСопоставленияСсылок.СопоставитьСсылки(ТекущийКонтейнер, ТаблицаИсходныхСсылок);
				Для Каждого ЭлементСоответствия Из СоответствиеСсылок Цикл
					
					ТекущийПотокЗаменыСсылок.ЗаменитьСсылку(ИмяТипаXML, Строка(ЭлементСоответствия[ИмяКолонки].УникальныйИдентификатор()),
