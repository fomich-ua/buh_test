...
//
//  Параметры:
//      АдресОбъекта - Строка, УникальныйИдентификатор - адрес временного хранилища с параметрами дополнительной регистрации
//
//  Возвращаемое значение:
//      УникальныйИдентификатор - идентификатор фонового задания
//      Неопределено            - если задание уже выполнено
//
Функция ФоноваяРегистрацияДополнительныхИзменений() Экспорт
	...
	Возврат ОбменДаннымиСервер.ЗапуститьФоновоеЗадание(
		...
		ПараметрыЗадания, НСтр("ru='Регистрация дополнительных данных для отправки при синхронизации';uk='Реєстрація додаткових даних для відправлення при синхронізації'"));
...
//
//  Параметры:
//      АдресРезультата      - Строка, УникальныйИдентификатор - адрес во временном хранилище, куда будет помещен результат
//      СписокИменМетаданных - Массив - строки - имена для отчета
//
//  Возвращаемое значение:
//      УникальныйИдентификатор - идентификатор фонового задания
//      Неопределено            - если задание уже выполнено
...
//  Возвращает данные компоновщикв для общих отборов узла "УзелИнформационнойБазы"
//  Опирается на значения реквизитов "УзелИнформационнойБазы", "ДополнительнаяРегистрация"
//
//  Параметры:
//      АдресСохраненияСхемы - Строка, УникальныйИдентификатор - адрес временного хранилища для сохранения схемы компоновки
//
// Возвращаемое значение:
//      Структура  - поля:
...
//
// Параметры:
//      ПолноеИмяМетаданных - Строка, ДеревоЗначений- имя таблицы (например "Справочник.Валюты") или имя предопределенной группы
...
//
//  Параметры:
//      СписокИменМетаданных - Массив - имена метаданных (деревья значений группы ограничений, служебных идентификаторов 
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаОбъекта=Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';uk='Некоректне ім''я метаданих ""%1"" для реєстрації на вузлі ""%2""'"),
	...
	Для Каждого Элемент Из ЭлементыИсточника Цикл
		...
		Иначе
			...
			Если Набор = Неопределено Тогда
				...
				ВсеПоляТабличнойЧасти = РеквизитыТабличнойЧастиДляЗапроса(МетаТабличнаяЧасть, Псевдоним);
				Набор.Запрос = "
					|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ДобавитьДополнительныеОтборыКомпоновкиТабличныхЧастей(ЭлементыПриемника, ЭлементыИсточника, ЭлементыНабора, ПараметрыДобавления)
	...
	Для Каждого Элемент Из ЭлементыИсточника Цикл
		...
		ИначеЕсли ПутьКДанным <> "НомерСтроки" И ПутьКДанным <> "Ссылка"
			И МетаТабличнаяЧасть.Реквизиты.Найти(ПутьКДанным) = Неопределено 
		Тогда
...
//
//  Параметры:
//      УзелОбмена - ПланОбменаСсылка - ссылка на узел обмена для возвращаемых настроек. Если не указано, то используется 
//                                      текущее значение реквизита "УзелИнформационнойБазы"
//
//  Возвращаемое значение:
