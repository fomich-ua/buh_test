...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВремяНачалаОтложенногоОбновления) Тогда
			ТекстПредупреждения = НСтр("ru='Обработка данных еще не завершилась.';uk='Обробка даних ще не завершилася.'");
		Иначе
			...
			Для Каждого СтрокаОбработчика Из СтрокаДереваВерсия.Строки Цикл
				...
				Если СтрокаОбработчика.Статус = "Выполнено" Тогда
					...
					СтрокаСписка.ИнформацияОПроцедуреОбновления = 
						НСтр("ru='Процедура ""%1"" обработки данных завершилась успешно.';uk='Процедура ""%1"" обробки даних завершилася успішно.'");
					СтрокаСписка.СтатусОбработчика = НСтр("ru='Выполнена';uk='Виконано'");
					СтрокаСписка.Вес = 1;
					СтрокаСписка.СтатусКартинка = БиблиотекаКартинок.Успешно;
				ИначеЕсли СтрокаОбработчика.Статус = "Выполняется" Тогда
					ОбработчикиНеВыполнялись = Ложь;
					СтрокаСписка.ИнформацияОПроцедуреОбновления = 
						НСтр("ru='Процедура ""%1"" обработки данных в данный момент выполняется.';uk='Процедура ""%1"" обробки даних у цей момент виконується.'");
					СтрокаСписка.СтатусОбработчика = НСтр("ru='Выполняется';uk='Виконується'");
					СтрокаСписка.Вес = 3;
				ИначеЕсли СтрокаОбработчика.Статус = "Ошибка" Тогда
					...
					СтрокаСписка.СтатусОбработчика = НСтр("ru='Ошибка';uk='Помилка'");
					СтрокаСписка.Вес = 4;
					СтрокаСписка.СтатусКартинка = БиблиотекаКартинок.Остановить;
				Иначе
					ВыполненыВсеОбработчики = Ложь;
					СтрокаСписка.СтатусОбработчика = НСтр("ru='Не выполнялась';uk='Не виконувалася'");
					СтрокаСписка.Вес = 2;
					СтрокаСписка.ИнформацияОПроцедуреОбновления = НСтр("ru='Процедура ""%1"" обработки данных еще не выполнялась.';uk='Процедура ""%1"" обробки даних ще не виконувалася.'");
				КонецЕсли;
