...
"Пожалуйста, подождите.';uk='Іде оновлення повнотекстового індексу..."
...
"<li>Попробуйте уменьшить количество искомых слов.';uk='Комбінація слів ""%1"" ніде не зустрічається.<br><br>"
"<b>Рекомендації:</b>"
"<li>Переконайтеся, що всі слова написані без помилок."
"<li>Спробуйте використати інші ключові слова."
"<li>Спробуйте зменшити кількість слів, які шукаєте.'"),
			СокрЛП(СтрокаПоиска));
...
Процедура ОбновитьАктуальностьИндекса()
	...
	Попытка
		...
		Если ДатаАктуальностиИндекса <> '00010101000000' Тогда
			Элементы.СостояниеИндекса.Подсказка = НСтр("ru='Последнее обновление индекса:';uk='Останнє оновлення індексу:'") + " " + Строка(ДатаАктуальностиИндекса);
		КонецЕсли;
		...
		Если Не ИндексАктуален Тогда
			СостояниеИндекса = НСтр("ru='Индекс давно не обновлялся, поэтому результаты поиска могут быть неточными.';uk='Індекс давно не оновлювався, тому результати пошуку можуть бути неточними.'");
		КонецЕсли;
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
