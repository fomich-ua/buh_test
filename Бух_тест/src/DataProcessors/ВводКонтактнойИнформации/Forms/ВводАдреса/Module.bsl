...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		Если Страна<>СтранаУкраина Тогда
			Если Не ПустаяСтрока(Параметры.ЗначенияПолей) Тогда
				...
				ТестПредставленияАдреса = СокрЛП(
					СокрЛП(Страна) + " " + СокрЛП(ПредставлениеАдресаОднойСтрокой)
				);
		...
		Иначе 
			...
			ТекстПредупрежденияПриОткрытии = ТекстПредупрежденияПриОткрытии
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Страна ""%1"" не найдена в справочнике стран мира.';uk='Країна ""%1"" не знайдена в довіднику країн світу.'"), ИсходноеПредставлениеСтраны
				);
		КонецЕсли;
	КонецЕсли;
	...
	Если Страна = СтранаУкраина Тогда
		Элементы.ТипАдреса.ТекущаяСтраница = Элементы.ПострочныйАдрес;
		ТекущийЭлемент = Элементы.НаселенныйПункт;
	Иначе
		Элементы.ТипАдреса.ТекущаяСтраница = Элементы.АдресОднойСтрокой;
		ТекущийЭлемент = Элементы.ПредставлениеАдресаОднойСтрокой;
	КонецЕсли;
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыФормы = Новый Структура("СкрыватьНеактуальныеАдреса, КодКлассификатораНаселенногоПункта, Улица, Заголовок", 
			СкрыватьНеактуальныеАдреса, -1, Улица, НаселенныйПункт + " " + НСтр("ru='(не найден)';uk='(не знайдений)'"));
	КонецЕсли;
...
"Продолжить?';uk='Зміни, введені вручну, будуть загублені."
"Продовжити?'");
	Иначе
		ТекстВопроса = НСтр("ru='Ввести адрес в свободной форме?"
"Адреса, введенные в свободной форме, могут не пройти проверку по адресному классификатору.';uk='Ввести адресу у вільній формі?"
"Адреси, введені у вільній формі, можуть не пройти перевірку за адресним класифікатором.'");
	КонецЕсли;
...
Функция РезультатаВыбораПоОбновлениюФлагов(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	Контекст.ВидКонтактнойИнформации.АдресТолькоУкраинский      = ЗначениеФлагов.АдресТолькоУкраинский;
...
Процедура ОбработкаИзмененияСтраныКлиент()
	...
	Элементы.ВвестиАдресВСвободнойФорме.Доступность             = ЭтоПострочныйАдрес;
	Элементы.ВвестиАдресВСвободнойФормеВсеДействия.Доступность  = ЭтоПострочныйАдрес;
...
Процедура УстановитьСписокВыбораЭлемента(ЭлементВид, ЭлементЗначение, Данные)
	...
	Если ЭлементВид.КнопкаВыпадающегоСписка Тогда
		Представления = КонтактнаяИнформацияКлиентСерверПовтИсп.ПредставленияОбъектовАдресации();
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	Иначе
		...
		Если ВРЕГ(НаименованиеСтраны)=ВРЕГ(СокрЛП(СсылкаНаУкраину.Наименование)) Тогда
	...
	Если КонтактнаяИнформацияСлужебный.ЭтоПострочныйАдрес(ДанныеАдреса) Тогда
...
Функция КонтактнаяИнформацияПоЗначениюРеквизитов(Контекст)
	...
	Если Контекст.РазрешитьВводАдресаВСвободнойФорме И Контекст.ПредставлениеАдресаИзменено Тогда
		...
		Иначе
			...
			АдресПострочно.Адрес_по_документу = ВведенноеПредставление;
		КонецЕсли;
	Иначе
		АдресПострочно.Сбросить("Адрес_по_документу");
		Результат.Представление = РасчетноеПредставление;
...
Функция КонтекстФормыКлиент(НовыеДанные=Неопределено)
	СписокРеквизитов = "
		...
		|Страна, КодСтраны, СтраныДляПострочногоВвода, СкрыватьНеактуальныеАдреса,
		|Индекс, ПредставлениеАдреса, ПредставлениеАдресаОднойСтрокой, Комментарий, КопияКомментария,
...
Процедура СостояниеВводаПредставленияАдреса(Режим, ФормироватьПредставление=Истина)
	...
	Если Режим Тогда
		...
		Элементы.ПредставлениеАдреса.Заголовок          = НСтр("ru='Адрес в свободной форме';uk='Адреса у вільній формі'");
...
Процедура СостояниеГруппыВвода(Группа, Режим)
	...
	Для Каждого Элемент Из Группа.ПодчиненныеЭлементы Цикл
		...
		Если ТипЭлемента = Тип("ГруппаФормы") Тогда
			Если Элемент <> Элементы.АдресОднойСтрокой Тогда
