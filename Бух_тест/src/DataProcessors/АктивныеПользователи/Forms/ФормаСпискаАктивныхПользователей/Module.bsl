...
Процедура ЗавершитьСеанс(Команда)
	...
	Для Каждого Обработчик Из ОбработчикиСобытия Цикл
		Обработчик.Модуль.ПриЗавершенииСеанса(ЭтотОбъект, ЗавершаемыйСеанс, СтандартнаяОбработка);
	КонецЦикла;
...
"администрирования кластера серверов';uk='Для завершення сеансу необхідно ввести параметри"
"адміністрування кластера серверів'");
			СоединенияИБКлиент.ПоказатьПараметрыАдминистрирования(ОписаниеОповещения, Ложь,, ПараметрыАдминистрирования, ЗаголовокФормы, ПоясняющаяНадпись);
...
Процедура ОткрытьЖурналРегистрации()
	...
	Если Элементы.СписокПользователей.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Для просмотра журнала регистрации выберите в списке только одного пользователя.';uk='Для перегляду журналу реєстрації виберіть у списку тільки одного користувача.'"));
		Возврат;
	КонецЕсли;
		
	ТекущиеДанные = Элементы.СписокПользователей.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Невозможно открыть журнал регистрации для выбранного пользователя.';uk='Неможливо відкрити журнал реєстрації для обраного користувача.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗавершитьСеансПродолжение(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Фильтр = Новый Структура("Номер", НомерСеансаДляЗавершения);
	...
	ТекстОповещения = НСтр("ru='Сеанс %1 завершен.';uk='Сеанс %1 завершений.'");
	ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОповещения, НомерСеансаДляЗавершения);
