...
Процедура ВычислитьИтоговыйAPDEX(ТаблицаКлючевыхОпераций)
	...
	Если МинимальныйПриоритет < 1 Тогда
		ТекстСообщения = НСтр("ru='Неверно заполнены приоритеты. Расчет итогового APDEX невозможен.';uk='Невірно заповнені пріоритети. Розрахунок підсумкового APDEX неможливий.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	
	МаксимальноеКоличествоОперацийЗаПериод = ТаблицаКлючевыхОпераций[ИндексСтрокиИтогов][ИндексНачальнойКолонки];
...
// Устарела. Следует использовать функцию СтруктураПараметровДляРасчетаАпдекса.
//
// Создает структуру параметров необходимую для расчета APDEX
//
// Возвращаемое значение:
