...
Процедура ПроизводительностьВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Лев(Поле.Имя, 18) <> "Производительность"
		Или СтрокаТЧ.КлючеваяОперация = Объект.ОбщаяПроизводительностьСистемы
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура ПереместитьВверх(Команда)
	...
	Если Врем.Количество() <= 1 ИЛИ
		ТекущийИндекс = 0 ИЛИ
		Врем[ТекущийИндекс - 1].КлючеваяОперация = Объект.ОбщаяПроизводительностьСистемы ИЛИ
		ТД.КлючеваяОперация = Объект.ОбщаяПроизводительностьСистемы
		Тогда
			Возврат;
	КонецЕсли;
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.ОценкаПроизводительности.Форма.АвтоматическийЭкспортЗамеровПроизводительности", , ЭтотОбъект);
...
Процедура УказатьAPDEX(Команда)
	...
	Если Лев(Элемент.Имя, 18) <> "Производительность"
		Или СтрокаТЧ.КлючеваяОперация = Объект.ОбщаяПроизводительностьСистемы
		Тогда
			Возврат;
	КонецЕсли;
...
"Рассчитать целевое время невозможно.';uk='Відсутні заміри продуктивності."
...
"Допустимые значения от 0 до 1.';uk='Введено неправильний показник APDEX."
...
Процедура Экспортировать(АдресВХранилище, ПараметрыЭкспорта)
	...
	Пока НачалоИнтервала < Конец Цикл
		...
		Если ЭкспортироватьИнтервал(ИмяВременногоФайла, НачалоИнтервала, КонецИнтервала, МассивКлючевыхОпераций) Тогда
...
//
&НаСервереБезКонтекста
Функция ЭкспортироватьИнтервал(ИмяВременногоФайла, Начало, Конец, МассивКлючевыхОпераций)
...
Функция НастройкаВыполнена()
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			И СтрокаТЧ.КлючеваяОперация <> Объект.ОбщаяПроизводительностьСистемы
		Тогда
