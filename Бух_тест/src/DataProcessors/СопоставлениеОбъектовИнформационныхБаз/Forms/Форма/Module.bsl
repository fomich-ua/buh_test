...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВариантыОтбораСтатусаСопоставления.Вставить(СписокВыбора.Добавить(
		"ВсеОбъекты", НСтр("ru='Все данные';uk='Всі дані'")
	).Значение, Новый ФиксированнаяСтруктура);
	
	ВариантыОтбораСтатусаСопоставления.Вставить(СписокВыбора.Добавить(
		"СопоставленныеОбъектыНеутвержденные", НСтр("ru='Изменения';uk='Зміни'"),
	).Значение, Новый ФиксированнаяСтруктура("СтатусСопоставления",  3));
	
	ВариантыОтбораСтатусаСопоставления.Вставить(СписокВыбора.Добавить(
		"СопоставленныеОбъекты", НСтр("ru='Сопоставленные данные';uk='Зіставлені дані'"),,
	).Значение, Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 0));
	
	ВариантыОтбораСтатусаСопоставления.Вставить(СписокВыбора.Добавить(
		"НесопоставленныеОбъекты", НСтр("ru='Несопоставленные данные';uk='Незіставлені дані'"),
	).Значение, Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 1));
	
	ВариантыОтбораСтатусаСопоставления.Вставить(СписокВыбора.Добавить(
		"НесопоставленныеОбъектыПриемника", НСтр("ru='Несопоставленные данные этой базы';uk='Незіставлені дані цієї бази'"),
	).Значение, Новый ФиксированнаяСтруктура("СтатусСопоставления",  1));
	
	ВариантыОтбораСтатусаСопоставления.Вставить(СписокВыбора.Добавить(
		"НесопоставленныеОбъектыИсточника", НСтр("ru='Несопоставленные данные второй базы';uk='Незіставлені дані другої бази'"),
	).Значение, Новый ФиксированнаяСтруктура("СтатусСопоставления", -1));
...
"Оставшиеся несопоставленные данные можно связать друг с другом вручную.';uk='Для зіставлення даних ""%1"""
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ВыполнитьЗагрузкуДанныхЗавершение1", ЭтотОбъект);
...
&НаКлиенте
Процедура ВыполнитьЗагрузкуДанныхЗавершение1(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ВыполнитьЗагрузкуДанныхЗавершение2", ЭтотОбъект);
		
		ПоказатьВопрос(Оповещение, НСтрока, РежимДиалогаВопрос.ДаНет, ,КодВозвратаДиалога.Нет);
		Возврат;
	КонецЕсли;
	
	ВыполнитьЗагрузкуДанныхЗавершение3();
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьЗагрузкуДанныхЗавершение2(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьЗагрузкуДанныхЗавершение3()
...
"Перейти в журнал регистрации?';uk='При одержанні даних виникли помилки."
"Перейти в журнал реєстрації?'");
		
		Оповещение = Новый ОписаниеОповещения("ВыполнитьЗагрузкуДанныхЗавершение4", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, НСтрока, РежимДиалогаВопрос.ДаНет, ,КодВозвратаДиалога.Нет);
...
&НаКлиенте
Процедура ВыполнитьЗагрузкуДанныхЗавершение4(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (
		    ТекущиеДанные.СтатусСопоставления=-1	// Несопоставленный объект источника
		Или ТекущиеДанные.СтатусСопоставления=+1	// Несопоставленный объект приемника
	) Тогда
	...
	Иначе
		...
		Если Не (
			(
				  Строка1.СтатусСопоставления = -1 // Несопоставленный объект источника
				И Строка2.СтатусСопоставления = +1 // Несопоставленный объект приемника
			) Или (
				  Строка1.СтатусСопоставления = +1 // Несопоставленный объект приемника
				И Строка2.СтатусСопоставления = -1 // Несопоставленный объект источника
			) )
		Тогда
			НельзяУстановитьСвязьБыстро = Истина;
		КонецЕсли;
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Возврат СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Результат, ", ");
КонецФункции
...
Процедура ВыполнитьСопоставлениеОбъектов(Отказ)
	...
	Попытка
		...
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			РезультатСопоставления = Обработки.СопоставлениеОбъектовИнформационныхБаз.РезультатСопоставленияОбъектов(ПараметрыМетода);
			ПослеСопоставленияОбъектов(РезультатСопоставления);
