...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодтверждениеЗакрытияФормы<>Истина Тогда
		...
		Текст = НСтр("ru='Закрыть объединение элементов?';uk=""Закрити об'єднання елементів?""");
...
Функция ПроверитьОбъединяемыеСсылки(Знач НаборСсылок)
	...
	Если КоличествоСсылок < 2 Тогда
		Результат.Ошибка = НСтр("ru='Для объединения необходимо указать несколько элементов.';uk=""Для об'єднання необхідно вказати кілька елементів.""");
		Возврат Результат;
	КонецЕсли;
...
"Группы не могут быть объединены.';uk=""Один з елементів для об'єднання є групою."
...
"Такие элементы не могут быть объединены.';uk=""У елементів для об'єднання різні власники."
"Такі елементи не можуть бути об'єднані.""");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ru='Все элементы для объединения  должны быть одного типа.';uk=""Всі елементи для об'єднання повинні бути одного типу.""");
	Иначе 
		// Все нормально
...
"использования на ""%2"" (отмечен стрелкой)';uk='Об''єднати елементи (%1) у ""%2""."
"Всі елементи зі списку будуть %3 і замінені у всіх місцях"
"використання на ""%2"" (відмічений стрілкою)'"),
...
"автоматическая замена на ""%3""';uk='Не вдалося об''єднати елементи (%1 з %2). В деяких місцях використання не може бути проведена"
...
Процедура ЗаполнитьМестаИспользования(Знач ТаблицаИспользования)
	...
	Для Каждого Строка Из ТаблицаИспользования Цикл
		...
		СтрокаИспользования.НеИспользуется = ?(Мест = 0, НСтр("ru='Не используется';uk='Не використається'"), "");
	КонецЦикла;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Объединение элементов %1 в один';uk=""Об'єднання елементів %1 в один"""),
		Представление
	);
КонецПроцедуры
...
Процедура ИнициализироватьСценарийМастера()
	...
	ДобавитьШагМастера(Элементы.ШагПоискМестИспользования, 
			...
			КнопкаШагаМастера("Отмена", НСтр("ru='Прервать';uk='Перервати'"), НСтр("ru='Отказаться от объединения элементов';uk=""Відмовитися від об'єднання елементів"""),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагВыборОсновногоЭлемента, 
			...
			КнопкаШагаМастера("Далее", НСтр("ru='Объединить';uk=""Об'єднати""") + " >", НСтр("ru='Начать объединение элементов';uk=""Почати об'єднання елементів"""), Истина, Истина, Истина,
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", НСтр("ru='Отмена';uk='Відмінити'"),  НСтр("ru='Отказаться от объединения элементов';uk=""Відмовитися від об'єднання елементів"""),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагОбъединение, 
			...
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Отмена", НСтр("ru='Прервать';uk='Перервати'"), НСтр("ru='Прервать объединение элементов';uk=""Перервати об'єднання елементів"""),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагУспешногоЗавершения, 
			...
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", НСтр("ru='Закрыть';uk='Закрити'"), НСтр("ru='Закрыть результаты объединения';uk=""Закрити результати об'єднання"""), Истина,  Истина,  Истина,
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагПовторОбъединения,
			...
			КнопкаШагаМастера("Далее", НСтр("ru='Повторить объединение';uk=""Повторити об'єднання""") + " >", НСтр("ru='Повторить объединение';uk=""Повторити об'єднання"""),  Истина, Истина, Истина, 
			КнопкаШагаМастера("Назад", "< " + НСтр("ru='Назад';uk='Назад'"), НСтр("ru='Вернутся к выбору основного элемента';uk='Повернутися до вибору основного елемента'"), Истина, Истина, Ложь,
			КнопкаШагаМастера("Отмена", НСтр("ru='Закрыть';uk='Закрити'"), НСтр("ru='Закрыть результаты объединения';uk=""Закрити результати об'єднання"""),
		)))
	);
...
Процедура ШагПоискМестИспользованияПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = НСтр("ru='Прервать поиск мест использования и закрыть?';uk='Перервати пошук місць використання і закрити?'");
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ПараметрыПредупреждения.Вставить("Заголовок", НСтр("ru='Невозможно объединить элементы';uk=""Неможливо об'єднати елементи"""));
...
Процедура ШагВыборОсновногоЭлементаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = НСтр("ru='Отменить объединение элементов и закрыть?';uk=""Скасувати об'єднання елементів і закрити?""");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
...
Процедура ШагОбъединениеПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = НСтр("ru='Прервать процесс объединения и закрыть?';uk=""Перервати процес об'єднання і закрити?""");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
