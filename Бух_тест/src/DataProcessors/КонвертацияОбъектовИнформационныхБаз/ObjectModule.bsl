...
"Если подходящего значения приемника нет, то указать пустое значение.';uk='У правилі конвертації значень (ПКЗ) необхідно зіставити значення Джерела значенню Приймача."
...
Функция ВерсияФорматаВходящегоСообщенияОбмена()
	...
	Если РазрядыВерсий.Количество() < 4 Тогда
		...
		ПолеВерсияФорматаВходящегоСообщенияОбмена = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(РазрядыВерсий, ".");
...
"Имя файла ""%1""."
"Описание ошибки:"
"%2';uk='Помилка при відкритті файлу для запису повідомлення обміну."
"Ім''я файлу ""%1""."
"Опис помилки:"
"%2'"),
...
Процедура ЗагрузитьПравилоКонвертации(ПравилаОбмена, ЗаписьXML)
	...
	ПоляТаблицы = "";
	ПоляПоиска = "";
	Если НоваяСтрока.Свойства.Количество() > 0 Тогда
		...
		ПоляТаблицы = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СвойстваМассив);
		ПоляПоиска  = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СвойстваПоискаМассив);
...
"Требуется выполнить загрузку правил обмена повторно.';uk='Версія формату зберігання правил обміну не відповідає очікуваній."
...
""
"Подробности:"
"%1';uk='Помилка установки блокування на обмін даними."
...
Процедура ЗакончитьЧтениеСообщения(Знач ЧтениеСообщения)
	...
	Попытка
		...
		Если Не ЧтениеСообщения.АнализДанных Тогда
			...
			ЧтениеСообщения.ОтправительОбъект.ДополнительныеСвойства.Вставить("Загрузка");
...
"%2';uk='Операція не виконана: %1"
"%2'");
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения,
				СтрокаДвижения.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение ТекстИсключения;
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Если Не Результат Тогда
		...
		КлючевыеПоляПоиска = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(МассивКлючевыхПолейПоиска);
...
"Имя файла ""%1""."
"Описание ошибки:"
"%2';uk='Помилка при створенні тимчасового файлу для вивантаження даних."
"Ім''я файлу ""%1""."
"Опис помилки:"
"%2'"),
...
"Нельзя помечать на удаление предопределенный элемент. Пометка на удаление для объекта не установлена.';uk=""Помилка установки властивості напередвизначеного елемента."
...
Процедура ПроизвестиЧтениеДанныхВРежимеАнализа(ЧтениеСообщения, ПараметрыАнализа = Неопределено)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "УдалениеОбъекта" Тогда
			...
			СтрокаТаблицы.ПоляТаблицы = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(МассивПодстрок);
...
Процедура ПрочитатьОбщиеДанныеУзлов(ЧтениеСообщения)
	...
	Если ОбменДаннымиСобытия.ДанныеРазличаются(ОбщийУзел, ОбщийУзел.Ссылка.ПолучитьОбъект()) Тогда
		...
		Попытка
			...
			ОбщийУзел.ДополнительныеСвойства.Вставить("Загрузка");
			ОбщийУзел.Записать();
...
Функция ПоляПоискаМеханизмаСопоставленияОбъектов(Знач ПоляПоиска)
	...
	ПоляПоискаКоллекция = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПоляПоиска);
	
	ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ПоляПоискаКоллекция, "ЭтоГруппа");
	
	Возврат СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ПоляПоискаКоллекция);
КонецФункции
