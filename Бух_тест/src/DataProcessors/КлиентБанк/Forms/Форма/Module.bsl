	...
	Если ВозможностьВыбораФайлов Тогда
		...
		Если ТребуетсяПовторноеЧтениеФайла Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.ФайлЗагрузки) Тогда
					...
					Нстр("ru='Не указан файл данных для загрузки в 1С:Бухгалтерию';uk='Не вказаний файл даних для завантаження в 1С:Бухгалтерію'")
					,, "Объект.ФайлЗагрузки");
				Возврат;
			КонецЕсли;
			...
			Если НЕ ФайлВыгрузки.Существует() Тогда
				ТекстСообщения = НСтр("ru='Файл данных для загрузки в 1С:Бухгалтерию не обнаружен';uk='Файл даних для завантаження в 1С:Бухгалтерію не виявлено'");
...
"Выберите файл загрузки';uk='Файл даних для завантаження в 1С:Бухгалтерію не коректний - обраний ""каталог""."
...
"%1';uk='При читанні файлу даних для завантаження в 1С:Бухгалтерію виникла помилка"
"%1'");
	...
	Если Операция = "Загрузка" Тогда
		...
		Если СтруктураДанныхКлиента.Свойство("КоличествоЗагруженныхДокументов")
			...
			ПоказатьОповещениеПользователя(ТекстОповещения,
				"e1cib/list/ЖурналДокументов.БанковскиеВыписки", НСтр("ru='Банковские выписки';uk='Банківські виписки'"));
		Иначе
	...
	Если Объект.ПлатежныеДокументы.Количество() > 0 Тогда
		...
		Если ВозможностьВыбораФайлов Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.ФайлВыгрузки) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Не указан файл данных для выгрузки из 1С:Бухгалтерии';uk='Не вказаний файл даних для вивантаження з 1С:Бухгалтерії'")
					,, "Объект.ФайлВыгрузки");
				Возврат;
			КонецЕсли;
...
"Выберите файл выгрузки';uk='Файл даних для вивантаження з 1С:Бухгалтерії не коректний - обраний ""каталог""."
...
"%1.'");
		Если ВозможностьВыбораФайлов Тогда
			Если ПустаяСтрока(Объект.ФайлЗагрузки) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ТекстШаблона, НСтр("ru='Не указан файл данных для загрузки в 1С:Бухгалтерию';uk='Не вказаний файл даних для завантаження в 1С:Бухгалтерію'"));
