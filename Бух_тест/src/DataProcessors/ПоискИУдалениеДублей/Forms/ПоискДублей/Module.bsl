...
"Задайте условия отбора и сравнения и нажмите ""Найти дубли"".';uk='Пошук дублів не виконувався. "
...
"Измените условия отбора и сравнения, нажмите ""Найти дубли""';uk='Не виявлено дублів за вказаними параметрами."
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодтверждениеЗакрытияФормы<>Истина Тогда
		...
		Текст = НСтр("ru='Закрыть поиск и замену дублей?';uk='Закрити пошук і заміну дублів?'");
...
Процедура ОбновитьМестаИспользованияКандидата(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = ФорматСтроки(
			НСтр("ru='Для элемента ""%1"" найдены дубли (%2)';uk='Для елементу ""%1"" знайдено дублі (%2)'"),
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = ФорматСтроки(НСтр("ru='Элемент ""%1"" не используется';uk='Елемент ""%1"" не використовується'"), 
			ДанныеСтроки.Наименование
		);
		
		Элементы.СтраницыМестаИспользования.ТекущаяСтраница = Элементы.ОписаниеГруппы;
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = ФорматСтроки(НСтр("ru='Места использования ""%1"" (%2)';uk='Місця використання ""%1"" (%2)'"), 
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = ФорматСтроки(НСтр("ru='Элемент ""%1"" успешно обработан';uk='Елемент ""%1"" успішно оброблено'"), 
			ДанныеСтроки.Наименование
		);
		
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = ФорматСтроки(НСтр("ru='Не удалось заменить дубли в некоторых местах (%1)';uk='Не вдалося замінити дублі в деяких місцях (%1)'"), 
...
Процедура УстановитьЦветаИУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Не используется';uk='Не використається'"));
...
Процедура ЗапуститьФоновыйПоискДублей()
	...
	ЗапуститьДлительнуюОперациюМастера("ФоновыйПоискДублей", ПараметрыПоиска, Истина, НСтр("ru='Поиск групп дублей';uk='Пошук груп дублів'") );
КонецПроцедуры
...
Процедура ИзменитьОсновнойЭлементИерархически(Знач ДанныеСтроки, Знач Родитель)
	...
	Родитель.Наименование = ФорматСтроки( НСтр("ru='%1 (%2)';uk='%1 (%2)'"), ДанныеСтроки.Наименование, Родитель.Количество);
КонецПроцедуры
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач Данные)
	...
	Если ВсегоНайденоДублей = 0 Тогда
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ",
			НСтр("ru='Не обнаружено дублей по указанным условиям';uk='Не виявлено дублів за вказаними умовами'")
		);
	Иначе
...
"и заменены во всех местах использования на оригиналы (отмечены стрелкой).';uk='Знайдені дублі елементів: %2 (серед елементів: %1). Всі відзначені елементи будуть відмічені для вилучення "
"і замінені у всіх місцях використання на оригінали (позначені стрілкою).'"),
...
"в ""%2""';uk='Всі знайдені дублі (%1) успішно об''єднані"
"у ""%2""'"),
		...
		Иначе
			...
			ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ", ФорматСтроки(
				НСтр("ru='Все найденные дубли (%1) успешно объединены."
"Оставлено элементов (%2).';uk=""Всі знайдені дублі (%1) успішно об'єднані."
...
Процедура ОбновитьОписаниеПравилПоиска()
	...
	Для Каждого Правило Из ПравилаПоиска Цикл
		...
		Если Правило.Правило = "Равно" Тогда
			Сравнение = НСтр("ru='%1 совпадает';uk='%1 збігається'");
		ИначеЕсли Правило.Правило = "Подобно" Тогда
			Сравнение = НСтр("ru='%1 совпадает по похожим словам';uk='%1 збігається за схожими словами'");
		Иначе
			Сравнение = "";
		КонецЕсли;
	...
	Если ПустаяСтрока(ТекстПравил) Тогда
		Элементы.ПравилаПоиска.Заголовок = НСтр("ru='Правила не заданы';uk='Правила не задані'");
	Иначе
...
Процедура ИнициализироватьКомпоновщикОтбораИПравила()
	...
	ВсеВариантыСравнения.Добавить("Подобно", НСтр("ru='Совпадает';uk='Збігається'"));
...
Процедура ИнициализироватьСценарийМастера()
	...
	ДобавитьШагМастера(Элементы.ШагПоискНеВыполнялся, 
			...
			КнопкаШагаМастера("Далее",  НСтр("ru='Найти дубли';uk='Знайти дублі'") + " >", НСтр("ru='Найти дубли по указанным критериям';uk='Знайти дублі за вказаними критеріями'"), Истина, Истина, Истина,
			КнопкаШагаМастера("Отмена", НСтр("ru='Отмена';uk='Відмінити'"), НСтр("ru='Отказаться от поиска и замены дублей';uk='Відмовитися від пошуку і заміни дублів'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагВыполнениеПоиска, 
			...
			КнопкаШагаМастера("Отмена", НСтр("ru='Прервать';uk='Перервати'"), НСтр("ru='Прервать поиск дублей';uk='Перервати пошук дублів'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагВыборОсновногоЭлемента, 
			...
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Далее",  НСтр("ru='Удалить дубли';uk='Вилучити дублі'") + " >", НСтр("ru='Удалить дубли';uk='Вилучити дублі'"), Истина, Истина, Истина,
			КнопкаШагаМастера("Отмена", НСтр("ru='Отмена';uk='Відмінити'"), НСтр("ru='Отказаться от поиска и замены дублей';uk='Відмовитися від пошуку і заміни дублів'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагВыполнениеУдаления, 
			...
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Отмена", НСтр("ru='Прервать';uk='Перервати'"), НСтр("ru='Прервать удаление дублей';uk='Перервати вилучення дублів'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагУспешноеУдаление,
			...
			КнопкаШагаМастера("Назад", "< " + НСтр("ru='Новый поиск';uk='Новий пошук'"), НСтр("ru='Начать новый поиск с другими параметрами';uk='Почати новий пошук з іншими параметрами'"), Истина, Истина, Ложь, 
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Отмена", НСтр("ru='Закрыть';uk='Закрити'"), НСтр("ru='Закрыть';uk='Закрити'"), Истина, Истина, Истина
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагНеудачныеЗамены,
			...
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Далее",  НСтр("ru='Повторить удаление';uk='Повторити вилучення'") + " >", НСтр("ru='Удалить дубли';uk='Вилучити дублі'"), Истина, Истина, Истина,
			КнопкаШагаМастера("Отмена", НСтр("ru='Закрыть';uk='Закрити'"), НСтр("ru='Закрыть';uk='Закрити'"), Истина, Истина, Ложь
		)))
	);
...
Процедура ШагПоискНеВыполнялсяПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = НСтр("ru='Отказаться от поиска и замены дублей?';uk='Відмовитися від пошуку і заміни дублів?'");
...
Процедура ШагВыполнениеПоискаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = НСтр("ru='Прекратить поиск дублей?';uk='Припинити пошук дублів?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
...
Процедура ШагВыборОсновногоЭлементаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = НСтр("ru='Закрыть поиск дублей?';uk='Закрити пошук дублів?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
...
Процедура ШагВыполнениеУдаленияПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ЗапуститьДлительнуюОперациюМастера("ФоновоеУдалениеДублей", ПараметрыУдаления, Истина, НСтр("ru='Удаление дублей';uk='Вилучення дублів'") );
КонецПроцедуры
...
Процедура ШагВыполнениеУдаленияПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = НСтр("ru='Прервать удаление дублей?';uk='Перервати вилучення дублів?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
