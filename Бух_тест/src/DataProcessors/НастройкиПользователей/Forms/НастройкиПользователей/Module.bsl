...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяФормыПерсональныхНастроек = ОбщегоНазначения.ОбщиеПараметрыБазовойФункциональности(
...
Процедура ПользовательСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОтбора = Новый Структура("СкрытьПользователейБезПользователяИБ, РежимВыбора", Истина, Истина);
...
"Откроется окно выбора пользователей, которым необходимо очистить настройки.';uk='Очистити виділені настройки?"
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, ПользовательСсылка);
...
"Продолжить?';uk='Зараз будуть очищені всі настройки всіх користувачів."
"Продовжити?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, ПользовательСсылка);
	КнопкиВопроса = Новый СписокЗначений;
...
Процедура ОчиститьНаСервере(Пользователи = Неопределено, ВыбраннымПользователям = Ложь)
	...
	// Очистка настроек
	Обработки.НастройкиПользователей.УдалитьВыбранныеНастройки(ПользовательИнформационнойБазы, Результат.МассивНастроек, НазваниеХранилища);
	Если Результат.МассивПерсональныхНастроек.Количество() <> 0 Тогда
		Обработки.НастройкиПользователей.УдалитьВыбранныеНастройки(
			ПользовательИнформационнойБазы, Результат.МассивПерсональныхНастроек, "ХранилищеОбщихНастроек");
	КонецЕсли;
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = ПользователиСлужебныйКлиент.ФормированиеСтрокиКоличестваНастроек(КоличествоНастроек);
	Если КоличествоНастроек = 1
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстПояснения, НазваниеНастройки, Строка(ПользовательСсылка));
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстПояснения, ПрописьПредмета, Строка(ПользовательСсылка));
	КонецЕсли;
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстПояснения, НазваниеНастройки, ПояснениеКомуОчищатьНастройки);
		Иначе
			ТекстПояснения = НСтр("ru='Очищено %1 %2';uk='Очищено %1 %2'");
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстПояснения, ПрописьПредмета, ПояснениеКомуОчищатьНастройки);
		КонецЕсли;
...
"его пересохранить со снятой пометкой ""Только для автора"".';uk='Неможливо скопіювати особистий варіант звіту."
...
"Копирование настроек личных вариантов отчетов не предусмотрено.';uk='Неможливо скопіювати настройок особистого варіанта звіту."
...
Функция ВыделенныеНастройки()
	...
	Возврат Новый Структура("МассивНастроек, МассивПерсональныхНастроек, МассивВариантовОтчетов, МассивПользовательскихНастроек",
			МассивНастроек, МассивПерсональныхНастроек, МассивВариантовОтчетов, МассивПользовательскихНастроек);
КонецФункции
