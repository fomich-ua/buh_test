		...
		Для Каждого Источник Из Источники Цикл
			...
			Если ОчищаемаяНастройка = "НастройкиОтчетов" Тогда
				...
				Если ТаблицаПользовательскихВариантовОтчетов = Неопределено Тогда
					ТаблицаПользовательскихВариантовОтчетов = ПользовательскиеВариантыОтчетов(Источник);
				КонецЕсли;
				
				Для Каждого ВариантОтчета Из ТаблицаПользовательскихВариантовОтчетов Цикл
					
					СтандартнаяОбработка = Истина;
					ПользователиСлужебный.ПриУдаленииПользовательскихВариантовОтчета(ВариантОтчета, Источник, СтандартнаяОбработка);
					Если СтандартнаяОбработка Тогда
						ХранилищеВариантовОтчетов.Удалить(ВариантОтчета.КлючОбъекта, ВариантОтчета.КлючВарианта, Источник);
					КонецЕсли;
			...
			ПользователиСлужебный.УстановитьНачальныеНастройки(Источник);
...
Процедура СкопироватьНастройки(МенеджерНастроек, ТаблицаНастроек, ПользовательИсточник,
	...
	Для Каждого Настройка Из ТаблицаНастроек Цикл
		...
		Если МенеджерНастроек = ХранилищеПользовательскихНастроекОтчетов
			...
			КлючОтчета = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КлючОбъекта, "/");
...
Процедура УдалитьНастройкиВыбраннымПользователям(Пользователи, МассивНастроекДляУдаления, НазваниеХранилища) Экспорт
	...
	Для Каждого Пользователь Из Пользователи Цикл
		...
		УдалитьВыбранныеНастройки(Пользователь, МассивНастроекДляУдаления, НазваниеХранилища);
	КонецЦикла;
...
//
Процедура УдалитьВыбранныеНастройки(ИмяПользователя, МассивНастроекДляУдаления, ИмяХранилища) Экспорт
	...
	Если ИмяХранилища = "ХранилищеПользовательскихНастроекОтчетов" Или ИмяХранилища = "ХранилищеОбщихНастроек" Тогда
		...
		Для Каждого Элемент Из МассивНастроекДляУдаления Цикл
			...
			Для Каждого Настройка Из Элемент Цикл
				МенеджерНастроек.Удалить(Настройка.Значение, Настройка.Представление, ИмяПользователя);
			КонецЦикла;
	...
	ИначеЕсли ИмяХранилища = "ХранилищеСистемныхНастроек" Тогда
		...
		Для Каждого Элемент Из МассивНастроекДляУдаления Цикл
			...
			Для Каждого Настройка Из Элемент Цикл
				...
				Если Настройка.Представление = "Интерфейс" Или Настройка.Представление = "Прочие" Тогда
					...
					МенеджерНастроек.Удалить(Настройка.Значение, , ИмяПользователя);
				...
				Иначе
					...
					Для Каждого ЭлементФорм Из МассивНастроекФорм Цикл
						МенеджерНастроек.Удалить(Настройка.Значение + ЭлементФорм, Настройка.Представление, ИмяПользователя);
					КонецЦикла;
		...
		Если УстановитьНачальныеНастройки Тогда
			ПользователиСлужебный.УстановитьНачальныеНастройки(ИмяПользователя);
		КонецЕсли;
...
Процедура УдалитьВариантыОтчетов(МассивВариантовОтчетов, ТаблицаПользовательскихВариантовОтчетов, ПользовательИнформационнойБазы) Экспорт
	...
	Для Каждого Настройка Из МассивВариантовОтчетов Цикл
		...
		КлючОбъекта = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Настройка[0].Значение, "/");
		...
		СтандартнаяОбработка = Истина;
		ПользователиСлужебный.ПриУдаленииПользовательскихВариантовОтчета(НайденныйВариантОтчета[0], ПользовательИнформационнойБазы, СтандартнаяОбработка);
		Если СтандартнаяОбработка Тогда
...
Функция ПредопределенныеНастройки()
	...
	Если ИнтерфейсТакси Тогда
		МассивНастроекФорм.Добавить("Такси/НастройкиОкна");
		МассивНастроекФорм.Добавить("Такси/НастройкиОкнаВебКлиента");
	Иначе
	...
	МассивНастроекФорм.Добавить("/ТекущиеПользовательскиеНастройки");
...
Процедура ДобавитьТипФормыВПредставление(Объект, Форма, ИмяФормы)
	...
	Если Форма = Объект.ОсновнаяФормаСписка Тогда
		ИмяФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (список)';uk='%1 (список)'"), ИмяФормы);
	ИначеЕсли Форма = Объект.ОсновнаяФормаДляВыбора Тогда
		ИмяФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (выбор)';uk='%1 (вибір)'"), ИмяФормы);
	ИначеЕсли Форма = Объект.ОсновнаяФормаГруппы Тогда
		ИмяФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (группа)';uk='%1 (група)'"), ИмяФормы);
	КонецЕсли;
...
Процедура ДобавитьСтрокуВТаблицуЗначений(ТаблицаНастроек, ВыборкаНастроек)
	...
	Если Найти(ВыборкаНастроек.КлючОбъекта, "ВнешнийОтчет.") <> 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПредставлениеВариантаОтчета(НастройкаКлюч, ИмяВариантаОтчета)
	...
	Если ПредставлениеВариантаОтчета <> Неопределено
		Тогда
...
Функция ПользовательскиеВариантыОтчетов(ПользовательИнформационнойБазы)
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		СтандартнаяОбработка = Истина;
		ПользователиСлужебный.ПриПолученииПользовательскихВариантовОтчетов(ОтчетМетаданные, ПользовательИнформационнойБазы, ТаблицаВариантовОтчетов, СтандартнаяОбработка);
		Если СтандартнаяОбработка Тогда
...
Функция ФормированиеОтчетаОКопировании(НеСкопированныеНастройкиОтчетов,
	...
	Если ТаблицаПользовательскихВариантовОтчетов <> Неопределено
		...
		ОбластьЗаголовок.Параметры.Описание = НСтр("ru='"
...
"Список пропущенных вариантов отчетов:';uk='"
"Неможливо скопіювати особисті варіанти звітів."
"Для того щоб особистий варіант звіту став доступний іншим користувачам,"
"необхідно його перезберегти зі знятою позначкою ""Тільки для автора""."
"Список пропущених варіантів звітів:'");
	...
	Если НеСкопированныеНастройкиОтчетов.Количество() <> 0 Тогда
		ОбластьЗаголовок = ТабМакет.ПолучитьОбласть("Заголовок");
		ОбластьЗаголовок.Параметры.Описание = НСтр("ru='"
"У следующих пользователй недостаточно прав на отчеты:';uk='"
"У наступних користувачів недостатньо прав на звіти:'");
		ТабДок.Вывести(ОбластьЗаголовок);
...
Процедура ЗаполнитьСписокНастроекОтчетов(Форма)
	...
	ИмяФормы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Форма.ИмяФормы, ".");
	...
	Для Каждого Настройка Из Настройки Цикл
		...
		Если ПредставлениеВариантаОтчета = "" Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого ВариантОтчета Из ТаблицаВариантовОтчетов Цикл
		...
		Если Не ПустаяСтрока(Форма.Поиск) Тогда
			...
			Если Найти(ВРег(ВариантОтчета.Представление), ВРег(Форма.Поиск)) = 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ДобавитьНастройкиРабочегоСтолаИКомандногоИнтерфейса(Форма, ДеревоНастроек)
	...
	Если Не ПустаяСтрока(Форма.Поиск) Тогда
		Если Найти(ВРег(НСтр("ru='Рабочий стол и командный интерфейс';uk='Робочий стіл і командний інтерфейс'")), ВРег(Форма.Поиск)) = 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если КлючиВсехНастроек.Количество() > 0 Тогда
		...
		НоваяСтрокаИнтерфейса.Настройка = НСтр("ru='Рабочий стол и командный интерфейс';uk='Робочий стіл і командний інтерфейс'");
		НоваяСтрокаИнтерфейса.Картинка = БиблиотекаКартинок.Картинка;
		НоваяСтрокаИнтерфейса.ТипСтроки = НСтр("ru='Рабочий стол и командный интерфейс';uk='Робочий стіл і командний інтерфейс'");
	...
	Если КлючиНастроекРабочегоСтола.Количество() > 0 Тогда
		...
		НоваяПодстрокаИнтерфейса.Настройка = НСтр("ru='Рабочий стол';uk='Робочий стіл'");
...
Процедура ЗаполнитьСписокНастроекВнешнегоВида(Форма)
	...
	Для Каждого НастройкаФормы Из НастройкиФорм Цикл
		...
		Если Не ПустаяСтрока(Форма.Поиск) Тогда
			...
			Если Найти(ВРег(НастройкаФормы.Представление), ВРег(Форма.Поиск)) = 0 Тогда
				Продолжить;
			КонецЕсли;
	...
	РабочийСтолИКомандныйИнтерфейс = НастройкиВнешнегоВида.Строки.Найти(НСтр("ru='Рабочий стол и командный интерфейс';uk='Робочий стіл і командний інтерфейс'"), "Настройка");
...
Процедура ЗаполнитьСписокПрочихНастроек(Форма)
	...
	Если Ключи.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(Форма.Поиск) Тогда
			Если Найти(ВРег(НСтр("ru='Персональные настройки';uk='Персональні настройки'")), ВРег(Форма.Поиск)) = 0 Тогда
	...
	Если Не ПустаяСтрока(Форма.Поиск) Тогда
		...
		Если Найти(ВРег(НСтр("ru='Настройки печати табличных документов';uk='Настройки друку табличних документів'")), ВРег(Форма.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Форма.Поиск) Тогда
		...
		Если Найти(ВРег(НСтр("ru='Избранное';uk='Вибране'")), ВРег(Форма.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
		...
		Для Каждого ПрочаяНастройка Из ПрочиеНастройки Цикл
			...
			Если Результат.СписокНастроек.Количество() <> 0 Тогда
				...
				Если Не ПустаяСтрока(Форма.Поиск) Тогда
					...
					Если Найти(ВРег(Результат.НазваниеНастройки), ВРег(Форма.Поиск)) = 0 Тогда
						ВыводитьНастройку = Ложь;
					КонецЕсли;
	...
	Если Не ПустаяСтрока(Форма.Поиск) Тогда
		...
		Если Найти(ВРег(НСтр("ru='Прочие настройки';uk='Інші настройки'")), ВРег(Форма.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
