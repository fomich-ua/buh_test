...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() Тогда
		ТекстВопроса = НСтр("ru='Изменения не настроены. Выполнить перезапись элементов без изменений?';uk='Зміни не настроєні. Виконати перезапис елементів без змін?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	Для Каждого ПереданныйОбъект Из Параметры.МассивОбъектов Цикл
		Если ТипЗнч(ПереданныйОбъект) <> ТипПереданныхОбъектов Тогда
			ВызватьИсключение НСтр("ru='Групповое изменение возможно только для элементов одного типа.';uk='Групова зміна можлива тільки для елементів одного типу.'");
		КонецЕсли;
	КонецЦикла;
...
"Изменено: %1; Не удалось изменить: %3';uk='Зміни виконані частково."
...
"Всего изменено элементов: %1';uk='Реквізити вибраних елементів були змінені."
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			...
			Пояснение = Пояснение + ПодставитьПараметрыВСтроку(
				НСтр("ru='реквизиты (%1)';uk='реквізити (%1)'"), ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
	...
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("ru='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';uk='у тих рядках вибраних елементів, які задовольняють умовам відбору'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("ru='во всех строках выбранных элементов';uk='у всіх рядках вибраних елементів'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		...
		Иначе
			Пояснение = НСтр("ru='Выполнить перезапись выбранных элементов.';uk='Виконати перезапис вибраних елементів.'");
		КонецЕсли;
	Иначе
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		Если ПустаяСтрока(ТекстСообщенияОбОшибке) Тогда
			ТекстНадписи = ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбрано элементов: %1';uk='Вибрано елементів: %1'"), КоличествоВыбранныхОбъектов);
		Иначе
			ЕстьОшибкиВОтборе = Истина;
			ТекстНадписи = ПодставитьПараметрыВСтроку(
				НСтр("ru='Элементы не выбраны. %1';uk='Елементи не обрані. %1'"), ТекстСообщенияОбОшибке);
		КонецЕсли;
	Иначе
		ТекстНадписи = НСтр("ru='Изменять все элементы';uk='Змінювати всі елементи'");
	КонецЕсли;
