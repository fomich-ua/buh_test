...
// Сормирует значение ссылочного типа
// 
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Каждого СтрокаДанныхНачальногоЗаполнения Из ТаблицаДанныхНачальногоЗаполнения Цикл
		...
		Если ТекущийОбъект.Модифицированность() Тогда
			...
			Конецесли;
			Попытка
				ТекущийОбъект.ДополнительныеСвойства.Вставить("ЗаписьОбщихДанных",Истина);
...
"Объект не заполнен!';uk=""Помилка при заповненні об'єкта %1! Не знайдено поле %2."
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
			Возврат;
		КонецЕсли;
...
Процедура ЗаполнитьПланВидовРасчета(ИмяОбъекта,ЗаполнениеСинонимовОбъектов = Ложь,ИмяПредопределенногоФильтр ="") Экспорт
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Каждого СтрокаДанныхНачальногоЗаполнения Из ТаблицаДанныхНачальногоЗаполнения Цикл
		...
		Если ИмяПредопределенногоФильтр <> "" Тогда
			Если ИмяПредопределенногоФильтр <> ИмяПредопределенного Тогда
				Продолжить;
			КонецЕсли;	
		КонецЕсли;	
