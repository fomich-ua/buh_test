...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		...
		Если КонфигурацияГотоваКОбновлению = Неопределено Тогда
...
Процедура НадписьКакОформитьПодпискуИТСНажатие(Элемент)
	ОбновлениеКонфигурацииКлиент.ОткрытьВебСтраницу("http://its.1c.ua/db/aboutitsu#content:20:1");
КонецПроцедуры
...
&НаКлиенте
Процедура НадписьПодробнееОбИТСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОбновлениеКонфигурацииКлиент.ОткрытьВебСтраницу("http://its.1c.ua/db/aboutitsu");
КонецПроцедуры
...
&НаКлиенте
Процедура ДекорацияОбновитеПлатформуОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
...
Процедура НадписьРезервнаяКопияНажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ОбновлениеКонфигурации.Форма.НастройкаРезервнойКопии", ПараметрыФормы, ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура НадписьПорядокОбновленияФайлОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
...
&НаКлиенте
Процедура НадписьПорядокОбновленияСерверОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
...
Функция ВставитьПараметрыСкрипта(Знач Текст, Знач СтруктураПараметров)
	...
	Результат = СтрЗаменить(Результат, "[СтрокаПутиКФайлуИнформационнойБазы]"	, Форматировать(ОбновлениеКонфигурацииКлиентСервер.ДобавитьКонечныйРазделительПути(СтрЗаменить(СтрокаПутиКИнформационнойБазе, """", "")) +
		"1Cv8.1CD"));
	...
	Результат = СтрЗаменить(Результат, "[КаталогРезервнойКопии]"				, 
		Форматировать(?(Объект.СоздаватьРезервнуюКопию = 2, ОбновлениеКонфигурацииКлиентСервер.ДобавитьКонечныйРазделительПути(Объект.ИмяКаталогаРезервнойКопииИБ), "")));
	...
	Результат = СтрЗаменить(Результат, "[ПарольАдминистратораИБ]"		, Форматировать(СтруктураПараметров.ПараметрыАдминистрирования.ПарольАдминистратораИнформационнойБазы));
	Результат = СтрЗаменить(Результат, "[ПарольАдминистратораКластера]" , Форматировать(СтруктураПараметров.ПараметрыАдминистрирования.ПарольАдминистратораКластера));
	...
	Пока ТекущаяВерсияОт <> ВерсияДо Цикл
		...
		Если ТекущаяВерсияОт = НоваяТекущаяВерсияОт И НоваяТекущаяВерсияОт <> ВерсияДо Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недоступно обновление на версию %1 с текущей версии %2';uk='Недоступне оновлення на версію %1 з поточної версії %2'"), ВерсияДо, ВерсияОт);
		КонецЕсли;
...
&НаСервере
Функция ПолучитьСтрокуСоединенияИИнформациюОСоединениях(СообщенияДляЖурналаРегистрации, ПортКластера)
	// запись накопленных событий ЖР
	ЖурналРегистрации.ЗаписатьСобытияВЖурналРегистрации(СообщенияДляЖурналаРегистрации);
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ДоступноеОбновление Тогда
		...
		Если СтруктураДоступногоОбновления <> Неопределено Тогда
			...
			Если ДоступноОбновлениеДляСледующейРедакции Тогда
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ДлительнаяОперация Тогда
		...
		Элементы.ПанельОжиданиеДлительнойОперации.ТекущаяСтраница = СтраницыОжидания.Интернет;
	...
	Если ТипЗнч(СообщенияДляЖурналаРегистрации) = Тип("СписокЗначений") Тогда
...
Функция ВосстановитьРезультатыПредыдущегоЗапуска(Отказ = Ложь)
	...
	Обработано	= 	ИмяСтраницы = Страницы.ДоступноеОбновление.Имя Или
				 	...
				 	ИмяСтраницы = Страницы.ВыборРежимаОбновленияФайл.Имя;
...
Функция ОбработатьСтраницуПодключениеКСайту(ВыдаватьСообщения = Истина)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодПользователяСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите код пользователя сервера обновлений.';uk='Укажіть код користувача сервера оновлень.'"));
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПарольСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите пароль пользователя сервера обновлений.';uk='Укажіть пароль користувача сервера оновлень.'"));
		КонецЕсли;
...
&НаКлиенте
Функция ПерейтиКВыборуРежимаОбновления(ЭтоПереходДалее = Ложь)
...
"параметры администрирования информационной базы';uk='Для установки оновлення необхідно ввести"
...
"администрирования кластера серверов и информационной базы';uk='Для установки оновлення необхідно ввести параметри"
"адміністрування кластера серверів й інформаційної бази'");
			ЗапрашиватьПараметрыАдминистрированияКластера = Истина;
		КонецЕсли;
		
		СоединенияИБКлиент.ПоказатьПараметрыАдминистрирования(ОписаниеОповещения,, ЗапрашиватьПараметрыАдминистрированияКластера,
...
"корректные параметры администрирования информационной базы.';uk='Не вдалося встановити оновлення програми, т. я. не були введені"
...
Функция ПолучитьПараметрыАутентификацииАдминистратораОбновления()
	...
	ТекущиеСоединения = ПолучитьСтрокуСоединенияИИнформациюОСоединениях(СообщенияДляЖурналаРегистрации, ПортКластера);
	...
	Пользователь = ПараметрыАдминистрирования.ИмяАдминистратораИнформационнойБазы;
	Пароль = ПараметрыАдминистрирования.ПарольАдминистратораИнформационнойБазы;
	
	Результат.ИмяПользователя			= Пользователь;
	Результат.ПарольПользователя		= Пароль;
	Результат.СтрокаПодключения			= "Usr=""" + Пользователь + """;Pwd=""" + Пароль + """";
	Результат.ПараметрыАутентификации	= "/N""" + Пользователь + """ /P""" + Пароль + """ /WA-";
	Возврат Результат;
...
Функция ПроверитьДоступКИБ()
	...
	// Проверка подключения к информационной базе
	Попытка
		АдминистрированиеКластераКлиентСервер.ПроверитьПараметрыАдминистрирования(ПараметрыАдминистрирования,, Ложь);
	Исключение
		Результат = Ложь;
	...
	Если Результат И Не ПараметрыРаботыКлиента.ИнформационнаяБазаФайловая Тогда
		...
		Исключение
			Результат = Ложь;
			ТекстСообщения = НСтр("ru='Не удалось выполнить подключение к кластеру серверов:';uk='Не вдалося виконати підключення до кластера серверів:'") + Символы.ПС;
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
				"Ошибка", ТекстСообщения + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбнаруженнаяОшибкаПодключения = ТекстСообщения + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
...
Функция СоздатьЗадачуПланировщика(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Попытка
		...
		Если КодОшибки <> 0 Тогда	// Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
				"Ошибка", НСтр("ru='Ошибка при создании задачи планировщика:';uk='Помилка при створенні задачі планувальника:'")
					+ " " + КодОшибки);
			Возврат 0;
		КонецЕсли;
	...
	Исключение
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка",
			НСтр("ru='Ошибка при создании задачи планировщика:';uk='Помилка при створенні задачі планувальника:'")
				+ " " + ОписаниеОшибки());
		Возврат 0;
	КонецПопытки;
...
Функция УдалитьЗадачуПланировщика(КодЗадачи)
	...
	Попытка
		...
		Если НЕ Результат Тогда	// Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка",
				НСтр("ru='Ошибка при удалении задачи планировщика:';uk='Помилка при вилученні задачі планувальника: '")
					+ " " + КодОшибки);
			Возврат Результат;
		КонецЕсли;
...
"%2';uk='Помилка при вилученні тимчасового файлу %1"
"%2'");
		ИмяСобытияЖурнала =	ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
...
Функция ПолучитьФайлСпискаШаблонов(Знач ВыдаватьСообщения = Истина, ДоступноОбновлениеДляСледующейРедакции = Ложь)
	...
	Если Результат.Статус <> Истина Тогда
		...
		Исключение
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(),
				"Ошибка", 
				НСтр("ru='Ошибка при удалении временного файла';uk='Помилка при вилученні тимчасового файлу'") + " " + ПутьКФайлуСпискаШаблонов + Символы.ПС +
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
"%1';uk='Не вдалося підтвердити легальність отримання оновлення через Інтернет з причини:"
"%1'");
				...
				Если Результат.Статус <> Истина Тогда
					...
					Исключение
						ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
							"Ошибка", НСтр("ru='Ошибка при удалении временного файла';uk='Помилка при вилученні тимчасового файлу'") + " " +
...
Процедура ВыполнитьОбновлениеКонфигурации()
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ru='Выполняется процедура обновления конфигурации:';uk='Виконується процедура оновлення конфігурації:'")
