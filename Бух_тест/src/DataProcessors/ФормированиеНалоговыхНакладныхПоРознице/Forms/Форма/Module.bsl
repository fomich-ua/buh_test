...
Процедура ОтчетОжидаемыйИПодтвержденныйНДСПродажПоДням(Команда)
	...
	ОткрытьФорму("Отчет.ОжидаемыйИПодтвержденныйНДСПродажПоДнямРозница.Форма.ФормаОтчета", ПараметрыОткрытия, ЭтотОбъект);
...
Процедура ВывестиИнформациюПользователюОСозданныхДокументах(МассивСозданныхДокументов)
	...
	Если МассивСозданныхДокументов.Количество() Тогда
		...
		Если Объект.ПечататьРеестр Тогда
			...
			ПравоеЗначениеОтбора = Новый СписокЗначений();
			ПравоеЗначениеОтбора.ЗагрузитьЗначения(МассивСозданныхДокументов); 
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокСозданныхДокументов.КомпоновщикНастроек.Настройки.Отбор,
				"Ссылка", ПравоеЗначениеОтбора, ВидСравненияКомпоновкиДанных.ВСписке, , Истина);
			
			ПараметрыОткрытияРеестра = Новый Структура("Ключ, Заголовок, Период, Организация", СписокСозданныхДокументов, "Реестр документов", 
				Новый СтандартныйПериод(Объект.ДатаНачала, Объект.ДатаОкончания), Объект.Организация);
			
			ФормаПечатиРеестра = ПолучитьФорму("Отчет.РеестрДокументов.Форма.ФормаОтчета", ПараметрыОткрытияРеестра, ЭтаФорма, Новый УникальныйИдентификатор);
