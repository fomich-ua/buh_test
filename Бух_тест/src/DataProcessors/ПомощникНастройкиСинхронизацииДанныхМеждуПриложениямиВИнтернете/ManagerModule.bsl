...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникли ошибки при получении сообщения обмена из внешнего ресурса для корреспондента ""%1"".';uk='Виникли помилки при отриманні повідомлення обміну із зовнішнього ресурсу для кореспондента ""%1"".'"),
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникли ошибки при анализе сообщения обмена для корреспондента ""%1"".';uk='Виникли помилки при аналізі повідомлення обміну для кореспондента ""%1"".'"),
			Строка(Корреспондент));
	КонецЕсли;
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникли ошибки при выполнении автоматического сопоставления данных, полученных от корреспондента ""%1"".';uk='Виникли помилки при виконанні автоматичного зіставлення даних, отриманих від кореспондента ""%1"".'"),
			Строка(Корреспондент));
	КонецЕсли;
...
Процедура ВыполнитьСинхронизациюСправочников(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникли ошибки в процессе загрузки справочников от корреспондента %1.';uk='Виникли помилки в процесі завантаження довідників від кореспондента %1.'"),
			Строка(Корреспондент));
	КонецЕсли;
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникли ошибки в процессе выгрузки справочников для корреспондента %1.';uk='Виникли помилки у процесі вивантаження довідників для кореспондента %1.'"),
			Строка(Корреспондент));
	КонецЕсли;
...
Процедура ПолучитьСтатистикуСопоставления(Параметры, АдресВременногоХранилища) Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Возникли ошибки при получении информации статистики для корреспондента ""%1"".';uk='Виникли помилки при отриманні інформації статистики для кореспондента ""%1"".'"),
