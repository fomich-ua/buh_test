...
"Прервать?';uk='Вилучення відмічених ще виконується."
"Перервати?'");
...
Процедура ЗаполнитьДеревоПомеченныхНаУдалениеНаКлиенте()
	Состояние(НСтр("ru='Выполняется поиск помеченных на удаление объектов';uk=""Виконується пошук відмічених для вилучення об'єктів"""));
...
Процедура ЗапуститьУдалениеПомеченныхНаКлиенте(ПовторныйЗапуск)
	Если РежимУдаления = "Полный" Тогда
		Состояние(НСтр("ru='Выполняется поиск и удаление помеченных объектов';uk='Виконується пошук і вилучення відмічених об`єктів'"));
	Иначе
		Состояние(НСтр("ru='Выполняется удаление выбранных объектов';uk=""Виконується вилучення обраних об'єктів"""));
	КонецЕсли;
...
"по которой его не удалось удалить.';uk=""Виберіть об'єкт, щоб дізнатися причину,"
	...
	Если КоличествоМожноУдалить = 0 Тогда
		...
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ru='Удаление записи регистра выполняется из ее карточки.';uk='Вилучення запису регістра виконується з її картки.'");
...
Функция ИзменитьПометкуУдаленияОбъектов(МассивСсылок, ПометкаУдаления)
	...
	Если ПометкаУдаления Тогда
		Результат.ВыводОповещения.Заголовок = НСтр("ru='Пометка удаления установлена';uk='Позначка вилучення встановлена'");
	Иначе
		Результат.ВыводОповещения.Заголовок = НСтр("ru='Пометка удаления снята';uk='Позначка вилучення знята'");
	КонецЕсли;
	КоличествоОбъектов = МассивСсылок.Количество();
	Если КоличествоОбъектов = 0 Тогда
		Результат.ВыводОповещения.Заголовок = НСтр("ru='Объект не найден';uk=""Об'єкт не знайдений""");
...
"Удалено объектов: %1.';uk=""Вилучення відмічених об`єктів успішно завершено."
"Вилучено об'єктів: %1."""),
...
"Удалено объектов: %1,"
"Не удалено: %2.';uk=""Вилучення відмічених об'єктів завершено."
"Вилучено об'єктів: %1,"
"Не вилучено: %2."""),
			Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
	...
	Если ОповещениеТекст <> Неопределено Тогда
		Результат.ВыводОповещения.Использование = Истина;
		Результат.ВыводОповещения.Заголовок = НСтр("ru='Удаление помеченных';uk='Вилучення відмічених'");
