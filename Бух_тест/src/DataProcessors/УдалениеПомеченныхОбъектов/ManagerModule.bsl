...
Процедура УдалитьПомеченныеОбъекты(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ПомеченныеНаУдаление = Неопределено Тогда
		Если ПараметрыВыполнения.Интерактивное Тогда
			ДлительныеОперации.СообщитьПрогресс(1, НСтр("ru='Поиск объектов, помеченных на удаление.';uk=""Пошук об'єктів, які відмічені для вилучення."""));
		КонецЕсли;
...
Процедура КлиентОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Если ИмяКоллекции = "ТехнологическиеОбъекты" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подготовка к удалению (%1 из %2).';uk='Підготовка до вилучення (%1 з %2).'"),
	...
	ИначеЕсли ИмяКоллекции = "ПомеченныеНаУдаление" Тогда
		...
		Если НеУдалено = 0 Тогда
			Шаблон = НСтр("ru='Удалено: %1 из %2 объектов.';uk=""Вилучено: %1 з %2 об'єктів.""");
		Иначе
			Шаблон = НСтр("ru='Обработано: %1 из %2 объектов, из них не удалено: %3.';uk=""Оброблено: %1 з %2 об'єктів, з них не вилучено: %3.""");
		КонецЕсли;
