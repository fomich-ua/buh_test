...
"%1';uk='Є активні сеанси, які не можуть бути завершені примусово. Блокування не встановлене."
"%1'"), 
				НазванияСоединенийИБ);
...
"Подробности см. в Журнале регистрации.';uk='Не вдалося завершити роботу всіх активних користувачів."
...
"Рекомендуется установить время начала на 5 минут относительно текущего времени.';uk='Зазначене занадто близький час початку дії блокування, до якого користувачі можуть не встигнути зберегти всі свої дані й завершити роботу."
...
"Запланировать блокировку на указанное время?';uk='Зазначене занадто великий час початку дії блокування (більш, ніж через годину)."
...
"Продолжить?';uk='Завершення роботи всіх активних користувачів буде зроблено в період з %1 по %2."
"Продовжити?'");
			Иначе
				ТекстВопроса = НСтр("ru='Сеансы всех активных пользователей будут завершены к %2."
"Продолжить?';uk='Сеанси всіх активних користувачів будуть завершені до %2."
"Продовжити?'");
			КонецЕсли;
		...
		Попытка
			...
			Иначе
				АдминистрированиеКластераКлиентСервер.УстановитьБлокировкуРегламентныхЗаданийИнформационнойБазы(
					ПараметрыАдминистрирования,, Объект.ЗапретитьРаботуРегламентныхЗаданий);
			КонецЕсли;
...
"параметры администрирования кластера серверов и информационной базы';uk='Для керування блокуванням сеансів необхідно ввести"
"параметри адміністрування кластера серверів й інформаційної бази'");
		СоединенияИБКлиент.ПоказатьПараметрыАдминистрирования(ОписаниеОповещения,,, ПараметрыАдминистрирования, ЗаголовокФормы, ПоясняющаяНадпись);
...
"ввести параметры администрирования кластера серверов и информационной базы';uk='Для керування блокуванням регламентних завдань необхідно"
...
"Работа пользователей завершается. Осталось активных сеансов: %1';uk='Будь ласка, почекайте..."
...
"Вход в программу новых пользователей оставлен заблокированным.';uk='Завершення роботи активних користувачів скасоване. "
