...
Процедура ИсключитьФайл(Знач ПолныйПутьКФайлу) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл %1 не найден в составе контейнера!';uk='Файл %1 не знайдений в складі контейнера!'"), ПолныйПутьКФайлу);
	Иначе
...
Процедура ЗаменитьФайл(Знач ИмяВконтейнере, Знач ПолныйПутьКФайлу, Знач УдалятьФайлЗаменыСДиска = Ложь) Экспорт
	...
	Если СтрокаИсходногоФайла = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл с идентификатором %1 не найден в составе контейнера!';uk='Файл з ідентифікатором %1 не знайдений в складі контейнера!'"), ИмяВконтейнере);
	Иначе
...
"Возможно, файл был получен из предыдущих версий программы или поврежден!';uk='Помилка завантаження даних. Невірний формат файлу. В архіві не знайдений файл PackageContents.xml."
...
Функция ПолучитьПроизвольныйФайл(Знач ТипДанных = Неопределено) Экспорт
	...
	Если Файлы.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В выгрузке отсутствует произвольный файл с типом данным %1!';uk='У вивантаженні відсутній довільний файл з типом даних %1!'"),
			ТипДанных
		);
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ru='В выгрузке содержится дублирующаяся информация';uk='У вивантаженні міститься інформація, що дублюється'");
...
Функция ПолучитьПолноеИмяФайла(Знач ОтносительноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В контейнере не обнаружен файл с относительным именем %1!';uk=""У контейнері не виявлений файл з відносним ім'ям %1!"""),
...
Функция ПолучитьОтносительноеИмяФайла(Знач ПолноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В контейнере не обнаружен файл %1!';uk='У контейнері не виявлений файл %1!'"),
			ПолноеИмяФайла
		);
	Иначе
...
Функция ПолучитьКаталогДляРазмещенияФайла(Знач ВидФайла)
	...
	Иначе
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вид файла %1 не поддерживается!';uk='Вид файлу %1 не підтримується!'"), ВидФайла);
	КонецЕсли;
