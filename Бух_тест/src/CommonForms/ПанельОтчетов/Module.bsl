...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе // Такси.
		...
		ЦветРаздела = ЦветГруппыВариантовОтчетов;
		//ЦветРаздела = ЦветаСтиля.ЦветТекстаФормы;
...
Процедура ПриОткрытии(Отказ)
	...
	#Иначе
		ВебКлиент = Ложь;
	#КонецЕсли
	Если ПоказатьОповещениеОПодсказках Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru='Новая возможность';uk='Нова можливість'"),
			"e1cib/data/Справочник.ВариантыОтчетов.Форма.ОписаниеНовойВозможностиПоВыводуОписаний",
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = ВариантыОтчетовКлиентСервер.ИмяСобытияИзменениеВарианта() Тогда
		...
		Если ТипЗнч(Параметр) = Тип("Структура") Тогда
			Если Параметр.Свойство("ПоказыватьПодсказки") Тогда
				ПоказыватьПодсказки = Параметр.ПоказыватьПодсказки;
				Событие = "ПоказыватьПодсказкиПриИзменении";
			КонецЕсли;
		КонецЕсли;
		ОбновитьПанельОтчетов(Событие);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВариантНажатие(Элемент)
	...
	КонецЕсли;
	ПараметрыОткрытия.Вставить("Раздел",       ТекущийРазделСсылка);
	ПараметрыОткрытия.Вставить("Подсистема",   Вариант.Подсистема);
	
	Найденные = ПодсистемыПрограммы.НайтиСтроки(Новый Структура("Ссылка", Вариант.Подсистема));
	ПараметрыОткрытия.Вставить("ПодсистемаПредставление", Найденные[0].Представление);
...
Процедура СтрокаПоискаПриИзменении(Элемент)
	...
	ОбновитьПанельОтчетов("СтрокаПоискаПриИзменении");
...
Процедура ИскатьВоВсехРазделахПриИзменении(Элемент)
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		ОбновитьПанельОтчетов("ИскатьВоВсехРазделахПриИзменении");
...
&НаКлиенте
Процедура ЗаголовокРазделаНажатие(Элемент)
	ИмяРодителя = Элемент.Родитель.Имя;
	ИмяРаздела = Сред(ИмяРодителя, Найти(ИмяРодителя, "_")+1);
	
	ФормаПараметры = Новый Структура;
	ФормаПараметры.Вставить("ПутьКПодсистеме",      ИмяРаздела);
...
Процедура ПоказыватьПодсказкиПриИзменении(Элемент)
	ОбновитьПанельОтчетов("ПоказыватьПодсказкиПриИзменении");
КонецПроцедуры
...
Процедура ПереместитьВариантБыстрогоДоступа(Знач ВариантИдентификатор, Знач БыстрыйДоступ)
	...
	//ЭлементРасширеннаяПодсказка = Элементы.Найти(Вариант.НадписьИмя + "_РасширеннаяПодсказка");
	//Если ЭлементРасширеннаяПодсказка <> Неопределено Тогда
	//	Элементы.Переместить(ЭлементРасширеннаяПодсказка, ГруппаДляПереноса, ПередКакимЭлементом);
	//КонецЕсли;
...
&НаСервере
Процедура ОбновитьПанельОтчетов(Знач Событие = "")
	Если Событие = "" Или Событие = "СтрокаПоискаПриИзменении" Тогда
		Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
	...
	ИначеЕсли Событие = "ПоказыватьПодсказкиПриИзменении"
		...
		СохранитьНастройкиВсехПанелейОтчетов();
	КонецЕсли;
...
Процедура СохранитьПользовательскиеНастройки()
	...
	Измерения = Новый Структура("Пользователь", Пользователи.ТекущийПользователь());
...
"См. модуль ВариантыОтчетовПереопределяемый, процедуру ОпределитьРазделыСВариантамиОтчетов.';uk='Розділ ""%1"" не підключений до підсистеми ""%2""."
...
&НаСервере
Процедура СохранитьНастройкиВсехПанелейОтчетов()
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Иначе
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			...
			ЗаголовокНадписи = СтрЗаменить(ЗаголовокНадписи, "%1", ПараметрыЗаполнения.ПределВывода);
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения)
	...
	|	ВариантыРазмещение.Ссылка.Родитель,
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Для Каждого ИмяГруппы Из ПараметрыЗаполнения.ГруппыВажности Цикл
		...
		Если ПараметрыГруппы.Количество = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		//Если ИмяГруппы = "СГруппой" Тогда
			ВывестиВариантыСГруппой(ПараметрыЗаполнения);
		//Иначе
		//	ВывестиВариантыБезГруппы(ПараметрыЗаполнения);
		//КонецЕсли;
	КонецЦикла;
...
Процедура ОпределитьГруппыИДекорацииДляВыводаВариантов(ПараметрыЗаполнения)
	...
	ПараметрыЗаполнения.Префикс = "Раздел" + ИнформацияОРазделе.Приоритет + "_";
	...
	ЗаголовокРаздела.УстановитьДействие("Нажатие", "ЗаголовокРазделаНажатие");
	...
	Если ПараметрыЗаполнения.ОсталосьВывести = 0 Тогда // Ранее в других группах достигнут предел вывода - не нужно создавать подчиненные.
...
Процедура ВывестиВариантыСГруппой(ПараметрыЗаполнения)
	...
	Если ГруппаУровня2Имя = "БыстрыйДоступ" Или ГруппаУровня2Имя = "СмТакже" Тогда
		ОписаниеПодсистемы = ПараметрыЗаполнения.ТаблицаПодсистем.Найти(ПараметрыЗаполнения.РазделСсылка, "Ссылка");
		Варианты.ЗаполнитьЗначения(ОписаниеПодсистемы.Ссылка, "Подсистема");
		Варианты.ЗаполнитьЗначения(ОписаниеПодсистемы.Приоритет, "ПодсистемаПриоритет");
		Варианты.ЗаполнитьЗначения(ОписаниеПодсистемы.Представление, "ПодсистемаПредставление");
		Колонки = "";
		Для Каждого Колонка Из Варианты.Колонки Цикл
			Колонки = ?(Колонки = "", "", Колонки + ", ") + Колонка.Имя;
		КонецЦикла;
		Варианты.Свернуть(Колонки);
	КонецЕсли;
	
	// Сортировка вариантов
	Варианты.Сортировать("ПодсистемаПриоритет ВОЗР, Важный УБЫВ, Наименование ВОЗР"); // Есть группы и важные // ВерхнийУровень УБЫВ, 
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если СтрокаДерева.КоличествоВариантов = 0 И СтрокаДерева.ВсегоВложенныхВариантов = 0 Тогда
		Возврат ВГруппу;
	КонецЕсли;
	//УвеличениеРазмераШрифта = ПараметрыЗаполнения.МаксимальныйУровеньВложенности - СтрокаПорядкаВывода.УровеньВложенности;
	...
	//ШрифтЗаголовка = Новый Шрифт(ШрифтВажнойГруппы, , ШрифтВажнойГруппы.Размер + 2*УвеличениеРазмераШрифта);
	Если ТребуетсяПодсветка Тогда
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			Суффикс = НСтр("ru='(продолжение)';uk='(продовження)'");
			Если Прав(ПодсветкаПредставления.Значение, СтрДлина(Суффикс)) <> Суффикс Тогда
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	ВыведеноЭлементовВариантов = ВыведеноЭлементовВариантов + 1;
	НадписьИмя = "ДобавленныйЭлемент_" + Формат(ВыведеноЭлементовВариантов, "ЧГ=0");
...
Процедура ВывестиПодсказкуВарианта(Надпись, СоставПодсказки)
	...
	//Надпись.РасширеннаяПодсказка.РастягиватьПоВертикали   = Ложь;
