...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИнтернетПоддержкаПользователей.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение
			НСтр("ru='Настройка параметров подключения к интернет-поддержке пользователей доступна только администратору системы.';uk='Настройка параметрів підключення до інтернет-підтримки користувачів доступна тільки адміністратору системи.'");
	КонецЕсли;
	
	ЭтоКлиентСервернаяИБ = (ВРег(Лев(СтрокаСоединенияИнформационнойБазы(), 5)) <> "FILE=");
	...
	Если ПустаяСтрока(Логин) Тогда
		Элементы.ДекорацияПояснениеЛогинПароль.Заголовок = НСтр("ru='Вы еще не авторизованы';uk='Ви ще не авторизовані'");
		...
		Если Параметр.Свойство("Логин") Тогда
			...
			Иначе
				Элементы.ДекорацияПояснениеЛогинПароль.Заголовок =
					НСтр("ru='При подключении интернет-поддержки используется:';uk='При підключенні інтернет-підтримки використовується:'");
