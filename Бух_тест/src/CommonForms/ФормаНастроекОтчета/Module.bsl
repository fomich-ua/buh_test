...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("НастройкиОтчета", НастройкиОтчета) Тогда
		РежимСовместимости = Истина;
		НастройкиОтчета = ФункцииОтчетовКлиентСервер.ПолучитьНастройкиОтчетаПоУмолчанию();
		ОтчетМетаданные = РеквизитФормыВЗначение("Отчет").Метаданные();
	...
	Если Параметры.Свойство("КлючОбъекта") Тогда
		НастройкиОтчета.Вставить("КлючОбъекта", Параметры.КлючОбъекта);
	Иначе
		НастройкиОтчета.Вставить("КлючОбъекта", РеквизитФормыВЗначение("Отчет").Метаданные().ПолноеИмя());
	КонецЕсли;
	Если Параметры.Свойство("ОтчетСсылка") Тогда
		НастройкиОтчета.Вставить("Ссылка", Параметры.ОтчетСсылка);
	Иначе
		НастройкиОтчета.Вставить("Ссылка", НастройкиОтчета.КлючОбъекта);
	КонецЕсли;
	Если Параметры.Свойство("НаименованиеОтчета") Тогда
		НастройкиОтчета.Вставить("Наименование", Параметры.НаименованиеОтчета);
	Иначе
		НастройкиОтчета.Вставить("Наименование", РеквизитФормыВЗначение("Отчет").Метаданные().Представление());
	КонецЕсли;
	Если Параметры.Свойство("Подсистема") Тогда
		НастройкиОтчета.Вставить("Подсистема", Параметры.Подсистема);
		НастройкиОтчета.Вставить("ПодсистемаПредставление", Параметры.ПодсистемаПредставление);
	Иначе
		НастройкиОтчета.Вставить("Подсистема", Неопределено);
		НастройкиОтчета.Вставить("ПодсистемаПредставление", "");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантСсылка", ВариантСсылка) Тогда
		ВариантСсылка = ВариантыОтчетов.ПолучитьСсылку(НастройкиОтчета.Ссылка, КлючТекущегоВарианта);
	КонецЕсли;
...
Процедура ПриЗакрытии()
	Если Не РежимСовместимости
		И Не ФормаЗакрываетсяСПараметрами
		И ВладелецФормы <> Неопределено Тогда
		ВладелецФормы.ОбработкаВыбора(РезультатВыбора(Ложь), ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВариантНажатие(Элемент)
	...
	ПараметрыОткрытия.Вставить("Подсистема",              НастройкиОтчета.Подсистема);
	ПараметрыОткрытия.Вставить("ПодсистемаПредставление", НастройкиОтчета.ПодсистемаПредставление);
	...
	ИначеЕсли Не ЗначениеЗаполнено(Вариант.ИмяОтчета) Тогда
		...
		ТекстПредупреждения = СтрЗаменить(НСтр("ru='Не заполнено имя отчета для варианта ""%1"".';uk='Не заповнене ім''я звіту для варіанта ""%1"".'"), "%1", Вариант.Наименование);
...
Процедура Подключаемый_ТаблицаПорядка_Выбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИмяКолонки = Поле.Имя;
	Если Лев(ИмяКолонки, 23) = "Порядок_КолонкаПорядок_" Тогда // Изменение порядка.
...
Процедура Подключаемый_СписокСПодбором_ПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаНачалаРедактирования, ОтменаЗавершенияРедактирования)
	...
	Если ОтменаЗавершенияРедактирования Тогда
		// Откат значений.
		ЭлементСпискаДоНачалаИзменения = ВариантыОтчетовКлиентСервер.СвойствоСтруктуры(ПеременныеКлиента, "ЭлементСпискаДоНачалаИзменения");
		Если ЭлементСпискаДоНачалаИзменения <> Неопределено
			И ЭлементСпискаДоНачалаИзменения.Идентификатор = СтрокаИдентификатор Тогда
...
Процедура ВсеНастройки(Команда)
	ПоказыватьВсеНастройки = Не Элементы.ВсеНастройки.Пометка;
	Элементы.ВсеНастройки.Пометка = ПоказыватьВсеНастройки;
	Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("ПоказыватьВсеНастройки", ПоказыватьВсеНастройки);
	ПользовательскиеНастройкиМодифицированы = Истина;
	ВидимостьДоступностьКорректность("ВсеНастройки");
КонецПроцедуры
...
Процедура ЗаписатьИЗакрыть(Переформировать)
	ФормаЗакрываетсяСПараметрами = Истина;
	Если РежимСовместимости
		Или МодальныйРежим
...
Функция РезультатВыбора(Переформировать)
	...
	ПользовательскиеНастройкиМодифицированы = Истина; // Для 8.3.4 и ниже.
...
Процедура ВыборПериодаИзВыпадающегоСписка(Результат, ПараметрыВыбора) Экспорт
	...
	Если ПараметрыВыбора.Вариант = Неопределено Или ПараметрыВыбора.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ru='Относительный...';uk='Відносний...'"));
	...
	Иначе
		...
		ОписаниеНавигационногоЭлемента.Вставить("Вариант",                  Неопределено);
		ОписаниеНавигационногоЭлемента.Вставить("ИндексНачальногоЗначения", Неопределено);
		СписокПериодов.Добавить(ОписаниеНавигационногоЭлемента, НСтр("ru='Фиксированный...';uk='Фіксований...'"));
...
Процедура БыстрыеНастройкиЗаполнить(Знач ПараметрыЗаполнения)
	...
	// Чтение настроек отчета, не связанных с полями СКД.
	ПрочитатьНастройкиНеСвязанныеСПолями();
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	Если Изменения = "" Тогда
		...
		ЕстьМедленныеНастройки = ПользовательскиеНастройкиКД.Элементы.Количество() > 0;
	...
	Если Изменения = "" Или Изменения = "ВсеНастройки" Тогда
		...
		Конецесли;
		Если Элементы.ВсеНастройки.Пометка Тогда
			Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаВсеНастройки;
			Заголовок = НСтр("ru='Все настройки отчета';uk='Всі настройки звіту'") + " """ + Заголовок + """";
		Иначе
			Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаНастройкиИДругиеОтчеты;
			Заголовок = НСтр("ru='Настройки отчета';uk='Настройки звіту'") + " """ + Заголовок + """";
		Конецесли;
		Элементы.ГруппаДругиеВариантыОтчета.Заголовок = НастройкиОтчета.Наименование + " (" + НСтр("ru='варианты отчета';uk='варіанти звіту'") + "):";
	КонецЕсли;
...
Процедура ОбновитьСвязиВедущегоСПодчиненными(ИдентификаторЭлемента, Значение, Использование, ВсеСвязиВедущего, ЗначениеИзменено)
	...
	Если ВсеСвязиВедущего.Свойство("ПараметрыВыбораПодчиненных") Тогда
		Для Каждого СтрокаСвязиОтбора Из ВсеСвязиВедущего.ПараметрыВыбораПодчиненных Цикл
			...
			Если ЗначениеИзменено = Истина
				И СтрокаСвязиОтбора.Действие = РежимИзмененияСвязанногоЗначения.Очищать Тогда
				Если Не ПустаяСтрока(СтрокаСвязиОтбора.ПодчиненныйИмяРеквизита) Тогда
					ЗначениеРеквизита = ЗначениеРеквизита(СтрокаСвязиОтбора.ПодчиненныйИмяРеквизита);
					...
					КонецЕсли;
				КонецЕсли;
				Если Не ПустаяСтрока(СтрокаСвязиОтбора.ПодчиненныйИмяФлажка)
					И ЗначениеРеквизита(СтрокаСвязиОтбора.ПодчиненныйИмяФлажка) <> Неопределено Тогда
					ИдентификаторКД = БыстрыйПоискПользовательскихНастроек.Получить(СтрокаСвязиОтбора.ПодчиненныйИдентификатор);
					ПользовательскаяНастройка = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ПолучитьОбъектПоИдентификатору(ИдентификаторКД);
...
&НаСервере
Процедура ПрочитатьНастройкиНеСвязанныеСПолями()
	ПользовательскиеНастройкиКД = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки;
	
	ФормироватьСразу = Неопределено;
	ПользовательскиеНастройкиКД.ДополнительныеСвойства.Свойство("ФормироватьСразу", ФормироватьСразу);
	Если ТипЗнч(ФормироватьСразу) <> Тип("Булево") Тогда
		ФормироватьСразу = Ложь;
	КонецЕсли;
	Элементы.ФормироватьСразу.Пометка = ФормироватьСразу;
	
	ПоказыватьВсеНастройки = Неопределено;
	ПользовательскиеНастройкиКД.ДополнительныеСвойства.Свойство("ПоказыватьВсеНастройки", ПоказыватьВсеНастройки);
	Если ТипЗнч(ПоказыватьВсеНастройки) <> Тип("Булево") Тогда
		ПоказыватьВсеНастройки = Ложь;
	КонецЕсли;
	Элементы.ВсеНастройки.Пометка = ПоказыватьВсеНастройки;
КонецПроцедуры
...
Процедура БыстрыеНастройкиУдалитьСтарыеЭлементыИКоманды(ПараметрыЗаполнения)
	...
	ДобавитьПодчиненныеЭлементы(УдаляемыеЭлементы, Элементы.СтраницаГлавное.ПодчиненныеЭлементы);
	...
	Для Каждого СтраницаНастроек Из Элементы.СтраницыНастроек.ПодчиненныеЭлементы Цикл
		Если СтраницаНастроек <> Элементы.СтраницаГлавное
			И СтраницаНастроек <> Элементы.СтраницаДополнительно
			И СтраницаНастроек <> Элементы.СтраницаДругиеОтчеты
			И СтраницаНастроек <> Элементы.СтраницаНеОтсортированное Тогда
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения)
	...
	Для Каждого Реквизит Из ВсеРеквизиты Цикл
		...
		Если ПостоянныеРеквизиты.НайтиПоЗначению(РеквизитПолноеИмя) = Неопределено Тогда
			...
			Если ОписанияТиповСовпадают(Реквизит.ТипЗначения, ОписаниеТиповТаблицаЗначений) Тогда
	...
	ГруппаСортировки = Элементы.СтраницаНеОтсортированное;
		...
		Для Каждого ПользовательскаяНастройка Из НаборПользовательскихНастроек Цикл
			...
			ИначеЕсли СвойстваНастройки.Тип = "ЗначениеПараметраНастроек" Тогда
				...
				ИначеЕсли СвойстваНастройки.ДоступнаяСКД.ДоступенСписокЗначений Тогда
					...
					Если ЭтоБыстраяНастройка Тогда
						СвойстваНастройки.Шаблон = "СвязьСКомпоновщиком";
					Иначе
						СвойстваНастройки.Шаблон = "СписокСПодбором";
					КонецЕсли;
				Иначе
				...
				ИначеЕсли ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
					...
					Иначе
						...
						Если ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке
							Или ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
							ЭлементЗаголовок = ЭлементЗаголовок + " (" + НСтр("ru='за исключением выбранных';uk=' за винятком обраних'") + ")";
						КонецЕсли;
					КонецЕсли;
			...
			Иначе
				...
				Пока ТекущаяСтрока <> Неопределено Цикл
					Если ЗначениеЗаполнено(ТекущаяСтрока.Заголовок) И ТекущаяСтрока.Настройка <> НастройкиКД Тогда
				...
				Если КорневаяСтрокаДереваВарианта = Неопределено Тогда // Раздел не найден. Вывод в основную часть.
					Если СвойстваНастройки.Шаблон = "СписокСПодбором" Тогда
						ГруппаВывода = ГруппыВывода.Основные;
					ИначеЕсли СвойстваНастройки.Тип = "ЗначениеПараметраНастроек"
						Или СвойстваНастройки.Тип = "ЭлементОтбора"
						Или СвойстваНастройки.Тип = "ФиксированныйОтбор" Тогда
						ГруппаВывода = ГруппыВывода.Основные;
					ИначеЕсли СвойстваНастройки.Шаблон = "ТаблицаПорядкаСФлажками" Тогда
						// Действие не требуется, т.к. каждая таблица "падает" на свою закладку.
					ИначеЕсли СвойстваНастройки.Шаблон = "СвязьСКомпоновщиком" Тогда
						ГруппаВывода = ГруппыВывода.ДополнительныеПодвал;
					Иначе
						ГруппаВывода = ГруппыВывода.ДополнительныеШапка;
					КонецЕсли;
				Иначе // Раздел найден. Вывод в группу на закладке "Дополнительно":
					СвойстваНастройки.ВыводВЗакладке = Истина;
					...
					КонецЕсли;
					Если СвойстваНастройки.Шаблон = "ТаблицаПорядкаСФлажками" Тогда // Вместо большой таблицы с флажками...
			...
			Если СвойстваНастройки.ВыводВЗакладке
				И (СвойстваНастройки.Шаблон = "СписокСПодбором"
					Или СвойстваНастройки.Шаблон = "ТаблицаПорядкаСФлажками") Тогда
				СвойстваНастройки.Шаблон = "СвязьСКомпоновщиком";
			КонецЕсли;
			...
			Если СвойстваНастройки.Шаблон = "ТолькоФлажокИспользование" Или СвойстваНастройки.Шаблон = "ТолькоФлажокЗначения" Тогда
				...
				// Настройки вывода.
				Если СвойстваНастройки.Шаблон = "ТолькоФлажокИспользование"
					И СвойстваНастройки.ВыводВЗакладке
					И (СвойстваНастройки.Тип = "Группировка"
						Или СвойстваНастройки.Тип = "Таблица"
						Или СвойстваНастройки.Тип = "Диаграмма")
					И ГруппаВывода.ИмяФлажка = ""
					И СвойстваНастройки.СтрокаДерева = КорневаяСтрокаДереваВарианта Тогда
				...
				// Флажок.
				ИспользованиеОтбора = Элементы.Добавить(ФлажокИмя, Тип("ПолеФормы"), ГруппаСортировки);
			...
			ИначеЕсли СвойстваНастройки.Шаблон = "СвязьСКомпоновщиком" Тогда
				...
				ИменаОсновныхРеквизитовФормы.Вставить(ИдентификаторЭлемента, ПутьПользовательскихНастроек + ".Элементы[" + ИндексЭлемента + "].Значение");
				...
				Если СвойстваНастройки.ФлажокИспользование Тогда
					...
					// Флажок
					ПолеФлажка = Элементы.Добавить(ФлажокИмя, Тип("ПолеФормы"), ГруппаСортировки);
				...
				ПолеВвода = Элементы.Добавить(ЗначениеИмя, Тип("ПолеФормы"), ГруппаСортировки);
			...
			ИначеЕсли СвойстваНастройки.Шаблон = "СтандартныйПериод" Тогда
				...
				// Основная Группа
				Группа = Элементы.Добавить(ГруппаИмя, Тип("ГруппаФормы"), ГруппаСортировки);
				...
				Период.Заголовок = НСтр("ru='Период';uk='Період'");
			...
			ИначеЕсли СвойстваНастройки.Шаблон = "СписокСПодбором" Тогда
				...
				Если СвойстваНастройки.ФлажокИспользование Тогда
					...
					ПолеФлажка = Элементы.Добавить(ФлажокИмя, Тип("ПолеФормы"), ГруппаЗаголовокТаблицы);
					ПолеФлажка.Вид                = ВидПоляФормы.ПолеФлажка;
					ПолеФлажка.Заголовок          = ЭлементЗаголовок + ":";
					ПолеФлажка.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
					ПолеФлажка.УстановитьДействие("ПриИзменении", "Подключаемый_ФлажокИспользование_ПриИзменении");
			...
			ИначеЕсли СвойстваНастройки.Шаблон = "ТаблицаПорядкаСФлажками" Тогда
				...
				КнопкаПереместитьВверх.Заголовок  = НСтр("ru='Переместить строку вверх';uk='Перемістити рядок вверх'");
				...
				КнопкаПереместитьВниз.Заголовок  = НСтр("ru='Переместить строку вниз';uk='Перемістити рядок униз'");
				
				Если СвойстваНастройки.Тип = "Порядок" Тогда
					
					КомандаПереместитьВверх.Подсказка = НСтр("ru='Поднять приоритет сортировки по колонке';uk='Підняти пріоритет сортування по колонці'");
					КомандаПереместитьВниз.Подсказка  = НСтр("ru='Понизить приоритет сортировки по колонке';uk='Знизити пріоритет сортування по колонці'");
					...
					КнопкаПереместитьВверх.Заголовок  = НСтр("ru='Сортировать по возрастанию';uk='Сортувати за зростанням'");
					...
					КнопкаПереместитьВниз.Заголовок  = НСтр("ru='Сортировать по убыванию';uk='Сортувати по убуванню'");
					
				ИначеЕсли СвойстваНастройки.Тип = "ВыбранныеПоля" Тогда
					
					КомандаПереместитьВверх.Подсказка = НСтр("ru='Переместить поле выше или левее';uk='Перемістити поле вище або ліворуч'");
					КомандаПереместитьВниз.Подсказка  = НСтр("ru='Переместить поле ниже или правее';uk='Перемістити поле нижче або праворуч'");
				...
				ИначеЕсли СвойстваНастройки.Тип = "СтруктураНастроек" Тогда
					...
					КомандаПереместитьВверх.Подсказка = НСтр("ru='Переместить поле вверх';uk='Перемістити поле вверх'");
					КомандаПереместитьВниз.Подсказка  = НСтр("ru='Переместить поле вниз';uk='Перемістити поле вниз'");
			...
			Если ЭлементВывода.Размер <> Неопределено Тогда
				Если ЭлементВывода.ИмяЭлемента1 = Неопределено Тогда // Заголовок
					ЭлементВывода.ИмяЭлемента1 = СвойстваНастройки.Тип + "_Заголовок_" + ИдентификаторЭлемента;
					ПолеНадписи = Элементы.Добавить(ЭлементВывода.ИмяЭлемента1, Тип("ДекорацияФормы"), Элементы.БыстрыеНастройки);
					ПолеНадписи.Вид       = ВидДекорацииФормы.Надпись;
					ПолеНадписи.Заголовок = ЭлементЗаголовок + ":";
				КонецЕсли;
				Если СвойстваНастройки.Шаблон = "СтандартныйПериод" Тогда
	...
	ВывестиПоПорядку(ГруппыВывода.Быстрые, Элементы.БыстрыеНастройки, 2, Ложь);
	ВывестиПоПорядку(ГруппыВывода.Основные, Элементы.СтраницаГлавное, 2, Ложь);
	ВывестиПоПорядку(ГруппыВывода.ДополнительныеШапка, Элементы.СтраницаДополнительноШапка, 1);
	ВывестиПоПорядку(ГруппыВывода.ДополнительныеПодвал, Элементы.СтраницаДополнительноПодвал, 1);
	...
	Для Каждого ОписаниеЗакладки Из ГруппыВывода.ДополнительныеЗакладки Цикл
		...
		Если ГруппаВывода.Размер > 0 Тогда
			...
			ВывестиПоПорядку(ГруппаВывода, Колонки, 1);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СвойстваНастройки Из ДобавленныеСпискиЗначений Цикл
		...
		Для Номер = 1 По Количество Цикл
			...
			КонецЕсли;
			ДобавитьУникальноеЗначениеВСписок(СписокСФлажками, Значение, ЭлементСписка.Представление, Истина);
		КонецЦикла;
	...
	Для Каждого СвойстваНастройки Из ДобавленныеТаблицыСФлажками Цикл
		...
		ИначеЕсли СвойстваНастройки.Тип = "СтруктураНастроек" Тогда
			...
			Для Каждого ЭлементСтруктуры Из ПользовательскаяНастройка.Структура Цикл
				...
				Иначе
					Представление = НСтр("ru='Таблица / диаграмма';uk='Таблиця / діаграма'");
				КонецЕсли;
...
&НаСервере
Процедура ВывестиПоПорядку(ГруппаВывода, Родитель, КоличествоКолонок, ГибкаяБалансировка = Истина)
...
Процедура УстановитьСвязьПараметровВыбора(ПодчиненныйЭлемент, СтруктураСвязи, ВключитьСвязь)
	Если ПустаяСтрока(СтруктураСвязи.ПодчиненныйИмяПараметраФормы) Тогда
		Возврат;
	КонецЕсли;
	Если СтрЧислоВхождений(СтруктураСвязи.ПодчиненныйИмяПараметраФормы, ".") > 1 Тогда
		Возврат; // Механика связей предъявляет особые требования: путь к данным может держать не более одной точки.
	КонецЕсли;
	Если СтрЧислоВхождений(СтруктураСвязи.ВедущийИмяРеквизита, ".") > 2 Тогда
		Возврат; // Связи с СКД не поддерживаются.
	КонецЕсли;
...
Процедура ЗаполнитьКомандыВыбораПользовательскихНастроек()
	...
	КонецЕсли;
	КлючНастроек = НастройкиОтчета.КлючОбъекта + "/" + КлючТекущегоВарианта;
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Отчет", НастройкиОтчета.Ссылка);
	...
	Если ЗначениеЗаполнено(НастройкиОтчета.Подсистема) Тогда
...
Функция Подсистемы()
	...
	Результат.Добавить(НастройкиОтчета.Подсистема);
	
	ДеревоПодсистем = ВариантыОтчетовПовтИсп.ПодсистемыТекущегоПользователя();
	Найденные = ДеревоПодсистем.Строки.НайтиСтроки(Новый Структура("Ссылка", НастройкиОтчета.Подсистема), Истина);
...
Процедура ПослеИзмененияКлючевыхСостояний(ПараметрыЗаполнения)
	Если ПараметрыЗаполнения.Событие.Имя <> "ПослеФормирования" Тогда
		Если ФормаОткрыта
			И НастройкиОтчета.ФормироватьСразу
			И ПараметрыЗаполнения.Событие.Имя = "ПриОбновленииСоставаПользовательскихНастроекНаСервере" Тогда
...
Процедура ДеревоВариантаЗарегистрироватьЭлемент(ЭлементСтруктурыСКД, НаборСтрокДерева)
	...
	Если СтрокаДерева.ТипНастройки = "НастройкиВложенногоОбъекта" Тогда
		ПараметрыВывода = ЭлементСтруктурыСКД.Настройки.ПараметрыВывода;
	Иначе
		ПараметрыВывода = ЭлементСтруктурыСКД.ПараметрыВывода;
	КонецЕсли;
	ПараметрЗаголовок = ПараметрыВывода.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Заголовок"));
...
Функция ПредставлениеПользовательскойНастройки(СвойстваНастройки)
	...
	Если СвойстваНастройки.Тип = "Отбор" Тогда
		ЭлементЗаголовок = НСтр("ru='Отбор';uk='Відбір'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Порядок" Тогда
		ЭлементЗаголовок = НСтр("ru='Сортировка';uk='Сортування'");
		
	ИначеЕсли СвойстваНастройки.Тип = "ВыбранныеПоля" Тогда
		ЭлементЗаголовок = НСтр("ru='Поля';uk='Поля'");
		
	ИначеЕсли СвойстваНастройки.Тип = "УсловноеОформление" Тогда
		ЭлементЗаголовок = НСтр("ru='Оформление';uk='Оформлення'");
		
	ИначеЕсли СвойстваНастройки.Тип = "ЭлементУсловногоОформления" Тогда
		ПредставлениеЗначения = Строка(СвойстваНастройки.ПользовательскаяСКД.Оформление);
		ПредставлениеТипа = НСтр("ru='Элемент оформления';uk='Елемент оформлення'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Группировка" Тогда
		...
		ЭлементЗаголовок = НСтр("ru='Группировка';uk='Групування'") + ?(Суффикс = "", "", " '" + Суффикс + "'");
		
	ИначеЕсли СвойстваНастройки.Тип = "ГруппировкаТаблицы" Тогда
		ЭлементЗаголовок = НСтр("ru='Группировка таблицы';uk='Групування таблиці'");
		
	ИначеЕсли СвойстваНастройки.Тип = "ГруппировкаДиаграммы" Тогда
		Суффикс = СокрЛП(Строка(СвойстваНастройки.ВариантаСКД.ПоляГруппировки));
		ЭлементЗаголовок = НСтр("ru='Группировка диаграммы';uk='Групування діаграми'") + ?(Суффикс = "", "", " '" + Суффикс + "'");
		
	ИначеЕсли СвойстваНастройки.Тип = "Таблица" Тогда
		ЭлементЗаголовок = НСтр("ru='Таблица';uk='Таблиця'");
		
	ИначеЕсли СвойстваНастройки.Тип = "Диаграмма" Тогда
		ЭлементЗаголовок = НСтр("ru='Диаграмма';uk='Діаграма'");
		
	ИначеЕсли СвойстваНастройки.Тип = "НастройкиВложенногоОбъекта" Тогда
		ЭлементЗаголовок = СтрЗаменить(НСтр("ru=""Вложенная группировка '%1'"";uk=""Вкладене групування '%1'"""), "%1", Строка(СвойстваНастройки.ПользовательскаяСКД));
		
	ИначеЕсли СвойстваНастройки.Тип = "СтруктураНастроек" Тогда
		
		Если СвойстваНастройки.Подтип = "ТаблицаСтроки" Тогда
			ЭлементЗаголовок = НСтр("ru='Строки';uk='Рядки'");
			
		ИначеЕсли СвойстваНастройки.Подтип = "ТаблицаКолонки" Тогда
			ЭлементЗаголовок = НСтр("ru='Колонки';uk='Стовпчики'");
			
		ИначеЕсли СвойстваНастройки.Подтип = "ДиаграммаСерии" Тогда
			ЭлементЗаголовок = НСтр("ru='Серии';uk='Серії'");
			
		ИначеЕсли СвойстваНастройки.Подтип = "ДиаграммаТочки" Тогда
			ЭлементЗаголовок = НСтр("ru='Точки';uk='Точки'");
			
		Иначе
			ЭлементЗаголовок = НСтр("ru='Структура';uk='Структура'");
...
&НаСервере
Функция ЗначениеРеквизита(Имя)
	Структура = Новый Структура(Имя);
	ЗаполнитьЗначенияСвойств(Структура, ЭтотОбъект);
	Возврат Структура[Имя];
КонецФункции
