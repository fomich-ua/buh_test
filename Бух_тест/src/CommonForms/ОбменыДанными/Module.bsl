...
Процедура ЗагрузитьПравилаСинхронизацииДанных(Команда)
	...
	ИмяПланаОбмена = ПолноеИмяПланаОбмена(УзелОбмена);
...
&НаСервере
Функция ПолноеИмяПланаОбмена(Знач УзелИнформационнойБазы)
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	ЗаголовокФормы = НСтр("ru='Синхронизация данных с ""%Программа%""';uk='Синхронізація даних з ""%Программа%""'");
...
"Последняя попытка: %ДатаПопытки%';uk='Дані відправлені: %ДатаОтправки%"
...
"Последняя попытка: %ДатаПопытки%';uk='Дані отримані: %ДатаПолучения%"
"Остання спроба: %ДатаПопытки%'");
	...
	Для Каждого Элемент ИЗ СписокПлановОбмена Цикл
		...
		Если МенеджерПланаОбмена.ИспользоватьПомощникСозданияОбменаДанными() 
			...
			Если Метаданные.ПланыОбмена[ИмяПланаОбмена].РаспределеннаяИнформационнаяБаза Тогда
				...
				Если ЕстьПравоНаАдминистрированиеДанных Тогда
			...
			Иначе
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					...
					Команды[ИмяКоманды].Заголовок = ЗаголовокКоманды + НСтр("ru=' (в сервисе)...';uk=' (у сервісі)...'");
...
Функция ПодробнаяИнформацияНаСервере(УзелОбмена)
	...
	СсылкаНаПодробноеОписание = МенеджерПланаОбмена.ПодробнаяИнформацияПоОбмену(УзелОбмена);
...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки)
	...
	Если ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(ИмяПланаОбмена, "ПредупреждатьОНесоответствииВерсийПравилОбмена")
		...
		Если ИнформацияОПравилах.ИмяКонфигурации <> Метаданные.Имя Тогда
			...
			ОписаниеОшибки.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ОписаниеОшибки.ТекстОшибки,
				ИнформацияОПравилах.СинонимКонфигурацииВПравилах);
