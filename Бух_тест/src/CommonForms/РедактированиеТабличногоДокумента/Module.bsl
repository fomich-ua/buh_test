...
Процедура ПриОткрытии(Отказ)
	...
	Если ТабличныйДокумент.ВысотаТаблицы = 0 Тогда
		Если Не ПустаяСтрока(Параметры.ИмяОбъектаМетаданныхМакета) Тогда
			ТабличныйДокумент =  ПолучитьИзВременногоХранилища(ПоместитьМакетВоВременноеХранилище());
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сохранить изменения в %1?';uk='Зберегти зміни в %1?'"), ИмяДокумента);
...
Процедура ВставитьПримечание(Команда)
	...
	ПоказатьВводСтроки(ОписаниеОповещения, Примечание, НСтр("ru='Примечание';uk='Примітка'"), , Истина);
КонецПроцедуры
...
Функция ПоместитьМакетВоВременноеХранилище()
	...
	Возврат ПоместитьВоВременноеХранилище(УправлениеПечатью.МакетПечатнойФормы(Параметры.ИмяОбъектаМетаданныхМакета));
