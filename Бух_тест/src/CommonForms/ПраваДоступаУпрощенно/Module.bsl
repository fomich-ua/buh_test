...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияФормы(
		Новый ОписаниеОповещения("ПередЗакрытиемПродолжение", ЭтотОбъект), Отказ);
...
&НаКлиенте
Процедура ПередЗакрытиемПродолжение(Ответ, Неопределен) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ЗаписатьИзменения(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект));
	Иначе
...
// Продолжение обработчика события ПередЗакрытием
&НаКлиенте
Процедура ПередЗакрытиемЗавершение(Отказ, Неопределен) Экспорт
...
Процедура ЗаписатьИзменения(ОбработкаПродолжения = Неопределено)
	...
	Иначе
		ЗаписатьИзмененияЗавершение("", ОбработкаПродолжения);
	КонецЕсли;
...
Процедура ЗаписатьИзмененияЗавершение(НовыйПарольПользователяСервиса, ОбработкаПродолжения) Экспорт
	...
	Если Отказ ИЛИ ОбработкаПродолжения = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьИзмененияНаСервере(Отказ)
	...
	Запрос.УстановитьПараметр(
		"ЗначенияДоступа", ЗначенияДоступа.Выгрузить(, "ГруппаДоступа, ВидДоступа, ЗначениеДоступа"));
