...
"Редактировать группу доступа могут ответственный за участников группы доступа и администратор.';uk='Недостатньо прав для редагування групи доступу."
...
Процедура ВключитьИсключитьИзГруппы(ГруппаДоступа, ВключитьВГруппуДоступа) Экспорт
	...
	Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().РазделениеВключено
	   И ГруппаДоступа = ПредопределенноеЗначение("Справочник.ГруппыДоступа.Администраторы") Тогда
		...
		Возврат;
	Иначе
		ВключитьИсключитьИзГруппыЗавершение("", ДополнительныеПараметры);
	КонецЕсли;
...
"не полноправный администратор.';uk='Неможливо включити користувача в групу доступу,"
...
"так как текущий пользователь"
"не ответственный за участников группы доступа и"
"не полноправный администратор.';uk='Неможливо виключити користувача з групи доступу,"
"так як поточний користувач"
"не відповідальний за учасників групи доступу і"
"не повноправний адміністратор.'");
		КонецЕсли;
	...
	Если НЕ Добавить И НЕ ПользовательВключенВГруппуДоступа(ТекущаяГруппаДоступа) Тогда
		ОписаниеОшибки =  НСтр("ru='Невозможно исключить пользователя из группы доступа,"
"так как он включен в нее косвенно.';uk='Неможливо виключити користувача із групи доступу,"
"тому що він включений у неї побічно.'");
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		И ГруппаДоступа = Справочники.ГруппыДоступа.Администраторы Тогда
