...
"Передано: %1';uk='Неприпустиме значення параметра ПараметрКоманды при виклику методу УправлениеПечатьюКлиент.ВыполнитьКомандуПечати."
...
&НаКлиенте
Процедура Подключаемый_ПолеТабличногоДокументаПриАктивизацииОбласти(Элемент)
...
Процедура ВычислитьСумму(Команда)
	...
	Если НастройкаПечатнойФормы <> Неопределено Тогда
		...
		СуммаВыделенныхЯчеек = ВычислитьСуммуСервер(ТабличныйДокумент, ВыделенныеОбласти(ТабличныйДокумент));
	КонецЕсли;
КонецПроцедуры
...
Процедура СформироватьПечатныеФормы(КоллекцияПечатныхФорм, ИменаМакетов, Отказ)
	...
	Иначе
		...
		УправлениеПечатью.СформироватьПечатныеФормы(Параметры.ИмяМенеджераПечати, ИменаМакетов,
			Параметры.ПараметрКоманды, Параметры.ПараметрыПечати.ДополнительныеПараметры, КоллекцияПечатныхФорм,
			ОбъектыПечати, ПараметрыВывода, ТипыОбъектовПечати);
	КонецЕсли;
	...
	Элементы.КодЯзыкаДляМногоязычныхПечатныхФорм.Видимость = (КоллекцияПечатныхФорм.Найти(Истина, "ЭтоМногоязычнаяПечатнаяФорма") <> Неопределено);
...
Процедура СоздатьРеквизитыИЭлементыФормыДляПечатныхФорм(КоллекцияПечатныхФорм)
	...
	Для Каждого РеквизитФормы Из НовыеРеквизитыФормы Цикл
		...
		Если РанееДобавленнаяНастройкаПечатнойФормы = Неопределено Тогда
			...
			НовыйЭлемент.УстановитьДействие("ПриАктивизацииОбласти", "Подключаемый_ПолеТабличногоДокументаПриАктивизацииОбласти");
...
Процедура Подключаемый_РассчитатьСуммуЯчеек()
	...
	Сумма = РассчитатьСуммуВыделенныхЯчеек(ТабличныйДокумент, Неопределено);
...
&НаСервереБезКонтекста
Функция ВычислитьСуммуСервер(Знач ТабличныйДокумент, Знач ВыделенныеОбласти)
	Сумма = РассчитатьСуммуВыделенныхЯчеек(ТабличныйДокумент, ВыделенныеОбласти);
