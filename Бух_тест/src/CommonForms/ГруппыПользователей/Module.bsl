...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(КоличествоПользователей > 0, 
		НСтр("ru='Не выбрано ни одного пользователя.';uk='Не обрано жодного користувача.'"));
	...
	Для каждого ПользовательИзМассива Из МассивПользователей Цикл
		...
		ОбщегоНазначенияКлиентСервер.Проверить(ТипПользователяИзМассива = Тип("СправочникСсылка.Пользователи")
			Или ТипПользователяИзМассива = Тип("СправочникСсылка.ВнешниеПользователи"), 
			НСтр("ru='Команда не может быть выполнена для указанного объекта.';uk=""Команда не може бути виконана для зазначеного об'єкта."""));
		ОбщегоНазначенияКлиентСервер.Проверить(ТипПользователей = ТипПользователяИзМассива, 
			НСтр("ru='Команда не может быть выполнена сразу для двух разных видов пользователей.';uk='Команда не може бути виконана відразу для двох різних видів користувачів.'"));
	КонецЦикла;		
...
"т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".';uk='Користувача ""%1"" не вдалося включити в обрані групи,"
"так як у них розрізняється тип або у груп встановлена ознака ""Всі користувачі заданого типу"".'");
		Иначе
			ПараметрыПредметаИсчисления = НСтр("ru='пользователю,пользователям,пользователям,,,,,,0';uk='користувачеві,користувачам,користувачам,,,,,,0'");
...
"т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".';uk='Не всіх користувачів удалося включити в обрані групи,"
"т.к. у них розрізняється тип або в груп установлена ознака ""Всі користувачі заданого типу"".'");
			...
			СтрокаПользователей = СтрокаПользователей + Строка(НеПеремещенныйПользователь.Ключ) + " : " + 
				СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(НеПеремещенныйПользователь.Значение) + Символы.ПС;
			КонецЦикла;
			УведомлениеПользователя.ПолныйТекстСообщения = НСтр("ru='Следующие пользователи не были включены в группы:';uk='Наступні користувачі не були включені в групи:'") +
	...
	ИначеЕсли КоличествоПользователей = 1 Тогда
		...
		СообщениеПользователю = НСтр("ru='Изменен состав групп у пользователя ""%1""';uk='Змінено склад груп у користувача ""%1""'");
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			СообщениеПользователю, НаименованиеПользователя);
	ИначеЕсли КоличествоПользователей > 1 Тогда
		
		СообщениеПользователю = НСтр("ru='Изменен состав групп у %1';uk='Змінено склад груп в %1'");
		СтрокаОбъект = ПользователиСлужебный.ФормированиеОкончанияСлова(
			КоличествоПользователей, НСтр("ru='пользователя,пользователей,пользователей,,,,,,0';uk='користувача,користувачів,користувачів,,,,,,0'"));
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
