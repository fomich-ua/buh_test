...
"для объектов типа ""%1"".';uk='Права доступу не настроюються"
"для об''єктів типу ""%1"".'"),
	...
	РодительЗаполнен =
		...
		И ЗначениеЗаполнено(ОбщегоНазначения.ПолучитьЗначениеРеквизита(Параметры.СсылкаНаОбъект, "Родитель"));
...
Процедура ДобавитьРеквизитыИлиЭлементыФормы(НовыеРеквизиты = Неопределено)
	...
	ОписанияВозможныхПрав = УправлениеДоступомСлужебныйПовтИсп.Параметры(
		).ВозможныеПраваДляНастройкиПравОбъектов.ПоТипамСсылок.Получить(
			ТипЗнч(Параметры.СсылкаНаОбъект));
...
"но и для ее нижестоящих папок';uk='Права не тільки для поточної папки,"
...
"вышестоящей папки ""%1"".';uk='Ці права успадковані, їх можна змінити у формі настройки прав"
"вищестоящої теки ""%1"".'"),
...
"снять флажок ""%1"".';uk='Для вилучення всіх успадкованих прав слід"
"зняти прапорець ""%1"".'"),
